...
# Adobe Systems Commerce 2.4.1 note sulla versione
...
#### Oltre 15 miglioramenti della sicurezza che aiutano a chiudere le vulnerabilità RCE (Remote Code Execution) e vulnerabilità cross-site scripting (XSS)
...
I miglioramenti della sicurezza per questa versione includono:
...
* La protezione **CAPTCHA** è stata aggiunta alle seguenti aree di prodotto:
  ...
  La protezione CAPTCHA per queste pagine aggiuntive è disattivata per impostazione predefinita. Può essere abilitato sull&#39;amministratore allo stesso modo delle altre pagine coperte dal CAPTCHA. Questa protezione è stata aggiunta come meccanismo anti-forza bruta per proteggere i negozi dagli attacchi di cardatura. Vedere [CAPTCHA.](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/captcha/security-captcha)

* **Supporto per l&#39;attributo SameSite per i cookie**. Per supportare l&#39;applicazione da parte di Google Effetto cromatura del nuovo sistema di classificazione dei cookie, le classi applicazione che gestiscono i cookie sono state aggiornate per supportare l&#39;attributo `SameSite` cookie. Questo attributo è impostato su `Lax` per impostazione predefinita, ma può essere sostituito in modo esplicito. <!-- MC-35389-->

* **Strumento** di scansione della sicurezza avanzato.Adobe Systems ha collaborato con [Sanguine Security](https://sansec.io/), leader nella prevenzione dello skimming digitale, per integrare il proprio database di oltre 8700 firme di minacce nello strumento di scansione della sicurezza. Questa partnership consentirà ai commercianti di ottenere informazioni in tempo reale sullo stato di sicurezza del loro sito attraverso il rilevamento proattivo di malware e la riduzione dei falsi positivi. I commercianti possono registrarsi allo strumento visitando `https://account.magento.com/scanner`. Per ulteriori informazioni, consulta il post del blog [Proteggi vetrina con lo strumento di analisi della sicurezza avanzata](https://magento.com/blog/magento-news/secure-your-storefront-enhanced-magento-security-scan-tool).

{{cve-notice}}

### Miglioramenti dell&#39;infrastruttura

Questa versione contiene miglioramenti alla qualità di base, che migliorano la qualità del framework e di queste aree funzionali: account cliente, catalogo, cms, OMS, Importa/esportazione, promozioni e targeting, carrello e checkout, B2B, staging e Anteprima.

* **Integrazione dello strumento di analisi a livello di sito con Admin**. Il [strumento](https://experienceleague.adobe.com/en/docs/commerce-operations/tools/site-wide-analysis-tool/access) fornisce approfondimenti di sistema e strumentazione per Adobe Systems su infrastruttura cloud installazioni con monitoraggio delle prestazioni in tempo reale, report e raccomandazioni self-service 24 ore su 24, 7 giorni su 7. Gli esercenti possono utilizzare la nuova risorsa](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-user-roles#role-resources) per il ruolo Amministratore [per accesso in modo sicuro le pagine dei dettagli cliente tramite l&#39;amministratore. Vedi le [FAQ](https://support.magento.com/hc/en-us/articles/360048646671) per una panoramica. <!-- SWAT-807-->

### Miglioramenti delle prestazioni

* **Riduzione delle dimensioni dei trasferimenti di rete tra Redis e Magento**. La configurazione dell&#39;elenco dei plug-in viene ora generata durante l&#39;esecuzione `bin/magento di:compile` del comando. Queste informazioni di configurazione vengono scritte in cartelle metadati generate in base all&#39;ambito. In precedenza, queste informazioni venivano memorizzate nella cache. I miglioramenti delle prestazioni risultanti includono una riduzione delle dimensioni della cache di rete e del tempo di esecuzione per molti scenari.<!-- MC-31617-->
...
### Galleria multimediale Nuovo

La Nuovo Media Gallery è ora abilitata per impostazione predefinita in Amministratore.I commercianti possono ora eseguire queste azioni sulle immagini in Media Gallery:
...
Page Builder ora supporta la modalità a tutto schermo, che supporta una modifica più semplice dei contenuti e offre un’esperienza di modifica coerente per l’amministratore. Vedi [Workspace](https://experienceleague.adobe.com/en/docs/commerce-admin/page-builder/walkthrough/3-catalog-content). <!-- PB-543-->

### GraphQL

Questa versione aggiunge la copertura GraphQL per le seguenti funzioni:

* **Recensioni prodotto**. Clienti e ospiti possono scrivere recensioni sui prodotti. I clienti possono recuperare la cronologia delle recensioni dei prodotti. Vedere [Crea una recensione](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-product-review.html) di prodotto e [una query](https://developer.adobe.com/commerce/webapi/graphql/queries/product-review-ratings-metadata.html) productReviewRatingsMetadata per informazioni sul recupero delle informazioni sull&#39;infrastruttura di recensioni.<!-- MC-32349-->

* **Opzioni regalo**. Tutti i clienti e gli ospiti possono aggiungere un messaggio regalo al loro ordine. Nelle installazioni di Adobe Systems Commerce, possono anche aggiungere confezioni regalo, ricevute regalo e biglietti stampati all&#39;ordine. Vedi [`setGiftOptionsOnCart` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/set-gift-options.html) e [`updateCartItems` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-cart-items.html) <!-- MC-32345-->

* **Punti premio**. I clienti possono applicare o rimuovere punti premio dai propri carrelli. Possono anche visualizzare la cronologia dei punti premio. Vedere [`applyRewardPointsToCart`](https://developer.adobe.com/commerce/webapi/graphql/mutations/apply-reward-points.html) e [`removeRewardPointsFromCart`](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-reward-points.html) per una discussione sulla gestione dei punti premio all&#39;interno di un carrello.<!-- MC-23366-->
...
* È stata aggiunta la mutazione [`requestPasswordResetEmail`](https://developer.adobe.com/commerce/webapi/graphql/mutations/request-password-reset-email.html), che attiva l&#39;e-mail di reimpostazione della password per l&#39;indirizzo e-mail specificato. _Correzione inviata da Oleh Usik nella richiesta di pull [27876](https://github.com/magento/magento2/pull/27876)_. [GitHub-28521](https://github.com/magento/magento2/issues/28521)

* **Klarna GraphQL**. Sono stati aggiunti o aggiornati argomenti su Klarna GraphQL nel [metodo di pagamento di Klarna](https://developer.adobe.com/commerce/webapi/graphql/payment-methods/klarna.html) e [`createKlarnaPaymentsSession`](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-klarna-payments-session.html)
...
* **Approvazioni e rifiuti di ordini in blocco**. I manager B2B e gli amministratori aziendali possono ora eseguire il rifiuto in blocco e l&#39;approvazione degli ordini di acquisto. Queste modifiche consentono agli approvatori di approvare o rifiutare più ordini di acquisto in un&#39;unica azione. <!-- BUNDLE-140 128 111 153 -->

* Gli esercenti possono ora cercare i campi **Si applica a** e **Richiede l&#39;approvazione da** della visualizzazione Ordini di acquisto personali e possono selezionare più ruoli utente durante la creazione della regola. <!-- BUNDLE-105 106-->
...
Vedi [Regole di approvazione](https://experienceleague.adobe.com/en/docs/commerce-admin/b2b/purchase-orders/account-dashboard-approval-rules)
...
Consulta [Funzioni B2B](https://experienceleague.adobe.com/en/docs/commerce-admin/config/general/b2b-features).

#### Sicurezza avanzata nella vetrina

Per ridurre la creazione di account fraudolenti o spam, gli esercenti possono ora abilitare Google reCAPTCHA nel modulo di richiesta della nuova azienda sulla vetrina. Vedi [reCAPTCHA](https://experienceleague.adobe.com/en/docs/commerce-admin/config/security/google-recaptcha-storefront).<!-- BUNDLE-154 -->

#### Registrazione estesa delle azioni amministratore

Le azioni di amministrazione eseguite nei moduli aziendali vengono ora registrate nel registro delle azioni di amministrazione. Le azioni vengono registrate da tutti i moduli aziendali rilevanti: `Company`, `NegotiableQuote`, `CompanyCredit`, `SharedCatalog`.
<!-- BUNDLE-180 181 182 183 -->

Questa versione include anche diverse correzioni di bug. Consulta [B2B note](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html) sulla versione.

### Framework di test funzionali (MFTF)

MFTF 3.1.0 è ora disponibile. Consulta [Changelog framework test funzionali](https://github.com/magento/magento2-functional-testing-framework/blob/develop/CHANGELOG.md).
...
* [Vertici cloud](https://marketplace.magento.com/vertexinc-vertex-tax-module.html)

* [Recensioni prodotti Yotpo](https://marketplace.magento.com/yotpo-module-yotpo.html)
...
* L&#39;installazione con estensioni di terze parti che hanno dipendenze dalle API per il modulo Store nei comandi CLI non riesce più. In precedenza, il applicazione visualizzato questo messaggio di errore: `The default website isn't defined. Set the website and try again`. Si trattava di un problema noto nella versione 2.4.0.
...
* L&#39;aggiornamento non riesce più quando viene dichiarato un plugin su `Magento\Framework\Encryption\Encryptor`.

<!-- MC-36231-->

* Il applicazione ora visualizza un messaggio di errore informativo quando alcuni temi non vengono distribuiti dopo l&#39;esecuzione `bin/magento setup:static-content:deploy`di . In precedenza, quando la distribuzione veniva completata correttamente ma non tutti i pacchetti venivano distribuiti, il applicazione non visualizzava un errore. Quando questo comando viene eseguito con l&#39;elaborazione parallela abilitata e ogni tema richiede più tempo per essere distribuito rispetto al tempo di esecuzione massimo specificato, questo comando può essere completato correttamente, anche se i temi non vengono distribuiti.

<!-- MC-35001-->

* La **casella di controllo Usa predefinito** per i pagamenti Klarna (**Negozi** > **configurazione** > **Vendite** > **metodi** di pagamento > **Klarna**) ora rimane selezionata come previsto quando il sito Web ambito cambia.

<!-- MC-17959 ENGCOM-7629-->

* L&#39;esecuzione `/bin/magento config:show vendor_module/general/value` ora restituisce `0` o una stringa vuota come previsto. In precedenza, ha restituito `Configuration for path: "vendor_module/general/value" doesn't exist`. _Correzione inviata da Vadim Malesh nel richiamare richiesta [28549](https://github.com/magento/magento2/pull/28549)._[GitHub-23290](https://github.com/magento/magento2/issues/23290)
...
* La disattivazione del modulo PageBuilder non influisce più sul rendering della pagina prodotto. In precedenza, i layout personalizzati nella pagina di prodotto scomparivano quando il modulo veniva disabilitato e il applicazione visualizzava una pagina vuota.

<!-- ENGCOM-7219-->

* È ora possibile utilizzare `bin/magento sampledata:deploy` per distribuire i dati di esempio come previsto dopo l&#39;installazione di Adobe Systems Commerce utilizzando Composer. In precedenza, il applicazione generava questo errore: `Git installations must deploy sample data from GitHub; see https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/next-steps/sample-data/git-repositories for more information`. _Correzione presentata da Andrii Beziazychnyi nel richiamare richiesta [27481](https://github.com/magento/magento2/pull/27481)_. [GitHub-19481](https://github.com/magento/magento2/issues/19481)

<!-- ENGCOM-7459-->

* Le prestazioni della vetrina sono migliorate eliminando il caricamento non necessario del `Datepicker` componente. _Correzione inserita da Mateusz Krzeszowiak nel richiamare richiesta [27860](https://github.com/magento/magento2/pull/27860)_. [GitHub-28823](https://github.com/magento/magento2/issues/28823)
...
* L&#39;impostazione **Crea Reindirizzamento permanente per le vecchie URL** è ora disabilitata per impostazione predefinita per le categorie. _Correzione inviata da Vadim Malesh nel richiamare richiesta [28752](https://github.com/magento/magento2/pull/28752)_. [GitHub-24922](https://github.com/magento/magento2/issues/24922)

### AdminGWS

<!-- MC-36164-->

* Il applicazione non visualizza più il **pulsante Aggiungi attributo** (**Archivia** > **attributi** o **Aggiungi set** di attributi pulsante (**Archivia** > attributi **>****Cliente**) quando l&#39;amministratore connesso non dispone delle autorizzazioni appropriate per creare tali entità. In precedenza, il applicazione generava un errore 404 quando un amministratore di siti Web che non disponeva delle autorizzazioni appropriate tentava di creare un set **di attributi o** un **attributo cliente**.
...
* Il `Login failed` messaggio visualizzato dall&#39;applicazione quando un commerciante fa clic su **Licenza** per un&#39;immagine Adobe Stock precedentemente salvata e senza licenza non contiene più tag HTML. _Correzione inserita da yolouiese in richiamare richiesta [29398](https://github.com/magento/magento2/pull/29398)_. [GitHub-1684](https://github.com/magento/adobe-stock-integration/issues/1684)

<!-- ENGCOM-7976 -->

* Facendo clic sui collegamenti nella **sezione Usato in** della pagina Dettagli immagine viene visualizzata una griglia contenente tutte le entità filtrate dall&#39;immagine. Anche il filtro delle risorse viene impostato e visualizzato correttamente. In precedenza, l&#39;applicazione non visualizzava il titolo della risorsa nella sezione **Filtri applicati**. _Correzione inviata da Nazar Klovanych nella richiesta di pull [29367](https://github.com/magento/magento2/pull/29367)_. [GitHub-1694](https://github.com/magento/adobe-stock-integration/issues/1694)
...
* L’applicazione ora aggiunge correttamente i tag quando si modificano più immagini in successione in Media Gallery. _Correzione presentata da Nazar Klovanych nel richiamare richiesta [29429](https://github.com/magento/magento2/pull/29429)_. [GitHub-1755](https://github.com/magento/adobe-stock-integration/issues/1755)

<!-- ENGCOM-8015-->

* Il applicazione ora rimuove i tag per Adobe Stock immagini dopo che un commerciante li elimina e salva i dettagli dell&#39;immagine. In precedenza, i tag venivano eliminati solo dopo l’aggiornamento della pagina. _Correzione inviata da Honeymay Louiese Ignacio nella richiesta di pull [29400](https://github.com/magento/magento2/pull/29400)_. [GitHub-1703](https://github.com/magento/adobe-stock-integration/issues/1703)
...
* Gli errori del browser non si verificano più durante il checkout sui dispositivi desktop quando Venmo è abilitato dall&#39;amministratore.

* L&#39;estrazione non riesce più quando l&#39;acquirente immette caratteri speciali nei campi Nome **dell&#39;workflow** di estrazione. In precedenza, l&#39;autenticazione non riusciva perché l&#39;API Braintree 3DS non supportava caratteri non ASCII.

* Il applicazione ora visualizza il nome del destinatario corretto nella sezione di spedizione della workflow di checkout quando si effettua un ordine utilizzando PayPal.
...
* I commercianti possono ora creare una nota di credito per i prodotti in bundle che fornisce un rimborso senza richiedere la restituzione del prodotto. In precedenza, il applicazione generava un errore. _Correzione inviata da Dzung Nguyen in richiamare richiesta [27455](https://github.com/magento/magento2/pull/27455)_. [GitHub-23440](https://github.com/magento/magento2/issues/23440)

<!-- ENGCOM-7655-->

* Il applicazione non visualizza più messaggi di convalida ridondanti quando un acquirente aggiunge un prodotto in bundle al carrello senza selezionare un&#39;opzione richiesta. _Correzione inviata da Dzung Nguyen in richiamare richiesta [27455](https://github.com/magento/magento2/pull/27455)_. [GitHub-23440](https://github.com/magento/magento2/issues/23440)
...
* Il applicazione non genera più un errore quando si tenta di ordinare un prodotto referenza di magazzino quando le cifre inserite corrispondono a un referenza di magazzino valido, ma il caso di queste cifre è diverso. In precedenza, quando si immetteva un referenza di magazzino su **Il mio account** > **Order by referenza di magazzino** che non corrispondeva esattamente a un referenza di magazzino valido, il applicazione generava un errore.

<!-- MC-25042-->

* L&#39;indirizzo di spedizione di un cliente viene ora selezionato per impostazione predefinita al momento del pagamento quando l&#39;indirizzo si trova nel paese identificato nell&#39;elenco Consenti paesi e tale elenco include solo quel paese. In precedenza, il applicazione non selezionava l&#39;indirizzo come predefinito e visualizzava questo messaggio di errore: `Please specify a regionId in shipping address`.
...
* Gli attributi dell&#39;indirizzo personalizzati sono ora inclusi, come previsto, nel modulo visualizzato per la fase di pagamento nella workflow di pagamento.

<!-- MC-36060-->

* La **casella di immissione Stato/Provincia/Regione** è ora abilitata come previsto in **Il mio account** Rubrica **>**> **Aggiungi nuovo indirizzo**.

<!-- ENGCOM-7746-->

* Gli sconti vengono ora applicati come previsto alle spese di spedizione quando **Applica all&#39;importo** della spedizione è abilitato. _Correzione presentata da Andrii Kalinich nel richiamare richiesta [28839](https://github.com/magento/magento2/pull/28839)_. [GitHub-26723](https://github.com/magento/magento2/issues/26723)

<!-- ENGCOM-7752-->

* Il codice che supporta la chiusura del mini carrello è stato rielaborato per rimuovere la funzione `closeSidebar`. L&#39;associazione di clic appropriata è stata aggiunta all&#39;elemento `[data-action="close"]` . _Correzione inserita da lumnn in richiamare richiesta [28906](https://github.com/magento/magento2/pull/28906)_. [GitHub-29161](https://github.com/magento/magento2/issues/29161)

<!-- ENGCOM-7585-->

* La nuova **Mostra &quot;Cancella carrello&quot; pulsante sull&#39;impostazione di configurazione della pagina** del carrello fornisce il controllo sulla visualizzazione di un **pulsante Cancella carrello** nella pagina di visualizzazione del carrello. Per impostazione predefinita, questa impostazione è disabilitata. _Correzione inserita da Pavlo Sydorenko nel richiamare richiesta [27917](https://github.com/magento/magento2/pull/27917)_. [GitHub-28705](https://github.com/magento/magento2/issues/28705)

<!-- ENGCOM-7457-->

* La convalida è stata aggiunta al campo telefono nella workflow di pagamento. _Correzione inviata da Oleh Usik nella richiesta di pull [27537](https://github.com/magento/magento2/pull/27537)_. [GitHub-28800](https://github.com/magento/magento2/issues/28800)
...
* È ora possibile eseguire correttamente azioni di massa sulle scorte dei prodotti di magazzino. In precedenza, quando si tentava di eseguire un&#39;azione di massa sulle scorte di prodotti di inventario, il applicazione visualizzava una pagina vuota. Se hai eseguito questa azione con il applicazione in modalità sviluppatore, il applicazione generato questo errore: `Notice: Undefined offset: 32000 in /Users/kodithuw/sites/m23inventory/inventory/InventoryCatalogAdminUi/view/adminhtml/templates/catalog/product/edit/action/inventory.phtml on line 24`

<!-- MC-32552-->

* Il conteggio totale dei record visualizzato quando si fa clic su **Aggiungi prodotti** nella cartella Prodotti scheda quando si aggiungono prodotti a una categoria di catalogo non cambia più in base all&#39;ordinamento dei prodotti.

<!-- ENGCOM-7854 -->

* La `{products(filter: {sku: {eq: "some sku"}}) {…}` query restituisce ora i valori che sono stati convertiti nella valuta prevista. In precedenza, `price_tiers.final_price.value` visualizzava prezzi speciali nella valuta di base. _Correzione presentata da Petkovski Marjan nel richiamare richiesta [28890](https://github.com/magento/magento2/pull/28890)_. [GitHub-26121](https://github.com/magento/magento2/issues/26121)
...
* È stato corretto il nome del `getRegionNameExpresion` metodo in `getRegionNameExpression`. _Correzione presentata da Pierre Grimaud in richiamare richiesta [28832](https://github.com/magento/magento2/pull/28832)_. [GitHub-28829](https://github.com/magento/magento2/issues/28829)

<!-- ENGCOM-7657-->

* Un metodo ridondante `init` è stato rimosso da `app/code/Magento/AdvancedPricingImportExport/Model/Import/AdvancedPricing/Validator/Website.php` e `app/code/Magento/AdvancedPricingImportExport/Model/Import/AdvancedPricing/Validator/TierPriceType.php`. _Correzione inviata da Oleh Usik nella richiesta di pull [28650](https://github.com/magento/magento2/pull/28650)_. [GitHub-29009](https://github.com/magento/magento2/issues/29009)
...
* L’applicazione ora genera un errore quando un esercente crea una pagina CMS con lo stesso URL della pagina Struttura società. In precedenza, l&#39;applicazione visualizzava la pagina CMS invece della pagina Struttura società.
...
* Il sintetico ordine ora visualizza l&#39;importo dello sconto corretto quando è stata applicata una regola del prezzo del carrello. In precedenza, il regola non arrotondava correttamente gli importi nel calcolo degli sconti sulla spedizione.

<!-- MC-33765-->

* Gli account utente di amministrazione creati da un account amministratore con un ambito limitato possono ora creare un prodotto configurabile con gli attributi previsti. In precedenza, il applicazione generava questo errore: `Notice: Undefined index: value_index in 23develop/app/code/Magento/ConfigurableProduct/Helper/Product/Options/Factory.php on line 101`.

<!-- ENGCOM-7787-->

* Il applicazione non genera più un errore convalida quando si utilizza POST `/V1/products` a un prodotto configurabile con un `int` valore pari a 0. In precedenza, il applicazione generava questo errore: `Product with id "%1" does not contain required attribute "%2"."` Correzione inviata da Vadim Malesh in richiamare richiesta [29001](https://github.com/magento/magento2/pull/29001)_._ [GitHub-13210](https://github.com/magento/magento2/issues/13210)
...
* Invia messaggio coda configurazione consumer è stata estesa con nuovi parametri che aiutano a controllare i consumatori e risparmiare risorse del server e che potenzialmente riducono il consumo di CPU coda consumer. Vedere [Configurare le](https://developer.adobe.com/commerce/php/development/components/message-queues/configuration/) code di messaggi per una descrizione di `maxIdleTime`, `sleep`e `onlySpawnWhenMessageAvailable` dei parametri.

<!-- ENGCOM-7863-->

* `cron` Tali espressioni sono `3/10 * * * *` ora pianificate come previsto. _Correzione inviata da Anton Evers in richiamare richiesta [28930](https://github.com/magento/magento2/pull/28930)_. [GitHub-29240](https://github.com/magento/magento2/issues/29240)
...
* La workflow di checkout non visualizza più i valori degli attributi dell&#39;indirizzo cliente personalizzati quando il cliente non ha inserito alcun dato.

### Cliente

<!-- MC-33679-->

* I nomi dei area geografica negli indirizzi dei clienti Admin vengono ora tradotti come previsto.

<!-- MC-36226-->

* I **campi Stato/Provincia** vengono ora compilati come previsto nella pagina Indirizzo Modifica (**Il mio account** > **Rubrica**).

<!-- MC-34655-->

* Il applicazione non genera più un errore quando un cliente fa clic più volte sul **pulsante Invia** sui moduli in tutto il negozio per i quali è stato abilitato il reCAPTCHA invisibile. In precedenza, facendo clic più volte su questa pulsante si verificava un errore interno simile al seguente: `Internal error: Make sure you are using reCaptcha V3 api keys`.

<!-- MC-33522-->

* Il salvataggio di un cliente eliminato dall&#39;amministratore ora genera solo un messaggio di errore. In precedenza, il applicazione visualizzava una pagina vuota e generava un rapporto contenente la stringa seguente: `"0":"No such entity with customerId = 3","1":"#1 Magento\\Customer\\Model CustomerRegistry->retrieve() called at [app\/code\/Magento\/Customer\/Model\/ResourceModel\/CustomerRepository.php:340"`.

<!-- MC-33357-->

* Il applicazione ora visualizza un messaggio di errore come previsto quando un amministratore tenta di salvare un indirizzo per un cliente il cui account è stato appena eliminato. In precedenza, nel applicazione veniva visualizzata una finestra di messaggio vuota.

<!-- MC-33150-->

* Le etichette per i campi indirizzo nel workflow di check-out e nella rubrica sono state modificate per coerenza.

<!-- ENGCOM-7244 -->

* Le etichette front-end ora rientrano nelle etichette store se i loro valori non sono nulli. In precedenza, gli attributi del cliente utilizzavano le etichette front-end predefinite. _Correzione inserita da Toan Nguyen in richiamare richiesta [27064](https://github.com/magento/magento2/pull/27064)_. [GitHub-27063](https://github.com/magento/magento2/issues/27063)

<!-- ENGCOM-7147 -->

* Il `sortOrder`numero di collegamenti nel layout XML è stato corretto. In precedenza, questo ordine veniva invertito e utilizzato l&#39;ordine decrescente. _Correzione inviata da Tu Nguyen in richiamare richiesta [27340](https://github.com/magento/magento2/pull/27340)_. [GitHub-27162](https://github.com/magento/magento2/issues/27162)

### Segmento di clientela

<!-- MC-33184-->

* Le operazioni in blocco sono state sottoposte a refactoring per salvare e aggiornare i segmenti di clienti in modo asincrono, il che ha migliorato le prestazioni di queste attività per le distribuzioni che includono molti clienti (superiori a 3.000.000).

<!-- MC-36224-->

* Le condizioni del segmento di clienti ora funzionano come previsto in una distribuzione di database divisi. In precedenza, il applicazione generava un errore quando si tentava di modificare un segmento di clienti aggiungendo una condizione: `SQLSTATE[42S02]: Base table or view not found`.

### Directory

<!-- MC-32271-->

* Il formato del menu a discesa Stato/Provincia è ora coerente in tutto l&#39;amministratore.

### dotdigital

* L&#39;ordine Sincronizzazione non riesce più quando un ordine contiene SKU di prodotto che non esistono più nel catalogo.

* Le categorie di prodotti vuote non sono più incluse nei dati della informazione approfondita Web.

* Il monitoraggio del comportamento web ora funziona per i commercianti con determinate configurazioni tematiche. Un nuovo selettore di fallback risolve questo problema.
...
* Il metodo obsoleto `addWarning` è stato sostituito con il `addWarningMessage` metodo nel modulo di sicurezza. _Correzione inserita da kishorekumarkesavan in richiamare richiesta [28264](https://github.com/magento/magento2/pull/28264)_. [GitHub-28308](https://github.com/magento/magento2/issues/28308)

<!-- ENGCOM-7588-->

* Code generati utilizzando i comandi della riga di `bin/magento` comando sono ora coerenti con i requisiti e gli standard di codifica. _Correzione inserita da Lukasz Bajsarowicz nel richiamare richiesta [28351](https://github.com/magento/magento2/pull/28351)_. [GitHub-28376](https://github.com/magento/magento2/issues/28376)

<!-- MC-33744-->

* Il applicazione non visualizza più di una pagina CMS più di una volta nella gerarchia del sito se la pagina è assegnata a più visualizzazioni store.

<!-- ENGCOM-7511-->

* Il salvataggio di un attributo con `backend_type = static` non rimuove più il contenuto del campo `frontend_class`. _Correzione inserita da jiten-patel in richiamare richiesta [27369](https://github.com/magento/magento2/pull/27369)_. [GitHub-27051](https://github.com/magento/magento2/issues/27051)
...
* Il feed RSS ora viene caricato correttamente. In precedenza, il feed non veniva caricato la prima volta, anche se veniva caricato come previsto al momento dell’aggiornamento della pagina. _Correzione inviata da Vadim Malesh nel richiamare richiesta [29455](https://github.com/magento/magento2/pull/29455)_. [GitHub-25211](https://github.com/magento/magento2/issues/25211)

<!-- MC-35550-->

* Una sessione persistente scaduta viene rinnovata come previsto quando il acquirente esegue nuovamente l&#39;accesso.

<!-- MC-35230-->

* Password a vita impostato in Archivi > configurazione > ora viene onorato Avanzate **amministratore >****.********** In precedenza, se si faceva clic su Dimenticato **password?** quando viene richiesto di reimpostare la password, è possibile ignorare la reimpostazione della password.
...
* Gli acquirenti possono selezionare il messaggio regalo e le opzioni di imballaggio durante il pagamento. GraphQL ora descrive le opzioni dei messaggi regalo per diversi tipi di articoli del carrello. Vedere [`setGiftOptionsOnCart` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/set-gift-options.html) e [`updateCartItems` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-cart-items.html). _Correzione inviata da Oleh Usik nelle richieste pull [28519](https://github.com/magento/magento2/pull/28105), [27956](https://github.com/magento/magento2/pull/27956), [28072](https://github.com/magento/magento2/pull/28072), [28072](https://github.com/magento/magento2/pull/28072) e 246 nei partner repository privati-magento2ee_. [GitHub-253](https://github.com/magento/magento2/issues/28519)

<!-- ENGCOM-7508 -->

* Clienti e ospiti possono scrivere recensioni sui prodotti. I clienti possono anche recuperare la cronologia delle recensioni dei prodotti. Vedere [Crea una recensione](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-product-review.html) di prodotto e [una query](https://developer.adobe.com/commerce/webapi/graphql/queries/product-review-ratings-metadata.html) productReviewRatingsMetadata per informazioni sul recupero delle informazioni sull&#39;infrastruttura di recensioni. _Correzione presentata da Eduard Chitoraga in richiamare richiesta [27882](https://github.com/magento/magento2/pull/27882)_. [GitHub-28523](https://github.com/magento/magento2/issues/28523)

<!-- ENGCOM-7800 7841-->

* I clienti possono applicare o rimuovere punti premio dai propri carrelli. Possono anche visualizzare la cronologia dei punti premio. Per una discussione sulla gestione dei punti premio all&#39;interno di un carrello, vedere [`applyRewardPointsToCart`](https://developer.adobe.com/commerce/webapi/graphql/mutations/apply-reward-points.html) e [`removeRewardPointsFromCart`](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-reward-points.html). _Correzione inviata da Petkovski Marjan nella richiesta di pull 285 nei partner repo privati-magento2ee e Dmitriy Gallyamov nelle richieste di pull 284 e 281 nei partner repo privati-magento2ee_. [GitHub-28835](https://github.com/magento/magento2/issues/28835), [GitHub-28833](https://github.com/magento/magento2/issues/28833)

<!-- ENGCOM-7968 -->

* La [`addProductsToCart` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-cart.html) permette di aggiungere qualsiasi tipo di prodotto al carrello attivo. Si consiglia di utilizzare questa mutazione invece di mutazioni monoscopo come `addSimpleProductsToCart`. _Correzione presentata da Yaroslav Rogoza nel richiamare richiesta [27914](https://github.com/magento/magento2/pull/27914)_. [GitHub-28524](https://github.com/magento/magento2/issues/28524)

<!-- ENGCOM-7801 7816-->

* Le query GraphQL per i prodotti correlati ora restituiscono valori per i prodotti correlati creati da regole destinazione. In precedenza, le query per i prodotti correlati (up-selling e effettuare azioni di cross-selling) restituivano valori solo se i prodotti correlati venivano aggiunti dalle impostazioni del prodotto. _Correzione presentata da Ulzii in richiamare richiesta 288 in repo partner privati-magento2ee_. [GitHub-28566](https://github.com/magento/magento2/issues/28566)

<!-- MC-34187-->

* La nuova query `availableStores` restituisce un elenco di attributi di configurazione per più archivi disponibili nello stesso sito Web (in base all&#39;archivio corrente). Non espone l’elenco dei siti web. Vedi [`availableStores` query](https://developer.adobe.com/commerce/webapi/graphql/queries/available-stores.html). _Correzione inviata da Dmitriy Gallyamov nella richiesta di pull [28794](https://github.com/magento/magento2/pull/28794)_. [GitHub-28569](https://github.com/magento/magento2/issues/28569)
...
* È possibile utilizzare la mutazione `subscribeEmailToNewsletter` per sottoscrivere i clienti a una newsletter. Vedere [`subscribeEmailToNewsletter` mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/subscribe-email-to-newsletter.html). _Correzione inviata da Alexander Taranovsky nella richiesta di pull [27586](https://github.com/magento/magento2/pull/27586)_. [GitHub-27337](https://github.com/magento/magento2/issues/27337)
...
* È stato aggiunto il supporto per le liste dei desideri in Magento Open Source. Puoi [aggiungere, [[aggiornare e](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-wishlist.html) rimuovere articoli](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-products-in-wishlist.html)](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-products-from-wishlist.html) da una lista dei desideri. _Correzione presentata da Eduard Chitoraga in richiamare richieste [28205](https://github.com/magento/magento2/pull/28205) e 264 in private repo partners-magento2ee_. [GitHub-28551](https://github.com/magento/magento2/issues/28551)
...
* Aggiunta della mutazione [`resetPassword`](https://developer.adobe.com/commerce/webapi/graphql/mutations/reset-password.html). _Correzione inviata da Oleh Usik nella richiesta di pull [27876](https://github.com/magento/magento2/pull/27876)_. [GitHub-28521](https://github.com/magento/magento2/issues/28521)

* È stata aggiunta la mutazione [`requestPasswordResetEmail`](https://developer.adobe.com/commerce/webapi/graphql/mutations/request-password-reset-email.html), che attiva l&#39;e-mail di reimpostazione della password per l&#39;indirizzo e-mail specificato. _Correzione inviata da Oleh Usik nella richiesta di pull [27876](https://github.com/magento/magento2/pull/27876)_. [GitHub-28521](https://github.com/magento/magento2/issues/28521)

* Aggiunta discussione su Klarna GraphQL nel [metodo di pagamento di Klarna](https://developer.adobe.com/commerce/webapi/graphql/payment-methods/klarna.html) e[`createKlarnaPaymentsSession`](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-klarna-payments-session.html)
...
* Il `error_report.csv` file ora viene scaricato con contenuto ed è disponibile all&#39;interno della `var/import_history/` directory come previsto. In precedenza, questo file non veniva generato dopo l&#39;importazione.

<!-- ENGCOM-7616-->

* L&#39;importazione di un prodotto tramite un file CSV genera ora un `error_report.csv` file come previsto. In precedenza, l&#39;applicazione generava il file ma lo rimuoveva al termine dell&#39;importazione. _Correzione inviata da Vadim Malesh in richiamare richiesta [28460](https://github.com/magento/magento2/pull/28460)_. [GitHub-28420](https://github.com/magento/magento2/issues/28420)

<!-- ENGCOM-7673-->

* Sono state rimosse le importazioni di classi ridondanti in tutta la codebase. _Correzione inviata da Oleh Usik nella richiesta di pull [28696](https://github.com/magento/magento2/pull/28696)_. [GitHub-29012](https://github.com/magento/magento2/issues/29012)
...
* La posizione dei prodotti nella tabella `catalog_category_product` ora viene aggiornata come previsto quando un amministratore crea un prodotto nell&#39;amministratore e lo assegna a una categoria. In precedenza, alla posizione dei nuovi prodotti veniva sempre assegnato un valore 0.
...
* Sono stati rimossi i parametri di costruzione non utilizzati in `AdvancedPricing.php`. [GitHub-29531](https://github.com/magento/magento2/issues/29531)

### Indice

<!-- ENGCOM-7073-->

* `Magento_CacheInvalidate` ora gestisce correttamente i pattern di tag di grandi dimensioni quando si esegue un&#39;operazione `PURGE`. `sendPurgeRequest` è stato modificato per gestire un array di tag invece di richiedere al chiamante di utilizzare `implode()`. _Correzione presentata da Matthew O&#39;Loughlin nel richiamare richiesta [26256](https://github.com/magento/magento2/pull/26256)_. [GitHub-26255](https://github.com/magento/magento2/issues/26255)
...
* Non si verificano più problemi durante il caricamento delle pagine di catalogo e di prodotto nelle implementazioni che eseguono PHP 7.4.9. In precedenza, il applicazione generava questo errore quando si tentava di caricare le pagine del catalogo e dei prodotti: `There has been an error processing your request. Exception printing is disabled by default for security reasons`. [GitHub-29502](https://github.com/magento/magento2/issues/29502)

<!-- ENGCOM-7994-->

* Tutte le eccezioni che si verificano quando si esegue il rendering dei layout in modalità di produzione vengono ora registrate nel file di registro delle eccezioni (`var/report`). In precedenza, l&#39;applicazione registrava questi messaggi nel registro di sistema come critico problemi.

<!-- ENGCOM-7154-->

* Le espressioni regolari ora funzionano correttamente per pagine di grandi dimensioni come risultato di un aumento di `ipcre.backtrack_limi`t e `pcre.recursion_limit` a circa 1000000. _Correzione inserita da Mateusz Krzeszowiak in richiamare richiesta [27270](https://github.com/magento/magento2/pull/27270)_. [GitHub-26026](https://github.com/magento/magento2/issues/26026)

<!-- ENGCOM-7483-->

* La generazione di intercettori è stata migliorata. `} else {` Le dichiarazioni sono state rimosse dagli intercettori e `array_map` hanno sostituito `foreach`. _Correzione inviata da Lukasz Bajsarowicz nella richiesta di pull [27902](https://github.com/magento/magento2/pull/27902)_. [GitHub-28383](https://github.com/magento/magento2/issues/28383)
...
* I dati del modulo ora persistono quando il applicazione genera un&#39;eccezione di integrazione quando si salva un&#39;integrazione utilizzando un nome già in uso. _Correzione inserita da Aditya Yadav in richiamare richiesta [26660](https://github.com/magento/magento2/pull/26660)_. [GitHub-28143](https://github.com/magento/magento2/issues/28143)

<!-- ENGCOM-7756-->

* Il applicazione non tronca `X-Forwarded-For` più le intestazioni a 32 caratteri. _Correzione inserita da Ihor Sviziev nel richiamare richiesta [27221](https://github.com/magento/magento2/pull/27221)_. [GitHub-28693](https://github.com/magento/magento2/issues/28693)
...
* È stato aggiunto il seguente supporto per i metodi magici per `DataObject`:
   ...
   * Nuova estensione per supportare `get/set/has/uns` i metodi magici (con l&#39;utilizzo di `__call`)
...
* Commenti di codice non necessari rimossi da `app/code/Magento/CatalogInventory/Model/StockState.php`. _Correzione inserita da Vitaliy Prokopov in richiamare richiesta [27758](https://github.com/magento/magento2/pull/27758)_. [GitHub-26702](https://github.com/magento/magento2/issues/26702)
...
* Sono stati aggiunti test per coprire i collegamenti **Usati in** per un&#39;immagine nella raccolta multimediale. _Correzione inviata da Nazar Klovanych nella richiesta di pull [29392](https://github.com/magento/magento2/pull/29392)_. [GitHub-1963](https://github.com/magento/adobe-stock-integration/issues/1693)
...
* È stato aggiunto il test per la `StorefrontShareCustomerWishlistActionGroup` condivisione della lista dei desideri dei clienti con non valido indirizzi e-mail. Il test esistente per la condivisione della lista dei desideri (`StorefrontShareWishlistEntityTest`) è stato sottoposto a nuovo fattorizzazione. `StorefrontCustomerShareWishlistActionGroup` è obsoleto.

<!-- ENGCOM-7682 -->

* È stato aggiunto un test per verificare l&#39;implementazione dell&#39;impostazione di configurazione **Only X left Threshold**. _Correzione presentata da Oleh Usik nel richiamare richiesta [27549](https://github.com/magento/magento2/pull/27549)_. [GitHub-28755](https://github.com/magento/magento2/issues/28755)
...
* Aggiunge un test per l’eliminazione del sottoscrittore di una newsletter come utente amministratore. _Correzione inviata da Dmitry Tsymbal nel richiamare richiesta [28972](https://github.com/magento/magento2/pull/28972)_. [GitHub-29032](https://github.com/magento/magento2/issues/29032)

<!-- ENGCOM-7739-->

* Sono stati aggiunti test per l’abbonamento a newsletter per gli ospiti con un’opzione non consentita nelle istruzioni di configurazione. Questo test sostituisce i valori obsoleti `VerifyRegistredLinkDisplayedForGuestSubscriptionNoTest` e `StorefrontCreateNewSubscriberActionGroup`. _Correzione inviata da Dmitry Tsymbal nel richiamare richiesta [28872](https://github.com/magento/magento2/pull/28872)_. [GitHub-29039](https://github.com/magento/magento2/issues/29039)

### Ordini

<!-- ENGCOM-7858-->

* `GetAssetIdByContentFieldInterface` e la relativa implementazione nei moduli `MediaContent` ora consentono all&#39;integrazione Adobe Stock di estendere la funzionalità filtro `MediaGallery`. _Correzione inserita da Gabriel da Gama nel richiamare richiesta [29058](https://github.com/magento/magento2/pull/29058)_. [GitHub-1464](https://github.com/magento/magento2/issues/1464)

<!-- ENGCOM-7885-->

* I valori `CustomerAddressI` per un cliente appena creato ora sono convalidati tra virgolette. _Correzione inviata da Andrii Kalinich nella richiesta di pull [29139](https://github.com/magento/magento2/pull/29139)_. [GitHub-28793](https://github.com/magento/magento2/issues/28793)

<!-- ENGCOM-7798-->

* Lo stato dell&#39;ordine per una nota di credito con zero totale è ora `Closed`. In precedenza, l&#39;applicazione segnalava lo stato dell&#39;ordine come `Complete`. _Correzione presentata da Andrii Kalinich nel richiamare richiesta [29023](https://github.com/magento/magento2/pull/29023)_. [GitHub-22762](https://github.com/magento/magento2/issues/22762)

### Pagina Builder

<!-- ENGCOM-7918-->

* Gli `description` attributi e `short_description` il prodotto sono stati riordinati per adattarsi meglio alle modifiche pianificate per Pagina Builder contenuto la fase di staging. _Correzione inviata da Matt Walters nel richiamare richiesta [29238](https://github.com/magento/magento2/pull/29238)_. [GitHub-543](https://github.com/magento/magento2-page-builder/issues/543)
...
* La pagina di revisione dell&#39;ordine del workflow di checkout ora visualizza l&#39;importo di spedizione corretto per PayPal tramite Braintree ordini per i quali il metodo di spedizione è stato modificato durante il checkout. In precedenza, quando un cliente modificava il metodo di spedizione nella pagina PayPal Revisione ordine del workflow di pagamento, il applicazione non aggiornava il totale dell&#39;ordine con il metodo corretto.

<!-- MC-34152-->

* I commercianti possono ora annullare correttamente gli ordini autorizzati utilizzando PayPal. In precedenza, il applicazione non annullava l&#39;ordine e visualizzava questo errore: `Declined: 10601-Authorization has expired`.

<!-- MC-33330-->

* Il applicazione non svuota più il carrello quando annulli un ordine chiudendo la finestra popup di pagamento PayPal dopo aver completato un altro ordine.

### Prestazione

* Tre nuove impostazioni di configurazione supportano una riduzione del consumo consumer coda della CPU. Questi parametri facoltativi forniscono un maggiore controllo sui consumer e consentono di risparmiare risorse del server. Per una descrizione dei parametri `maxIdleTime`, `sleep` e `onlySpawnWhenMessageAvailable`, vedere [Configurare le code di messaggi](https://developer.adobe.com/commerce/php/development/components/message-queues/configuration/).
...
* I deadlock non si verificano più quando il processo di importazione esegue un inserimento bulk e il processo di reindicizzazione esegue contemporaneamente un inserimento di grandi dimensioni da select. In precedenza, il applicazione visualizzato questo errore: `PDOException: SQLSTATE[40001]: Serialization failure: 1213 Deadlock found when trying to get lock`. [GitHub-8933](https://github.com/magento/magento2/issues/8933)

### Autorizzazioni alla restituzione della merce (RMA)

<!-- MC-35984-->

* La pagina Resi ora funziona come previsto dopo aver creato un&#39;etichetta di spedizione per un&#39;autorizzazione al reso della merce (RMA). In precedenza, i commercianti non potevano interagire con gli elementi della pagina Resi dopo aver creato un&#39;etichetta di spedizione per un RMA. Si trattava di un problema noto per la versione 2.4.0 e `MC-35984-2.4.0-CE-composer.patch` ha risolto questo problema per la versione 2.4.0.

<!-- MC-35826-->

* Il applicazione ora visualizza un messaggio di errore informativo o tutti i prodotti disponibili quando un amministratore con autorizzazioni limitate tenta di creare un reso per i prodotti che non si trovano più nel sito Web assegnato ambito. In precedenza, l’applicazione visualizzava una pagina vuota.

### Recensioni

<!-- MC-33405-->

* Alla tabella `review_entity_summary` è stata aggiunta una chiave univoca per `entity_pk_value-entity_type-store_id`, che impedisce la creazione di righe duplicate per lo stesso ID prodotto (`entity_pk_value`). In precedenza, la mancanza di una chiave univoca generava righe duplicate per lo stesso ID prodotto ed errori SQL.

### Vendite

<!-- MC-35675-->

* L&#39;applicazione non assegna più lo stato `Complete` dopo la fatturazione a un ordine che richiede il pagamento zero.

<!-- MC-35888-->

* L&#39;e-mail di spedizione Nuovo generata dall&#39;API REST ora contiene le stesse informazioni sulla spedizione e sul cliente delle spedizioni create manualmente dall&#39;amministratore. In precedenza, questa e-mail non conteneva il nome del cliente, le informazioni di tracciamento, i prodotti ordinati e altre informazioni sull’ordine.

<!-- MC-35837-->

* I nomi dei utente degli ospiti sono ora visibili come previsto nelle e-mail relative a fatture e spedizioni quando le e-mail vengono caricate o personalizzate dall&#39;amministratore.

<!-- MC-35707-->

* La fattura PDF viene ora tradotta nella lingua della vista store in cui è stato effettuato l&#39;ordine. In precedenza, la fattura PDF veniva tradotta nella lingua dell&#39;amministratore.
...
* I valori dell&#39;ambito vengono ora reimpostati come previsto nel modulo Nuovo Gruppo di sinonimi. _Correzione inserita da Sathish Subramanian in richiamare richiesta [29206](https://github.com/magento/magento2/pull/29206)_. [GitHub-28947](https://github.com/magento/magento2/issues/28947)
...
* I clienti possono ora rimuovere correttamente i punti premio nella pagina di revisione dell’ordine del flusso di lavoro di pagamento durante l’estrazione con più indirizzi. In precedenza, l&#39;applicazione generava un errore `404 Not Found` quando un cliente faceva clic sul pulsante **Rimuovi** per rimuovere i punti per qualsiasi indirizzo. Questo era un problema noto nella versione 2.4.0.
...
* L&#39;etichetta **dell&#39;attributo Imposta prodotto come attributo Nuovo** ora viene visualizzata come previsto nelle pagine dei dettagli e degli aggiornamenti pianificati di un prodotto.

<!-- MC-34484-->

* Ora è possibile visualizzare correttamente l&#39;anteprima del programmare di staging per un blocco CMS. In precedenza, il applicazione generava un errore irreversibile.

<!-- MC-33789-->

* È ora possibile accesso la pagina Anteprima CMS quando si modifica una pagina CMS con un aggiornamento pianificato e la si riassegna dal store predefinito a un altro store. In precedenza, il applicazione generava un errore 404 quando si faceva clic sull&#39;collegare di anteprima.

<!-- MC-33572-->

* È ora possibile utilizzare POST `/V1/products/special-price` per aggiornare il prezzo speciale di un prodotto senza specificare un valore di parametro `price_to`. In precedenza, i tentativi di impostare l&#39;aggiornamento del prezzo senza il parametro `price_to` specificato non avevano esito positivo.
...
* Ora puoi generare un token cliente con Swagger. In precedenza, Swagger non generava un codice di risposta quando venivano immesse informazioni valide sulla accesso del cliente e il applicazione visualizzato questo errore: `The account sign-in was incorrect or your account is disabled temporarily. Please wait and try again later`. _Correzione inviata da Vadim Malesh nel richiamare richiesta [28822](https://github.com/magento/magento2/pull/28822)_. [GitHub-27098](https://github.com/magento/magento2/issues/27098)
...
* È stata aggiunta `\Magento\TestFramework\TestCase\WebapiAbstract` una funzione che consente il confronto di grandi array nidificati di risultati attesi ed effettivi nei test e consente ai tester di ignorare le chiavi irrilevanti nel risultato effettivo. _Correzione inviata da Jekabs in richiamare richiesta [29458](https://github.com/magento/magento2/pull/29458)_. [GitHub-29498](https://github.com/magento/magento2/issues/29498)

<!-- ENGCOM-7887 -->

* È stata corretta l&#39;inversione del valore previsto con il valore effettivo in `app/code/Magento/Newsletter/Test/Unit/Model/SubscriptionManagerTest.php`. _Correzione inserita da Lukasz Bajsarowicz in richiamare richiesta [29271](https://github.com/magento/magento2/pull/29271)_. [GitHub-29283](https://github.com/magento/magento2/issues/29283)
...
* Il applicazione ora controlla l&#39;area (frontend o `adminhtml`) prima di eseguire il rendering del markup di traduzione in linea. In precedenza, l&#39;esecuzione di una chiamata API con credenziali non valide `/rest/V1/integration/admin/token` mentre la traduzione in linea era abilitata comportava l&#39;inclusione di markup di conversione in linea attorno al messaggio di errore nella risposta API. _Correzione inviata da Zach Nanninga nel richiamare richiesta [28856](https://github.com/magento/magento2/pull/28856)_. [GitHub-28656](https://github.com/magento/magento2/issues/28656)
...
* Il applicazione ora visualizza correttamente il widget dei prodotti visualizzati di recente nella visualizzazione mobile. _Correzione inviata da Tu Nguyen in richiamare richiesta [27572](https://github.com/magento/magento2/pull/27572)_. [GitHub-27058](https://github.com/magento/magento2/issues/27058)

<!-- ENGCOM-7575-->

* La modifica della posizione di un prodotto da parte dell&#39;amministratore si riflette ora nella posizione del prodotto in vetrina. _Correzione presentata da Oleg Aleksin nel richiamare richiesta [28150](https://github.com/magento/magento2/pull/28150)_. [GitHub-28149](https://github.com/magento/magento2/issues/28149)
...
* La classe CSS che controlla la larghezza del campo viene ora applicata come previsto per l&#39;elemento Inizia Time in tutto l&#39;amministratore. _Correzione inviata da Shankar Konar nel richiamare richiesta [29511](https://github.com/magento/magento2/pull/29511)_. [GitHub-29496](https://github.com/magento/magento2/issues/29496)

<!-- ENGCOM-7992-->

* L&#39;etichetta di testo Termini e condizioni ora può visualizzare correttamente le stringhe di testo più lunghe. _Correzione presentata da Bartłomiej Szubert nel richiamare richiesta [29413](https://github.com/magento/magento2/pull/29413)_. [GitHub-24060](https://github.com/magento/magento2/issues/24060)
...
* È stato rimosso un test `AdminAnalytics` non utilizzato (`TrackingScriptTest`). _Correzione inviata da Lukasz Bajsarowicz nella richiesta di pull [28605](https://github.com/magento/magento2/pull/28605)_. [GitHub-28850](https://github.com/magento/magento2/issues/28850)
...
* Ora puoi visualizzare in anteprima la pianificazione di staging per un blocco CMS. In precedenza, l’applicazione generava un errore irreversibile.
...
* Il applicazione ora visualizza lo stock di origine anziché lo stock di prodotto predefinito quando si ordinano i prodotti in Visual Merchandiser e l&#39;inventario è abilitato.

### Framework API Web

<!-- MC-36084-->

* Le fatture create utilizzando REST ora includono informazioni sui scheda dei regali simili alle fatture create nell&#39;amministratore. In precedenza, quando si utilizzava il POST `/V1/order/:id/invoice` per fatturare l&#39;ordine, non veniva visualizzato il codice gift card o l&#39;importo gift card applicato.
...
* L&#39;ID del gruppo di clienti viene convalidato per le richieste POST `/rest/V1/customers` . In precedenza, l&#39;applicazione non visualizzava un messaggio di errore quando si utilizzava un `group_id` non valido in queste richieste. _Correzione inviata da Vadim Malesh nel richiamare richiesta [28903](https://github.com/magento/magento2/pull/28903)_. [GitHub-28064](https://github.com/magento/magento2/issues/28064)

<!-- ENGCOM-7611 -->

* Le sessioni PHP non vengono create per impostazione predefinita per le chiamate REST anonime. In precedenza, le sessioni venivano avviate lineare caso non necessarie. _Correzione presentata da Maciej Pawłowski nel richiamare richiesta [26032](https://github.com/magento/magento2/pull/26032)_. [GitHub-7213](https://github.com/magento/magento2/issues/7213)
...
* È ora possibile utilizzare la funzionalità ricerca lista dei desideri per trovare un prodotto in una lista dei desideri pubblica nelle distribuzioni in cui è abilitato il supporto per più liste di desideri. In precedenza, dopo che un cliente utilizzava la ricerca della lista dei desideri per trovare un prodotto, selezionandolo e facendo clic su **Aggiungi al carrello**, il applicazione non aggiungeva il prodotto al carrello e visualizzava questo errore: `Invalid Form Key. Please refresh the page`.

<!-- MC-35622-->

* I clienti possono ora modificare il quantità di un prodotto in una lista dei desideri dalla lista dei desideri stessa. In precedenza, il applicazione non aggiornava il quantità del prodotto e non visualizzava alcun messaggio.

<!-- MC-35618-->

* I clienti possono ora modificare il quantità di un prodotto raggruppato in una lista dei desideri dalla lista dei desideri stessa. In precedenza, il applicazione non aggiornava il prodotto quantità né visualizzava alcun messaggio.

<!-- MC-34408-->

* La pagina Categoria vetrina ora visualizza la lista dei desideri ricerca il widget come previsto.

<!-- ENGCOM-7580 7285-->

* Polyfill per `Map`, `WeakMap`, `FormData`, e `MutationObserver` vengono ora caricati solo se necessario. _Correzione presentata da Dmitry Tsymbal nel richiamare richiesta [28330](https://github.com/magento/magento2/pull/28330) e Mateusz Krzeszowiak nel richiamare richiesta [27618](https://github.com/magento/magento2/pull/27618)_. [GitHub-28377](https://github.com/magento/magento2/issues/28377), [GitHub-28382](https://github.com/magento/magento2/issues/28382)

<!-- ENGCOM-7561-->

* Il processo di aggiornamento della lista dei desideri ora supporta l&#39;aggiornamento simultaneo di una voce della lista dei desideri e della sua descrizione. _Correzione presentata da Eduard Chitoraga nel richiamare richiesta [28222](https://github.com/magento/magento2/pull/28222)_. [GitHub-28261](https://github.com/magento/magento2/issues/28261)

<!-- ENGCOM-7564-->

* Il applicazione ora convalida correttamente il massimo consentito di liste di desideri che un acquirente può creare. _Correzione presentata da Eduard Chitoraga nel richiamare richiesta 247 in private repo partners-magento2ee_.

<!-- MC-36250-->

* Gli amministratori possono ora configurare un prodotto configurabile che è stato aggiunto da un cliente a una lista di desideri da un store non predefinito. In precedenza, quando il cliente aveva aggiunto anche il prodotto configurabile da un store non predefinito, il applicazione generava un errore.

<!-- ENGCOM-7660-->

* È stato aggiunto un test per la condivisione della lista desideri di un cliente con un numero di e-mail superiore a quello consentito. _Correzione inviata da Dmitry Tsymbal nel richiamare richiesta [28641](https://github.com/magento/magento2/pull/28641)_. [GitHub-28720](https://github.com/magento/magento2/issues/28720)

<!-- ENGCOM-7674-->

* È stato aggiunto un test per l&#39;eliminazione di elementi dalla lista desideri di un cliente come utente amministratore. _Correzione inviata da Dmitry Tsymbal nel richiamare richiesta [28632](https://github.com/magento/magento2/pull/28632)_. [GitHub-28721](https://github.com/magento/magento2/issues/28721)

<!-- ENGCOM-7675-->

* Aggiunto un test per disabilitare la funzionalità lista dei desideri. _Correzione inviata da Dmitry Tsymbal nel richiamare richiesta [28635](https://github.com/magento/magento2/pull/28635)_. [GitHub-28744](https://github.com/magento/magento2/issues/28744)

<!-- ENGCOM-7580-->

* È stato aggiunto un test per l&#39;utilizzo di indirizzi e-mail non valido per la condivisione delle liste dei desideri dei clienti. `StorefrontShareWishlistEntityTest` è stato sottoposto a nuova fattorizzazione. `StorefrontCustomerShareWishlistActionGroup` è obsoleto. _Correzione inviata da Dmitry Tsymbal nella richiesta di pull [28330](https://github.com/magento/magento2/pull/28330)_. [GitHub-28377](https://github.com/magento/magento2/issues/28377)
...
Siamo grati al più ampio Magento Open Source community e vorremmo like riconoscere il loro contributo a questa versione. Dai un&#39;occhiata ai seguenti modi per conoscere i contributi community alle nostre versioni correnti:

* Se un membro community ha fornito una correzione per questa versione, identifichiamo la correzione nella sezione Problema risolto di queste note con la frase &quot;*Correzione fornita da community membro*&quot;.

* La Community Engineering team [Magento Contributors](https://magento.biterg.io/app/kibana) mantiene un elenco delle persone e dei partner che contribuiscono maggiormente per mese, trimestre e anno. Da quella pagina Collaboratori, puoi seguire collegamenti ai loro PR uniti su GitHub.

### Contributi dei partner

Nella tabella seguente vengono evidenziati i contributi dei partner. Questa tabella elenca il partner che ha contribuito al richiesta richiamare, il richiesta richiamare esterno e il numero di problema GitHub ad esso associato (se disponibile).

{{$include /help/_includes/release-notes/engcomm-2-4-1-partner.md}}

### Contributi dei singoli collaboratori

La seguente tabella identifica i contributi dei nostri membri community. Questa tabella elenca le richieste di richiamare esterne, il numero di problema GitHub associato (se disponibile) e il membro community che ha contribuito alla richiesta richiamare.
