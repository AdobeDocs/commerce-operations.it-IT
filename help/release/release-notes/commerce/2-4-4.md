...
Adobe Commerce 2.4.4 include la risoluzione di tutti i problemi risolti dai seguenti hotfix, forniti per Adobe Commerce e Magenti Open Source 2.4.3, 2.4.3-p1 e 2.3.7-p2:
...
* `braintree-disabled-partial-capture-for-applepay-googlepay.patch`.  Gli esercenti non possono inviare rimborsi parziali per ordini pagati con Apple Pay tramite Braintree. Quando un commerciante tenta di creare una nota di accredito per un rimborso parziale dalla fattura dell&#39;ordine, **Qtà da rimborsare** non è modificabile. Questo hotfix risolve tale problema. Consulta la [Adobe Commerce 2.4.4: impossibile creare fatture parziali](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) Articolo della Knowledge Base.  <!-- BUNDLE-3088-->
...
* HTTPS è ora abilitato per impostazione predefinita per la vetrina Adobe Commerce. Il **Usa URL protetti in Storefront** e **Utilizzare URL protetti in Admin** Le impostazioni di sono abilitate per impostazione predefinita e tutti i cookie incorporati sono ora impostati come protetti.  <!-- AC-1173-->
...
* Il nuovo **Abilita controllo magazzino al caricamento del carrello** l’opzione di configurazione fornisce un controllo dell’inventario commutabile quando si carica un prodotto nel carrello. È attivata per impostazione predefinita. Quando disattivi questa opzione, Adobe Commerce ignora il controllo dell’inventario durante il caricamento del preventivo, velocizzando così il pagamento, in particolare per i carrelli contenenti molti articoli.

* Il nuovo `multiple_processes` L&#39;opzione di configurazione supporta l&#39;esecuzione di consumer paralleli in più processi. Precedentemente, `cron` gestiva un singolo consumatore quando necessario. Avviare più consumer per eseguire i processi in parallelo può migliorare la velocità di esecuzione delle attività. Per abilitare questa funzione, aggiungi `multiple_processes` al `app/etc/env.php` file.

* Le operazioni del carrello contenente oltre 750 prodotti configurabili sono state migliorate aumentando il limite di memoria impostato da `max_input_vars` nel `php.ini` per supportare il volume delle variabili di input.

* Ottimizzazione dell&#39;elaborazione delle regole di vendita durante il pagamento mediante il differimento del calcolo del totale. In genere, l’elaborazione delle regole di vendita si verifica ogni volta che un acquirente aggiunge un prodotto al carrello, aggiorna la quantità di prodotto o fa clic su  **Inserisci ordine** pulsante. Gli esercenti possono abilitare questo differimento impostando `checkout/deferred_total_calculating` variabile in `env.php` file. In alternativa, è possibile eseguire `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->
...
Consulta [Elaborazione degli ordini ad alta velocità](../../../performance/high-throughput-order-processing.md).
...
Ad eccezione di [Braintree](https://docs.magento.com/user-guide/payment/braintree.html), tutte le estensioni incluse nel fornitore sono state rimosse dalla base di codice di Adobe Commerce 2.4.4. Gli esercenti devono effettuare la migrazione alle estensioni ufficiali, disponibili sul [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
PWA Studi v.12.3.0 è compatibile con Adobe Commerce 2.4.4. Include il supporto per reCaptcha, l’ottimizzazione dei contenuti di Page Builder e i contenuti personalizzati. Per informazioni sui miglioramenti e sulle correzioni di bug, consulta [Versioni di PWA Studi](https://github.com/magento/pwa-studio/releases). Consulta [Compatibilità delle versioni](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) per un elenco delle versioni di PWA Studi e delle relative versioni di base compatibili di Adobe Commerce.
...
Il nuovo `Magento_ElasticsearchCatalogPermissionsGraphQl` è disabilitato durante la Live Search aggiornata [installazione](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/onboard/install.html). Le installazioni Live Search esistenti devono essere aggiornate a Live Search 2.0.0 per sfruttare queste funzioni. Per ulteriori informazioni, consulta Live Search [Note sulla versione](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).
...
* `cron` ora cancella la cache della categoria di prodotto come previsto durante `indexer_update_all_views` esecuzione. In precedenza, Adobe Commerce mostrava un conteggio di prodotti errato nella pagina della categoria dopo l’aggiornamento di massa di molti prodotti nella categoria.
...
* Adobe Commerce non genera più un errore quando un acquirente fa clic su **Aggiorna carrello** dopo aver modificato la quantità di un prodotto nel mini carrello. In precedenza, Adobe Commerce visualizzava questo errore:  `The quote item isn't found. Verify the item and try again`.
...
* Adobe Commerce non genera più questo errore quando un acquirente aggiunge un indirizzo di fatturazione al quale manca un campo della via: `Uncaught TypeError: Unable to process binding "text: function(){return currentBillingAddress().street.join(', ') }"`. [GitHub-33826](https://github.com/magento/magento2/issues/33826)
...
* Adobe Commerce non genera più un errore dopo l’utilizzo di `addConfigurableProductsToCart` per aggiungere un prodotto configurabile a un carrello direttamente dopo aver modificato la vista store.

<!-- ACP2E-176-->

* Adobe Commerce non genera più un errore quando utilizzi il `addConfigurableProductsToCart` mutazione per aggiungere al carrello un elemento secondario disattivato di un prodotto configurabile. In precedenza, Adobe Commerce generava un errore di violazione del vincolo di integrità.

<!-- ACP2E-206-->

* I prodotti di cross-selling non sono più visibili nella pagina del carrello quando **Mostra articoli di cross-selling nel carrello** l&#39;impostazione di configurazione è disabilitata.
...
* Ora è possibile aggiungere al carrello un prodotto con il prezzo di 0 quando Adobe Commerce è configurato con **Sconto importo fisso per carrello intero** regola prezzo carrello. In precedenza, Adobe Commerce visualizzava questo errore: `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* Adobe Commerce non genera più un’eccezione durante l’esecuzione di un’azione di aggiornamento di massa degli attributi sulla griglia del prodotto quando un prodotto presenta `datetime` attributo.
...
* Facendo clic sull’anteprima di un modello e-mail, l’anteprima viene ora visualizzata in una finestra separata, come previsto.  In precedenza, Adobe Commerce apriva l’anteprima del modello simultaneamente dalla griglia dei modelli e-mail e in una finestra a comparsa.
...
* L&#39;aiutante `Magento\Payment\Helper\Data` non crea più nuovi layout nei costruttori. In precedenza, quando questo helper veniva utilizzato in comandi personalizzati senza specificare un indicativo di località, Adobe Commerce generava un errore. [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* Sono state aggiunte nuove opzioni richieste al `.htaccess` e `nginx.conf` file distribuiti con Adobe Commerce per il supporto di PHP 8.x. Sono state rimosse anche le opzioni obsolete. [GitHub-34358](https://github.com/magento/magento2/issues/34358)
...
* Sono stati corretti dei problemi relativi a `sprintf(__())` e `sprintf(Magento\Framework\Phrase())` per la compatibilità con PHP 8.x in tutto il codebase. In precedenza, Adobe Commerce generava questo tipo di errore: `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* Il `staging_synchronize_entities_period` `cron` Il processo ora funziona come previsto. In precedenza, il primo aggiornamento di staging veniva applicato correttamente, ma Adobe Commerce generava questo errore con gli aggiornamenti successivi: `report.ERROR: Cron Job staging_synchronize_entities_period has an error: The active update can't be deleted`.
...
* Adobe Commerce ora copia tutti i campi del prodotto in base ai loro ambiti definiti durante la duplicazione di un prodotto. Precedentemente, il **nome** e **descrizione** i campi sono stati reimpostati su `global` ambito.
...
* Prodotti con **Imposta come nuovo** l’attributo a cui è assegnata una data di inizio vuota e una data di fine scaduta ora può essere salvato correttamente. In precedenza, Adobe Commerce generava questo errore quando si salvava un prodotto con le seguenti impostazioni: `Make sure the To Date is later than or the same as the From Date`.
...
* Il `mage-messages` il cookie non viene reimpostato se non sono presenti nuovi messaggi.
...
* Il non supportato `str_contains` il metodo è stato sostituito con una funzione supportata. Questo era un problema noto per Adobe Commerce 2.4.3. [GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* Adobe Commerce ora visualizza un errore informativo quando un amministratore con una proprietà di sola lettura `pubs/media` tenta di accedere alla pagina dei dettagli di un prodotto che include immagini. In precedenza, Adobe Commerce generava un errore PHP. [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* Sono state aggiunte dichiarazioni dei tipi di ritorno ai metodi interni con tipi di ritorno incompleti per garantire la compatibilità con PHP 8.1. [GitHub-34498](https://github.com/magento/magento2/issues/34498) <!-- AC-1606-->

<!-- AC-1147-->

* Aggiornamento della directory principale `composer.json` metapacchetto e  `composer.json` per ciascun modulo a PHP 8.1. [GitHub-34009](https://github.com/magento/magento2/issues/34009)
...
* Adobe Commerce non visualizza più un errore durante il pagamento con il metodo PayPal Express. In precedenza, anche se il processo di pagamento era stato completato, Adobe Commerce mostrava questo errore: `Something went wrong`.

<!-- AC-722-->

* PayPal Express ora funziona come previsto dal carrello. In precedenza, quando si selezionava **PayPal** per avviare il pagamento rapido dal carrello, Adobe Commerce ha generato questo errore:  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* Per impostazione predefinita, Adobe Commerce ridimensiona le immagini in modo sincrono durante il salvataggio del prodotto. Gli esercenti possono ora ridimensionare le immagini come processo asincrono in background apportando modifiche minori a un `di.xml` file.
...
* Il nuovo `product_alert` consumer migliora l&#39;invio degli avvisi dei clienti creando messaggi di coda, eseguendo il consumer e migliorando i tempi di esecuzione. In precedenza, Adobe Commerce generava un’eccezione per memoria insufficiente quando venivano inviati più di 100.000 avvisi sui prodotti. Adobe Commerce ha inoltre impiegato più di 20 ore per inviare tutti gli avvisi.

### Video del prodotto

<!-- MC-42105-->

* Adobe Commerce ora abilita **Salva** e completa automaticamente i campi quando inserisci un URL incompleto durante l’aggiunta di un video (amministratore **Catalogo**  > **Prodotti** > **Aggiungi video**). Precedentemente, il **Salva** il pulsante è stato disattivato e i campi non sono stati compilati.

### Restituisci autorizzazioni Merchandising (RMA)

<!-- MC-42987-->

* Nella griglia Prodotti restituiti creata dall&#39;amministratore vengono ora visualizzate le imposte, inclusi i prezzi solo per i prodotti configurati per visualizzare le imposte con i prezzi. In precedenza, Adobe Commerce non controllava le impostazioni di visualizzazione della configurazione sul `tax/calculation/price_includes_tax` flag.
...
* I punti premio scaduti sono ora contrassegnati come **Scaduto** nella sezione Reason della griglia Cronologia punti premio della vetrina.
...
* Adobe Commerce ora rimuove i tag HTML come previsto dalla vetrina **Account** > **I miei ordini** > **Ordine di visualizzazione** pagina. In precedenza, Adobe Commerce visualizzava i tag HTML nella sezione dei commenti sugli ordini dei clienti della vetrina.
...
* `category_name` è stato aggiunto ai codici degli attributi del prodotto riservati al sistema. Adobe Commerce ora mostra un errore se un utente tenta di creare un attributo di prodotto con il codice dell’attributo `category_name`. In precedenza, i prodotti non venivano elencati nella categoria prevista ed era stato registrato un errore di Elasticsearch.
...
* Adobe Commerce non visualizza più un messaggio di errore durante il caricamento della home page di un archivio con un tema personalizzato preesistente. Precedentemente, Adobe Commerce visualizzava questo messaggio:  `The store will not work correctly in the case when cookies are disabled`.

### Traduzioni e lingue

<!-- AC-285 MC-41400-->

* Il `UserExpiration` la convalida non ha più esito negativo con `de_DE` e `uk_UA` impostazioni internazionali. In precedenza, Adobe Commerce generava un errore quando un amministratore tentava di impostare una data di scadenza durante la creazione di un nuovo utente dall’amministratore con le impostazioni internazionali impostate su `de_DE` o `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* Adobe Commerce non mostra più indirizzi duplicati quando un acquirente fa clic su **Cambia indirizzo** nella pagina Revisione e pagamenti in una distribuzione in cui **Ricerca indirizzo** l&#39;impostazione di configurazione è attivata.
...
* Visual Merchandiser **Sposta il grezzo in basso** l’opzione di ordinamento automatico ora funziona come previsto per i prodotti configurabili quando tutti i prodotti semplici sono esauriti.
...
* Adobe Commerce non riproduce più una lista dei desideri nella barra laterale delle categorie quando **Mostra nella barra laterale** l&#39;opzione della lista dei desideri è disabilitata. In precedenza, Adobe Commerce ignorava questa opzione.
...
**Problema**: gli esercenti non possono inviare rimborsi parziali per ordini pagati con Apple Pay tramite Braintree. Quando un commerciante tenta di creare una nota di accredito per un rimborso parziale dalla fattura dell&#39;ordine, **Qtà da rimborsare** non è modificabile.  **Soluzione alternativa**: applica la patch `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. Consulta la [Adobe Commerce 2.4.4: impossibile creare fatture parziali](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) Articolo della Knowledge Base.  <!-- BUNDLE-3088-->
