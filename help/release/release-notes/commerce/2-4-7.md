...
Adobe Commerce 2.4.7 è ancora compatibile con PHP 8.1 solo a scopo di aggiornamento. PHP 8.1 non è supportato e non è consigliato. Il codice di base di Adobe Commerce 2.4.7, tutte le estensioni in bundle, e tutte le estensioni possedute da Adobe e i servizi SaaS sono compatibili con PHP 8.3. <!-- AC-9668 -->

* **Supporto di RabbitMQ 3.13**. Questa versione è compatibile con l’ultima versione di RabbitMQ 3.13. La compatibilità rimane con RabbitMQ 3.11 e 3.12, che è supportato rispettivamente fino ad agosto 2024 e dicembre 2024, ma l’Adobe consigliato è l’utilizzo di Adobe Commerce 2.4.7 solo con RabbitMQ 3.13. <!-- AC-9659 -->

* **Compositore 2.7.x**. La compatibilità con Composer 2.2.x rimane. <!-- AC-9663 -->

* **Supporto per la cache di vernice 7.4**. Questa versione è compatibile con la versione più recente di Varnish Cache 7.4. La compatibilità rimane con le versioni 6.0.x e 7.2.x, ma abbiamo consigliato di utilizzare Adobe Commerce 2.4.7 solo con Vernice Cache versione 7.4 o 6.0 LTS. <!-- AC-9662 -->

* **Elasticsearch 8.11** compatibilità <!-- AC-9661 -->

* Supporto di Opensearch 2.12 e OpenSearch 1.3 <!-- AC-9659 -->
...
Tutte le librerie JavaScript e le dipendenze NPM nel codice core di Adobe Commerce sono state aggiornate alle versioni più recenti disponibili. Tutte le dipendenze della libreria Laminas sono state aggiornate alla versione più recente compatibile con PHP 8.3. <!-- AC-9667 9665 -->

#### Aggiornamenti aggiuntivi

* **Più coupon per supporto ordine**. Gli esercenti ora possono [configurare il numero massimo di coupon](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#multicoupon-settings) che può essere applicato per ordine con il nuovo **[!UICONTROL Maximum number of coupons per order]** opzione di configurazione. Questo valore è impostato su 1 per impostazione predefinita. Ora puoi utilizzare [REST](https://developer.adobe.com/commerce/webapi/rest/modules/multicoupon/) o [GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/) per applicare più coupon a un carrello.

* Il gateway API XML di Commerce UPS è stato migrato al nuovo [API REST di Commerce UPS](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups) per supportare gli aggiornamenti che UPS sta apportando ai propri [Modello di sicurezza API](https://developer.ups.com/oauth-developer-guide). (UPS sta implementando un modello di sicurezza OAuth 2.0 (token bearer) per tutte le API.) Tutte le API XML di Commerce UPS precedenti sono state rimosse dalla base di codice di Adobe Commerce 2.4.7. <!-- AC-8918 -->

* Adobe Commerce [integrazione](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex) con FedEx è stata migrata dai servizi Web WSDL FedEx legacy alle più recenti API RESTful FedEx. WSDLS di tracciamento dei servizi web FedEx, convalida degli indirizzi e convalida dei codici postali verrà ritirato a maggio 2024. <!-- AC-1470 -->
...
* Gli esercenti aziendali possono ora configurare fino a **un milione di regole di prezzo del carrello attive basate su coupon** in Adobe Commerce, senza alcun peggioramento significativo delle prestazioni del carrello e delle operazioni di pagamento. <!-- ACPT-1198 -->

* **Gestione avanzata dell&#39;indicizzatore**. Il nuovo `indexer:set-status` supporta la gestione dinamica dello stato dell&#39;indicizzatore. Gli utenti amministratori possono utilizzare questo comando per modificare lo stato dell’indicizzatore in `suspended`, `invalid`, o `valid`. Questa funzione è particolarmente utile per gestire le prestazioni del sistema durante operazioni di massa estese, come importazioni di prodotti o aggiornamenti, consentendo il controllo del momento in cui gli indicizzatori vengono attivati automaticamente dai processi cron del sistema. Consulta [Gestire gli indicizzatori](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cli/manage-indexers.html).
...
* **Operazioni di salvataggio più rapide delle configurazioni a livello di archivio per le distribuzioni con molti archivi**. Il salvataggio delle impostazioni di configurazione nelle distribuzioni con più di 500 archivi può richiedere molto tempo. Il nuovo [Configurazione asincrona](../../../performance/configuration.md#asynchronous-configuration-save) Il modulo abilita le operazioni di salvataggio della configurazione asincrona eseguendo un processo cron che utilizza un consumer per elaborare l&#39;operazione di salvataggio in una coda di messaggi. AsyncConfig è disabilitato per impostazione predefinita.
...
[Server applicazioni GraphQL](../../../performance/application-server.md) consente ad Adobe Commerce di mantenere lo stato tra le richieste API di Commerce GraphQL ed elimina la necessità di avviare il sistema, con conseguente aumento della velocità effettiva, riduzione della latenza e utilizzo efficiente delle risorse per tutte le API di GraphQL. Condividendo lo stato dell’applicazione tra i processi, le richieste API di GraphQL diventano notevolmente più efficienti e le risposte API di GraphQL sono in media più veloci del 30%.<!-- ACPT-27 -->
...
I webhook Commerce (v1.2.0 ) consentono agli sviluppatori di configurare una logica sincrona per eseguire chiamate a sistemi esterni quando viene attivato un evento Adobe Commerce. Le chiamate sincrone sono necessarie quando Commerce deve calcolare o convalidare immediatamente un valore come totale dell’ordine, imposta o pagamento utilizzando un endpoint di terze parti e successivamente riscrivere il risultato in Adobe Commerce.

I webhook di Commerce ora sono installati per impostazione predefinita. Consulta [Panoramica dei webhook di Adobe Commerce](https://developer.adobe.com/commerce/extensibility/webhooks/)

### Adobe Stock

Il pacchetto Adobe Stock (`adobe-stock-integration`) è ora incluso nel pacchetto metadati delle estensioni Commerce (`extensions-metapackage`) per supportare aggiornamenti più frequenti. <!-- CABPI-630 -->

### B2B

>[!NOTE]
>
>La versione 2.4.7 è compatibile con B2B 1.4.2. Tuttavia, l’Adobe consiglia ai clienti di attendere la prossima versione B2B 1.5.0 GA prima dell’aggiornamento, che include il supporto per PHP 8.3. Consulta [Note sulla versione B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).

### Braintree

* **[!DNL Vaulted PayPal]e [!DNL Pay Later] Modifiche**—I clienti che hanno effettuato l&#39;accesso e che hanno in precedenza archiviato/memorizzato il proprio conto PayPal hanno la possibilità di pagare con:
   * [!DNL Pay Now] (senza dover accedere al [!DNL PayPal] , l&#39;utente può pagare con la propria carta predefinita)
   * Pagamento con un&#39;altra fonte di finanziamento
   * Paga con un altro account
   * [!DNL PayPal Pay Later] o [!DNL PayPal Credit] pulsante

* **Supporto 3DS per[!DNL Google Pay]**- Supporto di verifica 3DS incluso per [!DNL Google Pay] carte non tokenizzate. Consulta la [Braintree documentazione](https://developer.paypal.com/braintree/docs/guides/3d-secure/client-side/android/v3#using-3d-secure-with-google-pay) per ulteriori informazioni.

* **[!DNL Vault Apple Pay Payments]**: consente ai clienti che hanno effettuato l&#39;accesso di eseguire il vaulting/archiviare i [!DNL Apple Pay] pagamenti sul conto del proprio punto vendita Commerce da utilizzare per transazioni future. Questo riduce il numero di passaggi al momento del pagamento e crea un’esperienza di pagamento più rapida per il cliente che ritorna.

* **[!DNL Vault Google Pay Payments]**: consente ai clienti che hanno effettuato l&#39;accesso di eseguire il vaulting/archiviare i [!DNL Google Pay] pagamenti sul conto del proprio punto vendita Commerce da utilizzare per transazioni future. Questo riduce il numero di passaggi al momento del pagamento e crea un’esperienza di pagamento più rapida per il cliente che ritorna.

* **[!DNL Vault Venmo Payments]**: consente ai clienti che hanno effettuato l&#39;accesso di eseguire il vaulting/archiviare i [!DNL Venmo] conti nel proprio conto del punto vendita Commerce da utilizzare in transazioni future. Questo riduce il numero di passaggi al momento del pagamento e crea un’esperienza di pagamento più rapida per il cliente che ritorna.

* **[!DNL Vault ACH Payments]**: consente ai clienti connessi di archiviare/archiviare i pagamenti ACH sul proprio account del punto vendita Commerce per utilizzarli in transazioni future. Questo riduce il numero di passaggi al momento del pagamento e crea un’esperienza di pagamento più rapida per il cliente che ritorna.

* **[!UICONTROL Express Payment]pulsanti nella parte superiore del checkout**- Per favorire un&#39;esperienza di pagamento più rapida, abbiamo introdotto [!DNL Express Payment] all&#39;inizio del pagamento. I clienti ora possono pagare entro [!DNL PayPal], [!DNL PayPal Pay Later], [!DNL Apple Pay], e [!DNL Google Pay Express] pagamenti.

* **[!DNL Braintree]Note sulla versione e collegamenti per il supporto all’interno di Admin Configuration**- I commercianti possono ora collegarsi direttamente dall’amministratore di Commerce al supporto Braintree e alle note sulla versione online.

* **Supporto di GraphQL per tutti [!DNL Braintree] metodi di pagamento tranne[!DNL Venmo]**: nell’API GraphQL sono esposte più configurazioni. Questa funzione è particolarmente utile per le applicazioni headless.

* **[!DNL Vaulting]pagamenti nell&#39;area conto**- I clienti connessi ora possono [vault/store](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings) nuove carte di credito/debito e [!DNL PayPal] account nell&#39;area Account cliente. In precedenza, i clienti potevano eseguire il vaulting/store solo quando salvavano i pagamenti per un uso successivo al completamento di una transazione al momento del pagamento, ora possono eseguire il vaulting di nuove carte di credito/debito e account PayPal senza dover creare una nuova transazione.
...
* **Webhook delle controversie**- Quando un cliente contesta una transazione per Braintree, lo stato della controversia viene ora trasmesso a Commerce. È ricercabile in **[!UICONTROL Sales]** > **[!UICONTROL Order]** e collegati a ciascun ordine.

### Integrazione di Commerce con Adobe IMS

Pacchetto di integrazione Adobe IMS (`adobe-commerce/adobe-ims-metapackage`) è ora incluso nel pacchetto metadati delle estensioni Commerce (`extensions-metapackage`) per supportare aggiornamenti più frequenti. <!-- CABPI-630 -->

### GraphQL

Commerce 2.4.7 include funzionalità avanzate di caching di GraphQL, supporto dello schema GraphQL per gli attributi personalizzati, supporto per l’annullamento di ordini headless e funzionalità migliorate di caching del risolutore.
...
* [`applyCouponsToCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/) mutazione
* [`clearCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/clear-cart/) mutazione
...
* È stato aggiunto il supporto GraphQL per le operazioni di base per tutti i metodi di pagamento eccetto Venmo. I nuovi endpoint di GraphQL per i servizi di pagamento sono descritti in [Pagamento](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->
...
* Il flusso di lavoro di pagamento ora include una sezione di pagamento rapido che fornisce PayPal, Google Pay e Apple Pay **[!UICONTROL Express]** pulsanti. <!-- BUNDLE-3322 -->

[Opzioni di pagamento](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/payments-checkout/payments-options.html) introduce metodi di pagamento supportati.

### PWA Studi

PWA Studi v14.0 è compatibile con Adobe Commerce 2.4.7. Include diversi miglioramenti per migliorare l’accessibilità. Per informazioni sulle correzioni di bug, consulta [Versioni di PWA Studi](https://github.com/magento/pwa-studio/releases). Consulta [Compatibilità delle versioni](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) per un elenco delle versioni di PWA Studi e delle relative versioni di base compatibili di Adobe Commerce.
...
>[!NOTE]
>
>Per la prima volta, diverse route di endpoint REST contengono la stringa `V2` invece di `V1`. Ad esempio: `GET /rest/default/V2/carts/{cartId}/coupons`. Per gli esercenti Commerce, questi endpoint sostituiscono `V1` versioni degli endpoint. Questi endpoint sono disponibili solo in Adobe Commerce.<!-- LYNX-208 -->
...
* **[!UICONTROL Select]** Le opzioni del menu a discesa sono ora visibili in Admin **[!UICONTROL Content]** > **[!UICONTROL Pages]** quando nella griglia sono selezionate più pagine. [GitHub-35386](https://github.com/magento/magento2/issues/35386)
...
* Pagina dei dettagli del prodotto `meta itemprop="image"` e `meta itemprop="description"` Le proprietà sono ora disponibili per l’immagine del prodotto principale. [GitHub-37497](https://github.com/magento/magento2/issues/37497)

<!-- ACP2E-2314 -->

* Nella sezione Report della griglia Prodotti in carrello ora viene visualizzato il prezzo del prodotto corretto quando **[!UICONTROL Catalog Price Scope]** è impostato su **[!UICONTROL Website]**. In precedenza, il prezzo del prodotto veniva visualizzato come zero quando il prodotto veniva assegnato a un sito Web diverso da quello predefinito.
...
* Le pagine dei dettagli del prodotto ora visualizzano il prezzo corretto per i prodotti bundle a cui è stato applicato uno sconto del 100%. In precedenza, Commerce non applicava uno sconto del 100% ai prezzi dei prodotti in bundle.
...
* Commerce ora visualizza messaggi informativi quando viene attivata un’azione di aggiunta al carrello. Precedentemente, il `The requested qty is not available` non sempre veniva visualizzato nella vetrina in base alle esigenze.

<!-- ACP2E-2166 -->

* I tempi di caricamento delle pagine di pagamento sono stati migliorati per i clienti con rubriche di grandi dimensioni. Commerce ora elabora solo il numero di indirizzi cliente specificati in **[!UICONTROL Customer Addresses Limit]** impostazione. In precedenza, Commerce caricava l’intera rubrica.
...
* Ora puoi salvare correttamente una nuova regola di prezzo del carrello con il `Magento_OfflineShipping` estensione disabilitata. In precedenza, Adobe Commerce generava questo errore: `report.ERROR: Warning: Undefined array key "simple_free_shipping"`. [GitHub-37580](https://github.com/magento/magento2/issues/37580)
...
* In esecuzione `bin/magento cache:clean` o `bin/magento cache:flush` non esegue più il flushing dell&#39;intera cache incorporata o di vernice a pagina intera, se non specificato. [GitHub-38301](https://github.com/magento/magento2/issues/38301)
...
* In Commerce non vengono più visualizzate immagini duplicate quando si modifica il colore del prodotto in una pagina dei dettagli del prodotto.  [GitHub-36243](https://github.com/magento/magento2/issues/36243)
...
* Commerce non inserisce più il testo della descrizione del prodotto nella pagina dei dettagli del prodotto **[!UICONTROL Meta Description]** quando questo campo viene lasciato vuoto. In precedenza, quando una descrizione del prodotto conteneva codice HTML generato da Page Builder o da un altro editor HTML, il HTML veniva visualizzato nel file Meta **[!UICONTROL Description]** e non è stato possibile lasciare vuoto questo campo.
...
* Il resto `V1/products/<sku>/media` l&#39;endpoint può ora elaborare il caricamento simultaneo di più immagini multimediali. In precedenza, durante l’elaborazione di diverse richieste simultaneamente, venivano creati dati incoerenti e Commerce generava un errore quando questi dati venivano salvati nel database.
...
* È stato migliorato il debug con l’aggiunta di `UNCACHEABLE` quando si esegue l’output di un’intestazione HTTP con vernice. Precedentemente, solo `HIT` o `MISS` sono stati generati dei valori. [GitHub-37912](https://github.com/magento/magento2/issues/37912)
...
* Il `bin/magento varnish:vcl:generate` Il comando ora include un `input-file argument`. Questo supporta l’aggiunta di un VCL personalizzato a un archivio Git, relativo alla directory principale di Commerce. [GitHub-37363](https://github.com/magento/magento2/issues/37363)
...
* Caratteri non conformi [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html) sono stati rimossi dalla base di codice. [GitHub-38355](https://github.com/magento/magento2/issues/38355)

<!-- AC-10692 -->

* È stato corretto un tag documento PHP errato in `Magento\Reports\Block\Adminhtml\Shopcart\Product\Grid` classe. [GitHub-38186](https://github.com/magento/magento2/issues/38186)

<!-- AC-10865 -->

* Codice sottoposto a refactoring in tutta la base di codice creata quando l&#39;esercitazione di codifica imponeva che qualsiasi sezione della cache inclusa nel `app/etc/env.php` Il file includerebbe una sezione front-end. Questo refactoring risolve potenziali problemi durante l’aggiornamento delle distribuzioni di Commerce precedenti alla versione 2.4.4. [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* Il `var/log/system.log` il file non contiene più `main.INFO error` messaggi. Il registro delle voci di menu è stato spostato dal livello INFO a DEBUG. Precedentemente, il `var/log/system.log` Il file era pieno di messaggi come questo: `Add of item with id Magento_Theme::design_config was processed … `.
...
* Errori critici di lettura del database quando `synchronous_replication` è attivato per le connessioni slave su Adobe Commerce Cloud non si verificano più.
...
* Gli acquirenti possono ora accedere alle pagine configurate come nodi gerarchici quando sono abilitate le restrizioni di accesso. In precedenza, si verificava un ciclo di reindirizzamento infinito e il browser generava questo errore: `ERR_TOO_MANY_REDIRECTS`.
...
* Gli utenti amministratori con accesso limitato a uno specifico archivio ora possono utilizzare un’azione di aggiornamento di massa per aggiornare le recensioni dei prodotti. In precedenza, Commerce generava questa eccezione: `report.CRITICAL: TypeError: array_intersect(): Argument #1 ($array) must be of type array, null given in app/code/Magento/AdminGws/Model/Models.php:439`.
...
* Le regole del prodotto correlate **È uno di** ora funziona correttamente se configurato con il **Prodotti corrispondenti** impostazione. In precedenza, questa regola non identificava alcun prodotto.
...
* Prestazioni migliorate del `catalogrule_product` indicizzatore quando non è impostata alcuna regola di catalogo. [GitHub-34784](https://github.com/magento/magento2/issues/34784)
...
* Aggiunta di un elemento mancante `return` all&#39;admin usage enable controller. In precedenza, non vi era `return` e il controller in `FrontendController` la classe è stata gestita in modo errato. [GitHub-31374](https://github.com/magento/magento2/issues/31374)
...
* I commercianti possono ora notificare agli acquirenti che un ordine è pronto per il ritiro quando **[!UICONTROL No Manage Stock]** è attivata per un prodotto. In precedenza, quando questa impostazione era abilitata, Commerce visualizzava questo messaggio di errore: `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* I commercianti possono ora creare una nota di credito per gli ordini da cui è stato eliminato un prodotto semplice associato a un prodotto configurabile. In precedenza, gli esercenti non potevano creare la nota di credito e Commerce generava un `TypeError` eccezione.
...
* Commerce ora applica l’importo dello sconto corretto agli ordini parzialmente annullati. In precedenza, l&#39;importo dello sconto calcolato nella `base_discount_cancelled` la colonna non è stata calcolata correttamente quando è stato modificato un ordine. Il supporto per i valori negativi è stato aggiunto alle fatture per gli ordini scontati.

<!-- ACP2E-2585 -->

* Il resto `V1/orders` l&#39;endpoint ora applica le gift card come previsto quando si importano gli ordini. In precedenza, le modifiche agli attributi di estensione gift card in questo endpoint non venivano salvate.
...
* La pagina Griglia ordini di vendita amministratore ora viene caricata come previsto quando un commerciante avvia una ricerca da questa pagina. In precedenza, quando un esercente tentava di cercare ordini da questa tabella, la pagina si bloccava e Commerce mostrava un `Request-URI Too Long` nella console del browser.
...
* Gli elementi di pagina di Page Builder ora vengono riprodotti a una velocità accettabile nel browser Chrome (v112). In precedenza, il salvataggio dei contenuti con Page Builder in esecuzione nel browser Chrome era lento e Commerce registrava questo errore nella console del browser: `[ERROR] Page Builder was rendering for 5 seconds without releasing locks`.
...
* Commerce ora gestisce correttamente gli errori 502 da PayPal impedendo la creazione di ordini quando si verifica questo errore. In precedenza, gli ordini venivano comunque creati nonostante un errore 502 da PayPal, che causava incongruenze tra i record Commerce e PayPal.
...
* Gli acquirenti possono ora selezionare un metodo di spedizione durante l&#39;applicazione del codice coupon durante il pagamento. In precedenza, in base a queste condizioni, Commerce ha generato un errore e ha assegnato la spedizione a tariffa fissa all’ordine.
...
* Più clic in **Vendite** > **Ordini** Il menu a discesa mass action (azione di massa) della pagina non genera più più più richieste POST.  [GitHub-37997](https://github.com/magento/magento2/issues/37997)

<!-- AC-10944 -->

* Solo `200` o `404` Le risposte vengono memorizzate nella cache quando un acquirente passa a una pagina che attiva un reindirizzamento 301 a meno che l’intestazione Cache-Control della pagina non contenga `private `. In precedenza, lo stato HTTP 200 con `private` nell’intestazione Cache-Control era memorizzato in cache e uno stato HTTP 301 era memorizzato in cache a meno che non contenesse `private` nell’intestazione Cache-Control. [GitHub-36492](https://github.com/magento/magento2/issues/36492)
...
* Il resto `POST /V1/returns` La richiesta ora crea un RMA per prodotto. In precedenza, questa richiesta poteva ignorare la convalida e restituire più RMA per prodotto.

<!-- ACP2E-2450 -->

* Gli attributi RMA vengono ora aggiunti automaticamente al modulo EAV predefinito quando **[!UICONTROL Show on Storefront]** L&#39;attributo RMA è impostato su **no**. In precedenza, la finestra a comparsa Dettagli elemento RMA era vuota quando veniva creato un attributo restituito personalizzato e il **[!UICONTROL Show on Storefront]** Attributo RMA impostato su **no**.

<!-- ACP2E-2211 -->

* Il valore della proprietà **[!UICONTROL Enable RMA]** corrisponde ora al valore specificato nella configurazione quando il **[!UICONTROL Use Config]** la casella di controllo nella pagina di modifica del prodotto è abilitata.
...
* I prodotti bundle ora vengono aggiornati correttamente se un acquirente li aggiunge al carrello quando è in corso un aggiornamento della pianificazione. In precedenza, se al carrello era stato aggiunto un bundle di prodotti con opzioni quando era in corso un’attività di pianificazione, Commerce generava questa eccezione: `Some of the products below do not have all the required options. Please select all required options`.
...
* Commerce ora visualizza il valore corretto per **[!UICONTROL Regular Price]** valore quando viene selezionata un’opzione personalizzata per un prodotto che presenta **[!UICONTROL Special Price]** valore quando **[!UICONTROL Price Display Settings]** è impostato per includere l&#39;imposta.
...
* Correzione del comportamento del cursore nel registro delle donazioni **[!UICONTROL Message]** campo di testo.
...
* Gli utenti amministratori possono ora modificare il logo della vetrina quando è abilitata la modalità negozio singolo. In precedenza, Commerce visualizzava il logo di fallback predefinito. [GitHub-35952](https://github.com/magento/magento2/issues/35952)
...
* Il `crossorigin` l&#39;attributo è stato aggiunto a `linkTypes` per `HEAD` elementi.[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* Commerce non genera più riscritture dell’URL del prodotto per le categorie principali e per i prodotti impostati su **[!UICONTROL Not Visible Individually]** nelle impostazioni per più store quando si generano riscritture URL mancanti per gli store personalizzati.
...
* Commerce non visualizza più un `Integration not secure` avviso in Amministratore **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** quando **[!UICONTROL Callback URL]** e **[!UICONTROL Identity link URL]** i campi sono vuoti. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
È possibile installare Adobe Commerce 2.4.7 utilizzando [Compositore](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
