---
title: Adobe Systems Commerce 2.4.7 note sulla versione
...
Adobe Commerce 2.4.7 introduce il supporto per PHP 8.3. Introduce centinaia di correzioni e miglioramenti di qualità. Le dipendenze di Core Composer e le librerie di terze parti sono state aggiornate alle ultime versioni disponibili. Questa versione include miglioramenti significativi delle prestazioni, tra cui il caricamento più rapido delle pagine di elenco dei prodotti per prodotti complessi con molte opzioni e miglioramenti per la gestione degli indicizzatori. Questa versione aumenta la copertura GraphQL per gli attributi personalizzati e le cache del resolver GraphQL. Introduce inoltre il supporto per i servizi FedEx e UPS recentemente aggiornati.
...
* **Modifiche al comportamento delle chiavi** cache non generate:

   * Le chiavi della cache non generate per i blocchi ora includono prefissi diversi dai prefissi per le chiavi generate automaticamente. Le chiavi di cache non generate sono chiavi impostate tramite la sintassi della direttiva del modello o i `setCacheKey` metodi OR `setData` .
...
* **Limitazioni relative al numero di codici coupon generati** automaticamente. Commerce ora limita il numero di codici coupon generati automaticamente. Il massimo predefinito è 250.000. I commercianti possono utilizzare la nuova **[!UICONTROL Code Quantity Limit]** opzione di configurazione (**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > > **[!UICONTROL Customers]** **[!UICONTROL Promotions]**) per evitare potenzialmente di sovraccaricare il sistema con molti coupon. <!-- AC-8753 -->

* **Ottimizzazione del processo** predefinito di generazione dei URL di amministrazione. La generazione del URL amministratore predefinito è stata ottimizzata per aumentare la casualità, rendendo gli URL generati meno prevedibili. <!-- AC-9028 -->

* **È stato aggiunto il supporto** SRI (Subresource Integrity) per soddisfare i requisiti PCI 4.0 per la verifica dell&#39;integrità degli script nelle pagine di pagamento. Il supporto SRI (Subresource Integrity) fornisce hash di integrità per tutti i risorse JavaScript residenti nel filesystem locale. La funzione SRI predefinita viene implementata solo nelle pagine di pagamento per le aree Admin e Storefront. Tuttavia, i commercianti possono estendere la configurazione predefinita ad altre pagine. Vedere [Subresource Integrity](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/) nella _Commerce PHP Developer Guide_.<!--AC-1153-->

* **Modifiche ai criteri di sicurezza dei contenuti (CSP):** aggiornamenti della configurazione e miglioramenti ai criteri di sicurezza dei contenuti (CSP) di Adobe Systems Commerce per conformarsi ai requisiti PCI 4.0. Per informazioni dettagliate, vedere [Criteri](https://developer.adobe.com/commerce/php/development/security/content-security-policies/) di sicurezza dei contenuti nella Guida _per sviluppatori PHP di_ Commerce.<!--AC-11513-->

   * La configurazione CSP predefinita per le pagine di pagamento per le aree Amministrazione commercio e vetrina è ora `restrict` la modalità. Per tutte le altre pagine, la configurazione predefinita è `report-only` modalità.  Nelle versioni precedenti alla 2.4.7, CSP era configurato in `report-only` modalità per tutte le pagine.

   * È stato aggiunto un provider nonce per consentire l&#39;esecuzione di script in linea in un CSP. Il provider nonce facilita la generazione di stringhe nonce univoche per ogni richiesta. Le stringhe vengono quindi associate all&#39;intestazione CSP.

   * Sono state aggiunte opzioni per configurare URI personalizzati per segnalare violazioni CSP per la pagina Ordine di Crea nella pagina Amministratore e la pagina Pagamento nello storefront. Puoi aggiungere la configurazione dall&#39;amministratore o aggiungendo l&#39;URI al `config.xml` file.
...
Platform aggiornamenti di questa versione migliorano la conformità alle più recenti procedure consigliate per la sicurezza.

Adobe Systems Commerce 2.4.7 include i seguenti aggiornamenti della piattaforma:

* **Compatibilità** PHP 8.3. Questa versione introduce il supporto per PHP 8.3. Commerce ora supporta sia PHP 8.3 che 8.2. PHP 8.2 sarà supportato fino alla data di fine servizio (EOS) nel dicembre 2025. Dopo dicembre 2025, tutti i commercianti che eseguono implementazioni 2.4.7 dovrebbero migrare a PHP 8.3. <!-- AC-8988 -->
...
* **Compositore 2.7.x**. Compatibilità con Composer 2.2.x rimane. <!-- AC-9663 -->

* **Supporto** per la cache Varnish 7.4. Questa versione è compatibile con la versione più recente di Varnish Cache 7.4. La compatibilità rimane con le versioni 6.0.x e 7.2.x, ma si consiglia di utilizzare Adobe Commerce 2.4.7 solo con Vernice Cache versione 7.4 o 6.0 LTS. <!-- AC-9662 -->
...
* È stato aggiunto il supporto per il nuovo metodo di spedizione USPS Ground Advantage. Questa è un&#39;integrazione pronta all&#39;uso con il nuovo metodo di spedizione di USPS, USPS Ground Advantage, che è stato rilasciato a luglio 2023. Questa nuova integrazione può essere utilizzata per recuperare le tariffe di spedizione e pianificare consegne e restituzioni tramite il servizio di spedizione USPS. Il metodo di spedizione USPS Ground Advantage sostituisce questi metodi di spedizione, che sono stati ritirati quando è stato rilasciato il metodo di spedizione USPS Ground Advantage:

   * USPS Retail Ground
   * Servizio pacchetti di prima classe
   * Seleziona terreno <!-- AC-9182 -->

* I moduli di spedizione Temando sono stati rimossi dalla base di codice di Commerce principale. Questa funzionalità è stata deprecata in Adobe Systems Commerce 2.4.4. <!-- AC-8130 -->
...
* I commercianti aziendali possono ora configurare fino a un milione di **regole** attive sui prezzi del carrello basate su coupon in Adobe Systems Commerce senza significativi degradazioni delle prestazioni delle operazioni di carrello e checkout. <!-- ACPT-1198 -->
...
* **Formato JSON ora supportato per l&#39;API REST [Importa](https://developer.adobe.com/commerce/webapi/rest/modules/import/)**. I commercianti possono ora importare fino a 100.000 record al minuto in Adobe Systems Commerce in formato JSON. <!-- ACPT-943 -->

* **Miglioramenti delle vendite regola delle prestazioni**. Miglioramento delle prestazioni delle implementazioni aziendali con molte regole di vendita attive (circa 100.000). Le distribuzioni aziendali che implementare pesantemente le promozioni spesso distribuire molte regole del carrello attive. Questi tipi di distribuzioni aziendali che eseguono Commerce 2.4.7 non vedranno alcun degrado delle prestazioni correlato al numero di regole di prezzo del carrello configurate durante le operazioni di checkout. <!-- ACPT-1198 -->

* **Operazioni di salvataggio più rapide delle configurazioni a livello di store per distribuzioni con molti archivi**. Salvare le impostazioni di configurazione in distribuzioni con più di 500 archivi può richiedere molto tempo. Il nuovo [modulo Async Config](../../../performance/configuration.md#asynchronous-configuration-save) consente operazioni di salvataggio della configurazione asincrona eseguendo un cron job che utilizza un consumer per elaborare l&#39;operazione di salvataggio in un messaggio coda. AsyncConfig è disabilitato per impostazione predefinita.

* **Generazione più rapida della cache di configurazione per configurazioni di grandi dimensioni**. Il `bin/magento cache:clean config` comando ora preriscalda la cache di configurazione quando la cache di configurazione è abilitata. Ciò riduce i tempi di inattività necessari per generare la cache di configurazione per configurazioni di grandi dimensioni. Le operazioni di salvataggio della configurazione non puliscono più la cache prima di `config_scopes` scrivere i dati nella cache, il che riduce anche il tempo in cui altre richieste vengono bloccate durante la scrittura dei dati di configurazione.

### Server applicazioni GraphQL

[GraphQL Application Server](../../../performance/application-server.md) consente a Adobe Systems Commerce di mantenere lo stato tra le richieste API GraphQL di Commerce ed elimina la necessità di bootstrap, il che si traduce in un throughput più elevato, una latenza inferiore e un uso efficiente delle risorse per tutte le API GraphQL. Condividendo applicazione stato tra i processi, le richieste API GraphQL diventano significativamente più efficienti e le risposte API GraphQL sono in media il 30% più veloci.<!-- ACPT-27 -->
...
>[!NOTE]
>
>GraphQL Application Server non è attualmente compatibile con [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/). I clienti di Adobe Commerce sull&#39;infrastruttura cloud che attualmente utilizzano [!DNL AWS S3] per l&#39;[archiviazione remota](../../../configuration/remote-storage/cloud-support.md) non possono utilizzare GraphQL Application Server fino a quando Adobe non rilascerà un hotfix più tardi nel 2024.<!-- ACPT-1840 -->

### Metapacchetto dell&#39;estensione Adobe Systems Commerce

Questa versione introduce il metapacchetto dell&#39;estensione Adobe Systems Commerce v2.0.0, che raggruppa automaticamente alcune estensioni Adobe Systems Commerce con questa versione principale. Le versioni di queste estensioni incluse in questo metapacchetto vengono installate quando `composer update` viene eseguito, semplificando il processo di aggiornamento di queste estensioni durante l&#39;aggiornamento all&#39;ultima versione principale. Queste estensioni mantengono pianificazioni di rilascio indipendenti.

Il metapacchetto dell&#39;estensione Adobe Systems Commerce per Adobe Systems Commerce 2.4.7 include le seguenti estensioni:

* [Adobe Systems Commerce Admin interfaccia SDK](https://developer.adobe.com/commerce/extensibility/admin-ui-sdk/)
* [Integrazione di Adobe Systems Commerce con Adobe Systems IMS](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html)
...
* **Supporto 3DS per[!DNL Google Pay]**: incluso il supporto per la verifica 3DS per le [!DNL Google Pay] carte non tokenizzate. Per ulteriori informazioni, consulta la [documentazione](https://developer.paypal.com/braintree/docs/guides/3d-secure/client-side/android/v3#using-3d-secure-with-google-pay) Braintree.

* **[!DNL Vault Apple Pay Payments]**- Consentire ai clienti che hanno effettuato l&#39;accesso di depositare/store i loro [!DNL Apple Pay] pagamenti al proprio store di Commerce account da utilizzare per transazioni future. Ciò riduce il numero di passaggi per il checkout e crea un&#39;esperienza di checkout più veloce per il cliente che ritorna.

* **[!DNL Vault Google Pay Payments]**- Consentire ai clienti che hanno effettuato l&#39;accesso di depositare/store i loro [!DNL Google Pay] pagamenti al proprio store di Commerce account da utilizzare per transazioni future. Ciò riduce il numero di passaggi per il checkout e crea un&#39;esperienza di checkout più veloce per il cliente che ritorna.

* **[!DNL Vault Venmo Payments]**- Consente ai clienti connessi di depositare/store i propri [!DNL Venmo] account ai propri store di eCommerce account da utilizzare per transazioni future. Ciò riduce il numero di passaggi per il checkout e crea un&#39;esperienza di checkout più veloce per il cliente che ritorna.

* **[!DNL Vault ACH Payments]**- Consente ai clienti connessi di depositare/store i pagamenti ACH al proprio account di store di Commerce da utilizzare per transazioni future. Ciò riduce il numero di passaggi per il checkout e crea un&#39;esperienza di checkout più veloce per il cliente che ritorna.

* **[!UICONTROL Express Payment]pulsanti nella parte superiore del checkout**: per incoraggiare un&#39;esperienza di checkout più veloce, abbiamo introdotto [!DNL Express Payment] delle opzioni all&#39;inizio del checkout. I clienti ora possono pagare tramite [!DNL PayPal], [!DNL PayPal Pay Later], [!DNL Apple Pay], e [!DNL Google Pay Express] pagamenti.

* **[!DNL Braintree]Collegamenti note sulla versione e supporto all&#39;interno della configurazione** amministratore: i commercianti possono ora collegare direttamente dall&#39;amministratore delle vendite per Braintree supporto e note sulla versione online.
...
* **Supporto per l&#39;annullamento dell&#39;ordine**. La mutazione [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) consente a un cliente di annullare un ordine, trasmettendo il relativo identificatore e un motivo di annullamento. <!-- LYNX-197 -->
   ...
   * I campi nuovo `order_cancellation_enabled` e `order_cancellation_reasons.description` di risposta nella finestra di dialogo `storeConfig`
Supporto per le query utente richieste di annullamento degli ordini avviate. Consulta [Interrogazione della configurazione di annullamento dell&#39;ordine di un store](https://developer-stage.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-order-cancellation-configuration)

* **Supporto migliorato** per attributi personalizzati. Il supporto degli attributi personalizzati di GraphQL è stato migliorato arricchendo i dati API per supportare tutti i tipi di attributi. Lo schema degli attributi EAV di GraphQL ora supporta l&#39;estensione degli attributi del cliente e degli oggetti indirizzo del cliente nell&#39;amministratore e il loro recupero tramite GraphQL. Le aree specifiche di miglioramento includono: <!-- LYNX-61 110-->
   ...
   * Supporto degli attributi personalizzati esistenti migliorato per i prodotti

* **Le funzionalità avanzate di caching di GraphQL migliorano la velocità** di caricamento della pagina. A queste query è stata aggiunta la funzionalità di memorizzazione nella cache, migliorando la velocità del tempo di caricamento delle pagine per la maggior parte delle pagine PWA:
...
* **Prestazioni del parser GraphQL migliorate**. Le prestazioni del parser GraphQL sono state migliorate riducendo il numero di volte in cui il metodo di analisi viene chiamato per richiesta. Ora si chiama una volta. In precedenza, il parser veniva chiamato almeno tre volte.
...
* [`attributeForms`](https://developer.adobe.com/commerce/webapi/graphql/schema/attributes/queries/attributes-form/) quesito<!-- LYNX-151 -->
* [`attributesList`](https://developer.adobe.com/commerce/webapi/graphql/schema/attributes/queries/attributes-list/) quesito<!-- LYNX-150 -->
...
* [`recaptchaFormConfig`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/recaptcha-form-config/) quesito
...
* [`confirmEmail`](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/mutations/confirm-email/) mutazione
* [`createGuestCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/create-guest-cart/) mutazione
* [`estimateShippingMethods`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/estimate-shipping-methods/) mutazione
* [`estimateTotals`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/estimate-totals/) mutazione
* [`removeCouponsFromCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/remove-coupons/) mutazione
...
PWA Studio v14.0 è compatibile con Adobe Systems Commerce 2.4.7. Include diversi miglioramenti per migliorare l&#39;accessibilità. Per informazioni sulla correzione dei bug, vedere [PWA Studio versioni](https://github.com/magento/pwa-studio/releases). Consulta [Versione compatibilità](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) per un elenco delle versioni di PWA Studio e delle relative versioni di base compatibili di Adobe Systems Commerce.
...
>[!NOTE]
>
>Per la prima volta, diverse route dell&#39;endpoint REST contengono la stringa `V2` anziché `V1`. Ad esempio, `GET /rest/default/V2/carts/{cartId}/coupons`. Per i commercianti Commerce, questi endpoint hanno lo scopo di sostituire le `V1` versioni degli endpoint. Questi endpoint sono disponibili solo in Adobe Systems Commerce.<!-- LYNX-208 -->

Restituisci tutti i codici coupon associati ad un carrello:

`GET  /rest/{store_code}/V2/carts/{cartId}/coupons`
`GET  /rest/{store_code}/V2/carts/mine/coupons`

Aggiungi un codice coupon al carrello:
...
**Endpoint aggiuntivi**
Questa versione introduce due nuovi endpoint REST che forniscono una soluzione alternativa per una limitazione con gli endpoint REST API `GET` e `POST V1/products/attributes`. Questi endpoint restituiscono lo stesso valore per l&#39;attributo `is_filterable` per entrambe le opzioni `Filterable(with results)` e `Filterable(no results)` dell&#39;opzione **[!UICONTROL Use in Layered Navigation]**. (La `is_filterable` proprietà attribute è di tipo `Boolean`, che non consente di impostare questa proprietà su `Filterable(no results)`.)
...
* `bin/magento setup:upgrade` è stato sottoposto a refactoring per essere eseguito correttamente durante l&#39;installazione di un nuovo modulo che installa sia le tabelle che gli indicizzatori associati `mview` . [GitHub-37304](https://github.com/magento/magento2/issues/37304)

<!-- ACP2E-1882 -->

* Il ripristino del database non riesce più a causa di un errore di delimitatore. In precedenza, Commerce generava questo errore quando veniva eseguito `bin/magento setup:rollback --db`: `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`.
...
* I report sulle entrate vengono ora visualizzati come previsto nel portale Google Analytics (GA4). [GitHub-37605](https://github.com/magento/magento2/issues/37605)

<!-- ACP2E-1880 -->

* `price` e `quantity` i valori nel livello dati di Google Tag Manager vengono ora inviati come valori numerici previsti per Google Analytics. In precedenza, questi valori venivano inviati come stringhe.

<!-- ACP2E-2030 -->

* Gli errori nella gestione dei dati di intestazione durante l&#39;invio dei dati di distribuzione al modulo Nuovo Relic sono stati risolti.

<!-- ACP2E-1833 -->

* Google Analytics 3 tag vengono attivati come previsto quando i tag Google Analytics 4 sono disabilitati.
...
* La `customerCart` query restituisce ora tutti gli sconti applicati sui prodotti in bundle come previsto. In precedenza, gli sconti totali applicati a un prodotto in bundle venivano restituiti come zero.
...
* Ora puoi salvare un aggiornamento pianificato di un prodotto bundle che include un prodotto semplice con un aggiornamento pianificato indefinito. Quando viene creata l’istantanea del prodotto del bundle, i prodotti collegati vengono ora acquisiti per ID, ID entità e anche per SKU. In precedenza, la versione originale del prodotto semplice veniva controllata quando veniva salvato l&#39;aggiornamento pianificato sul prodotto bundle anziché la versione corrente del prodotto semplice.

<!-- ACP2E-1650 -->

* È ora possibile utilizzare il `GET V1/shipment/` percorso API per creare una spedizione con un prodotto bundle quando il prodotto ha il **[!UICONTROL together]** tipo di spedizione impostato. Anche i prodotti in bundle vengono ora convalidati in base alla proprietà del tipo di spedizione.

<!-- ACP2E-2038 -->

* Gli acquirenti possono ora modificare la quantità di un prodotto in bundle nel loro carrello della spesa. In precedenza, Adobe Systems Commerce non gestiva valori null per `ItemId` quando i prodotti venivano modificati nel carrello della vetrina e visualizzavano un errore. [GitHub-37696](https://github.com/magento/magento2/issues/37696)
...
* Il plug-in il cui scopo è quello di aggiungere il tag della cache configurabile quando viene salvato un prodotto semplice associato a un prodotto configurabile ora funziona come previsto. Tutti i tag della cache configurabili rilevanti vengono rimossi come previsto quando un prodotto semplice viene aggiornato da `POST V1/products`. In precedenza, non tutti i prezzi venivano aggiornati sul negozio e le cache dovevano essere cancellate manualmente. [GitHub-36726](https://github.com/magento/magento2/issues/36726)

### Carrello e pagamento

<!-- ACP2E-2658 -->

* Commerce ora ottimizza le prestazioni non caricando i dati delle offerte specifiche per l’utente durante la generazione di pagine memorizzabili nella cache a pagina intera, come le pagine dei dettagli del prodotto (PDP) e le pagine CMS, quando il carrello permanente è abilitato. In precedenza, il sistema caricava inutilmente dati di sessioni e preventivi specifici per utente durante la generazione di queste pagine, il che influiva sulle prestazioni.

<!-- ACP2E-2684 -->

* Gli attributi personalizzati dell&#39;indirizzo personalizzato vengono ora salvati come previsto nel database. In precedenza, i valori degli attributi salvati nel database erano preceduti dal codice di attributo.
...
* Adobe Systems Commerce ora visualizza un messaggio di errore informativo quando si verifica un errore durante il checkout e non restituisce più il acquirente a un passaggio di estrazione precedente. In precedenza, il acquirente veniva restituito alla pagina di spedizione.

<!-- ACP2E-2520 -->

* La `cart` query non restituisce più il metodo di spedizione e l&#39;indirizzo per i carrelli virtuali. In precedenza, quando un carrello conteneva sia prodotti fisici che virtuali e tutti i prodotti fisici venivano rimossi, la risposta alla query del carrello continuava a includere informazioni sulla spedizione.

<!-- ACP2E-2267 -->

* Commerce ora visualizza messaggi informativi quando viene attivata un&#39;azione di aggiunta al carrello. In precedenza, non `The requested qty is not available` veniva sempre visualizzato come necessario sulla vetrina.

<!-- ACP2E-2166 -->

* I tempi di caricamento della pagina di checkout sono stati migliorati per i clienti con rubriche di grandi dimensioni. Commerce ora elabora solo il numero di indirizzi dei clienti specificato nell&#39;impostazione **[!UICONTROL Customer Addresses Limit]** . In precedenza, Commerce caricava l&#39;intera rubrica.
...
* Il `AdvancedSalesRule` filtro degli attributi prodotto ora funziona correttamente con il tipo di back-end dell&#39;attributo decimale. Di conseguenza, le regole del carrello sono `Coupon Type` impostate per **[!UICONTROL No Coupon]** funzionare come previsto.
...
* I coupon vengono ora applicati correttamente a un ordine quando un acquirente applica il coupon dopo aver selezionato il metodo di spedizione a tariffa fissa in cui si applica una regola di prezzo del carrello con il metodo di spedizione impostato come condizione. In precedenza, nella pagina del carrello veniva visualizzato l&#39;errore Coupon non valido e il coupon non veniva applicato. [GitHub-34866](https://github.com/magento/magento2/issues/34866)

<!-- AC-8596 -->

* La **[!UICONTROL My billing and shipping address are the same]** casella di controllo ora rimane selezionata per impostazione predefinita quando un acquirente utilizza Effetto cromatura completamento automatico per compilare il proprio indirizzo di spedizione e quindi fa clic su **Successivo** prima che il blocco dei metodi di spedizione venga ricaricato. [GitHub-33725](https://github.com/magento/magento2/issues/33725)
...
* L&#39;API guest richiede di `POST V1/guest-carts/<cartId>/shipping-information` non restituire più uno `cancel` stato se la pagina viene ricaricata durante login in un ambiente mobile. In precedenza, Adobe Systems Commerce generava un errore 500 e registrava questa eccezione: `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`.

<!-- AC-7886 -->

* L&#39;idoneità alla spedizione gratuita viene ora calcolata come previsto quando viene applicata la **[!UICONTROL Subtotal (Excl. Tax)]** condizione per le regole del carrello. [GitHub-36760](https://github.com/magento/magento2/issues/36760)

<!-- AC-6672 -->

* La `cart` query restituisce ora il prezzo di livello corretto per un prodotto. [GitHub-29655](https://github.com/magento/magento2/issues/29655)

<!-- ACP2E-1162 -->

* Sono stati aggiunti messaggi di errore informativi alla pagina di pagamento per guidare gli acquirenti quando il applicazione di un coupon nella pagina di spedizione è in conflitto con il metodo di spedizione selezionato. In precedenza, un messaggio indicava agli acquirenti di tornare alla pagina precedente.
...
* Il `If an item is FOUND/NOT FOUND in the cart with ALL/ANY of these conditions true` prezzo regola condizione del catalogo ora funziona correttamente con gli attributi categoria e referenza di magazzino. In precedenza, questa condizione non veniva applicata correttamente nella logica di coupon del prezzo del carrello e non valido coupon venivano applicati agli ordini. [GitHub-37660](https://github.com/magento/magento2/issues/37660)
...
* Le regole del prezzo del carrello vengono ora applicate come previsto quando un regola del prezzo del carrello relativo a un metodo di spedizione è configurato per il store e questo metodo di spedizione viene modificato in un altro durante il check-out. In precedenza, il `applied_rule_ids` valore della tabella non veniva mai modificato e la `sales_order_item` regola del prezzo del carrello non veniva applicata all&#39;ordine.

### Catalogo

<!-- AC-10746 -->

* Se non specificato, l&#39;esecuzione `bin/magento cache:clean` o `bin/magento cache:flush` meno esegue lo scaricamento dell&#39;intera cache incorporata o di Varnish a pagina intera. [GitHub-38301](https://github.com/magento/magento2/issues/38301)

<!-- ACP2E-2257 -->

* I commercianti possono ora utilizzare il carosello di prodotti per creare blocchi CMS quando `catalog_product_price dimensions-mode` è impostato su `website`. In precedenza, Commerce non salvava il blocco e generava questo errore SLQ: `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.
...
* I prezzi dei prodotti del pacchetto sono ora calcolati correttamente e un problema con l&#39;ordine delle operazioni durante il calcolo dei prezzi è stato risolto. [GitHub-35665](https://github.com/magento/magento2/issues/35665)
...
* I valori definiti dall&#39;utente `special_from_date`, `special_to_date`, `news_from_date`e `news_to_date` i valori non vengono ignorati quando un prodotto viene modificato in ambito di visualizzazione store.

<!-- AC-6761 -->

* Commerce ora visualizza il nome corretto dopo l&#39;ordinamento dei prodotti per nome in Admin > **[!UICONTROL Category]** > **[!UICONTROL Products in category]** nelle distribuzioni multistore in cui i nomi dei prodotti variano in base store. [GitHub-36208](https://github.com/magento/magento2/issues/36208)

<!-- ACP2E-1975 -->

* La `addProductsToCart` mutazione ora restituisce opzioni personalizzabili con dati. In precedenza, restituiva solo opzioni personalizzabili vuote. [GitHub-37599](https://github.com/magento/magento2/issues/37599)
...
* I commercianti possono ora aggiornare la data o l&#39;ora di inizio futura di una modifica pianificata per il prodotto scaricabile. In precedenza, Adobe Systems Commerce generava questo errore durante l&#39;aggiornamento della data o dell&#39;ora di inizio futura per il prodotto scaricabile: `The downloadable link isn't related to the product. Verify the link and try again`.

<!-- ACP2E-1943 -->

* Lo stato di backorder del prodotto viene ora visualizzato correttamente sullo storefront. In precedenza, i prodotti disponibili per la spedizione venivano erroneamente identificati come arretrati.

<!-- ACP2E-1948 -->

* L&#39;accesso ai prodotti scaricabili dall&#39;amministratore non genera più un errore quando l&#39;utente dell&#39;amministratore modifica la visualizzazione store dalla visualizzazione utilizzata durante la creazione del prodotto a un&#39;altra vista store. I prezzi dei prodotti vengono ora convertiti correttamente in virgola mobile quando un utente amministratore passa store visualizzazione da tutte le visualizzazioni store alla vista store predefinita. In precedenza, Adobe Systems Commerce generava un errore. [GitHub-37519](https://github.com/magento/magento2/issues/37519)

<!-- ACP2E-1994 -->

* I prodotti esportati ora hanno il valore `manage_stock` corretto quando il valore `use_config_manage_stock` è uguale a `1`. In precedenza, questo valore predefinito non era corretto.

<!-- ACP2E-1479 -->

* Adobe Systems Commerce non visualizza più un messaggio di avviso quando si salva un prodotto creato con opzioni personalizzate dopo l&#39;eliminazione dell&#39;immagine.

<!-- ACP2E-1419 -->

* Le miniature dei prodotti vengono ora visualizzate come previsto nell&#39;e-mail di avviso di stock del prodotto inviata quando un prodotto è di nuovo disponibile. In precedenza, questa immagine non veniva visualizzata nel messaggio e-mail di avviso

<!-- AC-3003 -->

* Il conteggio dei prodotti nell&#39;albero delle categorie (Admin **[!UICONTROL Catalog]** > **[!UICONTROL Category]**) è ora accurato. In precedenza, non era possibile recuperare il conteggio dei prodotti della categoria dalla tabella `catalog_category_product_index`. È stato corretto l’errore di battitura che ha attivato questo bug. [GitHub-35417](https://github.com/magento/magento2/issues/35417)
...
* I prodotti semplici esauriti che sono opzioni per un prodotto configurabile ora vengono visualizzati come disabilitati nella vetrina quando **[!UICONTROL Display Out of Stock Products]** è impostato su **Sì**. In precedenza, tutte le opzioni venivano visualizzate come disponibili, nonostante il loro stato e la reale disponibilità.

<!-- ACP2E-1384 -->

* L&#39;aggiornamento dell&#39;attributo ambito del sito Web in una visualizzazione store specifica non sostituisce più il valore di tale attributo nella ambito globale. In precedenza, l&#39;importazione dei prezzi dei prodotti quando erano disponibili più righe con la stessa referenza di magazzino generava `store_view_code` prezzi imprecisi per gli ambiti di visualizzazione predefiniti e Tutti gli Store.

<!-- ACP2E-1632 -->

* I commercianti possono ora aggiungere un&#39;immagine GIF alla galleria di immagini di un prodotto dall&#39;amministratore. In precedenza, Adobe Systems Commerce visualizzava questo errore: `imagecolorsforindex(): Argument #2 ($color) is out of range`.
...
* Il numero di record in sospeso nella `cron_schedule` tabella è stato ridotto impedendo ai cron job di tentare di acquisire blocchi cron job non necessari. In precedenza, i cron job tentavano di acquisire i blocchi quando era troppo tardi per eseguire il lavoro. I cron job non `cron_schedule` sono stati contrassegnati come mancati lineare anche se avevano già superato l&#39;orario pianificato. Anche Commerce ha visualizzato ripetutamente questo errore nel registro degli errori: `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`.

<!-- AC-9207 -->

* È stato risolto l&#39;errore irreversibile di divisione per zero nelle espressioni cron. [GitHub-37804](https://github.com/magento/magento2/issues/37804)
...
* Gli utenti amministratori possono ora visualizzare quale attività è attualmente in esecuzione nel `bin/magento cron:run` processo. Il titolo del processo attualmente in esecuzione riporta ora i nomi del gruppo e dei processi. [GitHub-34321](https://github.com/magento/magento2/issues/34321)
...
* I nomi dei mittenti delle e-mail non possono più contenere due punti. Una nuova regola di convalida ora impedisce il salvataggio dei nomi dei mittenti che contengono i due punti della sezione Indirizzi e-mail per l&#39;archivio dell&#39;amministratore **[!UICONTROL Stores]** > **[!UICONTROL General]**. In precedenza, i commercianti potevano salvare un nome mittente che conteneva due punti, causando errori del server di posta.

<!-- ACP2E-2018 -->

* Immagine opzioni non vengono più visualizzate come markup HTML nelle e-mail di restituzione dei clienti (modello predefinito) per i prodotti con opzioni di immagine personalizzate.
...
* È stato chiarificato il messaggio di `StockItemValidator.php` errore associato quando viene fornito un ID stock con valore 0 durante una chiamata API. [GitHub-31107](https://github.com/magento/magento2/issues/31107)

<!-- AC-10583 -->

* Aggiunto un array vuoto predefinito a `$tagGenerators` in `CompositeTagGenerator` per evitare errori durante la compilazione dell&#39;inserimento delle dipendenze. In precedenza, l&#39;esecuzione del `bin/magento setup:upgrade` comando quando i `Magento_DirectoryGraphQl` moduli e `Magento_StoreGraphQl` erano disabilitati causava un errore. [GitHub-38165](https://github.com/magento/magento2/issues/38165)

<!-- AC-9232 -->

* La classe inutilizzata `AlgorithmProviderFactory` è stata rimossa così come le dipendenze da questa classe in classi tra cui `JweAlgorithmManagerFactory.php`, `JweContentAlgorithmManagerFactory.php`, e `JwsAlgorithmManagerFactory`. [GitHub-37783](https://github.com/magento/magento2/issues/37783)

<!-- AC-9776 -->

* Il `var/log/system.log` file non contiene `main.INFO error` più messaggi. Il registro delle voci di menu è stato spostato dal livello INFO a DEBUG. In precedenza, il `var/log/system.log` file era inondato di messaggi like questo: `Add of item with id Magento_Theme::design_config was processed … `.

<!-- AC-8809 -->

* Il precaricamento di Redis non riesce più quando `exec()` restituisce false. È stata aggiunta la verifica al processo di precaricamento per impedire a `array_combine` di ricevere `false` come secondo argomento. Di conseguenza, il precaricamento continuerà a essere eseguito quando `exec()` viene restituito `false`. [GitHub-37509](https://github.com/magento/magento2/issues/37509)
...
* Aggiunta la nuova valuta nicaraguense (NIO) all&#39;elenco delle valute disponibili.

<!-- AC-10591 -->

* Il nome della `reponseBody` variabile è stato modificato in `responseBody` .`app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`[GitHub-38093](https://github.com/magento/magento2/issues/38093)

<!-- AC-9221 -->

* Sono stati risolti gli errori di arrotondamento del subtotale nelle transazioni per i prodotti per i quali è stato abilitato il quantità decimale. [GitHub-37817](https://github.com/magento/magento2/issues/37817)

<!-- ACP2E-2127 -->

* Il filtro delle date per la griglia del cliente amministratore ora funziona come previsto in base alle impostazioni internazionali specificate.
...
* `bin/magento config:show` E `set` i comandi che utilizzano `$_ENV` variabili ora supportano l&#39;utilizzo di siti Web e codici store contenenti caratteri camelcase o maiuscoli.

<!-- AC-8923 -->

* Aggiunte classi con `!important` as usato in Tailwind 3 per il supporto del layout CSS. [GitHub-37568](https://github.com/magento/magento2/issues/37568)

<!-- AC-9115 -->

* Una dipendenza mancante `jQuery` è stata aggiunta al `trim-input.js` file. [GitHub-37683](https://github.com/magento/magento2/issues/37683)

<!-- AC-9387 -->

* I dettagli specifici dell&#39;evento `log` vengono ora passati come secondo attributo (`context`) del metodo. [GitHub-37879](https://github.com/magento/magento2/issues/37879)
...
* `Param $options[`position`]` per i simboli di valuta ora funziona come previsto e supporta la personalizzazione della posizione del simbolo di valuta. In precedenza, dopo la migrazione da Zend Framework 1, il metodo `toCurrency` ignorava il parametro `position`. Questo problema si è verificato in tutta l&#39;interfaccia di amministrazione.

<!-- AC-8817 -->

* È stato modificato il codice obsoleto in relazione alla creazione di proprietà dinamiche in tutta la codebase.
...
* L&#39;istanza del provider di blocco passa ora attraverso `\Magento\Framework\Lock\Proxy` la classe. Il provider di blocco è ora configurato con l&#39;impostazione `lock` attraverso il `app/etc/env.php` file.

<!-- ACP2E-1492 -->

* Mutex è stato implementato per gli ordini per prevenire le condizioni di gara durante l&#39;aggiornamento da richieste simultanee. In precedenza, le richieste simultanee (race conditions) di annullamento degli ordini causavano voci duplicate nella `inventory_reservation` tabella.

<!-- ACP2E-1381 -->

* La `populateWithArray` funzione ora trasforma correttamente le proprietà dell&#39;oggetto in snake case, rendendolo compatibile con i getter e i `AbstractModel` setter. In precedenza, la formattazione delle maiuscole e minuscole era errata quando il nome dell&#39;attributo dati conteneva diverse lettere maiuscole di fila.

<!-- ACP2E-1522 -->

* Il valore di incremento automatico nella tabella `catalog_product_entity_varchar` ora viene incrementato correttamente dopo il salvataggio di un prodotto. In precedenza, questo valore aumentava di dieci.

<!-- ACP2E-1701 -->

* Adobe Systems Commerce non genera più questa eccezione del file system quando si tenta di svuotare la cache JavaScript/CSS: `No such file or directory`.

<!-- ACP2E-1358 -->

* Il valore di `value_id` in `customer_entity_int` viene ora incrementato correttamente. Viene eseguita una query di aggiornamento anziché durante l&#39;aggiornamento di `insert on duplicate key update` un&#39;entità che incorpora questi attributi. Di conseguenza, le colonne di incremento automatico utilizzate nel modello EAV ora crescono in modo lineare. In precedenza, il processo di incremento automatico per le colonne ignorava i valori a causa di query di inserimento non riuscite. [GitHub-28387](https://github.com/magento/magento2/issues/28387)
...
* Adobe Commerce non genera più un’eccezione nel file di registro quando un utente prova a stampare un’etichetta di spedizione. In precedenza, se un&#39;estensione modificava le intestazioni di risposta che contengono un allegato, il sistema generava un&#39;eccezione nel file di registro.

<!-- AC-7986 -->

* Il supporto per più classi HTML è stato aggiunto a `elements.xsd`. [GitHub-36891](https://github.com/magento/magento2/issues/36891)
...
* La pianificazione degli aggiornamenti dei prodotti non cancella più i dati degli attributi a selezione multipla. In precedenza, quando veniva pianificato un aggiornamento per un prodotto, i dati degli attributi a selezione multipla del prodotto venivano cancellati per errore. [GitHub-37675](https://github.com/magento/magento2/issues/37675)

<!-- AC-2904 -->

* Il salvataggio di un prodotto con un ambito di store non predefinito non comporta più l&#39;attribuzione di attributi invariati all&#39;ambito store quando vengono caricati con `ProductRepository`. [GitHub-8897](https://github.com/magento/magento2/issues/8897)

<!-- ACP2E-2132 -->

* Gli acquirenti possono ora accesso pagine configurate come nodi gerarchici quando sono abilitate accesso restrizioni. In precedenza, si era verificato un ciclo di reindirizzamento infinito e il browser ha generato questo errore: `ERR_TOO_MANY_REDIRECTS`.
...
* Il prodotto correlato regola&#39;s **Is one of** condition ora funziona correttamente se configurato con l&#39;impostazione **Prodotti corrispondenti** . In precedenza, questo regola non identificava alcun prodotto.
...
* È ora possibile salvare correttamente le modifiche nelle pagine di amministrazione e nella griglia Blocca dopo aver eliminato una colonna. [GitHub-37525](https://github.com/magento/magento2/issues/37525)

<!-- ACP2E-1933 -->

* Gli amministratori dell&#39;azienda possono ora aggiungere utenti aziendali dalla vetrina. In precedenza, Commerce registrava questo errore quando l&#39;amministratore utente tentava di aggiungere un nuovo utente: `report.CRITICAL: Error: Call to a member function __toArray() on null in app/code/Magento/LoginAsCustomerLogging/Observer/LogSaveCustomerObserver.php:123`.

<!-- ACP2E-1972 -->

* I clienti possono ora accedere correttamente con un nuovo password da una dispositivo dopo aver reimpostato il password in un dispositivo diverso.
...
* Adobe Systems Commerce non genera più un errore di violazione del vincolo quando si deseleziona per la prima volta, quindi si seleziona una pagina CMS appena creata.

<!-- ACP2E-1639 -->

* Una directory principale gerarchica del sito Web è ora correttamente assegnata a una pagina CMS appena creata. In precedenza, la selezione principale del sito Web della gerarchia non corrispondeva nelle pagine CMS appena create.

<!-- ACP2E-1508 -->

* Gli errori non si verificano più durante le richieste di un feed RSS per le categorie quando **[!UICONTROL RSS Feed Top Level Category]** è abilitato. In precedenza, l&#39;esplorazione della pagina della categoria feed RSS causava errori Elasticsearch `CRITICAL` nei file di registro quando **[!UICONTROL RSS Feed Top Level Category]** era abilitato.

<!-- ACP2E-1490 -->

* Il valore corretto viene ora salvato per `gws_store_groups` quando viene modificato ambito ruolo durante utente creazione di ruolo. In precedenza, quando ambito di ruolo veniva selezionato come `All`, il valore veniva salvato come nullo durante utente creazione ma non durante la `gws_store_groups` modifica del ruolo. Inoltre, se è stato selezionato ambito ruolo come `Custom`, il `gws_store_groups` valore è sempre stato salvato come null.

<!-- ACP2E-1446 -->

* È ora possibile accedere alla pagina reimpostare la password facendo clic sull&#39;collegare Admin reimpostare la password nella pagina Admin login quando l&#39;impostazione **[!UICONTROL Add Store Code to Urls]** è abilitata. Il reimpostare la password amministratore collegare precedentemente aperto la pagina login o 404.
...
* Facendo clic sul **[!UICONTROL Send invitation]** pulsante (Admin **[!UICONTROL Marketing]** > **[!UICONTROL Private sales]** > **[!UICONTROL Invitation]**) viene ora inviata una richiesta POST insieme alla chiave del modulo e l&#39;invito viene inviato correttamente. In precedenza, quando si faceva clic su questo pulsante, non erano presenti né il metodo HTTP POST né il tasto modulo.

<!-- ACP2E-1608 -->

* I file a zero byte possono ora essere copiati con successo nello storage remoto con AWS S3.

<!-- AC-7402 -->

* Gli utenti Amministratore possono ora aggiungere un nuovo indirizzo cliente dall&#39;Amministratore quando il cliente è stato creato per un altro sito Web il cui ID store non corrisponde all&#39;ID del sito Web. In precedenza, Adobe Systems Commerce visualizzava questo messaggio finestra a comparsa: `Something went wrong`. [GitHub-36582](https://github.com/magento/magento2/issues/36582)

<!-- AC-8247 -->

* Il blocco CMS editor pagina ora visualizza il valore corretto del widget `block_id` . [GitHub-29644](https://github.com/magento/magento2/issues/29644)

### Carte regalo e confezioni

<!-- AC-7372 -->

* I commercianti non possono più creare un scheda regalo con un valore negativo.

<!-- ACP2E-1890 1806-->

* La `setGiftOptionsOnCart` mutazione ora rimuove la confezione regalo da un ordine come previsto. Il valore di `gift_wrapping_id` non è impostato a livello di carrello. In precedenza, il `gift_wrapping_id` veniva aggiornato al valore precedente dopo che l&#39;ordine era stato effettuato lineare quando questo valore veniva annullato a livello di carrello.

<!-- ACP2E-1866 -->

* Gli account scheda regalo rimangono ora attivi come previsto quando il rimborso parziale di un prodotto semplice viene elaborato dall&#39;ordine a cui è stato applicato il scheda. In precedenza, il account scheda regalo era stato eliminato.

<!-- ACP2E-1805 -->

* La `setGiftOptionsOnCart` mutazione ora aggiorna correttamente il database quando si rimuove un messaggio regalo dal carrello. In precedenza, i dettagli del messaggio non venivano aggiornati nel database e il messaggio regalo non veniva rimosso dal carrello.

<!-- ACP2E-1741 -->

* L&#39;etichetta del registro `Event Information` dei regali viene ora tradotta come previsto.
...
* L&#39;emulazione ora viene avviata durante `send()` le chiamate una volta che l&#39;emulazione è stata completata durante la `getInfoBlockHtml()` chiamata e non viene registrato alcun errore. In precedenza, il `system.log` file è stato inondato da questo errore: `main.ERROR: Environment emulation nesting is not allowed`. [GitHub-35603](https://github.com/magento/magento2/issues/35603)

<!-- ACP2E-1612 -->

* Ora puoi utilizzare la `updateCartItems` mutazione per aggiornare un scheda di un regalo in un carrello. In precedenza, solo l&#39;importo scheda poteva essere aggiornato nel carrello da questa mutazione.

<!-- ACP2E-1669 -->

* La confezione regalo è ora inclusa come previsto in un&#39;offerta basata sui carrelli uniti. In precedenza, i messaggi regalo venivano uniti correttamente, ma non le selezioni a capo.

<!-- AC-7530 -->

* La `addProductsToCart` mutazione ora aggiunge con successo un scheda regalo a un carrello. In precedenza, questa query prevedeva il nome del destinatario della gift card, ma è stato restituito il nome del mittente. Adobe Systems Commerce ha generato questo errore: `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* Il `products` filtro di prodotto delle query può ora restituire risultati di corrispondenza parziale e corrispondenze complete. Il nuovo `match_type` attributo (PARTIAL, FULL) consente di specificare il tipo di corrispondenza. In precedenza, la query restituiva solo corrispondenze complete.

<!-- ACP2E-2155 -->

* Mutex è stato aggiunto alla query per impedire condizioni `addConfigurableProductsToCart` di gara durante un aggiornamento da richieste simultanee. In precedenza, quando un commerciante inviava due richieste parallele per aggiungere lo stesso prodotto configurabile a un carrello, venivano aggiunti due articoli separati con lo stesso referenza di magazzino di prodotto. [GitHub-37847](https://github.com/magento/magento2/issues/37847)
...
* Sono state migliorate le prestazioni della query per le `setShippingAddressForEstimate` operazioni relative al carrello che coinvolgono attributi di prodotto personalizzati. In precedenza, la query caricava tutti gli attributi del prodotto quando i parametri venivano inviati come variabili.

<!-- ACP2E-2255 -->

* Il `products` campo query `ConfigurableProduct.configurable_options` ora restituisce solo i valori che hanno almeno un prodotto abilitato. In precedenza, questo campo restituiva prodotti semplici disabilitati.

<!-- ACP2E-2151 -->

* Sono state migliorate le prestazioni del generando il `GetCategories query` numero di query SQL non necessarie.

<!-- AC-8623 -->

* La gestione degli errori di indirizzamento per i componenti libreria GraphQL ora funziona come previsto.

<!-- AC-9058 -->

* La `products` query ora restituisce tutti i dettagli dei prezzi rilevanti della vetrina quando i prezzi dinamici sono impostati su no e vengono applicati sconti. In precedenza, la fascia di prezzo per i prodotti non veniva restituita. [GitHub-35649](https://github.com/magento/magento2/issues/35649)

<!-- ACP2E-1744 -->

* I nomi delle transazioni GraphQL ora includono i nomi delle query di primo livello solo nei registri di Nuovo Relic. In precedenza, i nomi delle transazioni includevano anche nomi di query secondarie.
...
* Quando l&#39;impostazione Visibilità per un prodotto è impostata su `Catalog`, `products` le query su quel prodotto ora risolvono correttamente l&#39;impostazione. In precedenza, qualsiasi filtro fornito veniva passato all&#39;impostazione `Visibility: Catalog` . [GitHub-36591](https://github.com/magento/magento2/issues/36591)

<!-- ACP2E-1769 -->

* La `customer` query restituisce ora un `product_sale_price` campo che include l&#39;imposta sugli ordini quando il prezzo del prodotto è stato configurato per includere le imposte. [GitHub-36946](https://github.com/magento/magento2/issues/36946)

<!-- ACP2E-1584 -->

* Gli attributi di prodotto di tipo `DateTime` vengono ora associati al `FilterRangeTypeInput` filtro nelle query dei prodotti. In precedenza, questi attributi erano mappati a `FilterMatchTypeInput`. In seguito a questa modifica, le query che filtrano gli attributi `DateTime` richiedono valori `from` e `to` invece di valori di corrispondenza.
...
* La `route` query ora restituisce le route per categorie e prodotti come previsto senza un errore interno del server. [GitHub-36544](https://github.com/magento/magento2/issues/36544)

<!-- ACP2E-1652 -->

* La `products` query ora recupera `url_key` i valori quando sono selezionate più categorie.

<!-- LYNX-1716-->

* `product` Le query ora restituiscono aggregazioni solo per i prodotti assegnati al catalogo condiviso specificato. In precedenza, i dati di aggregazione per i prodotti non assegnati a un catalogo condiviso venivano visualizzati in navigazione a più livelli.

<!-- AC-6680 -->

* Il `OptionValueProvider` metodo class `get()` restituisce ora il valore di un&#39;opzione attributo basata su quanto specificato `option_id` , come previsto. In precedenza, restituiva un errore. [GitHub-35910](https://github.com/magento/magento2/issues/35910)
...
* Il processo di importazione del prodotto ora analizza i valori con il separatore corretto. In precedenza, il processo di importazione utilizzava l&#39;operatore pipe (&quot;|&quot;) per analizzare i valori a selezione multipla fino a quando il separatore di valori determinato non era uguale al valore di default. Il processo di importazione non è successivamente riuscito.
...
* I prodotti con opzioni personalizzabili ora possono essere importati correttamente. In precedenza, i dati delle opzioni venivano persi durante l&#39;importazione. [GitHub-37598](https://github.com/magento/magento2/issues/37598)

<!-- ACP2E-2214 -->

* I caratteri speciali nei file CSV esportati sono ora rappresentati come previsto in Excel. [GitHub-37921](https://github.com/magento/magento2/issues/37921)

<!-- ACP2E-2301 -->

* L&#39;importazione di fonti di magazzino e indirizzi dei clienti utilizzando l&#39;opzione **[!UICONTROL Customers and Addresses (single file)]** viene ora completata correttamente.

<!-- ACP2E-1930 -->

* L&#39;importazione dei prodotti non riesce più a causa della mancanza di memoria. In precedenza, l&#39;importazione di un numero qualsiasi di prodotti in un database che contiene già circa dieci milioni di prodotti non riusciva a causa della mancanza di memoria.

<!-- AC-9116 -->

* I controlli di convalida ora interrompono il processo di importazione come previsto quando non ci sono dati validi da importare e Commerce ora visualizza questo errore: `There are no valid rows to import`. In precedenza, convalida passato in queste condizioni, ma il processo di importazione non è riuscito con questo messaggio: `entity values are mixed`. [GitHub-32905](https://github.com/magento/magento2/issues/32905)

<!-- ACP2E-1754 -->

* L&#39;importazione di chiavi URL con lo stesso prodotto non sovrascrive o elimina più le chiavi di visualizzazione store predefinite esistenti. URL riscritture vengono ora rigenerate per store viste solo quando non è presente alcun valore sostituito `url_key` . In precedenza, l&#39;importazione URL la riscrittura con la stessa chiave URL sovrascrivevano la URL chiave di visualizzazione store predefinita esistente. `key_store`( URL riscritture sono state eliminate, ma la riscrittura URL sul livello di visualizzazione store predefinito per il prodotto era ancora impostata su `key_store`.)

<!-- ACP2E-1875 -->

* Il numero di prodotti non cambia più in modo imprevisto durante l&#39;importazione. Lo scaricamento della cache è ora posticipato a dopo il processo di importazione.
...
* Le configurazioni non predefinite (sito Web o store ambito) aggiunte come variabili di ambiente non interrompono più il `app:config:import` processo con un errore di ricorsione.
...
* Miglioramento delle prestazioni dell&#39;indicizzatore `catalogrule_product` quando non è impostata alcuna regola di catalogo. [GitHub-34784](https://github.com/magento/magento2/issues/34784)

<!-- ACP2E-2204 -->

* Sono stati risolti i problemi relativi alle prestazioni dell&#39;indicizzatore dei prezzi. Le prestazioni dell&#39;indicizzatore non degradano più gradualmente. L&#39;indicizzatore è stato modificato per aumentare la velocità di elaborazione dell&#39;indicizzatore modificando la modalità di creazione delle tabelle temporanee. Queste tabelle temporanee vengono ora ricreate anziché essere affrontate da istruzioni DELETE più dispendiose in termini di tempo.

<!-- ACP2E-2033 -->

* Sono state migliorate le prestazioni dell&#39;indicizzatore dei prezzi. È stato aggiunto un suggerimento indice all&#39;indicizzatore dei prezzi che migliora i tempi di esecuzione delle query MySQL che vengono attivate durante l&#39;indicizzazione.

<!-- AC-9223 -->

* Tutti gli indicizzatori utilizzano ora gli stessi colori per rappresentare lo stato dell&#39;indicizzatore. [GitHub-34648](https://github.com/magento/magento2/issues/34648)

<!-- ACP2E-1697 -->

* Sono state migliorate le prestazioni dell&#39;indicizzatore di prodotto Regola catalogo nelle distribuzioni in cui le regole non sono assegnate a tutti i siti Web.

<!-- ACP2E-1401 -->

* La creazione di un tipo di prodotto personalizzato quando gli indicizzatori sono impostati su **[!UICONTROL Update on schedule]** non causa più l&#39;esito negativo dell&#39;attività di aggiornamento `cron` dell&#39;indice. [GitHub-36471](https://github.com/magento/magento2/issues/36471)

<!-- ACP2E-1574 -->

* Sono stati risolti i problemi di visualizzazione nella pagina dei dettagli del prodotto vetrina dovuti all&#39;indicizzazione delle autorizzazioni del catalogo. In precedenza, Adobe Systems Commerce cancellava prima la tabella dell&#39;indice e poi la reindicizzava durante un processo completo `catalogpermissions_product` o `catalogpermissions_category` di indicizzazione. Le autorizzazioni di catalogo non hanno funzionato come previsto sullo storefront fino al completamento della reindicizzazione.
...
* Le opzioni di attributi fornite dal modello origine dati e installate utilizzando la patch dei dati di installazione (in genere da moduli di terze parti) possono ora essere utilizzate con le regole del segmento del cliente. In precedenza, questa combinazione non restituiva l&#39;elenco dei clienti che corrispondevano al segmento configurato.

<!-- AC-7093 -->

* Il motore di modelli e-mail può ora elaborare correttamente alcune direttive nidificate (ad esempio, `{{if}}{{depend}}...{{/depend}}{{/if}}`). [GitHub-36438](https://github.com/magento/magento2/issues/36438)

<!-- AC-7920 -->

* `product` Le query con dati di aggregazione restituiscono ora i valori di etichetta corretti per gli attributi di prodotto di tipo booleano. [GitHub-29123](https://github.com/magento/magento2/issues/29123)

<!-- https://github.com/magento/magento2/pull/29173 -->

* I file CSS non sono più danneggiati quando vengono uniti quando `var/tmp` e `pub/static` si trovano su filesystem diversi ed **[!UICONTROL CSS Merging Enabled]** è abilitato. I file temporanei sono stati creati nella stessa directory statica del file di destinazione per garantire che entrambi i file si trovino nello stesso file system. Di conseguenza, la chiamata per rinominarlo è atomica. [GitHub-29172](https://github.com/magento/magento2/issues/29172)

<!-- https://github.com/magento/magento2/pull/36769 -->

* È stato risolto un potenziale errore quando si chiamava una funzione `getId()` membro su `int`, che occasionalmente attivava un&#39;eccezione quando un cliente veniva visualizzato nell&#39;amministratore.
...
* Lo stato delle scorte di prodotto configurabili ora viene aggiornato correttamente quando un prodotto configurabile e il relativo valore di prodotto secondario `Stock Availability` vengono aggiornati da un aggiornamento di massa dell&#39;inventario. In precedenza, non era possibile restituire un prodotto configurabile e i relativi prodotti figlio in magazzino tramite aggiornamento di massa.

<!-- ACP2E-2096 -->

* Il tipo di dati per le origini non predefinite è stato aggiornato per supportare fino a `DECIMAL (12, 4)` otto cifre intere, che è lo stesso limite implementato per lo stock predefinito. Questo valore è ora in Sincronizzazione con l&#39;aggiunta e modifica delle pagine di prodotto da parte dell&#39;amministratore, che supportano convalida di input fino a otto cifre per tutti i tipi di origini di inventario. In precedenza, l&#39;input per il campo (per le **[!UICONTROL Quantity]** origini non predefinite) era supportato solo fino a sei cifre.

<!-- ACP2E-2054 -->

* Sia le fonti predefinite che quelle non predefinite ora visualizzano le informazioni corrette per il prodotto vendibile quantità quando una fonte non predefinita ordinata dal negozio viene elaborata per la spedizione. In precedenza, Adobe Commerce mostrava uno stato delle scorte errato per le origini dei prodotti.

<!-- ACP2E-1950 -->

* `\Magento\Catalog\Model\Product::getIsSalable()` Ora restituisce lo stato delle scorte in base ai ambito definiti nell&#39;oggetto prodotto (`\Magento\Catalog\Model\Product::getStoreId()`). In precedenza, `getIsSalable()` restituiva lo stato delle scorte in base all&#39;ambito corrente indipendentemente dall&#39;oggetto prodotto.
...
* Le fatture, le spedizioni e le note di credito possono ora essere spostate dall&#39;archivio insieme agli ordini associati quando gli ID che includono differiscono dagli ID degli ordini. In precedenza, se un ordine veniva spostato nell&#39;archivio e quindi ripristinato, i record di fatturazione, spedizione e nota di credito mancavano da quell&#39;ordine se i numeri ID non corrispondevano. [GitHub-36847](https://github.com/magento/magento2/issues/36847)
...
* Pagina Builder non aggiunge più l&#39;attributo `tabindex` HTML ai tag dei collegamenti ipertestuali `a` quando modifichi un collegamento ipertestuale, quindi salvi un modello.
...
* Sono stati risolti i problemi relativi agli URL di fallback per gli ordini pagati con pagamenti Ideal quando il pagamento locale è abilitato.
...
* Braintree informazioni di pagamento vengono ora salvate per store e gli acquirenti ora vedono solo scheda informazioni salvate nel store in cui effettuano acquisti. In precedenza, gli acquirenti potevano vedere le informazioni sui scheda memorizzati da tutti i negozi durante il check-out. [GitHub-34253](https://github.com/magento/magento2/issues/34253)

<!-- BUNDLE-3304 -->

* Gli acquirenti possono ora controllare un ordine spedito a più indirizzi utilizzando un metodo di pagamento Braintree quando la cassaforte per i pagamenti scheda è disabilitata. [GitHub-37615](https://github.com/magento/magento2/issues/37615)
...
* Adobe Commerce non genera più un errore durante la creazione di una seconda fattura parziale per un ordine pagato da Braintree quando **[!UICONTROL Enable Vault for Card Payments]** è abilitato. In precedenza, quando un commerciante tentava di creare una seconda fattura, Adobe Systems Commerce generava questo errore: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.

<!-- ACP2E-1229 -->

* L&#39;opzione del metodo di pagamento Braintree Vault non viene più deselezionata automaticamente nella workflow di pagamento quando un acquirente aggiorna il proprio indirizzo fatturazione. In precedenza, il metodo di pagamento veniva deselezionato quando il acquirente deselezionava il **[!UICONTROL My billing and shipping address are the same]** pulsante.

<!-- BUNDLE-3248 -->

* Risoluzione dei problemi relativi al check-out con PayPal tramite Braintree dopo l&#39;aggiornamento da Adobe Systems Commerce 2.4.5-p2. [GitHub-37242](https://github.com/magento/magento2/issues/37242)
...
* Gli acquirenti possono ora completare il checkout utilizzando Braintree Payments and Adobe Systems Commerce 2.4.6 con ReCAPTCHA v2 o v3 invisibile. [GitHub-37241](https://github.com/magento/magento2/issues/37241)

<!-- BUNDLE-3241 -->

* Gli acquirenti non possono più fare clic sul **[!UICONTROL PayPal]** pulsante per acquistare un prodotto configurabile senza prima selezionare le opzioni richieste. In precedenza, gli acquirenti potevano fare clic sul **[!UICONTROL PayPal]** pulsante e successivamente venivano reindirizzati alla pagina del prodotto con un errore:

<!-- BUNDLE-3148 -->

* Il **[!UICONTROL PayPal]** pulsante viene ora visualizzato come previsto sulle pagine del carrello e del mini carrello. In precedenza, questo pulsante non veniva visualizzato nella pagina del carrello e veniva duplicato nella pagina del mini carrello.
...
* Invisible reCAPTCHA v3 ora funziona come previsto per la pagina **[!UICONTROL Sign in]** di checkout pulsante. In precedenza, gli acquirenti non potevano login e Commerce generava JavaScript errori.

<!-- ACP2E-1539 -->

* Google reCAPTCHA nella workflow di checkout ora funziona come previsto per assegni, vaglia postali e metodi di pagamento personalizzati.

<!-- ACP2E-1311 -->

* Adobe Systems moduli Commerce non possono più essere inviati prima del caricamento della casella di controllo Google ReCAPTCHA.
...
* Gli utenti amministratori possono ora ricerca i record con e-mail che contengono `+` (più) segni dalla pagina Rapporto carrelli abbandonati. [GitHub-36560](https://github.com/magento/magento2/issues/36560)

<!-- AC-7394 -->

* I nomi delle transazioni GraphQL vengono ora visualizzati correttamente dopo Nuovo Relic è stato disabilitato dall&#39;amministratore Commerce. [GitHub-36112](https://github.com/magento/magento2/issues/36112)

### Recensioni

<!-- ACP2E-1998 -->

* Gli acquirenti non possono più inviare rapidamente recensioni di prodotti duplicati quando Invisible reCAPTCHA v3 è abilitato.

### Ricompense

<!-- ACP2E-2099 -->

* Le e-mail di scadenza dei punti Reward non vengono più inviate dopo che i punti sono scaduti. Le `magento_reward_balance_warning_notification` attività cron e `magento_reward_expire_points` cron sono state rifattorizzate per aumentare la flessibilità e la capacità di invio di e-mail. In precedenza, i commercianti non potevano inviare molte e-mail generate da queste attività cron in un breve periodo di tempo. Ciò potrebbe aver causato ritardi nella consegna delle notifiche urgenti.

<!-- ACP2E-2283 -->

* I punti premio scaduti vengono ora sottratti dal saldo corrente dei punti premio di un acquirente come previsto.

<!-- ACP2E-1473 -->

* L&#39;e-mail di aggiornamento del saldo punti premio notifica ora prende in account il contesto store corrente. In precedenza, nell&#39;e-mail di aggiornamento del saldo dei punti premio venivano utilizzate le impostazioni di configurazione per il store predefinito.
...
* Il valore dell&#39;attributo **[!UICONTROL Enable RMA]** corrisponde ora al valore specificato nella configurazione quando la **[!UICONTROL Use Config]** casella di controllo nella pagina di modifica del prodotto è abilitata.

<!-- ACP2E-1230 -->

* L&#39;attributo **[!UICONTROL Enable RMA]** a livello di singolo prodotto ora rispetta la configurazione delle impostazioni RMA store. In precedenza, questa impostazione veniva disattivata per impostazione predefinita quando si aggiungeva un prodotto, indipendentemente dalle impostazioni di configurazione store.
...
* Il prezzo di ricerca avanzato e i filtri referenza di magazzino ora funzionano correttamente se applicati insieme in un ricerca avanzato. [GitHub-37581](https://github.com/magento/magento2/issues/37581)
...
* Search risultati ora visualizzano risultati rilevanti in base ricerca peso degli attributi come previsto.

<!-- AC-7505 -->

* Il testo informativo sotto il `Search Weight` campo di immissione dell&#39;attributo `Storefront Properties` in Admin **[!UICONTROL Stores]** > **[!UICONTROL Attributes]** > **[!UICONTROL Product]** ora descrive i valori possibili. [GitHub-36646](https://github.com/magento/magento2/issues/36675)

### Navigazione

<!-- ACP2E-2515 -->

* Le regole dei prezzi del carrello ora funzionano come previsto quando un prodotto bundle viene aggiunto al carrello quando gli attributi di prezzo dinamici sono disabilitati. La spedizione gratuita e la tariffa di tabella corretta vengono applicate indipendentemente dal fatto che l’attributo dinamico sia abilitato o disabilitato. In precedenza, la spedizione gratuita non veniva applicata come previsto.
...
* Quando un utente amministratore crea un riordino, gli indirizzi di fatturazione e spedizione dell&#39;ordine iniziale compilano automaticamente i campi pertinenti quando l&#39;utente amministratore fa clic sul pulsante **[!UICONTROL Reorder]** (Admin **[!UICONTROL Sales]** > **[!UICONTROL Orders]** > **[!UICONTROL View]** ). Se gli stessi indirizzi vengono salvati nella rubrica del acquirente, il riordino incorporerà sempre una versione aggiornata di questi indirizzi. In precedenza, il campo dell&#39;indirizzo di spedizione era precompilato con i dati aggiornati, ma il campo dell&#39;indirizzo fatturazione non lo era.
...
* La `cart` query restituisce ora un `selected_shipping_method` valore che corrisponde a uno dei codici carrier e method `available_shipping_methods` . In precedenza, quando uno dei metodi di spedizione selezionati conteneva un carattere di sottolineatura, questi codici vettore/metodo non corrispondevano. [GitHub-36606](https://github.com/magento/magento2/issues/36606)

<!-- AC-2556 -->

* La `setShippingAddressesOnCart` mutazione non genera più un errore quando il secondo valore nell&#39;array street è nullo. In precedenza, veniva generata questa eccezione: `The address failed to save. Verify the address and try again`. [GitHub-35160](https://github.com/magento/magento2/issues/35160)
...
* I prodotti bundle ora vengono aggiornati correttamente se un acquirente li aggiunge al carrello quando è in corso un aggiornamento della pianificazione. In precedenza, se un prodotto bundle con opzioni veniva aggiunto al carrello durante un&#39;attività programmare, Commerce generava questa eccezione: `Some of the products below do not have all the required options. Please select all required options`.

<!-- ACP2E-2252 -->

* Gli aggiornamenti programmati ora mantengono i prezzi corretti per le opzioni di bundle. In precedenza, questi prezzi venivano modificati a zero per specifiche visualizzazioni di siti web o store.
...
* Commerce ora visualizza il valore corretto per il **[!UICONTROL Regular Price]** valore quando viene selezionata un&#39;opzione personalizzata per un prodotto che ha un **[!UICONTROL Special Price]** valore quando **[!UICONTROL Price Display Settings]** è impostato per includere le imposte.

<!-- ACP2E-2303 -->

* Le tasse sono ora incluse come previsto nel prezzo di checkout workflow spedizione, come specificato dalle impostazioni di configurazione.

<!-- AC-9758 -->

* Il valore della `cart` risposta alla query `applied_taxes` ora include tutte le singole regole fiscali come previsto. In precedenza, restituiva il valore fiscale totale quando venivano configurate più regole fiscali.

<!-- ACP2E-2247 -->

* L&#39;icona di compressione/espansione per la sezione durante la **[!UICONTROL Additional Settings]** creazione di regola fiscali ora funziona come previsto. In precedenza, funzionava solo l&#39;icona Espandi. [GitHub-37922](https://github.com/magento/magento2/issues/37922)

<!-- ACP2E-1527 -->

* Ora puoi modificare il valore di Paese e regione aliquota fiscale dalla pagina di modifica Regola fiscale come previsto. In precedenza, le modifiche apportate da quella pagina non venivano salvate.

<!-- ACP2E-1143 -->

* L&#39;importo delle imposte nell&#39;area di stima del carrello viene aggiornato come previsto quando viene aggiornato il metodo di spedizione. In precedenza, l&#39;aliquota fiscale non veniva aggiornata dopo che un acquirente cambiato paese durante il check-out.

<!-- AC-8227 -->

* Il `tax class` valore per i prodotti semplici appena creati è ora impostato sul valore del prodotto padre quando si creano nuove varianti di un prodotto configurabile esistente in Store Visualizza ambito. In precedenza, questo valore era impostato su none. Di conseguenza, non è stata aggiunta alcuna imposta a questi prodotti nel carrello e i clienti sono stati sottopagati. [GitHub-37180](https://github.com/magento/magento2/issues/37180)

### Test

<!-- ACP2E-2501 -->

* Migliorato il `AdminCreateOrderWithVirtualProductFromDefaultSourceWithBackorders` test per eliminare gli errori causati dal completamento prematuro del campo e-mail.

<!-- AC-10863 -->

* `StorefrontVerifyProductAfterPartialReindexOnSeveralWebsitesTest` è stato sottoposto a nuovo il factoring per migliorarne l&#39;affidabilità. [GitHub-38359](https://github.com/magento/magento2/issues/38359)

<!-- ACP2E-2501 -->

* È stato corretto il `AdminSimpleProductwithTextandVisualSwatch` test introducendo un passaggio aggiuntivo che impostava in modo esplicito gli attributi da utilizzare nel ricerca.

<!-- AC-9194 -->

* Corretto errore di battitura nello `SortbyTest` unit test. [GitHub-37792](https://github.com/magento/magento2/issues/37792)
...
* I test di integrazione per la funzionalità di modifica del registro degli omaggi non si basano più sugli ID entità esistenti. Tutti i dati richiesti sono ora ottenuti tramite dispositivi. In precedenza, `\Magento\GiftRegistry\Controller\Index\EditPostTest` non riuscito.
...
* I valori CLS (Cumulative Layout Shift) sono migliorati per le pagine dei dettagli del prodotto. Questa metrica quantifica la stabilità visiva dei layout di pagina.[GitHub-35410](https://github.com/magento/magento2/issues/35410)

<!-- AC-10567 -->

* È stata migliorata la messaggistica di errori che si verificano durante il rendering dei modelli. Commerce visualizza ora un messaggio informativo. In precedenza, Commerce visualizzava perdite HTML quando si verificava un errore PHP.
...
* L&#39;ordine dei tipi di attributo a discesa del prodotto corrisponde ora all&#39;ordine delle immagini nella pagina di prodotto configurabile. [GitHub-37285](https://github.com/magento/magento2/issues/37285)

<!-- ACP2E-1969 -->

* I messaggi di sistema vengono caricati più rapidamente. Vengono caricati solo i messaggi visualizzati. In precedenza, tutti i messaggi venivano caricati dal database, il che richiedeva molto tempo.
...
* Il modulo registrazione cliente viene ora caricato come previsto quando include attributi cliente a selezione multipla. In precedenza, Adobe Systems Commerce ha generato questa eccezione: `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)

<!-- ACP2E-1602 -->

* Benvenuti messaggi contenenti virgolette singole vengono ora visualizzati correttamente nello storefront senza errori nella console. In precedenza, se un messaggio di benvenuto conteneva una virgoletta singola, il messaggio di benvenuto non veniva caricato durante il rendering della vetrina del negozio. Anche Adobe Systems Commerce ha visualizzato errori della console.
...
* L&#39;icona del calendario viene ora visualizzata come previsto nei campi dell&#39;intervallo di date nella > **[!UICONTROL Cart Price Rules]** Amministrazione **[!UICONTROL Marketing]** dopo aver selezionato una data per un campo. In precedenza, dopo aver selezionato una data per un campo, l&#39;icona del calendario non veniva più visualizzata negli altri campi e Adobe Systems Commerce visualizzava il testo segnaposto. [GitHub-36499](https://github.com/magento/magento2/issues/36499)

<!-- ACP2E-1717 -->

* I moduli registrazione cliente che includono campi a selezione multipla vengono caricati correttamente. In precedenza, questo modulo non veniva caricato quando conteneva campi a selezione multipla e veniva attivato il seguente errore: `report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)

<!-- AC-7092 -->

* Facendo clic su una miniatura del video nella modalità a schermo intero della galleria prodotti, il video viene ora visualizzato come previsto. In precedenza, il video non veniva visualizzato e nella console browser veniva visualizzato un errore JavaScript. [GitHub-36456](https://github.com/magento/magento2/issues/36456)
...
* Ora puoi utilizzare l&#39;attributo per le `Categories` regole di Visual Merchandiser. In precedenza, Adobe Systems Commerce ha generato questa eccezione: `Something went wrong while saving the category`.

### Framework API Web

<!-- AC-6980 -->

* Commerce non visualizza più un `Integration not secure` avviso in Admin **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** quando i **[!UICONTROL Callback URL]** campi e **[!UICONTROL Identity link URL]** sono vuoti. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* L&#39;API `GET` REST e `POST V1/products/attributes` l&#39;endpoint non restituiscono più lo stesso valore per l&#39;attributo `is_filterable` per entrambe `Filterable(with results)` le opzioni e `Filterable(no results)` per le opzioni dell&#39;opzione **[!UICONTROL Use in Layered Navigation]** . In precedenza, questo endpoint restituiva lo stesso valore perché la `is_filterable` proprietà attribute era di tipo `Boolean`, che non consentiva di impostare questa proprietà su `Filterable(no results)`. [GitHub-37979](https://github.com/magento/magento2/issues/37979)

<!-- AC-9117 -->

* Il campo mancante `media_gallery_entries` è stato aggiunto all&#39;endpoint REST `V1/configurable-products/sku/children` . In precedenza, a questa funzione mancava il codice necessario per store e visualizzare `media_gallery_entries`. [GitHub-37607](https://github.com/magento/magento2/issues/37607)

<!-- ACP2E-1986 -->

* Il metodo REST `V1/carts/mine/estimate-shipping-methods` convalida ora tutti i livelli di dati del campo payload e restituisce un codice HTTP 400 per non valido dati. In precedenza, veniva convalidato solo il primo livello di payload API e, se non valido dati venivano restituiti in livelli non convalidati del payload, Commerce restituiva un codice di stato 500.

<!-- ACP2E-2053 -->

* Le operazioni API REST in blocco asincrone per aggiornare i prodotti con prezzi di livello e set di attributi diversi vengono ora completate correttamente. In precedenza, i prodotti con set di attributi diversi non potevano essere aggiornati in questo modo e Commerce visualizzava questo errore: `The product was unable to be saved. Please try again`.

<!-- AC-7002 -->

* L&#39;endpoint `POST /V1/products/tier-prices` restituisce ora un messaggio di errore più informativo quando viene utilizzato un nome di gruppo di clienti non valido durante la creazione del prezzo di livello. In precedenza, Adobe Systems Commerce restituiva un errore like seguente: `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`. [GitHub-36431](https://github.com/magento/magento2/issues/36431)

<!-- ACP2E-1776 -->

* È ora possibile eseguire un richiesta API in blocco asincrono autorizzato per creare clienti con un ID gruppo non predefinito. In precedenza, queste richieste reimpostavano l&#39;ID gruppo al valore predefinito.

<!-- ACP2E-1543 -->

* I clienti creati utilizzando l’API in blocco ora ricevono le e-mail di registrazione del cliente nella lingua configurata nel sito web assegnato. In precedenza, tutte le e-mail venivano inviate alla lingua predefinita, non alla lingua del sito Web a cui è assegnato il cliente.
...
* Adobe Systems Commerce ora restituisce correttamente l&#39;attributo `content` (codice immagine base64) nelle `GET V1/products/sku/media` risposte API del prodotto media. L&#39;attributo `content` non era apparso in precedenza nella risposta.

### Lista dei desideri

<!-- ACP2E-1784 -->

* Commerce ora visualizza un messaggio che indica che un articolo è già stato aggiunto a una lista dei desideri quando un acquirente tenta di aggiungere lo stesso articolo a una lista dei desideri più volte. In precedenza, in queste condizioni, Commerce aumentava il quantità del prodotto nella lista dei desideri.

<!-- AC-8610 -->

* Adobe Systems Commerce ora aggiunge correttamente i prodotti con i file di opzioni personalizzabili caricati richiesti alla lista dei desideri. In precedenza, Commerce aggiungeva due prodotti alla lista dei desideri, non uno. [GitHub-37437](https://github.com/magento/magento2/issues/37437)
