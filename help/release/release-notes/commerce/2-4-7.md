---
title: Note sulla versione di Adobe Commerce 2.4.7-beta3
description: Scopri le nuove funzioni, i miglioramenti, le correzioni di bug e i problemi noti nella versione 2.4.7-beta3 di Adobe Commerce.
badgeCoreBeta: label="2.4.7-beta" type="informative"
exl-id: 6ceb1d6d-5076-49a1-925f-913aa3598c90
source-git-commit: 8fabdd1e6e87995c625ea06c37ed00511081c91f
workflow-type: tm+mt
source-wordcount: '19280'
ht-degree: 0%

---


# Note sulla versione di Adobe Commerce 2.4.7-beta3
...
Adobe Commerce 2.4.7-beta3 introduce il supporto per PHP 8.3. Introduce oltre 150 correzioni e miglioramenti di qualità. Le dipendenze del Compositore core e le librerie di terze parti sono state aggiornate alle versioni più recenti disponibili. Questa versione include miglioramenti significativi delle prestazioni, tra cui un caricamento più rapido delle pagine di elenco dei prodotti per i prodotti complessi con molte opzioni.
...
## Ambiente di test di Adobe Commerce 2.4.7-beta3

Adobe Commerce 2.4.7-beta3 è stato testato sulle seguenti versioni dei componenti:
...
Adobe Commerce 2.4.7-beta3 include i seguenti aggiornamenti della piattaforma:
...
Adobe Commerce 2.4.7 è ancora compatibile con PHP 8.1 solo a scopo di aggiornamento. PHP 8.1 non è supportato e non è consigliato. Il codice core Adobe Commerce 2.4.7-beta3, tutte le estensioni in bundle e tutte le estensioni possedute da Adobe e i servizi SaaS sono compatibili con PHP 8.3. <!-- AC-9668 -->

* **Supporto di RabbitMQ 3.11**. Questa versione è compatibile con l’ultima versione di RabbitMQ 3.11. La compatibilità rimane con RabbitMQ 3.9, che è supportato fino ad agosto 2023, ma è consigliabile utilizzare Adobe Commerce 2.4.7-beta3 solo con RabbitMQ 3.11.

* **Compositore 2.6.x**. La compatibilità con Composer 2.2.x rimane. <!-- AC-9663 -->

* **Supporto per la cache di vernice 7.4**. Questa versione è compatibile con la versione più recente di Varnish Cache 7.4. La compatibilità rimane con le versioni 6.0.x e 7.2.x, ma si consiglia di utilizzare Adobe Commerce 2.4.7-beta3 solo con Vernice Cache versione 7.4 o 6.0 LTS. <!-- AC-9662 -->
...
* **Più coupon per supporto ordine**. Gli esercenti possono ora configurare il numero massimo di coupon applicabili per ordine con il nuovo **[!UICONTROL Maximum number of coupons per order]** opzione di configurazione. Questo valore è impostato su 1 per impostazione predefinita. Il supporto REST per questa funzione è stato aggiunto a questa versione e il supporto GraphQL sarà presto disponibile.
...
Commerce 2.4.7-beta3 include i seguenti miglioramenti alle prestazioni e alla scalabilità di Commerce:
...
* **Operazioni di salvataggio più rapide delle configurazioni a livello di archivio per le distribuzioni con molti archivi**. Il salvataggio delle impostazioni di configurazione nelle distribuzioni con più di 500 archivi può richiedere molto tempo. Il nuovo modulo di configurazione asincrona abilita le operazioni di salvataggio della configurazione asincrona eseguendo un processo cron che utilizza un consumer per elaborare l’operazione di salvataggio in una coda di messaggi. AsyncConfig è disabilitato per impostazione predefinita.
...
Questa versione include B2B 1.4.2.
...
* Aggiornamenti a REST e GraphQL
   ...
   * È stato aggiunto un endpoint per fornire la copertura API web per la funzionalità di offerta avviata dal venditore e introdotta in questa versione: `POST /V1/negotiableQuote/draft`. Consulta [Gestisci offerte negoziabili](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).
...
* I dati del dispositivo per 3DS vengono ora verificati e visualizzati nel pannello di controllo della Braintree. Il `collectDeviceData` Il parametro ora viene passato come `true` nel `verifyCard` chiamare per 3DS. <!-- BUNDLE-3327 -->

* La pagina di configurazione dei pagamenti amministratore ora include un collegamento alle note sulla versione e alla pagina Braintree estensioni. <!-- BUNDLE-3325 -->

* L’SDK Braintree è stato aggiornato alle versioni più recenti. <!-- BUNDLE-3323 -->
...
Commerce 2.4.7 include funzionalità avanzate di caching di GraphQL, supporto dello schema GraphQL per gli attributi personalizzati, supporto per l’annullamento di ordini headless e miglioramento del resolver cqching.
...
* È stato aggiunto il supporto GraphQL per le operazioni di base per tutti i metodi di pagamento ad eccezione di Venmo. I nuovi endpoint di GraphQL per i servizi di pagamento sono descritti in [Pagamento](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* Il vaulting delle carte di credito è stato migliorato per tutti i metodi di pagamento ad eccezione di Venmo. Gli acquirenti possono ora archiviare o salvare i pagamenti ACH e visualizzarli o eliminarli dalla pagina del proprio account. Gli esercenti possono abilitare e disabilitare i pagamenti archiviati ACH dall&#39;amministratore. <!-- BUNDLE-3324 -->
...
PWA Studi v.13.3.x è compatibile con Adobe Commerce 2.4.7-beta3. Include diversi miglioramenti per migliorare l’accessibilità. Per informazioni sulle correzioni di bug, consulta [Versioni di PWA Studi](https://github.com/magento/pwa-studio/releases). Consulta [Compatibilità delle versioni](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) per un elenco delle versioni di PWA Studi e delle relative versioni di base compatibili di Adobe Commerce.

### Framework API web

**Utilizzo di più coupon per carrello**

I nuovi endpoint REST supportano la visualizzazione, l’aggiunta e l’eliminazione di più coupon associati a un carrello. <!-- LYNX-208 -->
...
Rimuovi un singolo codice coupon da un carrello:
...
Rimuovi più codici coupon da un carrello:
...
Sono stati risolti centinaia di problemi relativi al codice core di Adobe Commerce 2.4.7-beta3. Di seguito è descritto un sottoinsieme dei problemi risolti inclusi in questa versione. Sono descritte anche le correzioni incluse nelle precedenti versioni di Commerce 2.4.7-beta.
...
* Commerce non inserisce più il testo della descrizione del prodotto nella pagina dei dettagli del prodotto **[!UICONTROL Meta Description]** quando questo campo viene lasciato vuoto. In precedenza, quando una descrizione del prodotto conteneva codice HTML generato da PageBuilder o da un altro editor di HTML, il HTML veniva visualizzato nel file Meta **[!UICONTROL Description]** e non è stato possibile lasciare vuoto questo campo.
...
È possibile installare Adobe Commerce 2.4.7-beta3 utilizzando [Compositore](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
