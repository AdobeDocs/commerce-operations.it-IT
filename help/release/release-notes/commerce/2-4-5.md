...
Adobe Commerce introduce miglioramenti alla qualità della piattaforma, ai metodi di pagamento, alle prestazioni di memorizzazione in cache GraphQL e all’accessibilità. Include aggiornamenti ai moduli Google integrati e allo strumento di compatibilità per l’aggiornamento e introduce il supporto Live Search B2B.
...
* I nuovi marcatori di distribuzione relativi ora funzionano come previsto quando la cache viene scaricata. [GitHub-32649](https://github.com/magento/magento2/issues/32649)
...
* La `products` query ora restituisce informazioni di prodotto che riflettono con precisione la configurazione &quot;Mostra prodotti correlati&quot;. La `related_products`, `upsell_products`e `crosssell_products` I campi in GraphQL ProductInterface sono ora risolti in base alla configurazione Mostra prodotti correlati, Mostra prodotti di upselling e Mostra prodotti di cross-selling rispettivamente.
...
* Mancante `price_range` l’attributo è stato aggiunto a GraphQL `BundleItemOption` digitare. [GitHub-35010](https://github.com/magento/magento2/issues/35010)
...
* Le sessioni di Adobe Commerce non terminano più dopo una richiesta GraphQL. Precedentemente, la `ClearCustomerSessionAfterRequest` plugin ha disconnesso l&#39;acquirente. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* Lo schema GraphQL è ora valido quando è configurato un `type` è definito l&#39;attributo di prodotto. In precedenza, lo schema non era valido perché il `type` l&#39;attributo sui tipi di prodotti è stato sovrascritto dall&#39;attributo personalizzato `type` attributo. [GitHub-34929](https://github.com/magento/magento2/issues/34929)
...
**Problema**: Gli elementi dell’infrastruttura RabbitMQ (scambi, code) potrebbero non essere inizializzati dopo una nuova installazione (non l’aggiornamento) di Adobe Commerce 2.4.5. Questa configurazione errata di RabbitMQ può causare un errore fatale quando vengono eseguite operazioni asincrone o viene generato un profilo di prestazioni. Le distribuzioni cloud non sono interessate da questa modifica. **Soluzione alternativa**: I commercianti che eseguono una nuova installazione on-premise di Adobe Commerce 2.4.5 dovrebbero eseguire `bin/magento setup:upgrade --keep-generated` dopo l&#39;utilizzo `bin/magento setup:config:set` per configurare RabbitMQ o installare Adobe Commerce passando le opzioni di configurazione RabbitMQ a `setup:install` comando. Vedi [RabbitMQ](../../../installation/prerequisites/rabbitmq.md).
