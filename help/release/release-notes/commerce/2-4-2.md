...
* Verifica del percorso aggiunta al plug-in caricatore `Mediagallery`. In precedenza, convalida non riusciva e il applicazione non importava prodotti quando **l&#39;opzione Attiva raccolta multimediale precedente** era disattivata. [GitHub-30649](https://github.com/magento/magento2/issues/30649)
...
* `Magento_Cms::media_gallery` risorse ACL secondarie aggiunte e applicate per la nuova raccolta multimediale per supportare le autorizzazioni configurate per le seguenti operazioni discrete:
   ...
   * Carica file risorse
   * Inserire risorse nell contenuto
   * Elimina risorse
   * Creare cartelle
...
* Gli acquirenti non possono più effettuare un ordine dopo che il loro cookie di sessione è scaduto quando il carrello persistente è abilitato. Al applicazione viene visualizzata la pagina login in modo che il acquirente possa effettuare l&#39;accesso. In precedenza, l&#39;applicazione effettuava l&#39;ordine. Il applicazione non convalidava correttamente il cliente persistente alla scadenza `PHPSESSID` ed è stato ricreato dalle informazioni persistenti `customerSession` .

<!-- MC-39127-->

* L’applicazione ora visualizza il messaggio di benvenuto come previsto dopo che un acquirente accede a un negozio in cui è abilitato il carrello acquisti permanente. In precedenza, nel applicazione veniva visualizzato il comando Non **sei?** dopo l registrazione ingresso.
...
* Le spedizioni create tramite l&#39;endpoint POST `/rest/V1/shipment` ora aggiornano gli ordini correttamente. In precedenza, il applicazione creato una spedizione, ma lo stato della spedizione rimaneva nello stato di elaborazione.

<!-- MC-37120-->

* Il applicazione non rimuove più i prodotti semplici dal carrello quando un altro prodotto viene rimosso dal carrello prima del checkout con Spedisci a più indirizzi. [GitHub-30259](https://github.com/magento/magento2/issues/30259)

<!-- MC-38031-->

* La pagina di revisione dell&#39;ordine ora visualizza l&#39;importo dell&#39;imposta prima dell&#39;importo della spedizione, come previsto per gli ordini spediti a più indirizzi.
...
* `AdminMediaGalleryInsertLargeImageFileSizeTest` è stato sottoposto a nuova fattorizzazione.
...
* `\Magento\TestFramework\TestCase\WebapiAbstract` ora contiene una funzione che supporta il confronto di matrici nidificate di grandi dimensioni di risultati attesi ed effettivi nei test. Le chiavi irrilevanti nel risultato possono ora essere ignorate. [GitHub-29498](https://github.com/magento/magento2/issues/29498)

<!-- ENGCOM-8024 -->

* `AdminMediaGalleryCatalogUiEditCategoryGridPageTest` non riesce più in modo casuale. [GitHub-1764](https://github.com/magento/adobe-stock-integration/issues/1764)
...
* Il applicazione non visualizza più codice HTML non sottoposto a rendering nelle pagine di panoramica delle fatture e delle spedizioni. [GitHub-29958](https://github.com/magento/magento2/issues/29958)
