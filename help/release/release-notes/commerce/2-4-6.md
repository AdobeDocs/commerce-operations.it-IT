...
Anche se il codice per queste funzioni è incluso con le versioni del codice core di Adobe Commerce, molti di questi progetti (ad esempio, B2B, Page Builder e Progressive Web Applications (PWA) Studio) vengono rilasciati in modo indipendente. Le correzioni di bug per questi progetti sono documentate nelle informazioni sulla versione specifiche per il progetto, disponibili nella documentazione di ciascun progetto.
...
* Elasticsearch 8.x. Vedere [Aggiornamento Elasticsearch](../../../upgrade/prepare/prerequisites.md#upgrade-elasticsearch). <!-- AC-6597 -->

* MariaDB 10.6 (versione LTS). Questa versione è ancora compatibile con MariaDB 10.4, ma consigliamo di eseguire l&#39;aggiornamento a MariaDB 10.6. <!-- AC-6921 -->
...
* La compatibilità PHP 7.4 è stata rimossa dal codebase di Commerce. <!-- AC-2841-->
...
* `jquery-migrate` è stato rimosso dalla codebase Commerce. [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->

* Libreria `jQuery/fileUpload` aggiornata alla versione v10.32. <!-- AC-2658-->

* I componenti Zend framework (ZF1) che hanno raggiunto la fine del ciclo di vita sono stati rimossi dalla base di codice. Vedi [Modifiche non compatibili con le versioni precedenti in evidenza](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->
...
* [Estrazione rapida bullone](https://experienceleague.adobe.com/docs/commerce/quick-checkout/overview.html)
* [Adobe I/O Events per Adobe Commerce](https://developer.adobe.com/commerce/events/get-started/)
...
La nuova versione 2.2.0 di `adobe-ims-metapackage` è inclusa in Adobe Commerce 2.4.6. Nessuna funzionalità è stata modificata in questa versione.<!-- CABPI-505-->
...
* Lo schema GraphQL dell’ordine di acquisto copre l’utente dell’azienda B2B Funzionalità di acquisto di Order Management come la creazione, la modifica e l’eliminazione degli ordini di acquisto. Vedi [Ordini di acquisto (B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/).
...
* Quando la contenuto della pagina viene aggiornata dopo che l&#39;utente ha fatto clic sull&#39;pulsante **[!UICONTROL Create an account]**<!---AC-6676-->
...
* La griglia prodotti non genera più un’eccezione quando i prodotti bundle hanno un prezzo speciale che supera i 1000. In precedenza, quando si cercava il prodotto per referenza di magazzino o si passava a **[!UICONTROL Catalog]** > **[!UICONTROL Products]**, Adobe Systems Commerce generava questo errore: `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284).
...
* Il pagamento può essere completato correttamente se l&#39;indirizzo di spedizione contiene un attributo di estensione. In precedenza, Adobe Commerce ha generato questo errore: `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`. [GitHub-34202](https://github.com/magento/magento2/issues/34202)
...
* Gli elenchi delle categorie ora vengono aggiornati come previsto quando un prodotto viene assegnato a un nuovo sito Web o store. In precedenza, l&#39;indicizzatore dei prezzi dei prodotti non veniva contrassegnato come non valido e successivamente riattivato in esecuzione `cron` come previsto. Di conseguenza, l’inserimento del prezzo del prodotto non era presente nella tabella dell’indice dei prezzi e non veniva visualizzato nel nuovo negozio o sito web.

<!-- AC-6395-->

* Il conteggio dell&#39;inventario della vetrina è ora accurato quando l&#39;impostazione di **[!UICONTROL Display Out of Stock Products]** configurazione è abilitata. [GitHub-35899](https://github.com/magento/magento2/issues/35899)
...
* Quando è abilitata, l&#39;opzione **[!UICONTROL Display Out of Stock Products]** di configurazione ora funziona come previsto. In precedenza, tutti i prodotti venivano visualizzati come esauriti in vetrina quando questa opzione era abilitata nell&#39;amministratore. [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* Il metodo `getFilteredTimerIds` ora funziona come previsto.  La classe `Magento\Framework\Profiler\Driver\Standard\Stat` è ora compatibile con PHP 8.1. In precedenza, Adobe Commerce generava un’eccezione durante la chiamata di questo metodo. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Adobe Commerce ora visualizza un intervallo di tempo preciso quando un utente amministratore seleziona l&#39;opzione **[!UICONTROL Last 24 Hours]** dal dashboard delle statistiche di amministrazione. In precedenza, Adobe Commerce mostrava un intervallo di tempo errato. Ciò si è verificato perché il fuso orario del database è rimasto invariato.
...
* La Raccolta file multimediali legacy è ora disabilitata per impostazione predefinita. In precedenza, **[!UICONTROL Enable old Media gallery]** era impostato su **[!UICONTROL Yes]**.
...
* Sono stati risolti i problemi di concorrenza con l’inserimento dell’ordine. In precedenza, se due ordini venivano effettuati da browser diversi nello stesso momento, o quasi nello stesso momento, a entrambi gli ordini veniva assegnato lo stesso ID ordine. [GitHub-35833](https://github.com/magento/magento2/issues/35833)

<!-- AC-6908-->

* L&#39;effettuazione di un ordine con fatturazione automatica per un prodotto gratuito quando l&#39;indicizzazione asincrona è abilitata ora funziona come previsto. Nessuna voce viene aggiunta alla `sales_order_grid or sales_invoice_grid` tabella per questi ordini. In precedenza, Adobe Systems Commerce generava un&#39;eccezione perché il applicazione tentava di indicizzare le griglie durante l&#39;ordine posizionamento invece di fare affidamento su `cron`. [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* Gli acquirenti possono ora utilizzare il metodo di pagamento Braintree nei negozi in cui **[!UICONTROL Enable Card Payments]** è stato disabilitato. In precedenza, Adobe Commerce ha generato questo errore JavaScript: `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* Il report del registro di amministrazione ora mostra un valore **[!UICONTROL Value before change]** preciso quando viene aggiornata una regola di categoria.
...
* Il campo del commento RMA (**[!UICONTROL Orders]** > **[!UICONTROL Return]**) non contiene più spazi vuoti ridondanti. (I tag `<textarea>` e `<?php` sono ora su un&#39;unica riga.

<!-- ACP2E-1279-->

* La casella di controllo **[!UICONTROL Use Config Settings]** per il campo **[!UICONTROL Enable RMA]** ora salva il relativo valore come previsto quando viene salvato un prodotto.
...
* Le ricerche di stringhe tra virgolette ora funzionano come previsto quando l&#39;impostazione di configurazione **[!UICONTROL Minimum Terms to Match]** è abilitata. In precedenza, Adobe Commerce ha generato questo errore: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* L&#39;assegnazione automatica del gruppo di clienti ora funziona quando non è associato alcun ID IVA all&#39;indirizzo del cliente. Se l’indirizzo non contiene un ID IVA, viene considerato come appartenente a un cliente non IVA registrato e Adobe Commerce modifica il gruppo di clienti in un gruppo non IVA. In precedenza, quando **[!UICONTROL Enable Automatic Assignment to Customer Group]** era abilitato, l&#39;account cliente non veniva assegnato automaticamente a `customerGroup` quando l&#39;ID IVA veniva eliminato dall&#39;account cliente.
...
* Ora l&#39;amministratore visualizza correttamente l&#39;ultima data di login di ogni utente amministratore. In precedenza, quando un amministratore utente eseguito l&#39;accesso all&#39;amministratore per la prima volta, nella griglia della utente di amministrazione veniva visualizzato un valore di data login vuoto.


<!-- AC-3762-->

* La funzione di anteprima del modello e-mail (**[!UICONTROL Marketing]** > **[!UICONTROL Communications]** > **[!UICONTROL Email Templates]**) ora visualizza correttamente i modelli. In precedenza, l&#39;immagine di anteprima era troncata. [GitHub-35697](https://github.com/magento/magento2/issues/35697)

<!-- magento/magento2/pull/35656-->

* È stata aggiunta la classe per supportare lo `new-shipping-address-modal` stile del nuovo indirizzo di spedizione modale.

<!-- magento/magento2/pull/35738-->

* È stato aggiunto del testo informativo sotto il campo di immissione dell&#39;attributo `Search Weight` `Storefront Properties` in Admin **[!UICONTROL Stores]** > [!UICONTROL Attributes] > **[!UICONTROL Product]**.
...
* Dopo l’accesso, gli acquirenti possono ora aggiungere prodotti alla loro lista dei desideri. In precedenza, a causa di problemi con la chiave del modulo convalida dopo l&#39;accesso, la chiave del modulo inviata con il richiesta aggiuntivo alla lista dei desideri veniva invalidata.

<!-- ACP2E-341-->

* I prodotti vengono ora aggiunti come previsto alla lista dei desideri dalla lista dei prodotti e vengono visualizzate le pagine dopo che un cliente ospite conferma il proprio account da un’e-mail di conferma. In precedenza, i prodotti non venivano aggiunti alla lista desideri dopo account conferma.
...
Passaggi generali per risolvere questo problema:

1. Scarica la patch richiesta. Vedere i problemi delle etichette salesRules durante l&#39;aggiornamento dalle versioni &lt; 2.4.5](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html) dell&#39;articolo [della Knowledge Base.
...
È possibile installare Adobe Commerce 2.4.6 utilizzando [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
