...
### Miglioramenti sostanziali alla sicurezza
...
Ad oggi non si sono verificati attacchi confermati relativi a questi problemi. Tuttavia, alcune vulnerabilità possono potenzialmente essere sfruttate per accedere alle informazioni dei clienti o assumere il controllo delle sessioni dell’amministratore. La maggior parte di questi problemi richiede che un utente malintenzionato ottenga prima l’accesso all’amministratore. Di conseguenza, ti ricordiamo di adottare tutte le misure necessarie per proteggere l’amministratore, tra cui, ma non solo, queste iniziative: inserire nell&#39;elenco Consentiti dell’IP a due fattori, [autenticazione a due fattori](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/), utilizzo di una VPN, utilizzo di una posizione univoca invece di `/admin` e buona igiene della password. Vedere [Adobe Systems Bollettino](https://helpx.adobe.com/security/products/magento/apsb21-64.html) sulla sicurezza per una descrizione di questi problemi risolti.
...
* Un **nuovo plugin** Composer aiuta a prevenire la confusione delle dipendenze e identifica i pacchetti dannosi con gli stessi nomi dei pacchetti interni sul pacchetto pubblico archivio. Consultate il [plug-in Nuovo Composer Adobe Systems Releases with 2.4.3 Release](https://magento.com/blog/best-practices/adobe-releases-new-composer-plugin-magento-243-release) articolo di blog.

* **La limitazione della velocità è ora integrata nelle API** per prevenire attacchi Denial-of-Service (DoS). Le API Web ora impongono restrizioni sulla dimensione o sul numero di risorse (il massimo predefinito è impostato su 20 e possono essere configurate su un valore diverso in base alle esigenze aziendali) che possono essere richieste da un client. Per informazioni sulla configurazione di queste restrizioni, vedere [Limitazione frequenza](https://developer.adobe.com/commerce/webapi/get-started/api-security/). <!-- MC-35358-->
...
* Le dipendenze della libreria Laminas sono state aggiornate alle versioni compatibili con PHP 8.x. Alcune dipendenze ridondanti sono state rimosse dal file `composer.json`. **Adobe Commerce 2.4.3 utilizza Laminas 3.4.0**. <!-- MC-39513-->
...
[Live Search](https://experienceleague.adobe.com/en/docs/commerce-merchant-services/live-search/overview) powered by [Adobe Sensei](https://www.adobe.com/sensei.html) offre un&#39;esperienza di ricerca intuitiva utilizzando algoritmi di intelligence artificiale e di apprendimento automatico per eseguire un&#39;analisi approfondita dei dati aggregati dei visitatori. Consulta [Note sulla versione di Live Search](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).

### GraphQL

Questa versione aggiunge il supporto GraphQL per le seguenti funzionalità:

* **Cataloghi condivisi**<!-- PWA-1294-->

* **Liste dei** desideri. La [mutazione](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-wishlist-items-to-cart.html) addWishlistItemsToCart sposta gli articoli dalla lista dei desideri specificata al carrello del cliente. <!-- magento/partners-magento2ee#449-->

* **Registri delle donazioni**. I compiti coperti includono: <!-- PWA-1290-->

   * Utilizzare la [query](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry.html) giftRegistry per restituire il contenuto dei registri dei regali del cliente.

   * Search per i registri dei regali per [tipo](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-type-search.html), [e-mail](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-email-search.html) o [ID](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-id-search.html).

   * [Crea](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-gift-registry.html), [aggiorna](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry.html) o [elimina](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry.html) registri regali.

   * [Aggiorna](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-items.html) o [rimuovi](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-items.html) elementi da un registro regali.

   * [Sposta tutti gli elementi](https://developer.adobe.com/commerce/webapi/graphql/mutations/move-cart-items-to-gift-registry.html) dal carrello al Registro regali. <!-- ENGCOM-9082 9087-->

   * [Aggiungi](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-gift-registry-registrants.html), [aggiorna](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-registrants.html) o [rimuovi](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-registrants.html) i registranti da un registro regali.

   * [Condividi](https://developer.adobe.com/commerce/webapi/graphql/mutations/share-gift-registry.html) un registro regali con gli invitati.

* **Citazioni negoziabili**. Visualizzare le query [`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quote.html) e [`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quotes.html). <!-- PWA-1292-->
...
* **Route condivise**. La [query](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html) di routing e [RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html) supportano le richieste di routing su pagine di prodotti, categorie e CMS. La query `urlResolver` è stata dichiarata obsoleta e la relativa funzionalità è stata sostituita dalla query `route`.
...
La versione 2.4.3 introduce B2B v1.3.2. Questa versione include più correzioni di bug. Consulta [Note sulla versione B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* [Amazon Pay](https://marketplace.magento.com/amzn-amazon-pay-magento-2-module.html). Amazon Pay è stato dichiarato obsoleto e verrà rimosso in una versione successiva della versione 2.4.x. La versione 2.4.3 e successive conterranno solo aggiornamenti per la compatibilità e correzioni per i bug principali.
...
* Tutti i messaggi coda per i consumatori `quoteItemCleaner` ora cambiano il loro stato come `complete` previsto dopo l&#39;eliminazione di diversi prodotti. In precedenza, solo un messaggio per questo consumatore cambiava il proprio stato in `complete`, mentre il resto cambiava lo stato in `in progress`.

<!-- MC-40657-->

* Il applicazione ora visualizza il messaggio Termini e condizioni convalida nel blocco pertinente solo quando un acquirente fa clic sul pulsante Effettua ordine ****. In precedenza, il applicazione visualizzava questo messaggio nel blocco Code Applica sconto ogni volta che un acquirente cambiava metodo di pagamento nella workflow di pagamento: `The order wasn't placed. First, agree to the terms and conditions, then try placing your order again`.

<!-- MC-40271 magento/partners-magento2ee#476-->

* Ora si viene reindirizzati alla pagina di pagamento come previsto dopo aver aggiunto un prodotto in bundle al carrello da un&#39;anteprima di aggiornamento pianificato e aver fatto clic sul carrello. [GitHub-447](https://github.com/magento/partners-magento2ee/issues/447)
...
* L&#39;applicazione visualizza ora un messaggio di errore informativo e non aggiorna la quantità di prodotto quando un acquirente aggiunge una quantità di prodotto non valida e fa clic sul pulsante **Aggiorna articoli e quantità** nella pagina Gestisci carrello. In precedenza, l’applicazione aggiornava la quantità di prodotto e non visualizzava un messaggio di errore. [GitHub-459](https://github.com/magento/partners-magento2ee/issues/459)
...
* L’applicazione ora svuota il carrello come previsto dopo che un amministratore ha completato un ordine dall’amministratore creato da un acquirente nella vetrina. In precedenza, quando il cliente effettuava nuovamente l’accesso dopo che l’amministratore aveva completato l’ordine, il carrello della vetrina conteneva ancora il contenuto dell’ordine. [GitHub-30262](https://github.com/magento/magento2/issues/30262)
...
* Adobe Commerce aggiorna il conteggio totale delle pagine come previsto quando modifichi il valore per pagina dell’elenco Prodotti correlati all’amministrazione, Vendite successive e Cross-Sell. [GitHub-31059](https://github.com/magento/magento2/issues/31059)
...
* È ora possibile assegnare una nuova pagina a più nodi dalla scheda Gerarchia pagina di modifica di CMS. In precedenza, si verificava una violazione di vincolo univoco quando si tentava di assegnare la pagina a un nodo. [GitHub-363](https://github.com/magento/partners-magento2ee/issues/363)

### Prodotti configurabili

<!-- MC-40719-->

* Il applicazione non duplica più le miniature dei prodotti nella galleria di immagini di un prodotto quando fai clic sulle opzioni configurabili di un prodotto.

<!-- MC-37418-->

* Il finestra a comparsa di configurazione visualizzato dal applicazione quando si modifica un prodotto configurabile da una lista dei desideri ora si chiude come previsto quando si fa clic sull&#39;pulsante **OK** .
...
* Gli acquirenti possono ora aggiungere prodotti configurabili al carrello da una vista store non predefinita. In precedenza, quando il acquirente in una vista store non predefinita tentava di aggiungere un prodotto configurabile, il applicazione visualizzato questo errore: `Could not add item to cart. Please check required options and try again`. [GitHub-31660](https://github.com/magento/magento2/issues/31660)

### Politica sulla sicurezza dei contenuti CSP)

<!-- ENGCOM-8577-->

* Content Security Policy ora supporta il caricamento di immagini e font codificati base64 tramite `data: scheme`.

### cron

<!-- MC-30804-->

* Le query di cron clean up sono state rifattorizzate per ridurre o eliminare i seguenti problemi di prestazioni: `cron` lavori che rimangono bloccati in uno stato in sospeso, query MySQL sempre più lente e un aumento dell&#39;utilizzo della CPU. [GitHub-26507](https://github.com/magento/magento2/issues/26507)
...
* `indexer_update_all_views` processi cron ora vengono eseguiti come previsto dopo un errore precedente. L&#39;esecuzione non riuscita viene contrassegnata come errore in `cron_schedule schedule`, e l&#39;esecuzione successiva non riesce automaticamente. In precedenza, la tabella era piena di processi in sospeso e `indexer_update_all_views` cron `cron_schedule` job non veniva eseguito. [GitHub-23054](https://github.com/magento/magento2/issues/23054)
...
* Il campo **State** nella rubrica dell&#39;account cliente storefront è ora caricato come e rimane un elemento della pagina a discesa. L&#39;pulsante **Invia** è disabilitata finché tutti gli elementi della pagina non sono stati completamente caricati. In precedenza, l&#39;applicazione caricava questo campo come casella di testo prima di eseguirlo come elemento a discesa e gli acquirenti potevano inserire e salvare i valori nel campo di testo, che in seguito causava un errore durante il checkout.

<!-- MC-40906 magento/partners-magento2ee#509-->

* Il applicazione ora visualizza correttamente gli attributi dell&#39;indirizzo cliente personalizzato sia nelle pagine degli ordini vetrina che in quelle degli ordini dell&#39;amministratore. In precedenza, l&#39;opzione selezionata dell&#39;attributo a discesa mancava dalla sezione Informazioni indirizzo e il valore dell&#39;attributo input conteneva il codice dell&#39;attributo. [GitHub-508](https://github.com/magento/partners-magento2ee/issues/508)

<!-- MC-40329-->

* Il applicazione non genera più un errore quando salvi un attributo indirizzo cliente con un file allegato nel campo Indirizzo cliente amministratore durante il caricamento dei file. Ciò si è verificato a causa di un&#39;istruzione return mancante nell&#39;azione del controller. In precedenza, l&#39;applicazione ha generato questo errore: `Something went wrong while saving the file`.
...
* È stato aggiunto un processore di eventi del segmento cliente per le richieste GraphQL. In precedenza, gli sconti delle regole di prezzo del carrello non venivano applicati al carrello quando un segmento di clienti veniva utilizzato in una condizione di regola di prezzo del carrello. [GitHub-371](https://github.com/magento/partners-magento2ee/issues/371)
...
* Il messaggio e-mail inviato dall&#39;applicazione quando si condivide un registro regali dall&#39;amministratore contiene ora un collegamento valido alla posizione del registro. In precedenza, l’utilizzo di questo collegamento generava un errore 404. [GitHub-504](https://github.com/magento/partners-magento2ee/issues/504)
...
* È stata aggiunta una convalida per gli URL per impedire l&#39;inclusione di parole riservate nelle chiavi URL. Vedere [Definizione di identificatori di risorse Uniforme noti (URI)](https://tools.ietf.org/html/rfc5785)

<!-- ENGCOM-9037-->

* Nelle distribuzioni che eseguono PHP 7.4, il applicazione ora restituisce un errore 404 quando l&#39;impostazione **Genera &quot;categoria/prodotto&quot; URL Riscritture** è impostata su **No** e un acquirente tenta di accesso un percorso di categoria inesistente. In precedenza, l&#39;applicazione restituiva un errore 500 che indicava: `Trying to access array offset on value of type bool`. [GitHub-31984](https://github.com/magento/magento2/issues/31984)

<!-- MC-32257 magento/partners-magento2ee#507-->

* È ora possibile modificare correttamente un prodotto creato utilizzando un set di attributi da cui **erano stati rimossi i gruppi Gruppo di progettazione e Pianifica aggiornamento** progettazione.  In precedenza, quando si tentava di modificare un prodotto creato da tale set di attributi, il applicazione generava un errore irreversibile. [GitHub-44](https://github.com/magento/partners-magento2ee/issues/44)
...
* Il nome del `cms_index_noroute.xml` file è stato corretto in `cms_noroute_index.xml`. [GitHub-31300](https://github.com/magento/magento2/issues/31300)

<!-- ENGCOM-8516-->

* Il `.editorconfig` file è stato modificato per correggere la formattazione automatica dei `db_schema_whitelist.json` file. [GitHub-31171](https://github.com/magento/magento2/issues/31171)

<!-- ENGCOM-9018-->

* Il ridimensionamento di una finestra di browser non attiva più eventi di lente d&#39;ingrandimento di binding duplicati. [GitHub-30788](https://github.com/magento/magento2/issues/30788)

<!-- ENGCOM-8722-->

* Il applicazione non genera più un&#39;eccezione SQL quando viene filtrato `Magento\Users\Model\ResourceModel\Users\Collection` per `user_id`. [GitHub-31216](https://github.com/magento/magento2/issues/31216)
...
* La mutazione `setGiftOptionsOnCart` crea correttamente i messaggi regalo. [GitHub-388](https://github.com/magento/partners-magento2ee/issues/388)

### Registro regali

<!-- MC-33057 magento/partners-magento2ee#467-->

* I valori di **Data evento** ora sono gli stessi nella vetrina e nell&#39;amministratore durante la creazione o la modifica del registro regali. Precedentemente, ogni volta che si modificava e si salvava il registro degli omaggi, l&#39;applicazione modificava la **Data evento** un giorno indietro sia nella vetrina che in Amministrazione. [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)

<!-- MC-32587  magento/partners-magento2ee#511-->

* L&#39;applicazione non visualizza più i prodotti disabilitati nei registri delle donazioni. In precedenza, se un prodotto veniva disabilitato dopo essere stato aggiunto a un registro regali, l&#39;applicazione visualizzava questo messaggio quando un acquirente accedeva al registro: `Error: Call to a member function getId() …`. [GitHub-510](https://github.com/magento/partners-magento2ee/issues/510)
...
* I totali delle note di credito ora sono corretti quando la nota di credito contiene il materiale promozionale e le spese configurabili per il prodotto. In precedenza, l&#39;importo dell&#39;imposta di confezione regalo non veniva sempre incluso nel totale della nota di accredito. [GitHub-34](https://github.com/magento/partners-magento2ee/issues/34)

### Google Analytics

<!-- ENGCOM-8450-->

* Il applicazione non genera più un errore quando un amministratore tenta di caricare una pagina sullo storefront quando Google Analytics è abilitato. È stata aggiunta una voce CSP per `connect-src` consentire l&#39;www.google-analytics.com delle richieste AJAX. [GitHub-30880](https://github.com/magento/magento2/issues/30880)

### Gestione tag Google

<!-- MC-39072-->

* Il componente interfaccia per l&#39;indirizzo fatturazione nella pagina di pagamento del checkout workflow ora utilizza correttamente l&#39;indirizzo preventivo quando Google Tag Manager è attivato. In precedenza, si verificava un errore JavaScript nella pagina di pagamento.
...
* La query [`dynamicBlocks`](https://developer.adobe.com/commerce/webapi/graphql/queries/dynamic-blocks.html) restituisce il contenuto dei blocchi dinamici che corrispondono ai filtri specificati.
...
* Ora puoi salvare i valori vuoti in un&#39;esportazione pianificata. In precedenza, applicationupdated vuote i valori del filtro dopo la creazione o il salvataggio dell&#39;esportazione. Entrambi `no` i valori e sono `not specified` stati rappresentati da zero nel database e un valore di `not selected` è stato ignorato con `no`.

<!-- MC-41092-->

* Il processo di esportazione assume ora account ruolo utente ambito quando si esportano prodotti, fonti di magazzino ed entità cliente. In precedenza, il processo di esportazione ignorava utente ruolo ambito, che consentiva l&#39;esportazione di entità esportatrici private utente ruolo.

<!-- MC-40943-->

* Il applicazione ora imposta la classe di imposta sul prodotto su `None` se un prodotto viene importato con `tax_class_name` valori `None` o `0`. In precedenza, se `product tax_class_name` era `None` nel file CSV, l&#39;applicazione creava una nuova classe fiscale `None`, che duplicava la classe fiscale esistente. Se `product tax_class_name` presenti `0` nel file CSV, il applicazione ignorato tale valore e la classe fiscale del prodotto non è cambiata dopo l&#39;importazione.

<!-- MC-40898-->

* Gli amministratori possono ora modificare correttamente i `name` prodotti di un pacchetto `bundle_values` dall&#39;amministratore. In precedenza, l&#39;applicazione visualizzava il prodotto come esaurito nel negozio dopo aver cambiato il nome da amministratore. Anche i prodotti sono stati fusi con gli stessi referenza di magazzino in un&#39;unica sezione di opzioni bundle.

<!-- MC-40333-->

* Il applicazione ora tiene conto account ambito utente durante l&#39;esportazione dei dati dei clienti. In precedenza, quando si tentava di esportare i clienti, i applicazione esportavano i dati dei clienti da tutti i siti Web.

<!-- MC-40305-->

* Il applicazione ora rimuove le relazioni di prodotto tra up sell, cross selling e prodotti correlati durante l&#39;importazione del file CSV come previsto quando `__EMPTY__VALUE__` specificato nel file CSV.
...
* La dipendenza `pelago/emogrifier` è stata aggiornata dalla versione 3.1.0 alla 5.0.0. Questo aggiornamento ha comportato l&#39;introduzione di modifiche incompatibili con le `Magento\Email\Model\Template\Filter` versioni precedenti nella classe. Il codice modificato viene eseguito durante il rendering dei modelli e-mail. Vedere [riferimento](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/reference/) BIC.

<!-- MC-42212-->

* Risolto un problema che `\Magento\CatalogInventory\Model\Indexer\Stock\CacheCleaner::getCategoryIdsByProductIds` impediva il salvataggio di un nuovo prodotto.

<!-- MC-42199-->

* La libreria TinyMCE v3 obsoleta è stata rimossa. Il `Magento_Tinymce3Banner` modulo e i test MFTF relativi a TinyMCE v3.x sono stati rimossi da Adobe Systems Commerce.
...
* L&#39;applicazione non genera più un errore quando un plug-in viene aggiunto a una classe padre. Il parametro `optionsProvider` è ora dichiarato dopo `getContentIdentities` in `Assest.php`. In precedenza, l&#39;applicazione ha generato questo errore: `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* L’applicazione non registra più ogni cookie come contesto separato. Anche la `$_COOKIE` matrice è stata convertita in stringa. In precedenza, poiché ogni cookie veniva registrato come contesto separato, quando il numero di cookie superava i 50, l&#39;applicazione registrava questo messaggio: `Unable to send the cookie. Maximum number of cookies would be exceeded`. [GitHub-31334](https://github.com/magento/magento2/issues/31334)
...
* `bin/magento media-gallery:sync` ora non riesce come previsto durante l&#39;elaborazione di immagini PNG prive di informazioni XMP.

<!-- ENGCOM-8814-->

* Le voci nella tabella vengono rimosse `catalog_product_entity_media_gallery` come previsto quando vengono eliminati i prodotti correlati. [GitHub-17727](https://github.com/magento/magento2/issues/17727)

<!-- ENGCOM-8945-->

* Immagine dettagli possono ora essere aggiornati in Media Gallery quando JavaScript minimizzazione è abilitata. In precedenza, i dettagli dell&#39;immagine non venivano salvati e nel applicazione veniva visualizzato questo errore: `TypeError: Cannot read property 'call' of undefined in jquery.validate`. [GitHub-31633](https://github.com/magento/magento2/issues/31633)
...
* L&#39;elenco dei metodi di pagamento viene ora aggiornato come previsto quando un ospite acquirente cambia l&#39;indirizzo di spedizione di un ordine in un altro paese durante il checkout. In precedenza, la modifica dell’indirizzo di fatturazione non attivava un aggiornamento dei possibili metodi di pagamento.

#### PayPal

<!-- MC-39895-->

* Gli acquirenti possono ora controllare con successo un ordine PayPal Payflow Pro con un indirizzo di spedizione che contiene caratteri speciali. In precedenza, l&#39;applicazione rifiutava il pagamento per questi ordini.

<!-- MC-39861-->

* Gli acquirenti vengono ora reindirizzati alla pagina dell&#39;ordine completato dopo che un pagamento andato a buon fine utilizzando PayPal. In precedenza, gli acquirenti venivano reindirizzati a una pagina vuota perché i dati della sessione venivano persi.

### Prestazione

<!-- MC-41647-->

* Le prestazioni di Admin referenza di magazzino ricerca su cataloghi di grandi dimensioni sono migliorate. I suggerimenti per Query Optimizer ora forzano l&#39;utilizzo dell&#39;indice durante l&#39;esecuzione della query.

<!-- MC-41440-->

* Le prestazioni del processo durante l&#39;esecuzione `catalog_product_alert` `cron` su tabelle di grandi dimensioni (diversi milioni di righe) sono state migliorate. In precedenza, `catalog_product_alert` venivano caricati tutti gli avvisi relativi ai prodotti, che causavano un&#39;eccezione di memoria esaurita.

<!-- ENGCOM-8752-->

* Il applicazione non carica più tutte le pagine CMS quando è necessaria una sola pagina di modifica per eseguire il rendering di un modulo di amministrazione.  Queste pagine ora vengono caricate più rapidamente. [GitHub-30936](https://github.com/magento/magento2/issues/30936)
...
* È ora possibile salvare i prodotti del bundle quando ai prodotti è stato assegnato un prezzo di livello e `Magento\Framework\Api\ExtensibleDataObjectConverter` viene utilizzato per convertire i dati del prodotto. In precedenza, quando `Magento\Framework\Api\ExtensibleDataObjectConverter` veniva utilizzato per convertire i dati di prodotto a un array quando veniva salvato un prodotto, il applicazione non salvava il prodotto e visualizzava questo errore: `Notice: Undefined index: price in app/code/Magento/Catalog/Model/Product/Type/Price.php on line 382.`

<!-- MC-36811-->

* Gli aggiornamenti pianificati dei prezzi vengono ora applicati ai prodotti già presenti in un carrello acquirente. [GitHub-356](https://github.com/magento/partners-magento2ee/issues/356)
...
* I pulsanti freccia navigazione (**Successivo** e **Precedente**) sono ora visibili come previsto nei video dei prodotti in vetrina.
...
* L’applicazione ora filtra i record in base ai valori dei filtri nella griglia restituita dal cliente amministratore. In precedenza, l&#39;applicazione ti indirizzava alla pagina Amministrazione tutti i clienti e facendo clic sul pulsante **Reimposta filtro** veniva reindirizzato anche a questa pagina. [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* È ora possibile aggiungere un mapper di campo di Elasticsearch personalizzato a `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`. [GitHub-24350](https://github.com/magento/magento2/issues/24350)

<!-- ENGCOM-9029 MC-24026-->

* Ora gli sviluppatori possono modificare le mappature Elasticsearch. In precedenza, i modelli dinamici inviati al server Elasticsearch nelle mappature predefinite erano hardcoded, impedendo agli sviluppatori di indicizzare in Elasticsearch eventuali dati aggiuntivi associati a un modulo personalizzato. [GitHub-24363](https://github.com/magento/magento2/issues/24363)

<!-- ENGCOM-8464 -->

* I campi di ricerca (ricerca minisearch modulo) ora funzionano come previsto quando i suggerimenti di ricerca sono disabilitati. [GitHub-30584](https://github.com/magento/magento2/issues/30584)
...
* L&#39;applicazione non deseleziona più la casella di controllo **Aggiungi commenti** quando un acquirente fa clic su **Ottieni metodi e tariffe di spedizione** e seleziona un metodo di spedizione durante la creazione di un ordine dall&#39;amministratore. [GitHub-30256](https://github.com/magento/magento2/issues/30256)

<!-- MC-36425-->

* Gli acquirenti possono ora utilizzare il pulsante browser Indietro per tornare alla pagina Seleziona metodo di spedizione durante il check-out di un ordine con più indirizzi. In precedenza, sul applicazione veniva visualizzata una pagina danneggiata Seleziona metodo di spedizione. [GitHub-30268](https://github.com/magento/magento2/issues/30268)

<!-- MC-40021 ENGCOM-8983-->

* La quantità di prodotto ora rimane invariata come previsto dopo che un acquirente ha modificato la quantità nella pagina Spedisci a più indirizzi e ha fatto clic sul pulsante Indietro del browser. [GitHub-31956](https://github.com/magento/magento2/issues/31956)

<!-- ENGCOM-9096-->

* Le spese forfettarie del metodo di spedizione non diventano più pari a zero quando viene applicato un regola di prezzo del carrello durante il checkout. [GitHub-21832](https://github.com/magento/magento2/issues/21832)

<!-- ENGCOM-8537-->

* Il applicazione non genera più un errore quando un commerciante tenta di spedire un ordine utilizzando DHL quando la **casella di controllo Crea etichetta di spedizione** è abilitata e il nome del prodotto contiene caratteri unicode. In precedenza, l&#39;applicazione visualizzava questo errore quando richiedeva la creazione dell&#39;etichetta: `The response is in wrong format`. [GitHub-31032](https://github.com/magento/magento2/issues/31032)
...
* Le regole del prezzo di catalogo ora si applicano come previsto ai prodotti con valori di attributo non definiti. In precedenza, gli sconti creati tramite una regola del prezzo di catalogo non venivano applicati ai prodotti con attributi non definiti. [GitHub-461](https://github.com/magento/partners-magento2ee/issues/461)
...
* I plugin per `\Magento\Framework\App\ActionInterface` under `lib/internal/Magento/Framework/App/Action/Plugin` sono stati rimossi per mantenere la linea guida che i plugin dovrebbero essere utilizzati per personalizzare il comportamento di un modulo da un altro modulo. [GitHub-28050](https://github.com/magento/magento2/issues/28050)

<!-- MC-24772-->

* Il applicazione ora visualizza questo messaggio quando si tenta di selezionare `Website` come predefinito quando `Store View` è disabilitato durante la creazione del sito Web: `Please enable your Store View before using this Web Site as Default`. In precedenza, il sito Web si arrestava in modo anomalo e il applicazione non visualizzava alcun avviso.

### Tassa

<!-- MC-39571-->

* Il applicazione ora prende in account nascosto l&#39;imposta durante convalida dell&#39;importo minimo dell&#39;ordine.

<!-- MC-38025 ENGCOM-8612-->

* Il applicazione ora visualizza le imposte sui prodotti fissi (FPT) come previsto quando un acquirente torna al carrello e procede al pagamento dopo aver aggiunto i prodotti in bundle al carrello. [GitHub-30250](https://github.com/magento/magento2/issues/30250)

<!-- MC-37657 magento/magento2#31850-->

* L&#39;applicazione ora precompila i **campi di immissione del numero** di partita IVA per gli indirizzi di fatturazione e di spedizione della sezione Informazioni sull&#39;indirizzo della pagina Nuovo ordine amministratore con numeri di partita IVA salvati quando un amministratore crea un ordine per un cliente esistente. [GitHub-31846](https://github.com/magento/magento2/issues/31846)

<!-- ENGCOM-8617 magento-engcom/magento2ee#558-->

* Il applicazione ora visualizza un **campo Partita** IVA nella pagina registrazione cliente quando `customer/create_account/vat_frontend_visibility` è abilitato. [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* Rimosso `CacheCleaner::cleanAll();` dai test di integrazione. [GitHub-31074](https://github.com/magento/magento2/issues/31074)

<!-- ENGCOM-8898-->

* È stato aggiunto un test per questo scenario: gli utenti amministratori possono modificare un account cliente quando il cliente è abbonato a una newsletter in coda. [GitHub-30645](https://github.com/magento/magento2/issues/30645)

<!-- ENGCOM-8484-->

* Il comando `cache:flush` è stato rimosso dai test per migliorare il tempo di esecuzione per i moduli `Catalog`, `CatalogUrlRewrite` e `LoginAsCustomer`. [GitHub-31056](https://github.com/magento/magento2/issues/31056)
...
* Le parentesi che vengono aggiunte alle stringhe non hanno più l’escape quando viene configurata la traduzione in linea. In precedenza, il metodo `escapeHtmlAttr` convertiva le parentesi in codici di entità HTML. [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* Le stringhe di testo nel modello utilizzate per gestire i metodi di pagamento archiviati (**Account personale** > **Metodi di pagamento archiviati** ) sono ora traducibili. [GitHub-31211](https://github.com/magento/magento2/issues/31211)

<!-- MC-30798 ENGCOM-8717-->

* Le e-mail di ordine inviate dall’amministratore ora utilizzano le impostazioni locali dell’archivio e non le impostazioni locali associate all’account dell’amministratore. [GitHub-26521](https://github.com/magento/magento2/issues/26521)
...
* Sono stati risolti i problemi di allineamento con le schede nella pagina dell&#39;invito dell&#39;amministratore (Amministratore **Marketing** > **Vendite private** > **Inviti**). In precedenza, la visualizzazione Scheda per gli inviti accettati da un cliente non era allineata. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* La pagina di revisione dell&#39;ordine visualizzata durante il pagamento con PayPal Express Checkout ora viene caricata correttamente. In precedenza, il modello conteneva il pulsante **Aggiorna metodo di consegna** non utilizzato, nascosto solo parzialmente dallo script durante il rendering della pagina. [GitHub-32006](https://github.com/magento/magento2/issues/32006)

<!-- MC-37941 magento/magento2#31892-->

* Le stelle di valutazione e il testo di revisione nella sezione Customer Reviews della pagina del prodotto ora sono distanziati correttamente quando sono presenti nomi di valutazione lunghi. [GitHub-31890](https://github.com/magento/magento2/issues/31890)
...
* Le caselle di controllo che consentono ai commercianti di passare dal mostrare e nascondere le password sono state aggiunte a queste pagine:
   * Login cliente
...
* L&#39;applicazione visualizza ora un filtro `DateRange` nell&#39;area Accesso eseguito della pagina **Cliente** > **Accesso**. In precedenza, nella applicazione veniva visualizzato un `Text` filtro. [GitHub-30328](https://github.com/magento/magento2/issues/30328)

<!-- ENGCOM-9009 -->

* JavaScript è stato rimosso dai file modello e spostato in file separati per ridurre i problemi di rendering sulle pagine di amministrazione. In precedenza, il rendering delle pagine di amministrazione non veniva eseguito correttamente nelle distribuzioni in cui era stata abilitata la minimizzazione di HTML. Il applicazione visualizzato questo errore: `An error has happened during application run. See exception log for details`. [GitHub-32454](https://github.com/magento/magento2/issues/32454)
...
* Le riscritture dei URL di prodotto per un sito Web specifico in una distribuzione multisito vengono ora generate come previsto dopo che i prodotti sono stati assegnati a un sito Web tramite aggiornamento in blocco.

<!-- MC-40780 magento/magento2#32598-->

* Il applicazione ora genera correttamente il percorso URL per le categorie secondarie quando la **casella di controllo Usa Valore** predefinita per la chiave URL è abilitata per la categoria principale. In precedenza, lo spostamento di una categoria nella gerarchia generava un valore non corretto `url_path` quando si utilizzavano chiavi URL diverse in una distribuzione con visualizzazione multi store. [GitHub-16202](https://github.com/magento/magento2/issues/16202)

<!-- MC-40371-->

* Il applicazione ora aggiorna il `url_path` della categoria assegnata a tutti i store ambito quando si sposta una categoria nella gerarchia delle categorie. In precedenza, lo spostamento di una categoria nella gerarchia causava un errore `url_path`.
...
* L’applicazione ora utilizza il modello e-mail personalizzato corretto durante la generazione dei messaggi e-mail per i nuovi amministratori. In precedenza, l&#39;applicazione utilizzava il vecchio modello predefinito, che ometteva il nome e il cognome dell&#39;amministratore.

<!-- MC-38122-->

* L’applicazione ora utilizza il modello e-mail corretto per l’invio di e-mail a nuovi utenti. In precedenza, l&#39;applicazione utilizzava il modello predefinito lineare quando veniva selezionato un modello personalizzato.

### VersioneCMS

<!-- MC-39953-->

* Il applicazione ora applica il tema corretto a una pagina CMS dopo averne modificato il layout. In precedenza, il tema assegnato applicazione a Luma dopo aver salvato le modifiche al layout, indipendentemente dal tema assegnato alla pagina.

<!-- MC-40431-->

* Gli amministratori con ruoli limitati possono ora aggiungere, aggiornare o duplicare correttamente le pagine a cui ha accesso che sono assegnate a un nodo gerarchico. In precedenza, l’applicazione generava un errore irreversibile in queste condizioni.
...
**Problema**: _anomalie nella visualizzazione PayPal crediti degli importi_ dei scheda regalo. Quando PayPal credito è abilitato e sono configurati più importi scheda regalo, se un acquirente modifica l&#39;importo per il valore di un scheda regalo, la vetrina non aggiorna l&#39;importo per i pagamenti rateali. Una correzione per questo problema verrà inclusa in Adobe Systems Commerce 2.4.4 e Magento Open Source 2.4.4.<!-- MC-42499 AC-344-->

**Problema: La pulsante Aggiungi all&#39;ordine** non funziona per i prodotti aggiunti al carrello da referenza di magazzino _.**_**  Il applicazione visualizza questo messaggio di errore quando si fa clic sul **pulsante Aggiungi all&#39;ordine** dopo aver aggiunto prodotti all&#39;ordine per referenza di magazzino: `An error has happened during application run`. I prodotti non vengono aggiunti al carrello. **Soluzione alternativa**: utilizzare la funzionalità Aggiungi prodotti. <!-- AC-345-->

**Problema**: GraphQL `category` e `categoryList` le query ignorano le autorizzazioni Categoria che determinano se le categorie in un catalogo condiviso sono nascosto o esposte. La vetrina visualizza attualmente tutte le categorie che sono state assegnate o non assegnate al catalogo condiviso. Questo influisce sulle distribuzioni di Adobe Commerce 2.4.3 che implementano una vetrina PWA per la quale sono stati abilitati i cataloghi condivisi B2B. <!-- MC-42528-->
...
È possibile installare Adobe Commerce 2.4.3 utilizzando [Composer](../../../installation/composer.md).
