...
* Un plug-in **new Composer** consente di evitare confusione nelle dipendenze e identifica i pacchetti dannosi con gli stessi nomi dei pacchetti interni nell&#39;archivio dei pacchetti pubblico. Consulta il post sul blog [Adobe Releases New Composer Plugin with 2.4.3 Release](https://magento.com/blog/best-practices/adobe-releases-new-composer-plugin-magento-243-release).
...
[Live Search](https://experienceleague.adobe.com/it/docs/commerce/live-search/overview) basato su [Adobe Sensei](https://www.adobe.com/sensei.html) offre un&#39;esperienza di ricerca intuitiva utilizzando algoritmi di intelligenza artificiale e machine learning per eseguire un&#39;analisi approfondita dei dati aggregati dei visitatori. Consulta [Note sulla versione di Live Search](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html?lang=it).
...
* **Elenchi desideri**. La mutazione [addWishlistItemsToCart](https://developer.adobe.com/commerce/webapi/graphql/schema/wishlist/mutations/add-items-to-cart/) sposta gli elementi dalla lista dei desideri specificata al carrello del cliente. <!-- magento/partners-magento2ee#449-->

* **Registri regali**. Le attività coperte includono: <!-- PWA-1290-->

   * Utilizzare la query [giftRegistry](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/gift-registry/) per restituire il contenuto dei registri regalo del cliente.

   * Cerca registri regali per [tipo](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/type-search/), [e-mail](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/email-search/) o [ID](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/id-search/).

   * [Crea](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/create/), [aggiorna](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update/) o [elimina](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove/) registri regali.

   * [Aggiorna](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-items/) o [rimuovi](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-items/) elementi da un registro regali.

   * [Sposta tutti gli elementi](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/move-cart-items/) dal carrello al Registro regali. <!-- ENGCOM-9082 9087-->

   * [Aggiungi](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/add-registrants/), [aggiorna](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-registrants/) o [rimuovi](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-registrants/) i registranti da un registro regali.

   * [Condividi](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/share/) un registro regali con gli invitati.

* **Offerte negoziabili**. Visualizzare le query [`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quote/) e [`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quotes/). <!-- PWA-1292-->
...
La versione 2.4.3 introduce B2B v1.3.2. Questa versione include più correzioni di bug. Consulta [Note sulla versione B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html?lang=it).

### Page Builder

Page Builder è ora disponibile come estensione in bundle in Magento Open Source. Ora è lo strumento predefinito per la modifica dei contenuti per Adobe Commerce 2.4.3 e Magento Open Source 2.4.3. Può sostituire l’editor WYSIWG con qualsiasi modulo di terze parti.
...
L&#39;ambito di [Upgrade Compatibility Tool](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/upgrade-compatibility-tool/overview.html?lang=it) è stato espanso in base al feedback ricevuto dalla community. Unisciti al nostro canale Slack [#upgrade-compatibility-tool](https://magentocommeng.slack.com/archives/C019Y143U9F) per ottenere supporto dal team di prodotto di Adobe e dalla community, nonché per guidare la direzione futura dello strumento.
...
* I valori di configurazione vengono ora mantenuti a ricaricare del modulo quando la creazione di un nuovo prodotto configurabile non riesce. In precedenza, i valori andavano persi durante il ricaricamento del modulo e l&#39;applicazione visualizzava questo errore: `The value specified in the URL Key field would generate a URL that already exists`. [GitHub-32102](https://github.com/magento/magento2/issues/32102)

<!-- MC-37596-->

* L&#39;applicazione non genera più un&#39;eccezione quando si esegue `bin/magento setup:upgrade` per eseguire l&#39;aggiornamento da una distribuzione Magento Open Source con Redis ad Adobe Commerce.
...
* Ora vieni reindirizzato alla pagina di pagamento come previsto dopo aver aggiunto al carrello un bundle di prodotto dall’anteprima di aggiornamento della pianificazione e aver fatto clic sul carrello. [GitHub-447](https://github.com/magento/partners-magento2ee/issues/447)

<!-- MC-39581-->

* Il applicazione ora elimina le modifiche al modulo dell&#39;indirizzo di fatturazione nel passaggio di pagamento del pagamento se il acquirente non fa clic sul **pulsante di aggiornamento** e torna al passaggio di spedizione.

<!-- MC-39517  magento/partners-magento2ee#460-->

* Il applicazione ora visualizza un messaggio di errore informativo e non aggiorna il quantità del prodotto quando un acquirente aggiunge un quantità prodotto non valido e fa clic sul **pulsante Aggiorna articoli e quantità nella pagina Gestisci** carrello. In precedenza, l&#39;applicazioneaggiornava il prodotto quantità e non visualizzava alcun messaggio di errore. [GitHub-459](https://github.com/magento/partners-magento2ee/issues/459)

<!-- MC-37807 ENGCOM-8530-->

* I prodotti con un&#39;opzione `(File)` personalizzabile ora includono collegamenti attivi come previsto durante il processo di checkout multi-spedizione. In precedenza, questo collegare mancava. [GitHub-31095](https://github.com/magento/magento2/issues/31095)

<!-- MC-37689-->

* Il carrello Amministratore ora visualizza i prezzi dei prodotti nelle valute corrette per i negozi che supportano più valute. In precedenza, i prezzi venivano convertiti nella valuta specificata più di una volta: in primo luogo, quando i prodotti venivano aggiunti al carrello dal negozio, e poi di nuovo quando l&#39;ordine veniva successivamente reso sull&#39;amministratore.

<!-- MC-36703  magento/partners-magento2ee#439-->

* Il applicazione ora svuota il carrello acquisti come previsto dopo che un amministratore ha completato un ordine dell&#39;amministratore creato da un acquirente nel negozio. In precedenza, quando il cliente effettuava nuovamente l&#39;accesso dopo che l&#39;amministratore aveva completato l&#39;ordine, il carrello del negozio conteneva ancora il contenuto dell&#39;ordine. [GitHub-30262](https://github.com/magento/magento2/issues/30262)
...
* L&#39;ordinamento in base alla posizione nella ricerca prodotti utilizzando GET `/rest/V1/products/?searchCriteria[filterGroups]` ora funziona come previsto. In precedenza, la raccolta di prodotti non presentava un valore `position` per l&#39;ordinamento del campo. [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* È stato eseguito il refactoring delle query di pulizia delle celle per ridurre o eliminare i seguenti problemi di prestazioni: `cron` processi rimanenti bloccati in uno stato in sospeso, query MySQL sempre più lente e un aumento dell&#39;utilizzo di CPU. [GitHub-26507](https://github.com/magento/magento2/issues/26507)
...
* È stato aggiunto un osservatore dell’evento del segmento del cliente per l’API REST/SOAP. Di conseguenza, i segmenti dei clienti ora vengono aggiornati automaticamente come previsto se i criteri definiti nei segmenti vengono soddisfatti quando gli ordini vengono effettuati tramite l’API REST/SOAP.
...
* Le fatture e i PDF delle fatture ora includono gli stessi prezzi previsti per i prodotti bundle. In precedenza, i PDF delle fatture includevano il prezzo errato per i prodotti bundle. [GitHub-12856](https://github.com/magento/magento2/issues/12856)
...
* Le richieste ridondanti di AJAX alla sezione carrello del carrello sono state ridotte. In precedenza, l’applicazione non caricava correttamente il subtotale del carrello, riattivando il caricamento del carrello.
...
* È ora possibile modificare un prodotto creato utilizzando un set di attributi da cui sono stati rimossi i gruppi **Design Group And Schedule Design Update**.  In precedenza, quando si tentava di modificare un prodotto creato da tale set di attributi, l’applicazione generava un errore irreversibile. [GitHub-44](https://github.com/magento/partners-magento2ee/issues/44)
...
* L’applicazione non genera più un errore quando un amministratore tenta di caricare una pagina nella vetrina quando Google Analytics è abilitato. È stata aggiunta una voce CSP per `connect-src` per consentire le richieste AJAX to www.google-analytics.com. [GitHub-30880](https://github.com/magento/magento2/issues/30880)
...
* L&#39;applicazione non invia più richieste AJAX per ricaricare le sezioni dei dati del cliente (`Magento_Customer/js/section-config`) che non sono interessate dalla richiesta. [GitHub-31948](https://github.com/magento/magento2/issues/31948)
...
* La chiamata REST a GET `/V1/customers/search` ora restituisce le informazioni corrette per i clienti abbonati a più newsletter. [GitHub-31168](https://github.com/magento/magento2/issues/31168)
...
* I punti premio per le recensioni dei prodotti sono ora associati al sito Web pertinente quando più recensioni vengono approvate dalla griglia Admin **Marketing** > **Recensioni in sospeso** . In precedenza, i punti premio non venivano visualizzati sul negozio e non erano collegati al sito Web pertinente nell&#39;amministratore.
...
* L&#39;applicazione ora utilizza il logo caricato nelle impostazioni **Logo per la visualizzazione a stampa di HTML** quando gli acquirenti stampano un ordine dal proprio account. In precedenza, l’applicazione visualizzava il logo LUMA invece del logo caricato.
...
* La pagina della categoria non contiene più gli ID duplicati dell&#39;elemento HTML: `modes-label`, `mode-list`, `toolbar-amount`, `sorter`, `limiter`.
...
* L’applicazione non genera più un errore quando si visualizza una pagina categoria vuota con Elasticsearch abilitato. Viene invece eseguito il rendering della pagina come previsto e viene visualizzato un messaggio informativo. In precedenza, un indice full-text vuoto attivava un’eccezione in una pagina categoria.
...
* I filtri di navigazione a livelli ora visualizzano conteggi accurati dei prodotti. In precedenza, i valori del conteggio dei prodotti da Elasticsearch non venivano filtrati in base alle autorizzazioni del catalogo.

<!-- MC-23881 ENGCOM-8618-->

* Elasticsearch non genera più un errore quando il parametro di pagina dell’URL della categoria supera l’impaginazione. [GitHub-23843](https://github.com/magento/magento2/issues/23843)

<!-- MC-24033  magento/magento2#3274-->

* È ora possibile aggiungere un mapper di campi Elasticsearch personalizzato a `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`. [GitHub-24350](https://github.com/magento/magento2/issues/24350)

<!-- ENGCOM-9029 MC-24026-->

* Gli sviluppatori possono ora modificare le mappature di Elasticsearch. In precedenza, i modelli dinamici inviati al server Elasticsearch nelle mappature predefinite erano hardcoded, impedendo agli sviluppatori di indicizzare in Elasticsearch eventuali dati aggiuntivi associati a un modulo personalizzato. [GitHub-24363](https://github.com/magento/magento2/issues/24363)
...
* L&#39;applicazione visualizza ora il campo **Partita IVA** nella pagina di registrazione del cliente quando `customer/create_account/vat_frontend_visibility` è abilitato. [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* È stato aggiunto un test per questo scenario: gli utenti amministratori possono modificare un account cliente quando il cliente è iscritto a una newsletter in coda. [GitHub-30645](https://github.com/magento/magento2/issues/30645)

<!-- ENGCOM-8484-->

* Il comando è stato rimosso dai test per migliorare i `cache:flush` tempi di esecuzione per i `Catalog`moduli , `CatalogUrlRewrite`e `LoginAsCustomer` . [GitHub-31056](https://github.com/magento/magento2/issues/31056)

<!-- ENGCOM-8499-->

* I parametri ridondanti sono stati rimossi e POST modificati in GET dove necessario in `\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`.
...
* I collegamenti degli account nelle intestazioni ora seguire gli standard WCAG. In precedenza, account collegamenti nelle intestazioni contenevano ID duplicati, che causavano errori nelle convalida WCAG.

<!-- MC-40278-->

* Il applicazione ora assegna la priorità alla configurazione di store per un&#39;immagine del logo store rispetto alla configurazione del layout. In precedenza, la dimensione delle immagini del logo era fissa e non variava di store.

<!-- MC-35227-->

* Il applicazione ora visualizza gli elementi della pagina in modo coerente nelle pagine vetrina che utilizzano temi standard di Adobe Systems Commerce. In precedenza, non tutti gli stili venivano applicati in Tema vuoto, quindi non tutti gli elementi di pagina venivano visualizzati nelle pagine che utilizzavano questo tema. Nel campo di ricerca della pagina Ordini personali, ad esempio, non era presente alcuna icona di ingrandimento.
...
* JavaScript è stato rimosso dai file modello e spostato in file separati per ridurre i problemi di rendering sulle pagine di amministrazione. In precedenza, il rendering delle pagine di amministrazione non veniva eseguito correttamente nelle implementazioni in cui era stata abilitata la minimizzazione di HTML. L&#39;applicazione ha visualizzato questo errore: `An error has happened during application run. See exception log for details`. [GitHub-32454](https://github.com/magento/magento2/issues/32454)
