...
* Le dipendenze della libreria Laminas sono state aggiornate alle versioni compatibili con PHP 8.x. Alcune dipendenze ridondanti sono state rimosse dal `composer.json` file. **Adobe Commerce 2.4.3 utilizza Laminas 3.4.0**. <!-- MC-39513-->
...
* **Lista dei desideri**. La [mutazione addWishlistItemsToCart](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-wishlist-items-to-cart.html) sposta gli elementi dall&#39;elenco di desideri specificato al carrello del cliente. <!-- magento/partners-magento2ee#449-->

* **Regolamenti regalo**. I compiti coperti comprendono: <!-- PWA-1290-->
   ...
   * [Aggiorna](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry-items.html) o [remove](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry-items.html) elementi di un registro dei regali.

   * [Sposta tutti gli elementi](https://devdocs.magento.com/guides/v2.4/graphql/mutations/move-cart-items-to-gift-registry.html) dal carrello al registro dei regali. <!-- ENGCOM-9082 9087-->
...
* **Virgolette negoziabili**. Consulta la sezione [`negotiableQuote`](https://devdocs.magento.com/guides/v2.4/graphql/queries/negotiable-quote.html) e [`negotiableQuotes`](https://devdocs.magento.com/guides/v2.4/graphql/queries/negotiable-quotes.html) query. <!-- PWA-1292-->
...
* La risposta alla `{ category(id: 2){ children { name children { name } } } }` La richiesta GraphQL ora include una struttura di categoria ordinata correttamente.
...
* Le richieste vuote a GraphQL ora lanciano il codice di risposta 200 invece di 500. In precedenza, il parser GraphQL aveva generato un&#39;eccezione prima che il risultato della query venisse generato.
...
* I prodotti disattivati non sono più inclusi nella risposta GraphQL quando GraphQL viene utilizzato per collegare i prodotti di upselling.

<!-- MC-40262-->

* I merchants ora possono utilizzare GraphQL `setShippingAddressesOnCart` metodo per impostare gli indirizzi di fatturazione e di spedizione per il carrello di un acquirente quando il pagamento per gli ospiti è disattivato.
...
* Ora è possibile visualizzare e scaricare tutti i rapporti di sistema dal progetto Adobe Commerce on cloud Infrastructure. In precedenza, non era possibile scaricare rapporti di sistema per Registri, Dati, Progettazione o Negozi.
...
* RMA `resolution` l&#39;attributo non viene più visualizzato come obbligatorio quando il valore del campo dell&#39;attributo `required` è impostato su **no**.

<!-- MC-41808-->

* Ora puoi selezionare più ordini da restituire dall’amministratore. In precedenza, l&#39;applicazione generava un errore quando si faceva clic su **Invia** dopo aver selezionato più resi e aver selezionato Chiusura dal menu a discesa Azioni.

<!-- MC-41759-->

* L’applicazione ora invia ai clienti le notifiche e-mail come previsto dopo aver modificato lo stato di un ritorno utilizzando l’API REST.

<!-- MC-39981  magento/partners-magento2ee#442 -->

* L&#39;applicazione ora filtra i record in base ai valori di filtro nella griglia restituita dal cliente amministratore. In precedenza, l&#39;applicazione veniva indirizzata alla pagina Amministrazione tutti i clienti e faceva clic sul pulsante **Ripristina filtro** viene reindirizzato a questa pagina. [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* I commercianti possono ora annullare un ordine che è stato effettuato in tutto o in parte utilizzando punti di premio dopo che il cliente che ha effettuato l’ordine è stato rimosso dal sistema.

<!-- MC-40037-->

* I commercianti ora possono rimuovere i punti di ricompensa dalle regole del prezzo del carrello. Precedentemente, quando un commerciante cercava di impostare **Punti premio** a 0 quando si crea una regola di prezzo del carrello, il valore precedente per la quantità del punto di premio è persistito.

<!-- MC-39890-->

* I punti premio per le recensioni dei prodotti sono ora associati al sito web pertinente quando più recensioni sono approvate da Amministratore **Marketing** > **Revisioni in sospeso** griglia. In precedenza, i punti premio non venivano visualizzati sulla vetrina e non erano collegati al sito web rilevante nell’amministratore.
...
* È ora possibile aggiungere una mappatura campo Elasticsearch personalizzata a `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`. [GitHub-24350](https://github.com/magento/magento2/issues/24350)

<!-- ENGCOM-9029 MC-24026-->

* Gli sviluppatori ora possono modificare le mappature degli Elasticsearch. In precedenza, i modelli dinamici inviati al server di Elasticsearch nelle mappature predefinite erano codificati, il che impediva agli sviluppatori di indicizzare in Elasticsearch eventuali dati aggiuntivi associati a un modulo personalizzato. [GitHub-24363](https://github.com/magento/magento2/issues/24363)

<!-- ENGCOM-8464 -->

* I campi di ricerca (ricerca minisearch) ora funzionano come previsto quando i suggerimenti per la ricerca sono disabilitati. [GitHub-30584](https://github.com/magento/magento2/issues/30584)
...
* Gli amministratori che sono limitati a un sito Web specifico possono ora creare una spedizione per un ordine inserito sullo stesso sito web. In precedenza, l&#39;applicazione generava questa eccezione quando un amministratore che non disponeva dell&#39;autorizzazione per la visualizzazione archivio predefinita tentava di inviare un ordine inserito in una visualizzazione archivio a cui l&#39;amministratore aveva accesso: `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* L&#39;applicazione non deseleziona più il **Aggiungi commenti** quando un acquirente fa clic su **Ottieni metodi e tariffe di spedizione** e seleziona un metodo di spedizione quando si crea un ordine dall&#39;amministratore. [GitHub-30256](https://github.com/magento/magento2/issues/30256)
...
* La quantità di prodotto rimane invariata come previsto dopo che un acquirente cambia la quantità sulla pagina Indirizzo spedizione a più indirizzi e fa clic sul pulsante Indietro del browser. [GitHub-31956](https://github.com/magento/magento2/issues/31956)
...
* La funzione Pianifica aggiornamento blocco ora funziona come previsto nelle distribuzioni che implementano Flast. In precedenza, l’applicazione non aggiornava il contenuto del blocco perché i tag della cache non venivano invalidati e il contenuto obsoleto continuava infine a essere utilizzato.

<!-- MC-41142-->

* L’anteprima della gestione temporanea della regola del prezzo del carrello ora viene caricata nel sito web appropriato quando viene assegnata a più viste store del sito web. In precedenza, l&#39;anteprima della gestione temporanea della regola del prezzo del carrello veniva caricata nella visualizzazione store predefinita, indipendentemente dai siti web assegnati.

<!-- MC-40916-->

* I prezzi speciali per i prodotti nella visualizzazione store non predefinita sono ora impostati sul valore predefinito come previsto al termine di un aggiornamento pianificato.

<!-- MC-40850  magento/partners-magento2ee#489-->

* Dopo la creazione di un aggiornamento di staging futuro con una data di fine, gli esperti marketing possono ora salvare correttamente un prodotto scaricabile con un campione collegato dalla pagina del prodotto scaricabile. In precedenza, l&#39;applicazione non salvava il prodotto e visualizzava questo messaggio: `The downloadable sample isn't related to the product. Verify the link and try again`. [GitHub-488](https://github.com/magento/partners-magento2ee/issues/488)

<!-- MC-40734-->

* L&#39;applicazione non esegue più query di database di grandi dimensioni che includono tutti gli ID prodotto durante l&#39;anteprima di un aggiornamento della pianificazione di una categoria. Al contrario, il filtro categoria viene applicato alla raccolta di prodotti per creare una tabella temporanea per le anteprime di staging della categoria. In precedenza, l’applicazione inseriva tutti i prodotti in una tabella temporanea per l’anteprima di staging, che comprometteva le prestazioni.

<!-- MC-40714-->

* La `Magento_SalesRuleStaging` Il modulo non attiva più gli errori durante l&#39;aggiornamento dalla versione 2.3.x.

<!-- MC-40697-->

* L&#39;applicazione ora visualizza le condizioni della regola del catalogo con i dati corretti quando si visualizza un aggiornamento della pianificazione. In precedenza, se si visualizzava un aggiornamento della pianificazione creato con condizioni, l&#39;applicazione non visualizzava condizioni o condizioni con dati errati.

<!-- MC-40604-->

* La **Usa valore predefinito** la casella di controllo di un prodotto rimane selezionata dopo che un aggiornamento pianificato è deselezionato prima dell’aggiornamento.

<!-- MC-40588 magento/partners-magento2ee#463-->

* Le regole sul prezzo del catalogo ora si applicano come previsto ai prodotti con valori di attributo non definiti. In precedenza, gli sconti creati tramite una regola del prezzo di catalogo non venivano applicati ai prodotti con attributi non definiti. [GitHub-461](https://github.com/magento/partners-magento2ee/issues/461)

<!-- MC-40283-->

* Le regole del catalogo ora funzionano come previsto nelle implementazioni per più siti web con archivi in diversi blocchi orari. In precedenza, l&#39;applicazione applicava o disattivava le regole di catalogo su tutti i siti web contemporaneamente.

<!-- MC-40054-->

* I prezzi di livello non vengono più rimossi quando si crea un aggiornamento della pianificazione per un sito web specifico.

<!-- MC-39784 magento/partners-magento2ee#475-->

* È ora possibile salvare un prodotto dalla pagina Prodotto scaricabile dopo aver creato un aggiornamento di staging futuro con una data di fine. In precedenza, l&#39;applicazione non salvava il prodotto e visualizzava questo messaggio: `The downloadable link isn't related to the product. Verify the link and try again`. [GitHub-474](https://github.com/magento/partners-magento2ee/issues/474)
...
* L&#39;applicazione visualizza ora un **Numero IVA** nella pagina di registrazione del cliente quando `customer/create_account/vat_frontend_visibility` è abilitato. [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* Rimosso `CacheCleaner::cleanAll();` da test di integrazione. [GitHub-31074](https://github.com/magento/magento2/issues/31074)
...
* È stato rimosso il `cache:flush` dai test per migliorare il tempo di esecuzione per `Catalog`, `CatalogUrlRewrite`e `LoginAsCustomer` moduli. [GitHub-31056](https://github.com/magento/magento2/issues/31056)
...
* L’applicazione ora visualizza gli elementi di pagina in modo coerente sulle pagine della vetrina che utilizzano temi Adobe Commerce standard. In precedenza, non tutti gli stili venivano applicati al tema vuoto, quindi non tutti gli elementi di pagina venivano visualizzati nelle pagine che utilizzano questo tema. Ad esempio, nel campo di ricerca della pagina Ordini non era presente alcuna icona di ingrandimento.

### Traduzione e impostazioni internazionali

<!-- MC-23553 ENGCOM-8839-->

* Le parentesi graffe aggiunte alle stringhe non sono più precedute dalla configurazione della traduzione in linea. Precedentemente, la `escapeHtmlAttr` ha convertito le parentesi in codici di entità HTML. [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* Stringhe di testo nel modello utilizzate per gestire i metodi di pagamento memorizzati (**Il mio account**  >  **Metodi di pagamento memorizzati** ) sono ora traducibili. [GitHub-31211](https://github.com/magento/magento2/issues/31211)

<!-- MC-30798 ENGCOM-8717-->

* Le e-mail dell’ordine inviate dall’amministratore ora utilizzano le impostazioni internazionali dell’archivio non quelle associate all’account dell’amministratore. [GitHub-26521](https://github.com/magento/magento2/issues/26521)
...
* Problemi di allineamento con le schede nella pagina di invito per l’amministratore (Amministratore) **Marketing** > **Vendite private** > **Inviti**) sono state risolte. In precedenza, la visualizzazione della scheda per gli inviti accettati da un cliente non era allineata correttamente. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* La pagina di revisione dell&#39;ordine visualizzata durante il pagamento con Pagamento PayPal Express ora viene caricata correttamente. In precedenza, il modello conteneva il **Aggiorna metodo di consegna** , che era nascosto solo parzialmente dallo script durante il rendering della pagina. [GitHub-32006](https://github.com/magento/magento2/issues/32006)

<!-- MC-37941 magento/magento2#31892-->

* Le stelle di valutazione e il testo di revisione nella sezione Recensioni cliente della pagina del prodotto ora sono correttamente distanziate quando sono presenti nomi di valutazione lunghi. [GitHub-31890](https://github.com/magento/magento2/issues/31890)
...
* I reindirizzamenti URL creati dall’amministratore utilizzando un URL personalizzato ora funzionano correttamente. In precedenza, GraphQL aveva memorizzato nella cache la risposta da un GraphQL `urlResolver` e ha restituito il vecchio valore dopo l&#39;aggiornamento di riscrittura dell&#39;URL.
...
* L’applicazione ora utilizza il modello e-mail personalizzato corretto durante la generazione dell’e-mail per i nuovi amministratori. In precedenza, l&#39;applicazione utilizzava il vecchio modello predefinito, che ometteva i nomi e i cognomi dell&#39;amministratore.
...
* Gli amministratori con ruoli limitati ora possono aggiungere, aggiornare o duplicare correttamente le pagine a cui ha accesso che sono assegnate a un nodo gerarchico. In precedenza, l&#39;applicazione generava un errore irreversibile in queste condizioni.
...
* Sono state migliorate le prestazioni della pagina della categoria Amministratore caricate con Visual Merchandiser. Il caricamento delle pagine della categoria Amministratore non genera più query di database di grandi dimensioni.
...
### Restrizione sito web

<!-- MC-38516-->

* Gli acquirenti vengono ora reindirizzati alla pagina a cui stavano tentando di accedere prima che venisse richiesto loro di effettuare l&#39;accesso. In precedenza, nelle distribuzioni in cui erano abilitate le restrizioni ai siti web e la modalità di restrizione era impostata su **Vendite private: Solo accesso**, gli acquirenti sono stati reindirizzati alla home page del negozio.
...
* Il widget di collegamento nodo gerarchia CMS in TinyMCE ora esegue il rendering dei collegamenti gerarchici come previsto quando il **Ancoraggio testo personalizzato** campo vuoto.
...
* Il conteggio totale dei prodotti in un elenco di desideri per un cliente con più elenchi di desideri ora corrisponde al numero di elementi nell&#39;elenco di desideri. In precedenza, i prodotti non disponibili erano inclusi nel conteggio totale dei prodotti.
...
**Problema**: GraphQL `category` e `categoryList` le query ignorano le autorizzazioni di categoria che determinano se le categorie in un catalogo condiviso sono nascoste o esposte. La vetrina visualizza attualmente tutte le categorie assegnate o non assegnate al catalogo condiviso. Questo influisce sulle implementazioni di Adobe Commerce 2.4.3 che implementano una vetrina PWA per la quale sono stati abilitati i cataloghi condivisi B2B. <!-- MC-42528-->
...
È possibile installare Adobe Commerce 2.4.3 utilizzando [Compositore](../../../installation/composer.md).
