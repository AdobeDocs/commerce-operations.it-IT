---
title: Magento Open Source 2.4.5 note sulla versione
description: Scopri su nuove funzionalità, miglioramenti, correzioni di bug e problemi noti nella versione 2.4.5 Magento Open Source.
...
Questa versione include oltre 290 correzioni e miglioramenti della qualità.

{{bics}}

## Altre informazioni sulla versione

Sebbene il codice per queste funzionalità sia fornito in bundle con le versioni trimestrali del codice Magento Open Source core, molti di questi progetti vengono rilasciati anche in modo indipendente. Le correzioni di bug per questi progetti sono documentate nelle informazioni sulla versione separate e specifiche del progetto disponibili nella documentazione di ciascun progetto.

## Magento Open Source 2.4.5 caratteristiche principali
...
* Lo schema di integrazione DHL è stato aggiornato dalla versione 6.0 alla versione 6.2. <!-- AC-3022-->

* La URL Gateway predefinita per la spedizione USPS è stata aggiornata per essere utilizzata `https` al posto di `http`. <!-- AC-2426-->

* Il `Froogaloop` libreria è stato sostituito con il libreria Vimeo `Player.js` (2.16.4). <!-- AC-2527 2262-->

* La `grunt-eslint` libreria (NPM) è stata aggiornata alla versione più recente. <!-- AC-2700-->

* Le librerie `jQuery Storage` sono state sostituite con `julien-maurel/js-storage`. <!-- AC-2627-->

* Gli `php-cs-fixer` strumenti di analisi del codice statico sono `phpcs` ora compatibili con PHP 8.x. <!-- AC-2259-->

* `glob.js` dipendenza (aggiornato con vincolo a ~7.2.0) <!-- AC-2547-->

* `serve-static.js` Dipendenza (aggiornato con vincolo ~1.14.2) <!-- AC-2548-->

* `underscore.js` dipendenza (NPM) (1.14.2) <!-- AC-2546 2549-->

* `moment-timezone-with-data.js` (0.5.34) <!-- AC-2545-->

* Il libreria `jquery/jquery-cookie` è stato sostituito con `js-cookie/js-cookie`. <!-- AC-2626-->
...
Nuovo funzionalità di questa versione includono:

* I dati sul comportamento degli acquirenti vengono raccolti in PWA Studio vetrina per analisi web servizi.  I commercianti possono ora iscriversi ed estendere questi eventi in base alle esigenze.
...
* È ora possibile rinominare una patch di dati e aggiungere il nome della classe precedente come alias nella tabella del `patch_list` database. Magento Open Source ora controlla se nel database esistevano già alias di patch di dati prima di applicare la patch. In precedenza, l&#39;Origine applicationOpen generava un errore in queste condizioni.

<!-- AC-3036-->

* Magento Open Source non genera più un&#39;eccezione quando si tenta di modificare l&#39;URL di amministrazione in un URL personalizzato dall&#39;amministratore. In precedenza, dopo aver modificato l&#39;URL amministratore, non era possibile accedere. [GitHub-35416](https://github.com/magento/magento2/issues/35416)

<!-- AC-1480-->

* I commercianti possono ora eseguire correttamente l&#39;aggiornamento da una distribuzione Magento Open Source 2.4.2 con Klarna a Magento Open Source 2.4.3. [GitHub-33760](https://github.com/magento/magento2/issues/33760)

<!-- AC-2905-->

* Il percorso di Magento Open Source Analytics non è più codificato. In precedenza, questo percorso hardcoded generava conflitti quando venivano installate più istanze di Magento Open Source su un server. [GitHub-29373](https://github.com/magento/magento2/issues/29373)

### Accessibilità

<!-- AC-2786-->

* Il **pulsante Shopping bag** fornisce ora un&#39;indicazione programmatica o testuale del suo stato. Gli utenti dell&#39;utilità di lettura dello schermo vengono informati che facendo clic su questa pulsante verranno espansi altri contenuto oppure che il contenuto associato verrà espanso o compresso. In precedenza, questo pulsante non forniva un&#39;indicazione programmatica o testuale del suo stato.

<!-- AC-2496-->

* Informazioni sul pagamento Gli elementi di testo o le immagini di testo di scheda opzioni di credito ora soddisfano il rapporto di contrasto cromatico minimo richiesto dalle WCAG 2.0 di 4,5:1 per il testo standard di 18 pt (24 px) o 14 pt (19 px) se in grassetto. In precedenza, non soddisfacevano il rapporto di contrasto previsto.
...
* È ora possibile utilizzare l&#39;endpoint `PUT /V1/products` per aggiornare gli attributi del prezzo del prodotto per un sito Web specifico. In precedenza, se alcuni attributi del prodotto venivano ignorati per una visualizzazione store specifica, non era possibile aggiornare un attributo price per quel prodotto nella stessa visualizzazione store.
...
* L&#39;indirizzo ricerca finestra a comparsa nel passaggio fatturazione del checkout workflow non causa più errori DOM.

<!-- ACP2E-368-->

* La `addProductsToCart` mutazione ora funziona correttamente con più prodotti. In precedenza, questa query restituiva il primo prodotto con un subtotale accurato, ma restituiva un subtotale di 0 per gli altri prodotti.
...
* L&#39;attributo `Parent Only` ambito viene ora utilizzato correttamente nella condizione Regola carrello.

### Catalogo

<!-- ACP2E-758-->

* Le chiavi di URL prodotto ora rimangono invariate quando si aggiorna il nome del prodotto tramite `PUT /V1/products/` una visualizzazione store. In precedenza, veniva generata una nuova chiave URL in base al nuovo nome del prodotto e assegnata al prodotto, che sostituiva la chiave URL in quella visualizzazione store.
...
* Magento Open Source ora fornisce convalida messaggi di errore quando si tenta di aggiungere una chiave URL prodotto con un trattino finale. È inoltre disponibile un testo informativo per la descrizione dei comandi.

<!-- ACP2E-267-->

* Il nuovo `ConfigurableWishlistItem.configured_variant` campo è stato sostituito dal `ConfigurableWishlistItem.child_sku` campo. Quest’ultimo campo ha attivato un errore interno quando un elenco dei desideri del cliente conteneva un prodotto configurabile non configurato.
...
* I prezzi dei prodotti sono ora gli stessi nella pagina dei dettagli del prodotto e nelle ricerca in vetrina nelle distribuzioni multi-store dopo che l&#39;ambito dei prezzi del catalogo cambia da **sito web** a **globale**. In precedenza, la pagina Catalogo Search risultati visualizzava il **prezzo globale** e la pagina dei dettagli del prodotto visualizzava il prezzo del **sito Web** . [GitHub-34074](https://github.com/magento/magento2/issues/34074)
...
* È ora possibile modificare il limite di prodotti per pagina visualizzato all&#39;interno di una categoria quando **Ricorda paginazione categoria** è abilitato. In precedenza, il cookie `form_key` e l&#39;interfaccia utente `form_key` erano diversi e il Magento Open Source visualizzava questo errore: `Invalid Form Key. Please refresh the page`.
...
* Magento Open Source ora visualizza un messaggio di errore come previsto quando si tenta di creare un attributo dalla pagina del prodotto senza completare il campo Amministratore. [GitHub-33099](https://github.com/magento/magento2/issues/33099)
...
* Magento Open Source visualizza ora gli attributi configurabili come previsto durante la creazione degli attributi globali `select` tramite uno script di patch. In precedenza, gli attributi globali idonei erano nascosti.
...
* I clienti ora ricevono promemoria via e-mail sui loro carrelli abbandonati sul programmare corretto. La nuova `TIMESTAMPDIFF(DAY, ,)` funzione SQL ha sostituito la `TO_DAYS()` funzione e calcola la differenza nei timestamp sulla base della data e dell&#39;ora. In precedenza, i promemoria via e-mail non venivano inviati per programmare a causa del calcolo errato di due valori data-ora di abbandono del carrello (qualsiasi fuso orario) e ora del server (UTC).

<!-- AC-776-->

* Magento Open Source ora visualizza un messaggio di errore nella pagina Spedizione quando un acquirente inserisce un formato e-mail non valido dopo che il acquirente fa clic su **Effettua ordine**. In precedenza, il messaggio di errore veniva visualizzato nella pagina Pagamento. [GitHub-33590](https://github.com/magento/magento2/issues/33590)
...
* Il Magento Open Source non genera più un errore SQL dopo l&#39;assegnazione di una nuova origine a un prodotto e la modifica della quantità. [GitHub-35262](https://github.com/magento/magento2/issues/35262)
...
* Il Magento Open Source non visualizza più un errore `preg_replace()` in Admin. Il terzo argomento (`$subject`) è ora di tipo `array|string` invece di `bool`.
...
* Il Magento Open Source non genera più il seguente errore quando si crea un plug-in per qualsiasi metodo della classe `vendor/magento/module-backend/Model/Menu.php`: `Error: Call to undefined method ReflectionUnionType::getName()`. [GitHub-35292](https://github.com/magento/magento2/issues/35292)
...
* Il Magento Open Source non attiva più un errore `trim(): Passing null to parameter #1 ($string) of type string is deprecated` quando la connessione AMPQ è configurata senza la configurazione SSL.
...
* Il tempo di caricamento delle pagine dell&#39;elenco dei prodotti della categoria è stato migliorato aggiungendo `Magento_Ui/js/core/app` come a `deps` `app/code/Magento/Ui/view/frontend/requirejs-config.js`. [GitHub-34847](https://github.com/magento/magento2/issues/34847)

<!-- AC-2101-->

* Il metodo `ProductRepository.php:get` ora restituisce una volta le chiavi della cache. In precedenza, sono stati restituiti due volte. [GitHub-34958](https://github.com/magento/magento2/issues/34958)
...
* Il Magento Open Source visualizza ora un messaggio di errore quando imposti un dominio cookie non valido (**Archivio** > **Configurazioni** > **Web** > **Dominio cookie impostazioni cookie predefinito**). In precedenza, il sito web si bloccava. [GitHub-35048](https://github.com/magento/magento2/issues/35048)
...
* Il Magento Open Source non genera più un errore quando si attiva la casella di controllo **Seleziona qui per collegare un feed RSS alla tua lista dei desideri** prima di fare clic su **Condividi elenco dei desideri**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* Le sessioni di Magento Open Source non terminano più dopo aver effettuato una richiesta GraphQL. In precedenza, il plug-in `ClearCustomerSessionAfterRequest` ha disconnesso l&#39;acquirente. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* `products` Le query non restituiscono `price_range` più valori per i prodotti configurabili affetti da varianti disabilitate. [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* Gli esercenti non possono più creare una nota di accredito con una quantità totale decimale quando **Qtà decimale** è disabilitato a livello di prodotto o impostazione globale. In precedenza, i commercianti potevano creare una nota di credito per il totale decimale quantità dove non era applicabile.
...
* Gli attributi di prezzo che non hanno alcun valore nel ambito predefinito (ma che sono definiti a livello di visualizzazione store) sono ora indicizzati correttamente. In precedenza, le espressioni SQL che recuperano i valori degli attributi di prezzo dalla tabella EAV non tenevano in considerazione account lo scenario in cui il valore non era definito nel ambito predefinito.

<!-- ACP2E-313-->

* Il prezzo indicato nella pagina dei dettagli del prodotto è ora lo stesso del prezzo indicato nella workflow di checkout per i prezzi di livello che differiscono in base quantità selezionato (ad esempio, un prodotto ha un prezzo diverso in base all&#39;acquisto di 2 articoli rispetto a 5 articoli). In precedenza, il prezzo di pagamento rifletteva il prezzo per la quantità di prodotto più bassa.

### ReCAPTCHA

* L&#39;pulsante **Invia** nelle pagine Accedi e Crea un account è ora inattivo fino al completamento del caricamento di ReCaptcha.

### Ruoli

* La **pagina Store** >  **Configuration** > **Services** visualizza ora Magento Open Source informazioni API Web come previsto quando l&#39;opzione Accesso risorse è impostata su  **Personalizzato** nella scheda Risorse ruolo. [GitHub-35506](https://github.com/magento/magento2/issues/35506)

### Ricerca {#search-heading}

<!-- ACP2E-645-->

* Filtrando i prodotti in base al campione di colore sulla navigazione a più livelli viene visualizzata l&#39;immagine corretta per i prodotti dopo la correzione.

<!-- ACP2E-615-->

* Le query di Elasticsearch ora funzionano come previsto quando `int` è configurato come attributo di backend `type` ricercabile. In precedenza, applicationOpen Source generava un&#39;eccezione `Elasticsearch\Common\Exceptions\BadRequest400Exception`.
...
* In Magento Open Source ora viene visualizzato un conteggio accurato dei suggerimenti di ricerca nella vetrina delle distribuzioni in cui sono abilitati i suggerimenti di ricerca e l&#39;impostazione **Mostra conteggio risultati per ogni suggerimento**. In precedenza, il conteggio visualizzato accanto alle parole chiave era zero.
...
* È ora possibile creare un cliente dall&#39;amministratore quando `Magento_LoginAsCustomerAdminUi` è abilitato e **Memorizza Visualizza per accedere è** impostato sulla selezione manuale. In precedenza, l&#39;Origine applicationOpen generava questo errore: `(Magento\Framework\Exception\LocalizedException): Unable to get Customer ID`. [GitHub-33096](https://github.com/magento/magento2/issues/33096)
...
* L&#39;collegare Informativa sulla privacy nel piè di pagina amministrazione ora si collega al nuovo Informativa sulla privacy di Adobe Systems.
...
* Magento Open Source ora visualizza un messaggio di errore informativo quando un amministratore tenta di salvare un indirizzo con linee stradali eccessive in Admin **Store** > **Attributi**  > **Indirizzo** cliente.  L&#39;amministratore può ora eliminare le informazioni aggiuntive sull&#39;indirizzo e salvare correttamente l&#39;indirizzo. In precedenza, l&#39;Origine applicationOpen eseguiva il commit delle righe aggiuntive ma non salvava i dati.
...
* Le richieste PUT e DELETE in blocco API REST ora funzionano come previsto quando il modulo `Magento_ReCaptchaWebapiRest` è abilitato. [GitHub-35348](https://github.com/magento/magento2/issues/35348)
