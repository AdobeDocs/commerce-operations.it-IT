...
Il Magento Open Source introduce miglioramenti alla qualità della piattaforma, ai metodi di pagamento, alle prestazioni di memorizzazione in cache GraphQL e all’accessibilità. Include aggiornamenti ai moduli Google integrati.
...
* Il Magento Open Source non visualizza più un `preg_replace()` errore in Admin. Il terzo argomento (`$subject`) è ora di tipo `array|string` anziché `bool`.
