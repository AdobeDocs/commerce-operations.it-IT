...
* Il CAPTCHA non convalida più fallito in modo casuale nella pagina di pagamento della workflow di pagamento.

<!-- MC-41001-->

* Il applicazione ora visualizza i campi CAPTCHA come previsto dopo aver superato il numero di tentativi di completamento non riusciti. In precedenza, sebbene il applicazione richiedesse di tentare nuovamente la verifica CAPTCHA, i campi CAPTCHA non venivano visualizzati.

<!-- MC-40359-->

* CAPTCHA ora funziona come previsto nella pagina di checkout. In precedenza, dopo che un acquirente risposto correttamente a una richiesta CAPTCHA, il caricatore nella pagina di checkout non veniva mai completato e il applicazione visualizzato questo errore: `captchaData[formId] is undefined`. (Questo errore si verificava solo quando l’acquirente utilizzava lo stesso browser da cui aveva precedentemente effettuato l’accesso a una distribuzione in cui era in esecuzione la versione 2.3.5-p1.)
...
* Gli acquirenti possono ora trovare un ordine nella pagina Ordini e resi quando il cognome termina con uno spazio vuoto.

<!-- MC-41438-->

* Le fatture vengono ora create con il totale complessivo corretto quando un regola del prezzo del carrello che assegna uno sconto del 100% viene applicato a un ordine che è anche soggetto alle imposte sui prodotti e sugli sconti del catalogo e che si qualifica per la spedizione gratuito. In precedenza, l&#39;ordine aveva il prezzo corretto, ma la fattura no. [GitHub-30853](https://github.com/magento/magento2/issues/30853)

<!-- MC-40384-->

* Filtrando gli ordini per data ora si ottengono risultati accurati. In precedenza, il applicazione non restituiva un ordine effettuato dopo le 00:00 UTC quando filtravi gli ordini in base alla data dell&#39;ordine.
...
* Il applicazione ora visualizza un `DateRange` filtro nell&#39;area Accesso della **pagina Accesso >****clienti**. In precedenza, nella applicazione veniva visualizzato un `Text` filtro. [GitHub-30328](https://github.com/magento/magento2/issues/30328)

<!-- ENGCOM-9009 -->

* JavaScript è stato rimosso dai file modello e spostato in file separati per ridurre i problemi di rendering nelle pagine di amministrazione. In precedenza, il rendering delle pagine di amministrazione non veniva eseguito correttamente nelle distribuzioni in cui era stata abilitata la minimizzazione del codice HTML. Il applicazione visualizzato questo errore: `An error has happened during application run. See exception log for details`. [GitHub-32454](https://github.com/magento/magento2/issues/32454)
