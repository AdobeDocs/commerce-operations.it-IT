...
Magento Open Source 2.4.7 è ancora compatibile con PHP 8.1 solo a scopo di aggiornamento. PHP 8.1 non è supportato e non è consigliato. Il codice di base di Magento Open Source 2.4.7, tutte le estensioni in bundle e tutte le estensioni di proprietà di Adobe e i servizi SaaS sono compatibili con PHP 8.3. <!-- AC-9668 -->

* Supporto di **RabbitMQ 3.13**. Questa versione è compatibile con l’ultima versione di RabbitMQ 3.13. La compatibilità rimane con RabbitMQ 3.11 e 3.12, che è supportato rispettivamente fino ad agosto 2024 e dicembre 2024, ma Adobe consiglia di utilizzare Magento Open Source 2.4.7 solo con RabbitMQ 3.13. <!-- AC-9659 -->

* **Compositore 2.7.x**. La compatibilità con Composer 2.2.x rimane. <!-- AC-9663 -->

* **Supporto cache di vernice 7.4**. Questa versione è compatibile con la versione più recente di Varnish Cache 7.4. La compatibilità rimane con le versioni 6.0.x e 7.2.x, ma si consiglia di utilizzare Magento Open Source 2.4.7 solo con Vernice Cache versione 7.4 o 6.0 LTS. <!-- AC-9662 -->
...
Tutte le librerie JavaScript e le dipendenze NPM nel codice di base Magento Open Source sono state aggiornate alle versioni più recenti disponibili. Tutte le dipendenze della libreria Laminas sono state aggiornate alla versione più recente compatibile con PHP 8.3. <!-- AC-9667 9665 -->

#### Aggiornamenti aggiuntivi

* Il gateway API XML di Commerce UPS è stato migrato alla nuova [API](https://experienceleague.adobe.com/it/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups) REST di Commerce UPS per supportare gli aggiornamenti che UPS sta apportando al proprio [modello](https://developer.ups.com/oauth-developer-guide) di sicurezza API. (UPS sta implementando un modello di sicurezza OAuth 2.0 (token portatore) per tutte le API.) Tutte le precedenti API XML di Commerce UPS sono state rimosse dalla codebase di Magento Open Source 2.4.7. <!-- AC-8918 -->

* La Magento Open Source [integrazione](https://experienceleague.adobe.com/it/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex) con FedEx è stata migrata dai servizi Web FedEx WSDL legacy alle più recenti API FedEx RESTful. FedEx Web Services Tracking, Address Validation, and Validate Postal Codes WSDLS sarà ritirato nel maggio 2024. <!-- AC-1470 -->
...
* I moduli di spedizione Temando sono stati rimossi dalla base di codice principale di Magento Open Source. Questa funzionalità è stata rimossa in Magento Open Source 2.4.4. <!-- AC-8130 -->
...
* **[!DNL Vault Apple Pay Payments]** - Consenti ai clienti connessi di archiviare i pagamenti [!DNL Apple Pay] nel proprio account Commerce Store per utilizzarli in transazioni future. Ciò riduce il numero di passaggi per il checkout e crea un&#39;esperienza di checkout più veloce per il cliente che ritorna.
...
* [`guestOrder`](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/queries/guest-order/) quesito
...
* Magento Open Source ora ottimizza le prestazioni non caricando i dati dei preventivi specifici di utente quando si generano pagine memorizzabili nella cache a pagina intera come le pagine PDP (Product Details Pages) e le pagine CMS quando il carrello persistente è abilitato. In precedenza, il sistema caricava inutilmente dati di sessioni e preventivi specifici per utente durante la generazione di queste pagine, il che influiva sulle prestazioni.
...
* Magento Open Source ora visualizza messaggi informativi quando viene attivata un’azione di aggiunta al carrello. In precedenza, non `The requested qty is not available` veniva sempre visualizzato come necessario sulla vetrina.

<!-- ACP2E-2166 -->

* I tempi di caricamento della pagina di checkout sono stati migliorati per i clienti con rubriche di grandi dimensioni. Magento Open Source ora elabora solo il numero di indirizzi dei clienti specificato nell&#39;impostazione **[!UICONTROL Customer Addresses Limit]** . In precedenza, Magento Open Source caricava l&#39;intera rubrica.
...
* L&#39;intero importo di uno sconto sull&#39;intero carrello viene ora applicato come previsto ai carrelli che contengono sia pacchetti che prodotti configurabili.
...
* Il `bin/magento catalog:images:resize` comando ora genera correttamente le immagini dei prodotti per i prodotti assegnati a un sito Web personalizzato con un tema personalizzato.

<!-- ACP2E-1982 -->

* I metadati con `NULL` o `NOT NULL` valori vengono ora salvati per il store predefinito in base alla casella di **controllo Usa Valore** predefinita come previsto. In precedenza, metadati con `NULL` valore non veniva salvato quando questa casella di controllo era deselezionata.

<!-- ACP2E-2093 -->

* I commercianti possono ora eliminare correttamente un&#39;immagine esistente dalla pagina dei dettagli del prodotto, quindi caricare una nuova immagine con lo stesso nome. In precedenza, in tali circostanze, la pagina del prodotto vetrina non visualizzava l&#39;immagine corretta.

<!-- ACP2E-2128 -->

* La `productDetail` query ora restituisce il nome del prodotto anziché null quando l&#39;attributo dell&#39;immagine `alt` del prodotto è impostato su vuoto.

<!-- ACP2E-2215 -->

* Gli ordini che includono prodotti sia scaricabili che fisici ora includono collegamenti di lavoro al prodotto scaricabile non appena l&#39;ordine è stato completato. In precedenza, i collegamenti ai prodotti scaricabili non erano disponibili finché i prodotti fisici non venivano spediti nello stesso ordine.
...
* L&#39;ordinamento delle barre degli strumenti ora funziona come previsto nelle pagine Search quando l&#39;impostazione **[!UICONTROL Remember Category Pagination]** è abilitata. [GitHub-33220](https://github.com/magento/magento2/issues/33220)

<!-- ACP2E-1640 -->

* Gli attributi a selezione multipla vengono ora salvati come previsto quando **[!UICONTROL Use Default Value]** la casella di controllo nell&#39;ambito con vista store è selezionata. In precedenza, questa casella di controllo era deselezionata quando il prodotto veniva modificato e il valore predefinito veniva salvato.

<!-- ACP2E-1878 -->

* Gli utenti amministratori con accesso limitato al sito web possono ora aggiungere immagini a un prodotto che rientra nel loro ambito. Se l&#39;utente amministratore non dispone dei diritti di accesso sul prodotto, Magento Open Source visualizza un messaggio informativo. In precedenza, gli utenti amministratori con accesso limitato al sito web non potevano aggiungere immagini ai prodotti a cui erano accesso.
...
* `routes` Le query che utilizzano frammenti restituiscono ora le informazioni sulla categoria come previsto. In precedenza, si verificava un errore interno del server nella pagina della categoria. [GitHub-35906](https://github.com/magento/magento2/issues/35906)
...
* L&#39;etichetta **[!UICONTROL As low as]** non viene più visualizzata per un prezzo del prodotto configurabile quando tutte le opzioni hanno lo stesso prezzo.

<!-- ACP2E-2464 -->

* Gli utenti amministratori con autorizzazioni per il sito Web possono ora salvare un prodotto figlio che fa parte di un prodotto configurabile in un contesto store diverso. I processi di reindicizzazione ora ignorano questo tipo di prodotto configurabile.

<!-- ACP2E-2003 -->

* I prodotti configurabili con due prodotti figlio non vengono più contrassegnati come esauriti quando un prodotto figlio viene disabilitato da un aggiornamento pianificato. Questo era un problema noto per la versione 2.4.7-beta1.
...
* `Swaziland` è stato aggiornato a `Eswatini` tutta la codebase. [GitHub-37873](https://github.com/magento/magento2/issues/37873)
...
* È stato corretto il nome di una variabile in `CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`. [GitHub-38093](https://github.com/magento/magento2/issues/38093)
...
* Il ticker del conto alla rovescia degli eventi viene ora visualizzato come previsto nella pagina della categoria.
...
* Il serializzatore JSON nel `User` modulo è stato sostituito con un nuovo serializzatore più accurato (`JsonHexTag`). In precedenza, Magento Open Source a volte si bloccava quando si tentava di utilizzare i dati serializzati dal serializzatore normale quando contenevano elementi non di escape. [GitHub-31377](https://github.com/magento/magento2/issues/31377)
...
* `bin/magento config:show` E `set` i comandi che utilizzano `$_ENV` variabili ora supportano l&#39;uso di siti Web e codici store che contengono caratteri maiuscoli o maiuscoli.
...
* Magento Open Source ora registra un messaggio di eccezione come previsto quando il numero di partita IVA convalida dal servizio convalida VIES non riesce. [GitHub-36065](https://github.com/magento/magento2/issues/36065)

<!-- AC-8146 -->

* Il `Administrators` nome è stato rimosso dalla query utilizzata per recuperare il ruolo di un amministratore durante la creazione di un ruolo amministratore. [GitHub-36998](https://github.com/magento/magento2/issues/36998)

<!-- AC-8607 -->

* Il `bin/magento setup:config:set` comando non riesce più quando un database non valido nome utente è incluso nel `app/etc/env.php` file. [GitHub-37409](https://github.com/magento/magento2/issues/37409)
...
* La funzione `populateWithArray` ora trasforma correttamente le proprietà dell&#39;oggetto in maiuscole/minuscole, rendendolo compatibile con i getter e i setter `AbstractModel`. In precedenza, la formattazione delle maiuscole e minuscole era errata quando il nome dell&#39;attributo dati conteneva diverse lettere maiuscole di fila.

<!-- ACP2E-1522 -->

* Il valore dell&#39;incremento automatico nella tabella ora viene incrementato correttamente dopo il `catalog_product_entity_varchar` salvataggio di un prodotto. In precedenza, questo valore aumentava di dieci.
...
* La mutazione `addProductsToCart` non segnala più errori non correlati in `user_errors`. In precedenza, gli errori relativi al carrello venivano inclusi insieme `user_errors` agli errori operativi previsti. [GitHub-37908](https://github.com/magento/magento2/issues/37908)
...
* Mutex aggiunto alla query `addConfigurableProductsToCart` per evitare race condition durante un aggiornamento da richieste simultanee. In precedenza, quando un commerciante inviava due richieste parallele per aggiungere lo stesso prodotto configurabile a un carrello, venivano aggiunti due articoli separati con lo stesso referenza di magazzino di prodotto. [GitHub-37847](https://github.com/magento/magento2/issues/37847)
...
* Gli attributi di prodotto di tipo `DateTime` vengono ora associati al `FilterRangeTypeInput` filtro nelle query dei prodotti. In precedenza, questi attributi erano mappati a `FilterMatchTypeInput`. Come risultato di questa modifica, le query che filtrano gli `DateTime` attributi richiedono `from` e `to` valori anziché i valori corrispondenti.
...
* La `products` query restituisce ora tutte le aggregazioni disponibili (filtri) e le rispettive etichette correttamente tradotte. In precedenza, le etichette di aggregazione di prezzi e categorie non venivano tradotte come previsto. [GitHub-36140](https://github.com/magento/magento2/issues/36140)
...
* È stato corretto un errore in cui il metodo che imposta la store corrente in una mutazione GraphQL veniva eseguito prima del metodo che gestisce convalida. [GitHub-31336](https://github.com/magento/magento2/issues/31336)

### Immagine

<!-- ACP2E-1353 -->

* Il modulo di caricamento dell&#39;icona Admin favicon ora supporta `.ico` i tipi di file. [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* Il processo di importazione dei prodotti ora analizza i valori con il separatore corretto. In precedenza, il processo di importazione utilizzava l&#39;operatore pipe (&quot;|&quot;) per analizzare i valori a selezione multipla fino a quando il separatore di valori determinato non era uguale al valore di default. Il processo di importazione non è riuscito.
...
* Magento Open Source ora prende in considerazione l&#39;impostazione **[!UICONTROL Disable Automatic Group Change Based on VAT ID]** durante l&#39;importazione dei record dei clienti. In precedenza, il valore `disable_auto_group_change` nei file di importazione veniva ignorato. [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* I commercianti possono ora specificare le impostazioni internazionali in cui vengono presentati i dati di importazione e il modo in cui i validatori dei dati devono analizzarli. Se non si specifica un valore di impostazioni locali, Magento Open Source utilizza le impostazioni locali di configurazione predefinite (non la visualizzazione predefinita dell&#39;archivio) per analizzare i dati. In Amministrazione, i processi di importazione ed esportazione continuano a utilizzare le impostazioni locali dell’interfaccia utente di amministrazione per importare, analizzare e formattare i dati, rispettivamente.
...
* Magento Open Source visualizza ora un messaggio di errore informativo nella griglia di importazione pianificata quando un&#39;immagine non viene importata correttamente durante un&#39;operazione di importazione pianificata. In precedenza, non veniva visualizzato alcun messaggio di errore.

<!-- ACP2E-1565 -->

* Il processo di importazione del prodotto non viene più interrotto quando mancano le immagini del prodotto. In precedenza, quando mancava un&#39;immagine del prodotto durante l&#39;importazione di un servizio di terze parti tramite un file CSV, Magento Open Source visualizzava questo errore: `Maximum error count has been reached or a system error is occurred!`.
...
* Magento Open Source non include più il percorso completo della directory principale del server nel messaggio di errore visualizzato quando al server viene inviata una richiesta HTTP non valida. Solo le stringhe vengono passate al metodo `explode` come secondo argomento.

<!-- AC-8384 -->

* `restricted_classes.php` ha sostituito `Zend_Validate_File_Upload`. In precedenza, la convalida si arrestava in modo anomalo durante il caricamento di un file di importazione di dimensioni superiori al valore configurato in `php.ini `. [GitHub-37281](https://github.com/magento/magento2/issues/37281)
...
* I commercianti possono ora notificare agli acquirenti che un ordine è pronto per il ritiro quando l&#39;impostazione **[!UICONTROL No Manage Stock]** è abilitata per un prodotto. In precedenza, quando questa impostazione era abilitata, Magento Open Source visualizzava questo messaggio di errore: `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* I commercianti possono ora creare una nota di credito per gli ordini da cui è stato eliminato un prodotto semplice associato a un prodotto configurabile. In precedenza, gli esercenti non potevano creare la nota di credito e Magento Open Source generava un&#39;eccezione `TypeError`.
...
* La pagina Categoria non viene più reindirizzata al dashboard di amministrazione quando si ordinano i prodotti utilizzando l’opzione &quot;Sposta dal magazzino in basso&quot;. Il problema è stato risolto inserendo l&#39;istruzione join SQL all&#39;interno dell&#39;istruzione condizionale solo dopo aver verificato che non fosse già inclusa. In precedenza, la pagina veniva reindirizzata alla dashboard di amministrazione con il messaggio:`Invalid security or form key. Please refresh the page`.

<!-- ACP2E-1488 -->

* `AUTO_INCREMENT` `inventory_source_item` della tabella non viene più aumentata ad ogni `UPDATE` operazione. In precedenza, ogni aggiornamento aumentava il valore `AUTO_INCREMENT` di questa tabella, causando l&#39;esclusione del valore `AUTO_INCREMENT` dall&#39;intervallo durante l&#39;aggiunta di un nuovo record a `inventory_source_item table`. Di conseguenza, gli utenti amministratori hanno ricevuto il seguente errore quando hanno tentato di creare un nuovo prodotto dall&#39;amministratore: `Numeric value out of range...`.
...
* Magento Open Source non genera più un errore dopo il ricaricamento della pagina quando un acquirente detrae una quantità di prodotto da un ordine che rientra nell’intervallo di prodotti disponibili durante la selezione dell’origine di spedizione.
...
* Sia le origini predefinite che quelle non predefinite ora visualizzano le informazioni corrette per la quantità venduta del prodotto quando un&#39;origine non predefinita che è stata ordinata dalla vetrina viene elaborata per la spedizione. In precedenza, Magento Open Source mostrava uno stato delle scorte errato per le origini dei prodotti.
...
* Magento Open Source ora applica l&#39;importo dello sconto corretto agli ordini parzialmente annullati. In precedenza, l&#39;importo dello sconto calcolato nella colonna `base_discount_cancelled` veniva calcolato in modo errato quando un ordine veniva modificato. Il supporto per i valori negativi è stato aggiunto alle fatture per gli ordini scontati.

<!-- ACP2E-2412 -->

* Magento Open Source ora ordina gli attributi dell’indirizzo del cliente personalizzato in base all’ordinamento fornito quando l’utente amministratore ha creato l’ordine in Admin.

<!-- ACP2E-2400 -->

* La pagina Griglia ordini di vendita amministratore ora viene caricata come previsto quando un commerciante avvia una ricerca da questa pagina. In precedenza, quando un commerciante tentava di cercare ordini da questa tabella, la pagina si bloccava e Magento Open Source visualizzava un errore `Request-URI Too Long` nella console del browser.
...
* Gli acquirenti non possono più salvare gli stati degli ordini errati nella pagina dell’ordine. Lo stato dell&#39;ordine non viene più modificato da Completo a Elaborazione quando viene spedito un articolo. In precedenza, Magento Open Source non verificava lo stato attuale dell’ordine prima di salvarne lo stato dopo che un acquirente aveva inserito un commento. [GitHub-36562](https://github.com/magento/magento2/issues/36562)
...
* La pagina della nota di accredito non si blocca più se la spedizione gratuita è abilitata quando il prezzo di spedizione include le imposte e i calcoli delle imposte vengono applicati dopo gli sconti. In precedenza, Magento Open Source generava un&#39;eccezione `Division by zero`. [GitHub-36800](https://github.com/magento/magento2/issues/36800)
...
* L&#39;contenuto convertito in HTML viene ora visualizzato nei modelli di e-mail come previsto quando il `Magento_AsyncConfig` modulo è disabilitato. In precedenza, i modelli e-mail contenevano solo codice HTML grezzo.

<!-- AC-9128 -->

* Le prestazioni delle operazioni di salvataggio di Pagina Builder nella browser Effetto cromatura sono migliorate.
...
* Copiare e incollare testo da un editor di testo di Page Builder a un altro ora funziona come previsto. In precedenza, la pagina in cui si è tentato di incollare il testo non poteva essere salvata e Magento Open Source ha visualizzato questo errore della console: `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`.
...
* Il **[!UICONTROL Venmo]** pulsante ora può essere nascosto quando i pulsanti PayPal sono abilitati. In precedenza, il **[!UICONTROL Venmo]** pulsante non poteva essere rimosso dal carrello, dal mini carrello, dai dettagli del prodotto o dalle pagine di pagamento.

<!-- BUNDLE-3313 -->

* Gli acquirenti possono ora selezionare un metodo di spedizione mentre applicano coupon codice durante il checkout. In precedenza, in queste condizioni, Magento Open Source veniva generato un errore e assegnava all&#39;ordine la spedizione forfettaria.
...
* Magento Open Source non genera più un errore quando si crea una seconda fattura parziale per un ordine pagato da Braintree quando l&#39;opzione è **[!UICONTROL Enable Vault for Card Payments]** abilitata. In precedenza, quando un commerciante tentava di creare una seconda fattura, Magento Open Source generava questo errore: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* Sono state migliorate le prestazioni della mutazione `addProductToCart` durante l&#39;esecuzione con molti attributi di prodotto personalizzati. In precedenza, quando questa mutazione veniva eseguita con molti attributi di prodotto ( approssimativamente 1000), Magento Open Source tentava di caricare tutti gli attributi di prodotto.
...
* Magento Open Source passa ora a una nuova pagina di spedizione come previsto dopo aver creato una spedizione parziale per un ordine. In precedenza, dopo la creazione di una spedizione parziale, Magento Open Source non creava una pagina di spedizione completa per i restanti prodotti dell&#39;ordine. [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* Magento Open Source non genera più un’eccezione durante il pagamento con più indirizzi di spedizione quando l’utente amministratore utilizza la funzione di accesso come cliente.

<!-- ACP2E-2258 -->

* I prodotti ora vengono elencati correttamente nella pagina Crea Pacchetti Griglia Prodotti quando aggiungi prodotti a un pacco dopo che è stata creata l&#39;etichetta di spedizione.

<!-- ACP2E-2044 -->

* La `cart` query ora restituisce sconti di spedizione discreti come sconti di spedizione separati con etichette di sconto univoche. La nuova proprietà di sconto `applied_to` supporta la differenziazione tra gli sconti applicati a un elemento del carrello. In precedenza, tutti gli sconti sulla spedizione venivano combinati in un unico valore hardcoded **[!UICONTROL Shipping Discount]**.
...
* La query `cart` ora restituisce un valore `selected_shipping_method` che corrisponde a uno dei codici vettore e metodo `available_shipping_methods`. In precedenza, quando uno dei metodi di spedizione selezionati conteneva un carattere di sottolineatura, questi codici corriere/metodo non corrispondevano. [GitHub-36606](https://github.com/magento/magento2/issues/36606)

<!-- AC-2556 -->

* La mutazione `setShippingAddressesOnCart` non genera più un errore quando il secondo valore nella matrice stradale è nullo. In precedenza, veniva generata questa eccezione: `The address failed to save. Verify the address and try again`. [GitHub-35160](https://github.com/magento/magento2/issues/35160)

### Mappa del sito

<!-- AC-2465 -->

* Il mappa del sito XML ora funziona come previsto in una distribuzione a più piani. In precedenza, le Sitemap non contenevano le pagine predefinite assegnate per ogni store. [GitHub-35001](https://github.com/magento/magento2/issues/35001)
...
* Magento Open Source ora visualizza il valore corretto per il valore **[!UICONTROL Regular Price]** quando viene selezionata un&#39;opzione personalizzata per un prodotto con un valore **[!UICONTROL Special Price]** quando **[!UICONTROL Price Display Settings]** è impostato per includere l&#39;imposta.
...
* L&#39;icona di compressione/espansione per la sezione **[!UICONTROL Additional Settings]** durante la creazione della regola fiscale ora funziona come previsto. In precedenza, funzionava solo l&#39;icona Espandi. [GitHub-37922](https://github.com/magento/magento2/issues/37922)
...
* L&#39;importo delle imposte nell&#39;area di stima del carrello viene aggiornato come previsto quando viene aggiornato il metodo di spedizione. In precedenza, l’aliquota non veniva aggiornata dopo che un acquirente aveva cambiato paese durante il pagamento.
...
* L&#39;utilità di caricamento delle immagini ora funziona come previsto nelle righe dinamiche. Le immagini vengono ora caricate correttamente quando vengono inviate come array multinazionale. [GitHub-37957](https://github.com/magento/magento2/issues/37957)

<!-- AC-10877 -->

* File stili di caricamento non vengono più duplicati nel tema Luma. [GitHub-32373](https://github.com/magento/magento2/issues/32373)

<!-- AC-9984 -->

* I campi stradali nel modulo di modifica dell&#39;indirizzo del account del campo strada vengono ora salvati correttamente. Questi campi sono ora ordinati correttamente in base alle chiavi specificate nel modulo. [GitHub-37973](https://github.com/magento/magento2/issues/37973)
...
* È stata migliorata la messaggistica degli errori che si verificano durante il rendering del modello. Magento Open Source visualizza ora un messaggio informativo. In precedenza, Magento Open Source mostrava perdite di HTML quando si verificava un errore PHP.
...
* Gli utenti Admin ora possono cambiare il logo della vetrina quando è abilitata la modalità store singola. In precedenza, Magento Open Source visualizzato il logo di fallback predefinito. [GitHub-35952](https://github.com/magento/magento2/issues/35952)
...
* Magento Open Source ora visualizza i prezzi corretti per i prodotti in bundle fisso con prezzi speciali.
...
* Il modulo di registrazione cliente ora viene caricato come previsto quando include più attributi cliente selezionati. In precedenza, Magento Open Source ha generato questa eccezione: `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)

<!-- ACP2E-1602 -->

* I messaggi di benvenuto contenenti virgolette singole vengono ora visualizzati correttamente nella vetrina senza errori della console. In precedenza, se un messaggio di benvenuto conteneva una virgoletta singola, il messaggio di benvenuto non veniva caricato durante il rendering nella vetrina. Magento Open Source ha inoltre visualizzato errori della console.
...
* Magento Open Source non genera più riscritture dell&#39;URL del prodotto per le categorie principali e per i prodotti impostati su **[!UICONTROL Not Visible Individually]** nelle impostazioni multi-store durante la generazione di riscritture dell&#39;URL mancanti per gli archivi personalizzati.
...
* Magento Open Source ora esegue il rendering della struttura delle categorie a tutti i livelli durante la creazione delle riscritture degli URL delle categorie. In precedenza venivano visualizzati solo i primi tre livelli di categorie. [GitHub-36973](https://github.com/magento/magento2/issues/36973)
...
* Magento Open Source non visualizza più un avviso `Integration not secure` in Admin **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** quando i campi **[!UICONTROL Callback URL]** e **[!UICONTROL Identity link URL]** sono vuoti. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* L&#39;API REST Prezzi di livello ora supporta le quantità decimali. In precedenza, questa API non consentiva quantità decimali lineare quando l&#39;impostazione **[!UICONTROL Use Decimal Quantity]** era abilitata.

<!-- AC-8998 -->

* L&#39;endpoint REST `V1/orders/{id}/comments` non modifica più lo stato di un ordine quando si tenta di aggiungere un commento allo stato di un ordine in sospeso. Ora restituisce un errore informativo. In precedenza, questo endpoint modificava lo stato dell&#39;ordine quando si aggiungeva un commento.
...
* L&#39;endpoint `POST /V1/products/tier-prices` ora restituisce un messaggio di errore più informativo quando viene utilizzato un nome di gruppo di clienti non valido durante la creazione del prezzo di livello. In precedenza, Magento Open Source restituiva un errore come: `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`. [GitHub-36431](https://github.com/magento/magento2/issues/36431)

<!-- ACP2E-1776 -->

* Ora puoi eseguire una richiesta API in blocco asincrona autorizzata per creare clienti con un ID gruppo non predefinito. In precedenza, queste richieste reimpostavano l&#39;ID gruppo al valore predefinito.
...
* La `POST V1/guest-carts` chiamata non invia più ordini ospite quando **[!UICONTROL Allow Guest Checkout]** è disabilitata.

<!-- ACP2E-1526 -->

* Magento Open Source ora restituisce correttamente l&#39;attributo `content` (codice immagine base64) nelle `GET V1/products/sku/media` risposte API media del prodotto. L&#39;attributo `content` non era apparso in precedenza nella risposta.

## requisiti di sistema

Il nostro stack tecnologico è basato su PHP e MySQL. Per ulteriori informazioni, consulta [Requisiti di sistema](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html?lang=it).

## Istruzioni di installazione e aggiornamento

È possibile installare Magento Open Source 2.4.7 utilizzando [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html?lang=it).
