...
* L&#39;applicazione ora aggiorna correttamente gli attributi etichettati `Product Type`. In precedenza, poiché l&#39;attributo `product_type` era riservato nel codice, il applicazione visualizzato questo messaggio quando si tentava di aggiornare un `Product Type` attributo: `An attribute with the same code (product_type) already exists.`

<!-- MC-36214-->

* Quando salvi il prodotto, il applicazione non modifica più la posizione di un video caricato nella sezione Immagini e video della pagina dei dettagli del prodotto. In precedenza, alla nuova posizione dell&#39;immagine veniva assegnato un valore di indice (posizione nell&#39;elenco) e se le posizioni delle immagini esistenti non erano allineate con i valori degli indici, la nuova immagine veniva smarrita nell&#39;elenco.

<!-- MC-36548-->

* La **funzionalità Aggiungi al carrello** ora funziona come previsto ogni volta che è possibile fare clic sull&#39;pulsante **Aggiungi al carrello** . In precedenza, se si faceva clic su questo pulsante più volte in attesa del caricamento di una pagina di prodotto, il applicazione generava questo errore: `Invalid Form Key. Please refresh the page`.
...
* Il applicazione ora visualizza la valuta corretta nelle distribuzioni multisito durante la creazione del prezzo di livello.
...
* I commercianti possono ora salvare correttamente un nuovo attributo durante la creazione del prodotto. In precedenza, il applicazione non salvava l&#39;attributo e reindirizzava il commerciante alla home page della visualizzazione store quando tentava di salvare l&#39;attributo. [GitHub-30362](https://github.com/magento/magento2/issues/30362)
...
* Risoluzione dei problemi relativi agli aggiornamenti del compositore derivanti dalla dipendenza del modulo dotdigital da `magento/module-authorization`.

### Scaricabile

<!-- ENGCOM-7679 -->

* Il applicazione non elenca più un prodotto scaricabile in I miei prodotti scaricabili scheda dopo che l&#39;ordine a cui appartiene è stato parzialmente rimborsato. [GitHub-28388](https://github.com/magento/magento2/issues/28388)
...
* Le e-mail inviate ai clienti che contengono fatture parziali ora includono subtotali degli articoli accurati. In precedenza, il subtotale in questa e-mail era il totale della quantità ordinata, non l’importo totale della quantità fatturata.

<!-- MC-38337-->

* Il applicazione ora invia promemoria via e-mail a tutti i clienti interessati con indirizzi e-mail validi. In precedenza, l’applicazione cessava di inviare e-mail di promemoria ai clienti dopo aver incontrato un indirizzo non valido.
...
* Le prestazioni dei ricerca di catalogo sono migliorate. La **disabilitazione dell&#39;opzione Abilita suggerimenti** Search (**archivi** > **configurazione** > **catalogo** > **catalogo Search** ) funziona come previsto. In precedenza, l&#39;applicazione interrogava la tabella MySQL `search_query` anziché Elasticsearch per i suggerimenti di completamento automatico ricerca. [GitHub-25534](https://github.com/magento/magento2/issues/25534)
...
* Il collegare di tracciamento della spedizione nell&#39;e-mail di conferma della spedizione inviata ai clienti ora funziona come previsto. In precedenza, questo collegare restituito un errore 404.

<!-- ENGCOM-8025 -->

* Il applicazione ora visualizza un messaggio di errore informativo quando un acquirente esegue il check-out utilizzando più indirizzi, quindi rimuove tutti i prodotti diversi da un prodotto virtuale. [GitHub-25595](https://github.com/magento/magento2/issues/25595)
...
* Il `Vault` modulo ora riconosce i codici dei metodi di pagamento dal richiesta per il servizio di gestione delle informazioni di pagamento. In precedenza, quando un acquirente effettuava un ordine utilizzando un scheda di credito Braintree salvato, il applicazione generava questo errore lineare quando veniva utilizzato un metodo di pagamento valido: `The requested Payment Method is not available`.
...
* L&#39;imposta calcolata da Vertex viene ora considerata quando gratuito spedizione è configurata per un importo comprensivo di imposte.
