...
>
>Dopo questa data, Adobe Commerce 2.4.7 non fornirà compatibilità o>supporto per qualsiasi versione di MySQL rilasciata dopo MySQL 8.0. Adobe non>convalida o fornisce supporto per le versioni principali di MySQL più recenti in questo Adobe>Riga di rilascio Commerce.
>
>Tutti i clienti Adobe Commerce on-premise che eseguono le versioni 2.4.7 di sono>è stato consigliato di migrare i server di database a una versione compatibile di MariaDB.
...
#### Compatibilità con MariaDB 11.8

Adobe Commerce 2.4.7-p10 introduce la compatibilità con MariaDB versione 11.8. MariaDB 11.8 introduce modifiche nel comportamento SQL, nei valori predefiniti, nelle deprecazioni e nelle ottimizzazioni delle prestazioni. Questo aggiornamento affronta in modo proattivo potenziali problemi per mantenere la stabilità della piattaforma e la sua preparazione futura.

#### Supporto della versione secondaria più recente di OpenSearch 3

Adobe Commerce 2.4.7 ora supporta la versione minore più recente di OpenSearch 3 su Adobe Commerce per le distribuzioni on-premise, cloud native e infrastruttura cloud. La compatibilità con OpenSearch 2 viene mantenuta.

#### Supporto Valkey 8.1 LTS

Adobe Commerce 2.4.7 è ora compatibile con Valkey 8.1 LTS, fornendo un’opzione di back-end per la cache di supporto a lungo termine supportata su Adobe Commerce nell’infrastruttura cloud.

#### Supporto di RabbitMQ 4.2

Adobe Commerce 2.4.7 è ora compatibile con RabbitMQ 4.2, che affronta la data di fine del supporto di RabbitMQ 4.1 pianificata per febbraio 2026. La compatibilità con Apache ActiveMQ Artemis viene mantenuta e ActiveMQ rimane il servizio di coda messaggi predefinito per questa riga di rilascio di sola protezione.
...
Per risolvere il problema del ritiro di Laminas MVC, Adobe Commerce ora utilizza un fork di proprietà di Magento di `laminas-mvc` (pubblicato come `magento/magento-zf-mvc`). Questo fork garantisce l&#39;applicazione continua di patch e la conformità in materia di sicurezza a lungo termine per Adobe Commerce 2.4.7.
...
#### Aggiungi compatibilità con la versione più recente del Compositore

Adobe Commerce 2.4.7 è stato aggiornato per supportare Composer 2.9.x, pur rimanendo compatibile con Composer 2.2 LTS.
