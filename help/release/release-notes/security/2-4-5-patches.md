...
>
>Dopo questa data, Adobe Commerce 2.4.5 non fornirà compatibilità o >supporto per le versioni MySQL rilasciate dopo MySQL 8.0. Adobe non>convalida o fornirà supporto per le versioni principali di MySQL più recenti in questa versione di Adobe>Commerce.
>
>A tutti i clienti Adobe Commerce on-premise che eseguono le versioni 2.4.5 viene consigliato di >migrare i propri server di database a una versione MariaDB compatibile.
...
Adobe Commerce 2.4.5 è ora compatibile con Valkey 8.1 LTS, fornendo un’opzione di back-end per la cache di supporto a lungo termine supportata su Adobe Commerce nell’infrastruttura cloud.

#### Supporto di RabbitMQ 4.2

Adobe Commerce 2.4.5 è ora compatibile con RabbitMQ 4.2, che affronta la data di fine del supporto di RabbitMQ 4.1 pianificata per febbraio 2026. La compatibilità con Apache ActiveMQ Artemis viene mantenuta e ActiveMQ rimane il servizio di coda messaggi predefinito per questa riga di rilascio di sola protezione.

## 2.4.5-p16

Adobe Commerce 2.4.5-p16 è una [versione di sicurezza del supporto esteso](../../lifecycle-policy.md#security-only-transitional-period) che fornisce correzioni di bug di sicurezza per le vulnerabilità identificate nelle versioni precedenti della versione 2.4.5.
...
#### Compatibilità con Apache ActiveMQ Artemis

A causa del rischio di fine del supporto associato a RabbitMQ 4, Adobe Commerce ha avviato una migrazione strategica ad Apache ActiveMQ Artemis per garantire stabilità e supporto a lungo termine per l’infrastruttura della coda di messaggi.

* Adobe Commerce 2.4.5 ora supporta la versione più recente di Apache ActiveMQ Artemis.
* La compatibilità con RabbitMQ 4.1 viene mantenuta per supportare i clienti che preferiscono continuare a utilizzare il servizio MQ esistente.
* ActiveMQ è ora completamente supportato su Adobe Commerce Cloud, incluso AWS ActiveMQ per le distribuzioni native di Cloud.

#### 2.4.5-p16 compatibilità con MariaDB 10.11

Adobe Commerce 2.4.5-p16 è stato verificato per la compatibilità con MariaDB 10.11, pur continuando a supportare completamente MariaDB 10.6. I commercianti attualmente in esecuzione 2.4.5-px possono tranquillamente effettuare l&#39;aggiornamento a 2.4.5-p16 e passare da MariaDB 10.6 a MariaDB 10.11.

## 2.4.5-p15

Adobe Commerce 2.4.5-p15 è una [versione di sicurezza del supporto esteso](../../lifecycle-policy.md#security-only-transitional-period) che fornisce correzioni di bug di sicurezza per le vulnerabilità identificate nelle versioni precedenti della versione 2.4.5.
...
Per informazioni aggiornate sulle correzioni dei bug di sicurezza, consulta [Bollettino sulla sicurezza di Adobe](https://helpx.adobe.com/security/products/magento/apsb23-35.html).
