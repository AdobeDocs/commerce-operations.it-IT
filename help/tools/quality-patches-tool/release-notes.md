...
* **ACSD-56790** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7): è stato risolto il problema relativo al reindirizzamento di un utente alla Admin Dashboard durante l’ordinamento dei prodotti di categoria utilizzando **Sposta il grezzo in basso** e `Invalid security or form key. Please refresh the page` nella parte superiore dello schermo.
* **ACSD-56280** (per Adobe Commerce >=2.4.4 &lt;2.4.7): è stato risolto il problema che si verificava se l’ordine di articoli da un registro di articoli regalo generava un’eccezione.
...
* **ACSD-56193** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4) - Corregge il problema per cui la cache di Vernice/Fastly non viene aggiornata quando si utilizza un blocco pianificato nella descrizione della categoria utilizzando Page Builder.
...
* **ACSD-56023** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7): è stato risolto il problema per cui il contenuto dei widget non viene aggiornato nella pagina CMS quando la cache è abilitata.
* **ACSD-55427** (per Adobe Commerce >=2.4.5 &lt;2.4.7): risolve il problema che impediva all’utente amministratore di annullare l’assegnazione di un prodotto da un catalogo condiviso dalla pagina del prodotto in Admin.
* **ACSD-55352** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui dopo aver creato una nota di credito parziale con i punti premio del cliente, lo stato dell’ordine cambia in Chiuso e le opzioni della nota di credito scompaiono dalla pagina Ordine amministratore.
* **ACSD-55231** (per Adobe Commerce >=2.4.2 &lt;2.4.7): risolve il problema che impediva l’aggiunta di prodotti al carrello tramite la funzionalità di ordine rapido.
...
* **ACSD-53636** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.5) - Risolve il problema per cui il prezzo normale non viene visualizzato nelle pagine di elenco dei prodotti per i prodotti configurabili che hanno prodotti secondari con prezzi speciali.
* **ACSD-54885** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7): è stato risolto il problema dell’estrazione di più indirizzi quando l’utente amministratore utilizza *Accedi come cliente* funzionalità.
* **ACSD-55610** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Risolve il problema relativo a un ordine parzialmente annullato con un importo di sconto errato.
...
* **ACSD-54739** (per Adobe Commerce >=2.4.5 &lt;2.4.7): risolve il problema se la condizione dello stato dello stock del prodotto non viene applicata per le regole del prodotto correlate.
* **ACSD-53925** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7): è stato risolto il problema che impediva all’amministratore di salvare il blocco CMS con il carosello del prodotto quando `catalog_product_price` dimension-mode è impostato su *sito web*.
...
* **ACSD-55055** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Migliora le prestazioni di caricamento degli attributi del prodotto nelle regole del prezzo del carrello nel carrello.
* **ACSD-53790** (per Adobe Commerce >=2.4.6 &lt;2.4.7): risolve il problema della creazione di più RMA per un singolo prodotto tramite API REST.
* **ACSD-56090** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.5): risolve il problema se la richiesta GraphQL risponde con i dati di tutti gli archivi anziché con i dati di archivio richiesti in modo specifico.
* **ACSD-54983** (per Adobe Commerce >=2.4.2 &lt;2.4.7) - Corregge il problema per cui non è possibile ottenere l’UID utente della società con la richiesta GraphQL quando lo stato utente è impostato su *[!UICONTROL Inactive]*.
* **ACSD-53309** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7): risolve il problema della mancata applicazione completa dell’imposta nel *[!UICONTROL Regular Price]* quando è selezionata l’opzione personalizzabile.
* **ACSD-55305** (per Adobe Commerce >=2.4.4 &lt;2.4.7): risolve il problema se il *[!UICONTROL Edit Company User]* popup sul **[!UICONTROL myAccount]** > **[!UICONTROL Company Structure]** La pagina si blocca con un caricatore sullo schermo.
* Patch aggiornate: ACSD-49013

## v1.1.42 {#v1-1-42}

* **ACSD-53658** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema se *[!UICONTROL Recently Viewed]* i dati di prodotto non vengono aggiornati correttamente nella vista store.
* **ACSD-54626** (per Adobe Commerce >=2.4.6 &lt;2.4.7): risolve il problema che impediva la creazione di una nuova regola dell’ordine d’acquisto (`createPurchaseOrderApprovalRule`) con `NUMBER_OF_SKUS` attributo tramite [!DNL GraphQL].
* **ACSD-53845** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge i [!DNL MySQL] problema di timeout della connessione quando `consumer max_messages` = 0.
* **ACSD-54890** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema se `aggregate_sales_report_bestsellers_data` cause [!DNL MySQL] errori dovuti a `/tmp` spazio su disco insufficiente.
* **ACSD-55112** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema se *[!UICONTROL Submit review]* può essere cliccato più volte senza [!DNL Google reCAPTCHA v3] convalida.
* **ACSD-54264** (per Adobe Commerce >=2,4,4-p5 &lt;2,4,5 || >=2,4,5-p4 &lt;2,4,6 || >=2.4.6-p2 &lt;2.4.7) - Corregge il problema se il messaggio di errore *&quot;Impossibile aggiornare l’attributo richiesto. ID riga: store_id&quot;* viene visualizzato quando un cliente tenta di effettuare il Check-Out con un preventivo negoziabile da un&#39;altra visualizzazione del punto vendita.
* **ACSD-54418** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7): risolve il problema relativo all’applicazione errata di un importo fisso di sconto a ciascun prodotto secondario del bundle a prezzo dinamico.
...
* **ACSD-54966** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema per cui un codice coupon con utilizzo limitato per cliente non può essere riutilizzato se l’ordine precedente non è riuscito.
...
* **ACSD-55381** (per Adobe Commerce >=2.4.2 &lt;2.4.7) - Corregge un errore interno del server durante l’esecuzione della query `configurable_product_option_uid` e `configurable_product_option_value_uid` campi da un [!DNL B2B] *[!UICONTROL Requisition list]* tramite [!DNL GraphQL].
...
* **ACSD-54376** (per Adobe Commerce >=2.4.2 &lt;2.4.7): risolve il problema che si verifica nel carrello quando un prodotto viene rimosso dal catalogo condiviso dopo che è già stato aggiunto al carrello.
* **ACSD-53722** (per Adobe Commerce >=2.4.4 &lt;2.4.7): è stato risolto il problema che causava la variazione di prezzo delle opzioni di prodotto in bundle a $ 0 quando diventavano attivi aggiornamenti pianificati per ambiti diversi.
* **ACSD-53643** (per Adobe Commerce >=2.4.3 &lt;2.4.7): risolve il problema se l’ordine presenta un totale errato quando si effettua un ordine di acquisto con prodotti disabilitati o esauriti. Viene risolto nascondendo il *[!UICONTROL Place Order]* per tali ordini fornitore.
* **ACSD-54067** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7): risolve il problema se un video di prodotto non viene riprodotto su un dispositivo mobile.
...
* **ACSD-53118** (per Adobe Commerce >=2.4.0 &lt;2.4.7): risolve il problema se il *[!UICONTROL Cart Price Rule]* viene applicato utilizzando il codice coupon quando il prodotto ha un attributo vuoto.
* **ACSD-54324** (per Adobe Commerce >=2.4.5 &lt;2.4.7) - Corregge il problema per cui la richiesta GraphQL richieste_list non considera le impostazioni di paginazione e restituisce tutti i risultati.
...
* **ACSD-54040** (per Adobe Commerce >=2,4,4-p5 &lt;2,4,5 || >=2.4.5-p4 &lt;2.4.6) - Corregge il problema se il *[!UICONTROL Created]* è vuoto nei dettagli dell’ordine quando i moduli B2B sono abilitati.
* **ACSD-54319** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.6): risolve il problema se il prezzo del prodotto mostra zero nel campo *[!UICONTROL Product in Cart]* rapporto.
...
* **ACSD-53414** (per Adobe Commerce >=2.4.6 &lt;2.4.7): risolve il problema per cui un utente amministratore con restrizioni può visualizzare le pagine CMS al di fuori del proprio ambito di autorizzazioni.
* **ACSD-54472** (per Adobe Commerce >=2.4.6 &lt;2.4.7): risolve il problema per cui i clienti di un’azienda rifiutata possono ancora autenticarsi e i clienti di un’azienda bloccata e rifiutata possono ancora effettuare ordini. La patch aggiunge una convalida aggiuntiva per gli endpoint di GraphQL.
...
* **ACSD-54989** (per Adobe Commerce >=2,4,4-p5 &lt;2,4,5 || >=2,4,5-p4 &lt;2,4,6 || >=2.4.6-p2 &lt;2.4.7) - Risolve il problema che impediva all’amministratore dell’azienda di effettuare un ordine quando *[!UICONTROL Enable Purchase Orders]* è impostato su *[!UICONTROL Yes]* e *[!UICONTROL Purchase Order]* è impostato su *[!UICONTROL No]*.
* **ACSD-54007** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge l’errore *&quot;Chiave di matrice non definita &quot;_scope&quot;&quot;* all’importazione di dati dei clienti.
* **ACSD-55031** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.6) - Corregge i *Il tipo &quot;mixed&quot; non può ammettere valori Null* errore durante la compilazione.
* **ACSD-54961** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema per cui un utente amministratore con restrizioni non può aggiornare in massa il *Revisione prodotto* stato.
...
* **ACSD-53704** (per Adobe Commerce >=2.4.0 &lt;2.4.7): risolve il problema relativo al calcolo errato della cronologia del saldo dei punti di premio dopo la scadenza dei punti di premio.
* **ACSD-53583** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Migliora le prestazioni di reindicizzazione parziale per *Categoria Prodotti* e *Categorie di prodotti* indici.
* **ACSD-54026** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Corregge un messaggio di errore errato per un’ `updateCompanyRole` Richiesta GraphQL per un utente non autorizzato.
...
* **ACSD-52219** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7): è stato risolto il problema che causava il mancato funzionamento dei filtri salvati dalle griglie dell’amministratore quando si passava spesso da una visualizzazione all’altra dei segnalibri.
* **ACSD-54342** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge un messaggio di errore errato *Errore nella struttura dati: valori misti* quando si importa un file CSV senza dati validi.
* **ACSD-54660** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6) - Aggiunto un nuovo attributo di input *sort* per ordinare gli ordini dei clienti in GraphQL per `sort_field` e `sort_direction`.
* **ACSD-54776** (per Adobe Commerce >=2.4.5 &lt;2.4.7) - Corregge il problema se non è selezionata *[!UICONTROL Use Default Value]* e i valori dei campi prodotto non predefiniti non vengono salvati per la seconda visualizzazione sito Web, store e store.
* **ACSD-53998** (per Adobe Commerce e Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - Corregge il problema se un **[!UICONTROL Dynamic Block]** in base a **[!UICONTROL Customer Segment]** non funziona correttamente dopo la disconnessione da un account cliente.
* **ACSD-53204** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Correzioni *Impossibile salvare il prodotto.* errore durante l’esecuzione di richieste simultanee di aggiunta di immagini alla galleria di prodotti utilizzando `rest/V1/products/<sku>/media` endpoint.
* **ACSD-47657** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) È stato aggiunto un meccanismo di caching per le credenziali di AWS. Un provider di credenziali ora utilizza la cache del Magento per memorizzare nella cache le credenziali recuperate da AWS per la configurazione EC2.
...
* **ACSD-52613** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7): risolve il problema dell’aggiornamento delle cache e degli indici anche quando non vengono effettuati aggiornamenti a `Inventory_source` elementi per API REST.
* **ACSD-51884** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7): è stato risolto il problema che causava l’errata correzione del percorso della cache delle immagini del prodotto dopo l’esecuzione del comando di ridimensionamento.
* **ACSD-53628** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Corregge il problema relativo alla visualizzazione di caratteri speciali non corretti nel rapporto CSV ordini cliente.
* **ACSD-53148** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7): è stato corretto il problema per cui due richieste parallele in GraphQL per l’aggiunta dello stesso prodotto configurabile al carrello generavano due elementi separati sul carrello con lo stesso SKU di prodotto.
* **ACSD-52606** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema se il messaggio di errore *Il tuo ordine non è pronto per il ritiro* viene visualizzato quando l’utente fa clic su **[!UICONTROL Notify Order is Ready for Pickup]**.
* **ACSD-51574** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7): risolve il problema se l’immagine non viene aggiornata sul front-end dopo essere stata sostituita con un’altra immagine con lo stesso nome.
* **ACSD-53728** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7): è stato risolto il problema che causava un tempo di completamento dell’indicizzatore EAV del prodotto.
* **ACSD-53979** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7): è stato risolto il problema JS che si verificava nella home page se il messaggio di benvenuto conteneva una virgoletta singola.
* **ACSD-52085** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7): risolve il problema se un prodotto configurabile con un prezzo speciale non è visibile nel carosello del prodotto.
* **ACSD-53795** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema relativo a un tipo di dati non valido in `indexer_update_all_views` lavoro cron.
...
* **ACSD-53750** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge i *Tubo rotto o connessione chiusa* errore durante multithreading `catalog_product_price` reindicizzare.
* **ACSD-49843** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - Corregge il problema per cui il collegamento sul download del prodotto non è disponibile dopo la fatturazione automatica dell’articolo ordinato tramite metodo di pagamento online con **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** impostazione abilitata.
* **ACSD-47054** (per Adobe Commerce >=2.4.2 &lt;2.4.6): risolve il problema relativo alla reindicizzazione dell’anteprima per tutti gli archivi, causando rallentamento.
