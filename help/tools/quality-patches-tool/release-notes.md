...
* **ACSD-51114** (per Adobe Commerce e Magenti Open Source >=2.4.3 &lt;2.4.7) - Corregge il problema per cui un prodotto casuale scompare dai cataloghi di grandi dimensioni quando l’indicizzazione asincrona è abilitata. Migliora le prestazioni della reindicizzazione asincrona per i cataloghi di grandi dimensioni.
* **B2B-2598** (per Adobe Commerce e Magenti Open Source >=2.4.4 &lt;2.4.7) - Aggiungi la funzionalità di memorizzazione in cache al [!UICONTROL availableStores], [!UICONTROL countries], [!UICONTROL country], [!UICONTROL currency]e [!UICONTROL storeConfig] Query GraphQL.
...
* **ACSD-49389** (per Adobe Commerce e Magenti Open Source >=2.4.0 &lt;2.4.7) - Risolve il problema per cui un messaggio e-mail pronto per essere ritirato viene inviato dall’API quando l’ordine non è pronto per il ritiro.
* **ACSD-49822** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Corregge il problema per cui gli aggiornamenti nel [!UICONTROL Requisition List] le pagine non vengono visualizzate [!UICONTROL Print Requisition List].
* **ACSD-48771** (per Adobe Commerce e Magenti Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema con l&#39;aggiornamento del tipo di contenuto del blocco di colonna dalla versione precedente [!DNL Page Builder] versioni.
* **ACSD-49464** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Corregge il problema per cui fatture, spedizioni e note di credito non vengono rimosse dall&#39;archivio quando l&#39;orderId è diverso.
* **ACSD-49773** (per Adobe Commerce e Magenti Open Source >=2.4.2 &lt;2.4.6) - Corregge il problema in cui l&#39;esportazione del prodotto non riesce quando AWS S3 viene utilizzato come archiviazione remota.
* **ACSD-49748** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Corregge il problema a causa del quale gli inviti non possono essere inviati.
* **ACSD-49502** (per Adobe Commerce >=2.4.3 &lt;2.4.7) - Corregge il problema per cui il collegamento scaricabile non viene aggiornato correttamente dopo che un aggiornamento di staging è stato applicato al prodotto scaricabile.
* **ACSD-49527** (per Adobe Commerce >=2.4.2 &lt;2.4.7) - Corregge il problema a causa del quale i ruoli aziendali di GraphQL non visualizzano correttamente l’impaginazione.
* **ACSD-49706** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema in cui il valore predefinito viene salvato per un attributo di campione visivo quando non è selezionato alcun valore.
* **ACSD-49835** (per Adobe Commerce e Magenti Open Source >=2.4.5 &lt;2.4.7) - Risolve il problema per cui il valore della casella di controllo Usa predefinito non viene salvato correttamente a livello di archivio per un attributo con più selezioni.
* **ACSD-49898** (per Adobe Commerce e Magenti Open Source >=2.4.4 &lt;2.4.6) - Corregge il problema per cui la griglia dei prodotti genera un&#39;eccezione quando un prodotto bundle ha un prezzo speciale superiore a 1000.
* **ACSD-50234** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.5) - Corregge il problema con il nome del cliente sbagliato nell&#39;e-mail di conferma se effettui un ordine con [!DNL PayPal].
* **ACSD-49960** (per Adobe Commerce e Magenti Open Source >=2.4.5 &lt;2.4.7) - Risolve il problema per cui il filtro per data non funziona per la griglia degli ordini dei clienti.
* **ACSD-49849** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.6) - Corregge il problema per cui l’e-mail del cliente è stata sostituita con [!DNL PayPal] e-mail quando effettui un ordine con [!DNL PayPal Express] via GraphQL.
* **ACSD-49839** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Risolve il problema in cui la determinazione prezzi e la struttura del catalogo condiviso genera un errore in Amministratore quando i prodotti hanno virgolette singole o doppie in SKU.
* **ACSD-49970** (per Adobe Commerce e Magenti Open Source >=2.4.5 &lt;2.4.7) - Corregge la gestione errata degli errori GraphQL quando [!DNL New Relic] il reporting è attivato.
* **ACSD-50260** (per Adobe Commerce e Magenti Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema per cui i risultati della ricerca dei prodotti GraphQL sono limitati solo a 10.000 risultati.
* **ACSD-48813** (per Adobe Commerce e Magenti Open Source >=2.4.3 &lt;2.4.7) - Risolve il problema in cui la ricerca non mostra risultati rilevanti in base al peso della ricerca degli attributi.

## v1.1.28 {#v1-1-28}

* **ACSD-48204** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.3) - Corregge il problema per cui una regola del prezzo di catalogo creata in base all&#39;attributo Sì/No non considera l&#39;ambito selezionato.
* **ACSD-47704** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.7) - Corregge il problema per cui il prodotto in bundle mostra solo il prezzo di in prodotti di magazzino.
* **ACSD-49370** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema in cui la *Data e ora* l&#39;attributo di prodotto ha *FilterMatchTypeInput* digitare nello schema GraphQL.
* **ACSD-48807** (per Adobe Commerce e Magenti Open Source >=2.4.1 &lt;2.4.7) - Risolve il problema per cui le recensioni dei prodotti dei clienti non vengono filtrate tramite vetrina tramite GraphQL.
* **ACSD-49433** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.7) - Corregge il problema in cui l&#39;importo predefinito viene visualizzato come totale parziale nel carrello della carta regalo con un importo aperto.
* **ACSD-48866** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema in cui si verifica un errore durante la richiesta di feed RSS per le categorie.
* **ACSD-48784** (per Adobe Commerce e Magenti Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema per cui i prezzi dei segmenti del cliente non sono correttamente memorizzati nella cache tra i gruppi di clienti.
* **ACSD-48857** (per Adobe Commerce e Magenti Open Source >=2.4.3 &lt;2.4.7) - Risolve il problema che impediva all’utente di salvare le modifiche dopo la modifica con Page Builder.
* **ACSD-49065** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Corregge il problema a causa del quale gli articoli del preventivo non sono visibili nell’amministratore se assegnati solo alle azioni personalizzate.
* **ACSD-49179** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui il Rapporto Ordini mostra importi non corretti in caso di valute diverse per diversi store.
* **ACSD-49286** (per Adobe Commerce e Magenti Open Source >=2.4.3 &lt;2.4.7) - Corregge il problema per cui un prodotto viene aggiunto due volte a un carrello quando sulla pagina sono presenti più widget di prodotto.
* **ACSD-49574** (per Adobe Commerce >=2.4.4 &lt;2.4.7) - Aggiunge funzionalità per supportare gli aggiornamenti dei prodotti Gift Card in un carrello tramite GraphQL.
* Patch aggiornata: ACSD-48694.
