...
* **ACP2E-4698** (per Adobe Commerce e Magento Open Source >=2.4.7-p9 &lt;2.4.8 || >=2.4.8-p4 &lt;2.4.9) - Corregge il problema per cui modificando di nuovo un’immagine nel contenuto di testo di Page Builder è possibile salvare un URL multimediale assoluto invece di mantenere una direttiva sui supporti portatili.
* **ACP2E-4748** (per Adobe Commerce >=2.4.7 &lt;2.4.9) - Corregge il problema della scadenza dei punti premio che può essere elaborata lentamente nei negozi con una cronologia dei punti premio di grandi dimensioni, causando ritardi nella scadenza dei punti premio.
* **ACP2E-4797** (per Adobe Commerce >=2.4.8 &lt;2.4.9) - Corregge il problema se l&#39;immissione di caratteri Unicode a 4 byte nell&#39;editor di WYSIWYG o nel contenuto di Page Builder nell&#39;amministratore non viene bloccata correttamente anche quando il database è configurato per supportare utf8mb4.
* **ACP2E-4799** (per Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Corregge il problema per cui la query `requisition_lists` GraphQL restituisce un valore &#39;total_count&#39; che riflette solo il numero di elementi nella pagina corrente anziché il numero totale di elenchi di richieste che corrispondono ai criteri della query.
* **ACP2E-4805** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui le richieste API di pagamento possono diventare notevolmente più lente per i prodotti configurabili con molti prodotti secondari quando il primo prodotto secondario vendibile viene visualizzato in una posizione tardiva nell&#39;elenco.
* **ACP2E-4840** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema se il valore della quantità richiesto nella query GraphQL &quot;products&quot; restituisce null.
* **ACP2E-4870** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.10) - Corregge il problema per cui le notifiche e-mail degli avvisi sul prodotto ignorano le impostazioni e-mail della visualizzazione archivio.
* **ACP2E-4875** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.10) - Corregge il problema per cui la visualizzazione di account cliente con rubriche di grandi dimensioni nell&#39;amministratore può causare la disconnessione imprevista degli utenti amministratori.
* **ACP2E-4894** (per Adobe Commerce >=2.4.7 &lt;2.4.9) - Corregge il problema relativo al ritardo nella visualizzazione dei nuovi ordini nelle griglie di gestione degli ordini amministratore quando l&#39;indicizzazione asincrona è abilitata negli archivi di grandi volumi.
* **ACP2E-4981** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Risolve il problema per cui i caroselli di prodotti Page Builder visualizzano i prodotti in un ordine di posizione categoria errato e includono prodotti configurabili quando i prodotti secondari corrispondenti sono individualmente visibili.
* Versioni aggiornate: **ACSD-45255**, **ACSD-50849**
* Sostituzione delle patch: **ACSD-49970-V3**, **ACSD-50260-V2**, **ACSD-54966**, **ACSD-63325**, **ACSD-64212**, **ACSD-66865**, **ACSD-69237**
* Patch obsolete: **ACP2E-4815**

## v1.1.81 {#v1-1-81}

* **ACP2E-4300** (per Adobe Commerce >=2.4.5 &lt;2.4.9) - Corregge il problema se la modifica di un gruppo di clienti nell&#39;amministratore non aggiorna le autorizzazioni del catalogo nella vetrina, causando la visualizzazione di autorizzazioni obsolete nel catalogo e nel carrello.
...
* **ACP2E-4533** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema per cui le immagini segnaposto non vengono caricate sulla vetrina quando nell&#39;URL è incluso un codice store.
...
* **ACP2E-4626** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema relativo alla richiesta e all&#39;esecuzione di alcuni file JavaScript della vetrina due volte, con conseguenti caricamenti duplicati intermittenti e comportamento instabile.
...
* **AC-10737** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7-p5) - Corregge un problema per cui il comando bin/magento setup:db:status non riconosce il tipo di dati JSON.
* **ACP2E-4402** (per Adobe Commerce >=2.4.5 &lt;2.4.6 || >=2.4.8 &lt;2.4.9) - Corregge il problema per cui i prodotti creati come disabilitati non venivano aggiunti nuovamente ai risultati delle regole di destinazione correlate dopo l’abilitazione.
* **ACP2E-4505** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema relativo alla possibilità di salvare una categoria con dati non aggiornati da una scheda del browser duplicata, creando una dipendenza circolare.
* **ACP2E-4531** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema per cui la modifica della chiave URL di una pagina CMS non comporta l&#39;aggiornamento dell&#39;URL gerarchico della pagina.
* **ACP2E-4601** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema relativo al comportamento inefficiente dell&#39;elaborazione delle transazioni di pagamento in determinate condizioni.
* **ACP2E-4603** (per Adobe Commerce >=2.4.6 &lt;2.4.8): è stato risolto il problema che causava la mancata modifica delle righe dell&#39;indice delle autorizzazioni esistenti durante l&#39;esecuzione della reindicizzazione del prodotto Autorizzazioni catalogo, impedendo che le autorizzazioni per le categorie aggiornate si riflettessero in modo affidabile sui prodotti.
* **ACP2E-4706** (per Adobe Commerce >=2.4.5 &lt;2.4.9) - Corregge il problema per cui i prodotti non abilitati nell&#39;ambito Admin sono stati ignorati dall&#39;indicizzatore Regola di destinazione.
* **ACP2E-4720** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema per cui la spedizione gratuita non veniva applicata correttamente o rimossa per i prodotti bundle con regole di sconto sul carrello.
* **ACP2E-4411** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Risolve il problema relativo alla visualizzazione di un prezzo errato per un prodotto bundle nella pagina del carrello e nel mini-carrello per i negozi a più valute.
* **ACP2E-4475** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Risolve il problema per cui la pagina di elenco dei prodotti filtra e ordina erroneamente i prodotti del bundle esaurito in base al prezzo quando l&#39;opzione &quot;Visualizza prodotti esauriti&quot; è abilitata.
* **ACP2E-4110** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema per cui i prodotti bundle con un prezzo speciale mostravano importi non corretti su PDP e PLP in una valuta non predefinita.
* **AC-10698** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Risolve il problema relativo all&#39;invio della valuta a livello di tutti gli ordini anziché associarla a singoli ordini. I prezzi delle transazioni e i totali vengono ora inviati per ordine a Google Tag, migliorando la precisione del tracciamento dei dati di e-commerce.
* Versioni aggiornate: **ACSD-67383**, **ACSD-69203**
* Sostituzione delle patch: **ACSD-67370**, **ACSD-69319**

## v1.1.78 {#v1-1-78}

* **ACP2E-4416** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema per cui i punti premio cliente non vengono inizializzati quando vengono creati nell&#39;amministratore.
* **ACP2E-4419** (per Adobe Commerce >=2.4.6 &lt;2.4.8) - Corregge il problema relativo alla mancata applicazione corretta delle gift card al momento del pagamento dopo la convalida reCAPTCHA v2 (&#39;I am not a robot&#39;) nella vetrina.
* **ACP2E-4431** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema di eliminazione dei prodotti correlati corrispondenti alle regole di destinazione durante il processo di reindicizzazione.
* **ACP2E-4448** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui le modifiche alla configurazione apportate durante le interruzioni di Redis non vengono applicate dopo il ripristino di Redis, causando la persistenza di valori non aggiornati.
* **ACP2E-4452** (per Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Corregge il problema per cui i prezzi dei prodotti nella pagina Ordine rapido includono l&#39;imposta indipendentemente dalla configurazione della visualizzazione dell&#39;imposta.
* **ACP2E-4456** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge un problema per cui l&#39;annullamento di un ordine con una mutazione GraphQL non comporta la transizione di un ordine pagato interamente con gift card allo stato Chiuso.
* **ACP2E-4507** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui la configurazione Opzioni password non viene applicata per le richieste di reimpostazione della password del cliente effettuate tramite mutazioni GraphQL.
* **ACP2E-4513** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui le immagini CAPTCHA scadute non vengono eliminate dal sistema.
* **ACP2E-4522** (per Adobe Commerce >=2.4.7 &lt;2.4.9) - Corregge il problema relativo a un errore di chiave duplicata intermittente nella tabella quote_coupons quando vengono eseguite contemporaneamente più richieste di unione carrello o di salvataggio dei preventivi.
* **ACP2E-4528** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Risolve il problema della convalida della città negli indirizzi dei clienti, che ora consente di usare una barra (/) e rifiuta i caratteri non validi come !, &quot;, # e ?.
* **ACP2E-4535** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge un problema per cui l&#39;invio del modulo per password dimenticata determina la distruzione o la rigenerazione della sessione (modifiche PHPSESSID) e l&#39;eliminazione del carrello guest.
* **ACP2E-4540** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Risolve il problema relativo al mancato caricamento della libreria Fotorama, che causava la visualizzazione della prima immagine allegata.
* **ACP2E-4555** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema se i numeri di telefono moderni contengono &quot;.&quot; o &quot;/&quot; non sono convalidati correttamente.
* **ACP2E-4565** (per Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Risolve il problema per cui la query GraphQL della società restituisce &quot;Il cliente corrente non è autorizzato&quot; quando viene utilizzata l&#39;intestazione X-Adobe-Company.
* **ACP2E-4591** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Corregge il problema per cui i segmenti dei clienti basati sul conteggio degli ordini, ad esempio &quot;Acquirenti nuovi&quot;, non venivano aggiornati quando gli ordini venivano effettuati tramite l&#39;API REST.
* **ACP2E-4609** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.9) - Corregge il problema per cui la pagina Offerte personali non mostra virgolette quando alcune virgolette contengono prodotti eliminati.
* **ACP2E-4613** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.9) - Corregge il problema relativo alle strutture di directory multimediali di grandi dimensioni che causavano risposte lente, causando tempi di caricamento estesi della struttura di directory di Media Gallery.
* **ACP2E-4628** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui l&#39;importazione di clienti con indirizzi e-mail in maiuscolo genera l&#39;errore di chiave di array non definita quando la condivisione dell&#39;account è impostata su Globale.
* **ACP2E-4665** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui i prodotti secondari di prodotti configurabili contenenti video nelle raccolte prodotti non vengono elencati quando richiesto tramite API REST.
* **ACP2E-4732** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8): è stato risolto un problema che causava l&#39;interruzione dell&#39;indicizzazione parziale per i clienti con un numero elevato di aggiornamenti quando la colonna version_id nella tabella changelog raggiungeva il valore massimo.
* **ACP2E-4763** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema in cui la query GraphQL customerOrders restituisce i valori gonfiati original_price_included_tax e original_row_total_included_tax quando i prezzi del catalogo sono impostati su Includi imposta, a causa dell&#39;applicazione dell&#39;imposta due volte.
* **ACSD-60989** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema se la modifica di una colonna con una chiave esterna tramite uno schema dichiarativo causa errori in MariaDB.
* Versioni aggiornate: **ACSD-59280**, **ACSD-45255**, **ACSD-50336**, **ACSD-49737**, **ACSD-50849**, **ACSD-53750**, **ACSD-55031**, **ACSD-51819**, **ACSD-55628**, **ACSD-54965-V2**, **ACSD-56546**, **ACSD-61756**, **ACSD-68040**, **ACSD-62708**, **ACSD-63283**, **ACSD-64732**, **ACSD-65775**, **ACSD-66965**, **ACP2E-4050**
* Sostituzione delle patch: **ACSD-58446**, **ACSD-67904**

## v1.1.77 {#v1-1-77}

* **ACSD-63687** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema della visualizzazione di prezzi non corretti, perché non è possibile pulire la cache di Redis.
* **ACSD-68341** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Risolve il problema quando il cookie X-Magento-Vary viene impostato più volte durante il caricamento di PDP, quando vengono creati più segmenti di clienti nello store.
* **ACSD-68537** (per Adobe Commerce >=2.4.8 &lt;2.4.9) - Corregge il problema di riduzione delle prestazioni di estrazione con l&#39;aumento del numero di segmenti dei clienti.
* **ACSD-68664** (per Adobe Commerce >=2.4.6 &lt;2.4.9) - Risolve il problema relativo all&#39;interruzione dell&#39;anteprima dell&#39;aggiornamento pianificato durante il tentativo di visualizzare l&#39;anteprima del contenuto per gli archivi con domini personalizzati.
* **ACSD-68759** (per Adobe Commerce e Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.9) - Corregge il problema per cui la creazione di un account cliente non riesce quando si utilizza la lingua araba e l’attributo Data di nascita (DOB) è impostato per essere visualizzato nella vetrina.
* **ACSD-68892** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9): è stato risolto il problema che si verificava se le pagine memorizzabili in cache non venivano memorizzate o servite correttamente dalla cache Fastly, causando un comportamento di caching incoerente e prestazioni ridotte.
* **ACSD-69016** (per Adobe Commerce >=2.4.7 &lt;2.4.9) - Risolve il problema quando il prezzo speciale non ha effetto per i siti Web creati con fusi orari diversi.
* **ACSD-69020** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Risolve il problema relativo all&#39;inclusione automatica di un prodotto configurabile negli elenchi a carosello dei prodotti PageBuilder se uno dei suoi prodotti secondari soddisfa le condizioni di filtro.
* **ACSD-69237** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui il numero di voci che possono essere elaborate e inserite tramite i processi sales_*_async_insert cron è limitato a 100 per esecuzione.
* **ACSD-69311** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Risolve il problema relativo al calcolo dell&#39;imposta errato nelle note di accredito durante la creazione di un rimborso parziale da una fattura, se è stata creata una nota di accredito precedente dalla pagina di visualizzazione dell&#39;ordine.
* **ACSD-69351** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema che causa la visualizzazione dei saldi delle gift card e delle date di scadenza non conformi all&#39;ambito del sito Web assegnato.
* **ACSD-69494** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge i problemi relativi alle operazioni di rimborso asincrone in cui le richieste di rimborso con il parametro &quot;is_online&quot; non vengono elaborate correttamente.
* Versioni aggiornate: **ACSD-67250**
* Sostituzione delle patch: **ACSD-62629**, **ACSD-66157**
* Patch obsolete: **ACSD-66157**

## v1.1.76 {#v1-1-76}

* **ACSD-67091** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge l&#39;errore relativo alla dimensione massima del set di scrittura per garantire la pulizia dell&#39;indice del prodotto delle regole di catalogo implementando due strategie di eliminazione basate sul volume dei dati.
* **ACSD-67370** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge diversi problemi in cui venivano visualizzati prezzi errati per i prodotti Bundle su PDP/PLP e la pagina del carrello per gli store multi-valuta.
* **ACSD-68410** (per Adobe Commerce, B2B >=1.3.3 &lt;1.5.3) - Corregge un problema a causa del quale il posizionamento di un ordine per un preventivo negoziabile aggiunge o unisce erroneamente linee del carrello aggiuntive al preventivo. I prodotti vengono ora aggiunti correttamente al carrello dopo aver lasciato l’ultimo passaggio del pagamento delle offerte negoziabili.
* **ACSD-69086** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8): è stato risolto il problema che impediva al processo cron di cancellare le tabelle del registro delle modifiche, causando arresti anomali del cluster Galera durante la gestione di grandi quantità di dati.
* **ACSD-69115** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge un problema a causa del quale gli errori del carrello non venivano visualizzati all&#39;utente amministratore durante la gestione del carrello per un cliente assegnato a un sito Web non predefinito.
* **ACSD-69129** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - È stato risolto un problema che si verificava se l’eliminazione del sito web di base predefinito e l’utilizzo del sito web secondario come sito predefinito causavano un errore durante il tentativo di aggiornare il prezzo del sito web secondario tramite API REST.
* **ACSD-69203** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge un problema a causa del quale il widget Elenco prodotti restituiva risultati non corretti quando più categorie erano elencate nella condizione della categoria.
* **ACSD-69261** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge un problema a causa del quale un coupon della regola di prezzo del carrello configurato per l&#39;uso singolo per cliente è stato riutilizzato più volte a causa di una gestione errata dell&#39;attributo `times_used` in scenari di annullamento di quantità parziale e rimanente.
* **ACSD-69308** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge un problema per cui le regole del prezzo di catalogo non venivano applicate quando `special_price` era impostato solo a livello di sito Web (non in &quot;Tutte le visualizzazioni dello store&quot;). Dopo la correzione, le regole del prezzo del catalogo vengono applicate correttamente controllando prima il negozio predefinito del sito web.
* **ACSD-69319** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge un problema a causa del quale i prezzi dei bundle non venivano indicizzati correttamente quando i prodotti secondari disponevano di scorte in origini personalizzate.
* **ACSD-69325** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9): è stato risolto un problema che causava la mancata disponibilità del prodotto nella vetrina a causa della modifica del case SKU.
* **ACSD-69331** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.9) - È stato risolto un problema che impediva ai creatori di contenuto della raccolta multimediale di creare cartelle con la sola autorizzazione `create_folder`. Dopo la correzione, può creare le cartelle come previsto.
* **ACSD-69333** (per Adobe Commerce >=2.4.7 &lt;2.4.9) - Corregge un problema per cui le modifiche SKU erano consentite per i prodotti con un aggiornamento pianificato attivo. Dopo la correzione, le modifiche SKU non sono consentite durante gli aggiornamenti attivi; il salvataggio non riesce e viene visualizzato un errore di cancellazione e il campo SKU amministratore è disabilitato. In questo modo si evitano incoerenze nell’inventario MSI causate da modifiche SKU durante il rollback della gestione temporanea.
* **ACSD-69541** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge un problema per cui la riduzione della quantità di un prodotto nell&#39;amministratore a un valore inferiore a quello già esistente in un carrello rendeva impossibile modificare la quantità di prodotto nel carrello tramite GraphQL.
* Versioni aggiornate: **ACSD-46541**, **ACSD-53750**, **ACSD-66404**
* Sostituzione delle patch: **ACSD-66404**, **ACSD-68499**

## v1.1.75 {#v1-1-75}

* **ACSD-68289** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge un problema per cui la ricerca full-text ora restituisce prodotti corrispondenti se la condizione di corrispondenza minima viene soddisfatta collettivamente in tutti i campi ricercabili, anziché richiedere che la condizione sia soddisfatta da un singolo campo.
* **ACSD-68359** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - È stato risolto un problema che impediva la selezione di un archivio durante l&#39;estrazione con [!UICONTROL Pick in Store] a causa di URL lunghi quando molti prodotti erano nel carrello. Precedentemente, ciò causava un errore *414* causato da URL eccessivamente lunghi generati durante la selezione dell&#39;archivio, impedendo ai clienti di completare l&#39;estrazione.
* **ACSD-68451** (per Adobe Commerce, B2B >=1.5.2-p1 &lt;1.5.3): risolve un problema per più siti Web in cui un amministratore di società effettua l&#39;accesso a un sito Web, crea una società non correlata a un altro sito Web, ma è erroneamente collegato a tale società non correlata.
* **ACSD-68490** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Corregge il problema per cui il pulsante [!UICONTROL Add New Attribute] è visibile per un utente amministratore con restrizioni durante la creazione di un prodotto configurabile.
* **ACSD-68517** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge un errore di reinvio modulo nelle pagine di ricerca catalogo e catalogo.
* **ACSD-68573** (per Adobe Commerce >=2.4.5 &lt;2.4.9) - Corregge il problema per cui le autorizzazioni delle categorie non venivano applicate correttamente agli elementi della lista dei desideri del cliente. Dopo la correzione, le voci dell’elenco dei desideri vengono visualizzate e impaginate correttamente sia nel web che in GraphQL.
* **ACSD-68615** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui la CLI di compensazione delle prenotazioni di magazzino mostrava un&#39;eccezione se la combinazione elaborata aveva un ID ordine mancante.
* **ACSD-68793** (per Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Corregge un problema a causa del quale prodotti validi venivano erroneamente rifiutati durante l&#39;assegnazione a un catalogo condiviso.
* **ACSD-68925** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge un problema per cui le risposte per le richieste GraphQL ora sono allineate con le specifiche GraphQL su HTTP. Viene restituito un codice di risposta 4XX quando la richiesta non può essere analizzata, non è autorizzata o riscontra un problema generale. Se la richiesta viene analizzata e può essere elaborata, viene restituito un codice di risposta 200.
* Versioni aggiornate: **MDVA-19640**, **ACSD-47910**, **ACSD-68040**, **ACSD-62965**
* Sostituzione delle patch: **ACSD-62577**, **ACSD-68011**
...
* **ACSD-68499** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge un problema per cui la mutazione GraphQL `updateCartItems` restituisce una risposta di successo errata durante l&#39;aggiornamento delle quantità che superano le scorte disponibili, causando quantità e totali gonfiati.
* **ACSD-68810** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Risolve un problema per cui un ordine viene assegnato a un cliente creato su un sito Web diverso, nonostante la configurazione **[!UICONTROL Customer Account Sharing]**.
* Versioni aggiornate: **ACSD-49737**, **ACSD-57003-V2**
* Patch sostituite: **ACSD-61969**

## v1.1.73 {#v1-1-73}

* **ACSD-67171** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema per cui gli utenti B2B visualizzano una *[!UICONTROL Access Denied]* pagina quando la sessione è scaduta o è stata rimossa durante l&#39;estrazione.
* **ACSD-67908** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9): è stato risolto il problema che impediva la corretta unione dei file JS nelle impostazioni multi-store.
* **ACSD-68190** (per Adobe Commerce >=2.4.4 &lt;2.4.7) - Corregge il problema per cui gli sconti non vengono applicati, gli sconti applicati non vengono visualizzati correttamente nella risposta di visualizzazione del carrello GraphQL e gli sconti non coupon vengono rimossi quando si rimuove uno sconto coupon.
* **ACSD-68206** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge l&#39;errore quando si utilizza il server applicazioni GraphQL con la funzionalità **[!UICONTROL Rate Limiting]** con l&#39;estensione [!DNL PHP Redis] installata.
* **ACSD-68356** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema che causava la restituzione da parte di una query del carrello di GraphQL di un importo di sconto errato per i preventivi virtuali.
* **ACSD-68391** (per Adobe Commerce >=2.4.6-p10 &lt;2.4.9) - Corregge il problema di applicazione non corretta delle autorizzazioni relative alla categoria in **[!UICONTROL Quick Order]** e **[!UICONTROL Requisition Lists]**.
* **ACSD-68400** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui la quantità di gift card virtuale non veniva riportata in modo preciso nella tabella di prenotazione del magazzino.

## v1.1.72 {#v1-1-72}

* **ACSD-68040** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7): è stato risolto il problema relativo al peggioramento delle prestazioni della pagina di ricerca front-end in [!DNL MariaDB] 10.6 e 11.4 con numerose richieste di ricerca cronologica.
* **ACSD-67941** (per Adobe Commerce e Magento Open Source >=2.4.7-p1 &lt;2.4.8) - Corregge il problema per cui le richieste GraphQL con nomi di filtro sconosciuti causano i registri eccezioni PHP.
* **ACSD-68064** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8): è stato risolto il problema che causava la creazione di aggiornamenti pianificati con la creazione di voci duplicate in ambienti con un numero elevato di categorie nidificate.
* **ACSD-66807** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui la tabella `report_viewed_product_index` mostra un conteggio errato delle visualizzazioni della pagina di prodotto.
* **ACSD-67383** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema se l&#39;utilizzo di **[!UICONTROL Login as Customer]** con due account amministratore società nella stessa sessione causa un errore *Nessuna entità di questo tipo con cartId*.
* **ACSD-67518** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9): è stato risolto il problema che causava la generazione di righe di intestazione duplicate quando il conteggio delle righe superava le dimensioni del batch.
* **ACSD-67639** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9): è stato risolto il problema che impediva la creazione di una nota di credito per i prodotti bundle con **[!UICONTROL Dynamic Price]** impostato su *No*.
* **ACSD-67696** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui le `media_gallery` voci non vengono restituite nel nodo del prodotto Cart GraphQL dopo uno scaricamento della cache.
* **ACSD-67946** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema per cui gli aggiornamenti del carrello mostrano banner di errore duplicati.
* **ACSD-68011** (per Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Risolve il problema relativo all&#39;assegnazione di SKU inesistenti a un catalogo condiviso tramite l&#39;API `/V1/sharedCatalog/:id/assignProducts` [!DNL REST].
* **ACSD-68118** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.9) - Corregge il problema per cui la query `customerCart` GraphQL restituisce valori di attributi di prodotto che non riflettono l&#39;intestazione dell&#39;archivio, causando una localizzazione incoerente.
* **ACSD-68092** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema della perdita delle opzioni del prodotto bundle dopo più salvataggi, a causa di una sincronizzazione non corretta tra gli aggiornamenti pianificati e i dati del prodotto di base.
* **ACSD-67424** (per Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Corregge il problema per cui il valore `updated_at` nella risposta API `GET /carts/search` [!DNL REST] non corrisponde al valore mostrato in **[!UICONTROL Admin panel]** quando si utilizzano i preventivi negoziabili.
* **ACSD-67187** (per Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Corregge il problema per cui gli utenti amministratori limitati a siti Web non predefiniti visualizzano l&#39;errore *Crea almeno un catalogo condiviso pubblico per continuare* e non può accedere al pulsante **[!UICONTROL Add New Company]** nella griglia dell&#39;azienda.
* Versioni aggiornate: **ACSD-49737**, **ACSD-53750**, **ACSD-51819**, **ACSD-55566**, **ACSD-62965**, **ACSD-63323**, **ACSD-63406**, **ACSD-66139**, **ACSD-66404**, **ACSD-67659**, **ACSD-66301**
* Sostituzione delle patch: **ACSD-62577**, **ACSD-63325**, **ACSD-67102**
...
* **ACSD-67102** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema relativo al caricamento molto lento del back-end di Adobe Commerce **[!UICONTROL Categories]**.
...
* **ACSD-67686** (per Adobe Commerce e Magento Open Source >=2.4.4-p15 &lt;2.4.5 || >=2,4,5-p14 &lt;2,4,6 || >=2.4.6-p12 &lt;2.4.7) - Corregge il problema relativo a un `Syntax Error: Unexpected <EOF>` errore durante l&#39;invio di una richiesta [!DNL GraphQL] vuota.
* **ACSD-67250** (per Adobe Commerce >=2.4.7-p4 &lt;2.4.8) - Corregge il problema per cui l&#39;operazione di salvataggio **[!UICONTROL Shared Catalog]** aggiorna tutti gli elementi anziché solo quelli interessati, migliorando le prestazioni tramite l&#39;eliminazione delle operazioni non necessarie.
...
* **ACP2E-3731** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui le esportazioni di prodotti con **[!UICONTROL Catalog, Search]** visibilità includono erroneamente record di altre visualizzazioni di store in ambienti multi-store.
...
* **ACSD-66149** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema se il gestore IPN restituisce un errore *500* per tipi IPN non supportati o sconosciuti.
* **ACSD-66153** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema se una pagina restituisce un errore *500* a causa di una struttura di layout errata nella cache.
...
* **ACSD-67347** (per Adobe Commerce e Magento Open Source >=2.4.5-p11 &lt;2.4.6) - Corregge il problema se l&#39;ordine non riesce e *Non è possibile acquisire un errore di blocco* quando vengono utilizzati coupon con caratteri speciali e il blocco dei file è abilitato.
...
* **ACSD-67039** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui i record dei clienti non sono stati salvati a causa della convalida dell&#39;attributo di sistema `rp_token` e la convalida dei segni diacritici viene ora applicata solo all&#39;e-mail del cliente risultante.
* **ACSD-62146** (per Adobe Commerce >=2.4.7 &lt;2.4.8) - Corregge il problema relativo alla scomparsa dell&#39;indirizzo di fatturazione selezionato nella pagina Pagamento di pagamento di pagamento di pagamento di pagamento di pagamento quando la ricerca degli indirizzi è abilitata e &quot;Limite numero di indirizzi cliente&quot; è impostato su 1.
...
* **ACSD-65935** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Corregge il problema per cui la query `customerOrders` GraphQL ha restituito un errore interno del server quando un prodotto è stato eliminato.
...
* **ACSD-66118** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - Corregge il problema per cui l&#39;aggiornamento del codice della visualizzazione archivio cancella le impostazioni [!UICONTROL Design Configuration] se la cache di configurazione non viene aggiornata.
* **ACSD-66139** (per Adobe Commerce >=2.4.7 &lt;2.4.8) - Corregge il problema per cui le chiamate GraphQL per effettuare un ordine per un carrello inesistente o inattivo hanno restituito un codice di errore *NON DEFINITO*.
...
* **ACP2E-3789** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.9) - Risolve il problema relativo all&#39;aggiornamento di un prodotto tramite [!DNL WebAPI] file multimediali duplicati quando veniva fornito un ID multimediale.
...
* **ACSD-65750** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema per cui la query GraphQL `route` restituiva prodotti fuori servizio nei tipi di contenuto Prodotti Page Builder.
* **ACSD-65775** (per Adobe Commerce e Magento Open Source >=2.4.8 &lt;2.4.9) - Corregge il problema per cui i dettagli dell&#39;ordine API [!DNL REST] restituivano valori `base_row_total` e `row_total` errati quando venivano ordinate più quantità dello stesso elemento.
...
* **ACSD-65913** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.9) - Corregge il problema per cui [!DNL OpenSearch] ha generato un errore *legal_topic_exception* per le categorie con prodotti che hanno lo stesso prezzo.
* **ACSD-66041** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.9) - È stato risolto il problema che impediva la ricerca di località di prelievo nei codici postali di Irlanda (IE) a causa di un `CountryID` mancante.
...
* **ACSD-64813** (per Adobe Commerce >=2.4.4 &lt;2.4.9) - Corregge il problema per cui l&#39;annullamento dell&#39;assegnazione di categorie da un [!DNL B2B] catalogo condiviso tramite API [!DNL REST] richiede troppo tempo o si verifica un timeout con cataloghi di grandi dimensioni.
...
* **ACSD-65331** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema di cancellazione dell&#39;archivio selezionato in [!UICONTROL Pick in Store] dopo essere tornato ripetutamente alla pagina di pagamento.
...
* **ACSD-66093** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui gli indirizzi e-mail potevano essere immessi nei campi [!UICONTROL First Name] e [!UICONTROL Last Name] del cliente ospite, generando e-mail di conferma dell&#39;ordine non valide.
* Versioni aggiornate: **ACSD-51291**
* Patch sostituite: **ACSD-61522**

## v1.1.64 {#v1-1-64}

* **ACP2E-3838** (per Adobe Commerce e Magento Open Source >=2.4.4-p9 &lt;2.4.4-p13 || >=2,4,5-p8 &lt;2,4,5-p12 || >=2,4,6-p6 &lt;2,4,6-p10 || >=2.4.7 &lt;2.4.7-p5) - Corregge il problema se [!DNL Page Builder] errori CORS impediscono di salvare le modifiche nel pannello Admin in modalità di produzione.
...
* **ACSD-65127** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Risolve il problema per cui se si abilita la minimizzazione di JavaScript in modalità di produzione, [!DNL TinyMCE] 6 genera errori nella console del browser, influendo sulla funzionalità e sull&#39;esperienza utente.
...
* **ACSD-63406** (per Adobe Commerce e Magento Open Source >=2.4.4-p9 &lt;2.4.5 || >=2,4,5-p8 &lt;2,4,6 || >=2.4.6-p6 &lt;2.4.8) - Corregge il problema per cui le virgolette persistenti scadute non vengono cancellate da alcun processo cron durante l’esecuzione del processo `persistent_clear_expired` cron.
...
* **ACSD-65164** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.8) - Risolve il problema relativo al messaggio di errore *Alcune delle opzioni selezionate per l&#39;elemento non sono attualmente disponibili* che si verifica quando si riordina un prodotto configurabile con una singola opzione selezionata personalizzata per la casella di controllo.
...
* **ACP2E-3705** (per Adobe Commerce >=2.4.7 &lt;2.4.8) - Corregge un problema per cui l&#39;esecuzione di `indexer_update_all_views` cron non riesce quando `MAGE_INDEXER_THREADS_COUNT` è impostato.
* **ACSD-63883** (per Adobe Commerce >=2.4.4 &lt;2.4.7-p4) - Corregge il problema per cui l&#39;elenco richieste di acquisto restituisce un `items_count` errato nella risposta di GraphQL.
...
* **ACSD-64209** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Corregge il problema per cui il modulo di pianificazione cron recupera tutti i preventivi negoziabili senza escludere quelli con lo stato **[!UICONTROL ordered]**, causando l&#39;attivazione di un messaggio e-mail o e-mail.
* **ACSD-64431** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - La mutazione `placeOrder` che contiene le informazioni sul codice coupon nella richiesta non genera più un errore interno, ma mostra che l&#39;ordine è stato effettuato correttamente.
...
* **ACSD-64546** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema relativo alla presenza di un messaggio di errore generico nell&#39;interfaccia utente e di un&#39;eccezione *Array to string conversion* nei registri durante la creazione delle etichette di spedizione UPS, in modo che l&#39;errore effettivo venga visualizzato nell&#39;interfaccia utente e il messaggio di errore corretto venga archiviato nei registri.
...
* **ACSD-64111** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema quando si verifica un errore durante l&#39;impostazione delle condizioni nidificate per un componente Product in [!DNL Page Builder].
...
* **ACSD-64112** (per Adobe Commerce >=2.4.5 &lt;2.4.8) - Corregge un problema che impedisce l&#39;esecuzione di `indexer_update_all_views` cron se `MAGE_INDEXER_THREADS_COUNT` è impostato.
...
* **ACSD-48570** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Risolve il problema relativo all&#39;impossibilità di raggiungere la pagina di reimpostazione della password facendo clic sul collegamento [!UICONTROL Admin] reimpostazione della password quando **Aggiungi codice archivio agli URL** era *abilitato*, causando in precedenza la visualizzazione della pagina di accesso o di una pagina 404.
...
* **ACSD-63067** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui tutte le quantità di prodotto vengono evidenziate in modo errato e il messaggio *[!DNL Please specify the quantity of product(s).]* viene visualizzato per tutti i prodotti in un prodotto raggruppato quando una sola quantità non è corretta.
...
* **ACSD-63182** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Risolve il problema relativo alla presenza di un errore durante il salvataggio di un prodotto bundle duplicato con **[!DNL MSI]** *enabled*.
...
* **ACSD-63325** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema di un `Syntax Error: Unexpected <EOF>` errore durante l&#39;invio di una richiesta [!DNL GraphQL] vuota.
* **ACSD-63329** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui i valori predefiniti per gli attributi con **[!UICONTROL Date]** o **[!UICONTROL Date and Time]** tipi di input non vengono impostati durante la creazione di prodotti tramite [!DNL REST API].
* **ACSD-63572** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.8) - Corregge il problema per cui le tabelle temporanee dell&#39;`CatalogRule` indicizzatore non vengono pulite se il processo dell&#39;indicizzatore viene terminato.
...
* **ACSD-59083** (per Adobe Commerce >=2.4.4 &lt;2.5.0) - Corregge il problema per cui alcune operazioni di aggiornamento del database generano un errore *Tabella o vista di base non trovata* se l&#39;aggiornamento [!DNL mview] è in esecuzione contemporaneamente.
* **ACSD-61622** (per Adobe Commerce e Magento Open Source >=2.4.6-p1 &lt;2.4.7) - Corregge il problema per cui le tariffe specifiche dell&#39;account di [!DNL FedEx] non sono presenti nella risposta.
...
* **ACSD-62481** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.5.0) - Risolve il problema relativo al fatto che il carrello del cliente diventa vuoto anche se **[!UICONTROL Persistence]** è abilitato.
...
* **ACSD-62635** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.5.0): è stato risolto il problema che impediva la corretta visualizzazione dei prodotti correlati a più store nella query del prodotto [!DNL GraphQL].
...
* **ACSD-62708** (per Adobe Commerce e Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2,4,5-p10 &lt;2,4,6-p2 || >=2.4.6-p8 &lt;2.4.7-p1) - Corregge il problema per cui la dimensione del font dell&#39;editor [!DNL TinyMCE] 7 nell&#39;amministratore mostra *PT* e non *PX*. Ora puoi anche impostare la dimensione del font in *PX* invece di *PT*.
* **ACSD-62758** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.5.0) - Risolve il problema relativo al rendering errato dei video di prodotto nella pagina dei dettagli di **[!UICONTROL Configurable Product]** se [!DNL URL] contiene opzioni selezionate.
...
* **ACSD-63244** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema se un errore [!DNL JavaScript] impedisce il corretto rendering di [!DNL Google Maps]. È stato risolto il problema che causava molti *errori di tipo non rilevati: this._each non è una funzione* nella console nel pannello [!UICONTROL Admin].
* **ACSD-63242** (per Adobe Commerce e Magento Open Source >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - Risolve il problema della lentezza di importazione quando si aggiungono prodotti catalogo con più di 10.000 voci.
...
* **ACSD-62971** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema che causa l&#39;importazione di origini stock con valori non numerici nella colonna **quantity**, con l&#39;impostazione di **quantity** su *0*.
...
* **ACSD-62755** (per Adobe Commerce e Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2,4,5-p10 &lt;2,4,6 || >=2,4,6-p8 &lt;2,4,7 || >=2.4.7-p3 &lt;2.4.8) - Corregge il problema per cui [!DNL TinyMCE] 7 richiede che le dimensioni e i caratteri vengano aggiunti in modo specifico nelle impostazioni di inizializzazione dell&#39;editor.
* **ACSD-62670** (per Adobe Commerce e Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2,4,5-p10 &lt;2,4,6 || >=2,4,6-p8 &lt;2,4,7 || >=2.4.7-p3 &lt;2.4.8) - Corregge il problema per cui l&#39;[!UICONTROL Products Ordered] esportazione report in [!DNL CSV] e [!DNL XML] restituisce un errore.
* **ACSD-62577** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8): è stato risolto il problema di prestazioni lente delle query di ricerca storefront ottimizzando gli indici di query e di tabella.
...
* **ACSD-62428** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema per cui `is_out_of_stock` viene impostato su un valore errato nell&#39;indice di ricerca del catalogo quando [!DNL SKU] non è impostato come attributo ricercabile.
...
* **ACSD-58383** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Risolve il problema che causava la creazione di note di credito duplicate quando si emetteva un rimborso tramite [!DNL REST API] con due richieste identiche eseguite contemporaneamente.
...
* **ACSD-58566** (per Adobe Commerce >=2.4.6 &lt;2.4.8) - Corregge il problema per cui [!DNL GraphQL] restituisce un errore interno del server durante la query sul campo `created_at` nella mutazione `addPurchaseOrderComment`.
...
* **ACSD-60344** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Risolve il problema relativo all&#39;invio di e-mail di conferma dell&#39;ordine duplicate quando si utilizza un **[!UICONTROL Purchase Order]** con approvazione automatica.
...
* **ACSD-61534** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema che impediva l&#39;impostazione della configurazione di progettazione con il comando `bin/magento config:set` e la modifica dei valori bloccati tramite la manipolazione del modulo. Non è possibile aggiornare i valori bloccati impostati da [!DNL CLI] con `--lock-env` o `--lock-conf`.
* **ACSD-61785** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Corregge il problema per cui l&#39;aggiornamento dell&#39;attributo `reward_warning_notification` non era possibile tramite [!DNL GraphQL] mutazione e [!DNL REST API] chiamate, allineando il suo comportamento con `reward_update_notification`.
...
* **ACSD-62793** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema per cui gli attributi `datetime` nei dati esportati non includono il componente tempo. Inoltre, se **[!UICONTROL Fields Enclosure]** è *enabled*, i valori degli attributi nella colonna `additional_attributes` verranno racchiusi tra virgolette doppie.
* **ACSD-62332** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Corregge il problema per cui la query di elenco prodotti [!DNL GraphQL] era limitata a `total_count` di 10.000 prodotti. È stato risolto il problema per cui [!DNL Live Search] imposta la pagina corrente su *1* invece della pagina *2* nei criteri di ricerca quando viene eseguita una query tramite [!DNL GraphQL].
...
* **ACSD-61103** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema per cui il conteggio degli errori nella tabella `customer_entity` non viene azzerato dopo che un cliente ha effettuato correttamente l&#39;accesso tramite gli endpoint API.
* **ACSD-61134** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema per cui il [!DNL Braintree Vault] metodo di pagamento viene deselezionato automaticamente nel flusso di lavoro di pagamento quando un acquirente aggiorna il proprio indirizzo di fatturazione deselezionando la casella di controllo *[!UICONTROL My billing and shipping address are the same]*.
...
* **ACSD-61200** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Corregge il problema per cui nei calcoli per *[!UICONTROL Total Amount]* e *[!UICONTROL Total Amount Actual]* nelle vendite mancano *[!UICONTROL Discount Tax Compensation Amount]* e *[!UICONTROL Shipping Discount Tax Compensation Amount]*, causando discrepanze nei dati dell&#39;ordine di vendita.
...
* **ACSD-61756** (per Adobe Commerce >=2.4.4 &lt;2.4.7) - Migliora le prestazioni dei `AdvancedSalesRule` filtri.
...
* **ACSD-61845** (per Adobe Commerce e Magento Open Source >=2.4.7-p1 &lt;2.4.8) - Corregge l&#39;errore che si verifica quando una richiesta viene inviata con solo l&#39;intestazione *text/html* accept.
...
* **ACSD-62485** (per Adobe Commerce >=2.4.4 &lt;2.4.6-p8 || >=2.4.7 &lt;2.4.8) - Risolve il problema per cui `async.operations.all` il consumatore smette di lavorare durante la creazione di un&#39;azienda.
...
* **ACSD-61528** (per Adobe Commerce >=2.4.6 &lt;2.4.8) - Corregge il problema per cui il recupero dei ruoli da [!UICONTROL Admin] con [!DNL GraphQL] non restituisce alcun risultato.
* **ACSD-61553** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema per cui gli **[!UICONTROL Cart Price Rule]** sconti non vengono calcolati correttamente quando al prodotto vengono applicati più sconti con priorità diverse e **[!UICONTROL Maximum Qty Discount is Applied To]**.
...
* **ACSD-59865** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Risolve il problema per cui un [!UICONTROL Cart Price Rule] non annulla le regole applicate in precedenza se la quantità del prodotto nel carrello non è sufficiente per l&#39;applicazione delle regole.
...
* **ACSD-60590** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Migliora le prestazioni di generazione di [!UICONTROL Bestsellers Aggregated Daily Reports] per un grande volume di ordini.
* **ACSD-60673** (per Adobe Commerce >=2.4.4 &lt;2.4.8) - Corregge il problema per cui [!UICONTROL Cart Price Rule] per più metodi di pagamento al momento dell&#39;acquisto non viene applicato in modo appropriato al metodo di pagamento specifico.
...
* **ACSD-61322** (per Adobe Commerce >=2.4.6 &lt;2.4.8) - Corregge il problema per cui [!UICONTROL Products/Categories] non assegnato a [!UICONTROL Shared Catalog] per Default (General Group) è ancora incluso in XML Sitemap.
* **ACSD-61366** (per Adobe Commerce e Magento Open Source >=2.4.7 &lt;2.4.8) - Corregge il problema per cui il comando `setup:static-content:deploy --jobs 4` viene eseguito con più processi non riusciti e *Port deve essere configurato nell&#39;errore del parametro host* quando la porta è specificata per la connessione DB.
...
* **ACSD-60234** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Risolve il problema che causava la visualizzazione di un importo errato in [!DNL PayPal] quando lo sconto veniva applicato con il metodo di pagamento.
* **ACSD-59967** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7-p2): è stato risolto il problema che impediva il corretto rendering di [!DNL Google Maps] a causa di un errore di JavaScript.
...
* **ACSD-60538** (per Adobe Commerce >=2.4.7 &lt;2.4.8) - Risolve il problema per cui se un prodotto è disabilitato in *[!UICONTROL All Store Views]* e abilitato solo in ambiti di visualizzazione specifici dell&#39;archivio, gli attributi del prodotto non vengono visualizzati correttamente nella risposta di GraphQL, causando una visualizzazione non corretta del prodotto.
...
* **ACSD-60816** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7): è stato risolto il problema che impediva l&#39;esecuzione degli [!DNL New Relic Browser Monitoring] script inseriti dall&#39;agente APM non conformi ai criteri CSP (Content Security Policy).
...
* **ACSD-45049** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.4-p8 || >=2.4.5 &lt;2.4.6) - Corregge il problema per cui l’impostazione di un attributo *[!UICONTROL Is required]* del cliente non funziona correttamente in base all’ambito del sito web in Admin.
* **ACSD-46938** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6): è stato risolto il problema relativo alle prestazioni dei trigger del database durante la ricreazione `setup:upgrade`.
...
* **ACSD-58141** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.8) - Risolve il problema relativo alla rigenerazione di PHPSESSID nelle richieste POST nell&#39;area di vetrina per un cliente connesso se [!DNL L2 Redis cache] è abilitato e il cliente è aggiornato da Admin.
...
* **ACSD-59514** (per Adobe Commerce >=2.4.4 &lt;2.4.7-p2) - Risolve il problema relativo al rendering dei moduli nell&#39;area di amministrazione con [!DNL Page Builder] generazione dell&#39;errore *[!DNL Page Builder]per 5 secondi senza rilascio dei blocchi.* nella console del browser dopo l’invio del modulo, e le modifiche non possono essere salvate.
* **ACSD-60303** (per Adobe Commerce >=2.4.4-p9 &lt;2.4.5 || >=2,4,5-p8 &lt;2,4,6 || >=2.4.6-p6 &lt;2.4.8) - Corregge il problema per cui non è possibile effettuare un ordine dall’Amministratore se è abilitata la minimizzazione di HTML.
...
* **ACSD-58375** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui la configurazione *[!DNL YouTube API Key]* errata causa un errore durante l&#39;aggiunta di un video [!DNL YouTube] a livello di visualizzazione archivio.
...
* **ACSD-58163** (per Adobe Commerce >=2.4.3 &lt;2.4.7) - Corregge il problema per cui un *[!UICONTROL Cart Price Rule]* non applica uno sconto per un cliente ospite dal carrello *[!UICONTROL Customer Segment]* corrispondente senza un codice coupon.
...
* **ACSD-55566** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.7) - Corregge il problema per cui la `mergeCart` mutazione non riesce e viene restituito &quot;*Errore interno del server*&quot; nella risposta [!DNL GraphQL] durante l&#39;unione dei carrelli di origine e di destinazione con gli stessi elementi bundle.
* **ACSD-56546** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema per cui i prodotti configurabili e bundle vengono visualizzati come **esauriti** nella vetrina quando **visualizzazione esaurita dalla configurazione del prodotto** è *Disabilitato*.
* **ACSD-56635** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Risolve il problema relativo alla duplicazione del cliente importato con lo stesso indirizzo e-mail, quando viene utilizzata un&#39;importazione con **condivisione account** impostato su *Global*.
...
* **ACSD-57315** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Risolve il problema quando viene creata una nuova transazione in [!DNL PayPal Payflow Pro] ogni volta che si fa clic sul pulsante [!UICONTROL Fetch] nella schermata **[!UICONTROL View transaction]** dell&#39;amministratore.
* **ACSD-57337** (per Adobe Commerce >=2.4.4 &lt;2.4.6) - Corregge il problema per cui un utente amministratore con restrizioni di accesso a siti Web specifici è in grado di visualizzare le società di tutti i siti Web nella griglia **[!UICONTROL Companies]**.
...
* **ACSD-57565** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema per cui il dashboard **[!UICONTROL Order]** visualizza informazioni di ordine errate fino all&#39;aggiornamento del periodo di tempo. Il dashboard visualizza ora le statistiche dell’ordine corrette al primo caricamento.
* **ACSD-57854** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7): è stato risolto il problema che si verificava se le richieste di prodotto [!DNL GraphQL] restituivano categorie disabilitate nelle aggregazioni di categorie.
...
* **ACSD-55241** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui gli attributi *[!UICONTROL Used]* e *[!UICONTROL Times Used]* visualizzano valori errati per i coupon generati quando vengono utilizzati durante l&#39;estrazione con più indirizzi.
...
* **ACSD-57074** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema per cui l&#39;attributo personalizzato *Sì/No* con `attrbute_code` che inizia con `price_` non funziona correttamente con l&#39;indicizzazione e i prodotti con tali attributi non sono disponibili nel front-end.
...
* **ACSD-56415** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema relativo al rallentamento delle prestazioni dell&#39;indicizzazione parziale dei prezzi dovuto a una query `DELETE` quando nel database sono presenti molti dati di prezzo parziali da indicizzare.
...
* **ACSD-56790** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Risolve il problema relativo al reindirizzamento di un utente al dashboard di amministrazione durante l&#39;ordinamento dei prodotti di categoria tramite l&#39;opzione **Sposta le scorte in fondo** e l&#39;errore `Invalid security or form key. Please refresh the page` viene visualizzato nella parte superiore dello schermo.
...
* **ACSD-53925** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui l&#39;amministratore non è in grado di salvare il blocco CMS con il carosello prodotti quando `catalog_product_price` dimensioni-mode è impostato su *sito Web*.
...
* **ACSD-53658** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema per cui i dati di *[!UICONTROL Recently Viewed]* prodotto non vengono aggiornati correttamente nella visualizzazione archivio.
* **ACSD-54626** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Risolve il problema che impediva la creazione di una nuova regola ordine fornitore (`createPurchaseOrderApprovalRule`) con l&#39;attributo `NUMBER_OF_SKUS` tramite [!DNL GraphQL].
* **ACSD-53845** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il [!DNL MySQL] problema di timeout della connessione quando `consumer max_messages` = 0.
* **ACSD-54890** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema in cui `aggregate_sales_report_bestsellers_data` causa [!DNL MySQL] errori a causa di spazio insufficiente sul disco `/tmp`.
...
* **ACSD-54264** (per Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2,4,5-p4 &lt;2,4,6 || >=2.4.6-p2 &lt;2.4.7) - Corregge il problema per cui il messaggio di errore *&quot;Non è possibile aggiornare l’attributo richiesto. L&#39;ID riga: store_id&quot;* viene visualizzato quando un cliente tenta di effettuare il Check-Out con un preventivo negoziabile da un&#39;altra visualizzazione archivio.
...
* **ACSD-55238** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Correzioni per il salvataggio del prodotto vuoto *[!UICONTROL Meta Description]*.
...
* **ACSD-55381** (per Adobe Commerce >=2.4.2 &lt;2.4.7) - Corregge un errore interno del server durante la query `configurable_product_option_uid` e `configurable_product_option_value_uid` campi da un [!DNL B2B] *[!UICONTROL Requisition list]* tramite [!DNL GraphQL].
...
* **ACSD-54040** (per Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6) - Corregge il problema se il campo *[!UICONTROL Created]* è vuoto nell’ordine in cui i dettagli sono attivati i moduli B2B.
...
* **ACSD-55004** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Risolve il problema relativo agli arresti anomali della convalida durante il caricamento di un file di importazione di dimensioni superiori al valore configurato in `php.ini`.
* **ACSD-54989** (per Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2,4,5-p4 &lt;2,4,6 || >=2.4.6-p2 &lt;2.4.7) - È stato risolto il problema che impediva all&#39;amministratore della società di effettuare un ordine se *[!UICONTROL Enable Purchase Orders]* è impostato su *[!UICONTROL Yes]* e *[!UICONTROL Purchase Order]* è impostato su *[!UICONTROL No]*.
* **ACSD-54007** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge l&#39;errore *&quot;Undefined array key &quot;_scope&quot;* nell&#39;importazione dei dati dei clienti.
...
* **ACSD-54961** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema per cui un utente amministratore con restrizioni non può aggiornare in massa lo stato *Revisione prodotto*.
...
* **ACSD-54026** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Corregge un messaggio di errore non corretto per una richiesta `updateCompanyRole` di GraphQL per un utente non autorizzato.
...
* **ACSD-54342** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge un messaggio di errore non corretto *Errore nella struttura dati: i valori sono misti* quando si importa un file CSV senza dati validi.
...
* **ACSD-53998** (per Adobe Commerce e Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - Corregge il problema per cui un **[!UICONTROL Dynamic Block]** basato su **[!UICONTROL Customer Segment]** non funziona correttamente dopo la disconnessione da un account cliente.
* **ACSD-53204** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Correzioni *Impossibile salvare il prodotto.* errore durante l&#39;esecuzione di richieste simultanee di aggiunta di immagini alla raccolta prodotti utilizzando l&#39;endpoint `rest/V1/products/<sku>/media`.
...
* **ACSD-54018** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.6) - Corregge i problemi di prestazioni con il [!UICONTROL Product List] widget che utilizza un attributo non globale nella condizione del widget.
...
* **ACSD-47669** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.6) - Correzioni *Violazione del vincolo di integrità: 1452 Impossibile aggiungere o aggiornare una riga figlio: errore Un vincolo di chiave esterna non riesce* durante l&#39;importazione del file CSV dei prodotti.
...
* **ACSD-53824** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema per cui `UpdateMultiselectAttributesBackendTypes` la patch dei dati di migrazione supera il limite delle dimensioni delle transazioni del database durante `setup:upgrade`.

## v1.1.37 {#v1-1-37}

* **ACSD-52613** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema di aggiornamento delle cache e degli indici anche quando non vengono effettuati aggiornamenti agli `Inventory_source` elementi da parte dell&#39;API REST.
...
* **ACSD-52606** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema relativo alla visualizzazione del messaggio di errore *L&#39;ordine non è pronto per il ritiro* quando l&#39;utente fa clic su **[!UICONTROL Notify Order is Ready for Pickup]**.
...
* **ACSD-53795** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema relativo a un tipo di dati non valido nel `indexer_update_all_views` processo cron.
...
* **ACSD-49843** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - Corregge il problema per cui il collegamento sul download del prodotto non è disponibile dopo la fatturazione automatica dell’articolo ordinato tramite metodo di pagamento online con l’impostazione **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** abilitata.
...
* **ACSD-51666** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Corregge l&#39;errore *La sessione è scaduta. Accedere di nuovo.* questo accade dopo che un cliente tenta di accedere.
...
* **ACSD-52041** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corregge il problema per cui il messaggio di errore: *[ERROR] [!DNL Page Builder] eseguiva il rendering per 5 secondi senza rilasciare blocchi.* viene visualizzato nel browser Chrome quando si salva il contenuto modificato con [!DNL Page Builder].
...
* **ACSD-51984** (per Adobe Commerce >=2.4.5 &lt;2.4.7) - Corregge il problema per cui i valori *[!UICONTROL Use Default Value]* e *[!UICONTROL non-default product field]* non vengono salvati per la seconda visualizzazione sito Web, archivio e archivio.
...
* **ACSD-51683** (per Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corregge il problema per cui non è possibile aggiungere al carrello un&#39;opzione personalizzabile utilizzando GraphQL.
...
* **ACSD-52202** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.7) - Corregge il problema per cui la quantità vendibile delle scorte predefinite viene erroneamente modificata in 0 quando le scorte non predefinite vengono modificate in 0 qtà all&#39;evasione dell&#39;ordine.
* **ACSD-51265** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Risolve il problema relativo alle `catalog_product_price` prestazioni di reindicizzazione in presenza di troppi prodotti in bundle nel sistema.
* **ACSD-52831** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Risolve il problema che impediva ai clienti di inserire ordini di preventivo negoziabili se [!DNL Google reCAPTCHA v3 Invisible] è abilitato.
* **ACSD-51845** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Risolve il problema che impediva l&#39;aggiornamento in blocco asincrono dell&#39;API REST di prodotti successivi con prezzi di livello e set di attributi diversi.
...
* **ACSD-51739** (per Adobe Commerce >=2.4.6 &lt;2.4.7) - Risolve il problema relativo alla restituzione di un errore quando `structure_id` viene richiesto in una richiesta CompanyTeam GraphQL.
* **ACSD-51857** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corregge il problema per cui le prestazioni lente del report `aggregate_sales_report_bestsellers_data` cron sulle tabelle sales_order e `sales_order_item` di database di grandi dimensioni erano dovute al modo in cui è stata scritta la query di dati principale.
...
* **ACSD-52689** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.6) - Risolve il problema che impediva il caricamento delle immagini nell&#39;archiviazione Amazon S3 utilizzando l&#39;API REST.
...
* **ACSD-50512** (per Adobe Commerce >=2.4.5 &lt;2.4.7) - Corregge il *Errore: il collegamento scaricabile non è correlato al prodotto. Verificare il collegamento e riprovare.* errore che si verifica quando si aggiorna la data di inizio per un aggiornamento scaricabile della gestione temporanea del prodotto.
* **ACSD-50949** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui il filtro del prezzo in Ricerca avanzata non restituisce risultati corretti se utilizzato insieme al filtro SKU.
...
* **ACSD-50368** (per Adobe Commerce e Magento Open Source >= 2.4.3 &lt;2.4.5) - Corregge il problema per cui il `group_id` del cliente viene ignorato quando viene creato un cliente tramite API REST asincrona o API REST asincrona in blocco.
...
* **ACSD-51735** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.5) - Corregge il problema per cui lo stato dell&#39;elemento dell&#39;ordine non è impostato correttamente su *[!UICONTROL Ordered]* quando le scorte di prodotto sono *0*.
...
* **ACSD-51700** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.7) - Corregge l&#39;errore che si verifica quando si passa da una visualizzazione archivio a una pagina di modifica prodotto scaricabile nell&#39;amministratore.
...
* **ACSD-52148** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.4) - Corregge il problema che causa l&#39;errore occasionale dell&#39;[!DNL Google V3 reCAPTCHA] accesso amministratore.
* **ACSD-51431** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema relativo allo stato dell&#39;indicizzatore *funzionante* anche se non sono presenti nuove voci nel changelog.
...
* **ACSD-49628** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui [!UICONTROL Page Builder's] più errori impediscono all&#39;amministratore di salvare un prodotto senza autorizzazioni per il contenuto.
...
* **ACSD-50621** (per Adobe Commerce >=2.3.7 &lt;2.4.7) - Corregge il problema per cui [!UICONTROL Tier Prices] per siti Web diversi nel catalogo condiviso non è visibile quando si tenta di modificarli in un ambiente con più siti Web.
...
* **ACSD-51528** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema per cui la colonna *x_forwarded_for* contiene valori Null nella tabella *sales_order*.
...
* **ACSD-49737** (per Adobe Commerce e Magento Open Source >=2.4.1-p1 &lt;2.4.7) - Risolve il problema relativo all&#39;utilizzo errato di un coupon dopo un pagamento non riuscito con carta.
...
* **ACSD-49513** (per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.5): è stato risolto il problema che impediva la sincronizzazione dell&#39;archiviazione remota a causa di file a 0 byte.
* **ACSD-46683** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui il prezzo di spedizione è *Non ancora calcolato*.
* **ACSD-49129** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.6) - Corregge il problema per cui l&#39;attributo *[!UICONTROL content]* (codice immagine base64) non viene restituito nelle risposte API dei supporti di prodotto di `rest/V1/products/sku/media`.
...
* **ACSD-49973** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.5) - Migliora le prestazioni di recupero dei prodotti in bundle tramite GraphQL.
...
* **ACSD-48771** (per Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corregge il problema relativo all&#39;aggiornamento del tipo di contenuto a blocchi di colonna dalle versioni precedenti [!DNL Page Builder].
...
* **ACSD-49849** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.6) - Corregge il problema relativo alla sostituzione dell&#39;e-mail del cliente con [!DNL PayPal] e-mail al momento dell&#39;ordine con [!DNL PayPal Express] tramite GraphQL.
...
* **ACSD-47704** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Risolve il problema relativo alla visualizzazione del prezzo dei soli prodotti in magazzino nel prodotto incluso.
...
* **ACSD-48866** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.7) - Corregge il problema che si verifica quando si richiede il feed RSS per le categorie.
...
* **ACSD-48216** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - Corregge il problema se [!UICONTROL AUTO_INCREMENT] della tabella [!UICONTROL inventory_source_item] aumenta nell&#39;operazione [!UICONTROL UPDATE].
...
* **ACSD-48404** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.4) - Risolve il problema &quot;Ricorda paginazione categoria = Sì&quot; causa un errore quando si preme il pulsante Indietro del browser.
...
* **ACSD-48773** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corregge il problema per cui il modello e-mail dei punti premio proviene dall&#39;archivio errato.
...
* **ACSD-47988** (per Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.6): è stato risolto il problema che si verificava se l&#39;esportazione del prodotto tagliava i tag HTML dalla descrizione del prodotto del generatore di pagine.
* **ACSD-48366** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6) - Corregge il problema per cui l&#39;immagine del prodotto non viene visualizzata nel modello e-mail Torna a Stock.
...
* **ACSD-47520** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Corregge il problema della perdita di punti premio da parte dei clienti al momento della creazione di una nota di credito.
* **ACSD-48044** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.4): è stato risolto il problema che impediva l&#39;inserimento di ordini quando si applicano più gift card a un singolo ordine con spedizione multipla.
...
* **ACSD-47292** (per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6) - Corregge il problema per cui i prodotti in bundle esauriti non sono disponibili nella risposta di GraphQL se &quot;show out-of-stock products&quot; (Mostra prodotti esauriti) è impostato su Sì.
...
* **ACSD-48313** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.5) - Corregge il problema per cui la colonna &quot;configurable_variables&quot; non viene analizzata se il valore dell&#39;attributo contiene una virgola. Lo stesso algoritmo di analisi viene utilizzato per &quot;additional_attributes.
...
* **ACSD-47803** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6): è stato risolto il problema che determinava la visualizzazione dei campioni di prodotto configurabili esauriti come disponibili.
...
* **ACSD-46617** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Corregge il problema per cui il pulsante *Continua per estrazione* è disattivato anche se il subtotale è maggiore dell&#39;*importo ordine minimo* configurato.
...
* **ACSD-47336** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Correzioni *Si è verificato un errore.* durante la rimozione delle notifiche in Commerce Admin.
...
* **ACSD-47920** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Corregge il problema per cui è possibile effettuare gli ordini tramite l&#39;API REST come utente guest anche quando *Consenti estrazione utenti guest* è disattivato.
...
* **ACSD-47666** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Corregge il problema per cui la funzione di filtro non funziona nella griglia Amministrazione > Sistema > Autorizzazioni > Ruoli utente > Ruolo > Utenti ruolo.
...
* **ACSD-46581** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Corregge il problema per cui il totale imposte stimato non viene aggiornato dopo aver selezionato un paese nel carrello.
* **ACSD-46618** (per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Risolve il problema relativo alla visualizzazione di prezzi non corretti nella cache per un cliente connesso.
...
* **ACSD-47076** (per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.5) - Risolve il problema che impediva la riproduzione di video Vimeo nella vetrina.
* **ACSD-45071** (per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4) - Risolve il problema dell&#39;aggiunta dell&#39;origine predefinita al prodotto durante l&#39;importazione.
...
* **ACSD-46520** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.5*) - Corregge il problema che causa lo stato non corretto dell&#39;ordine quando l&#39;utente viene rimborsato con il credito del negozio.
* **ACSD-46703** (*per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6*) - Risolve il problema che impediva di trascinare le opzioni personalizzate in una pagina di modifica del prodotto.
* **ACSD-44851** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6*) - Risolve il problema che impediva l&#39;apertura o l&#39;espansione di una categoria con sottocategorie.
...
* **ACSD-46869** (*per Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6*) - Corregge il problema se un prodotto configurabile in un carrello non viene aggiornato tramite una richiesta *PUT REST API* senza modificare la quantità del prodotto.
* **MDVA-42768-V2** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.3*) - Corregge il problema per cui nel prodotto configurabile viene visualizzato il prezzo regolare come *0* quando *Visualizzazione esaurita* è *Sì*.
...
* **ACSD-45781** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.2*) - Corregge il problema che impedisce la visualizzazione del campo di ricerca iniziale dell&#39;archivio su dispositivi mobili.
...
* **ACSD-45817** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Corregge il problema se una mutazione di prodotto GraphQL per un archivio specifico restituisce tutte le varianti configurabili, incluse quelle non assegnate all&#39;archivio richiesto.
...
* **ACSD-45255** (*per Adobe Commerce >=2.4.3 &lt;2.4.6*) - Corregge un&#39;eccezione nella pagina Report Stock esauriti per un utente amministratore con restrizioni.
...
* **ACSD-43887** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.5*) - Corregge il problema relativo alla visualizzazione di dettagli non corretti nella pagina Pagamento di pagamento di pagamento quando gli ordini di acquisto per le società sono abilitati.
* **ACSD-45143** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.5*) - Corregge il problema per cui la mutazione `setShippingAddressesOnCart` non consente di impostare il codice dell&#39;area numerica come *region*.
...
* **ACSD-45169** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.6*) - Corregge il problema per cui [!DNL Visual Merchandiser] non visualizza le azioni e il prezzo corretti per un prodotto configurabile dopo l&#39;applicazione di un aggiornamento di staging.
...
* **MDVA-44703** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*) - Corregge il problema relativo al calcolo errato dei totali degli ordini nel report Ordini per l&#39;utente amministratore con restrizioni.
...
* **MDVA-44562** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.2-p2*) - Corregge il problema per cui l&#39;ID archivio non predefinito per gli elementi dell&#39;offerta viene sostituito dall&#39;ID archivio predefinito, nonostante la richiesta GraphQL proveniente dalla visualizzazione archivio non predefinita.
...
* **MDVA-44044** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.2-p2*) - Corregge il problema relativo alla mancata visualizzazione di un prodotto nella pagina della categoria dopo l&#39;assegnazione a un nuovo sito Web.
* **MDVA-42509** (*per Adobe Commerce e Magento Open Source >=2.3.3 &lt;2.4.4*) - Corregge il problema che impediva il caricamento di un file CSV per un ordine rapido, generando un errore *Impossibile inviare il cookie*.
...
* **MDVA-40961** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*) - Corregge il problema che impedisce l&#39;aggiunta di un elemento al carrello se la quantità minima dell&#39;elemento è già nel carrello.
...
* **MDVA-43859** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.5*) - Corregge il problema relativo all&#39;errore *Nessuna entità con customerId =* viene registrata quando un cliente eliminato tenta di accedere.
* **MDVA-44147** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.5*) - Corregge il problema per cui una richiesta di GraphQL non restituisce gli elenchi delle richieste di acquisto.
...
* **MDVA-42969** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.3*) - Corregge il problema in cui la regola prodotto correlata funziona solo quando il segmento cliente è impostato su *All*.
...
* **MDVA-43862** (*per Adobe Commerce e Magento Open Source >=2.3.3 &lt;2.4.5*) - Corregge il problema per cui il cliente non può aggiornare gli elementi del carrello a causa di un errore di mutazione *UpdateCartItems* di GraphQL.
* **MDVA-43824** (*per Adobe Commerce e Magento Open Source >=2.3.6 &lt;=2.3.7-p3 || >=2.4.1 &lt;2.4.5*) - Corregge il problema relativo alla visualizzazione di un errore nell&#39;annullamento degli ordini con uno sconto.
* **MDVA-43451** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.5*) - Corregge il problema per cui l&#39;errore *Impossibile trovare l&#39;archivio richiesto. Verificare l&#39;archivio e riprovare.* viene visualizzato durante la configurazione di un catalogo condiviso per un sito Web specifico.
...
* **MDVA-43601** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*): è stato risolto il problema relativo ai trigger mancanti dopo la reindicizzazione completa.
...
* **MDVA-43935** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.5*) - Risolve il problema della doppia visualizzazione del prodotto Upsell.
...
* **MDVA-39713** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.3.6*) - Corregge il problema per cui l&#39;utente può modificare l&#39;ora di inizio per un aggiornamento pianificato attivo.
* **MDVA-42410** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*): è stato risolto il problema che causava la visualizzazione della sola valuta di base predefinita nei rapporti coupon.
* **MDVA-41136** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*) - Corregge il problema se la data di scadenza di `mage-cache-sessid` non viene estesa, causando la pulizia dei dati del cliente.
* **MDVA-39993** (*per Adobe Commerce e Magento Open Source >=2.3.5 &lt;=2.3.7-p2 || >=2.4.0 &lt;2.4.4*) - Corregge il problema per cui le modifiche di inventario eseguite tramite API non vengono applicate alla pagina del prodotto sul front-end.
...
* **MDVA-43414** (*per Adobe Commerce e Magento Open Source >=2.3.6 &lt;=2.3.7-p2*) - Corregge l&#39;errore irreversibile PHP che si verifica quando si esegue il consumer della coda `inventory.reservations.updateSalabilityStatus` negli SKU numerici.
...
* **MDVA-43348** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.5*) - Corregge il problema per cui la richiesta GraphQL gift card visualizza un errore se `gift_card_options` contiene *uid*.
* **MDVA-39546** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*) - Corregge il problema per cui la data di inizio per l&#39;aggiornamento dello staging potrebbe essere impostata su una data precedente alla data corrente durante la modifica.
* **MDVA-42950** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*) - Risolve il problema della mancata riproduzione dei video nella pagina del prodotto.
...
* **MDVA-43731** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*) - Corregge il problema per cui *i sinonimi di ricerca* non funzionano più quando il valore viene aggiunto in *Termini minimi da abbinare*.
* **MDVA-43232** (*per Adobe Commerce e Magento Open Source >=2.3.4 &lt;2.4.5*) - Corregge il problema relativo all&#39;ordinamento dei prodotti in [!DNL Visual Merchandiser] in base al prezzo speciale In basso/Superiore causando un errore durante il salvataggio della categoria.
* **MDVA-43726** (*per Adobe Commerce e Magento Open Source >=2.3.3 &lt;2.4.3*) - Corregge il problema per cui la regola del prezzo di catalogo basata sulla corrispondenza dell&#39;attributo a livello di archivio non viene applicata dopo la reindicizzazione parziale.
...
* **MDVA-41350** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*) - Corregge il problema che genera un&#39;eccezione quando un utente amministratore con accesso limitato aggiunge un prodotto al di fuori del proprio ambito di ruolo da parte dello SKU in un ordine.
* **MDVA-42269** (*per Adobe Commerce e Magento Open Source >=2.4.3-p1 &lt;2.4.5*) - Corregge il problema che impedisce a un utente amministratore di accedere all&#39;amministratore a causa dell&#39;*errore TypeError: strtotime() prevede che il parametro 1 sia una stringa, null specificato*.
* **MDVA-40830** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.5*) - Corregge il problema per cui il credito dell&#39;archivio viene applicato più volte durante l&#39;inserimento dell&#39;ordine.
* **MDVA-42237** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.5*) - Risolve il problema che impediva l&#39;aggiornamento di un prezzo speciale del prodotto configurabile in seguito a modifiche nel prezzo del relativo sottoprodotto.
* **MDVA-42520** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*) - Risolve il problema relativo all&#39;applicazione dell&#39;aliquota fiscale due volte se si utilizza *Abilita commercio transfrontaliero*.
* Patch aggiornate: MDVA-27239, MDVA-39305, MDVA-41236, MDVA-36832.
* Patch obsoleta: MDVA-37725.

## v1.1.10 {#v1-1-10}

* **MDVA-38728** (*per Adobe Commerce e Magento Open Source >=2.3.2 &lt;2.4.5*) - Corregge il problema per cui l&#39;aggiornamento di massa degli attributi crea la riscrittura degli URL per l&#39;archivio predefinito solo dopo la modifica di *visibilità prodotto*.
* **MDVA-43091** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*) - Corregge il problema per cui l&#39;ordinamento di un prodotto bundle dall&#39;amministratore nel backend restituisce l&#39;errore *Non è possibile utilizzare quantità decimali per questo prodotto*.
...
* **MDVA-43201** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Corregge il problema di errore nell&#39;accesso del cliente quando si utilizza l&#39;attributo DOB con determinate impostazioni internazionali.
...
* **MDVA-42657** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.5*) - Corregge il problema per cui l&#39;utente amministratore non è in grado di selezionare categorie nelle condizioni del segmento cliente.
* **MDVA-42806** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Corregge il problema per cui viene inviata un&#39;e-mail di *nuova registrazione società* ogni volta che una società esistente viene aggiornata tramite API REST.
* **MDVA-37984** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.5*) - Corregge il problema per cui la funzionalità [!DNL Visual Merchandiser] *Corrispondenza prodotto per regola* non filtra correttamente i prodotti con aggiornamenti di gestione temporanea.
* **MDVA-40488** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Corregge il problema per cui i prodotti configurabili con prodotti secondari esauriti non vengono visualizzati nell&#39;intervallo di prezzo corretto.
...
* **MDVA-38526** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.4*): è stato risolto il problema che impediva all&#39;utente amministratore di accedere a [!DNL Site-Wide Analysis tool].
...
* **MDVA-41139** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.4*): è stato risolto il problema che causava la perdita di scorte dei prodotti configurabili in seguito all&#39;importazione di un prodotto semplice quando per una delle sue origini era impostato il valore qty=0.
* **MDVA-42326** (*per Adobe Commerce e Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Corregge il problema relativo a un errore del cliente al momento del pagamento dopo un timeout della sessione, anche se il carrello acquisti persistente è abilitato.
...
* **MDVA-38393** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.4*): risolve il problema se le regole del catalogo cessano di funzionare per un prodotto configurabile se il relativo prodotto semplice viene rinominato.
...
* **MDVA-40311** (*per Adobe Commerce e Magento Open Source >=2.4.2-p2 &lt;2.4.4*) - Corregge il problema che causa la visualizzazione del messaggio di errore *Protezione o chiave modulo non valida. Aggiornare la pagina* dopo l&#39;accesso all&#39;amministratore se il percorso di amministrazione personalizzato è configurato e la chiave segreta è abilitata.
* **MDVA-41631** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.4*) - Corregge il problema che si verifica quando si tenta di recuperare le informazioni sull&#39;ordine senza un valore *phone* facoltativo tramite GraphQL.
* **MDVA-27239** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.3.6*) - Risolve il problema della mancata visualizzazione dei prodotti di cross-selling.
* Patch aggiornate: MDVA-37068, MDVA-35254, MDVA-41164, MDVA-37916, MDVA-37478, MDVA-34551, MDVA-31791.

## v1.1.6 {#v1-1-6}

* **MDVA-40550** (*per Adobe Commerce e Magento Open Source >=2.3.5 &lt;2.4.4*): è stato risolto il problema relativo ai prodotti mancanti sul front-end durante la reindicizzazione.
* **MDVA-40120** (*per Adobe Commerce e Magento Open Source >=2.4.1 &lt;2.4.4*) - Corregge il problema che non consente l&#39;ordinamento GraphQL in base a DESC/ASC con prodotti con la stessa rilevanza o prezzo.
* **MDVA-41399** (*per Adobe Commerce e Magento Open Source >=2.3.3 &lt;2.4.2*) - Corregge il problema per cui gli utenti amministratori non possono accedere alla pagina *Gestisci carrello* se un cliente aggiunge un prodotto alla lista dei desideri.
...
* **MDVA-27456** (*per Adobe Commerce e Magento Open Source >=2.3.5 &lt;2.3.7*) - Corregge il problema che si verifica quando si tenta di caricare [!DNL Swagger].
...
* **MDVA-41046** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.4*) - Corregge il problema della disponibilità di prodotti semplici con opzioni personalizzate da assegnare a prodotti configurabili o raggruppati.
* **MDVA-40545** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;2.4.4*) - Corregge il problema per cui è stato recuperato solo il primo nodo di una pagina, anche se erano presenti più nodi per la stessa pagina.
...
* **MDVA-40101** (*per Adobe Commerce e Magento Open Source >=2.3.2 &lt;2.4.0*) - Corregge il problema per cui gli elementi non vengono rimossi dal mini carrello dopo un inserimento dell&#39;ordine eseguito correttamente tramite [!DNL PayPal Express] Checkout.
...
* **MDVA-40537** (*per Adobe Commerce e Magento Open Source >=2.3.4 &lt;=2.4.0-p1*) - Corregge il problema che si verifica quando si crea una visualizzazione archivio se esistono più pagine CMS con la stessa chiave URL.
...
* **MC-42528** (*per Adobe Commerce e Magento Open Source >=2.4.3 &lt;=2.4.3-p1*) - Corregge il problema per cui la query GraphQL `categoryList` restituisce sia le categorie assegnate che quelle non assegnate.
...
* **MDVA-40262** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Corregge il problema per cui le query di ricerca di GraphQL non vengono visualizzate nei termini di ricerca più comuni nell&#39;amministratore.
* **MDVA-40601** (*per Adobe Commerce e Magento Open Source >=2.3.1 &lt;=2.4.2-p2*) - Corregge il problema che si verifica quando si tenta di ottenere informazioni sulla categoria modificata dall&#39;aggiornamento pianificato tramite GraphQL.
* **MDVA-37234** (*per Adobe Commerce e Magento Open Source >=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*) - Corregge il problema per cui aggiungendo più volte un articolo al carrello (richiesta parallela) per lo stesso SKU si crea una riga duplicata per lo stesso ID carrello.
...
* **MDVA-37364** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;=2.4.3*) - Corregge il problema di interruzione dell&#39;interfaccia utente della griglia da parte dell&#39;attributo cliente personalizzato del tipo di data.
* **MDVA-39195** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Corregge il problema relativo all&#39;inattività del pulsante *Aggiungi al carrello* nella pagina delle categorie quando è abilitato il reindirizzamento al carrello.
* **MDVA-37115** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Corregge il problema se nella pagina del prodotto configurabile viene visualizzato un avviso non necessario di *Solo 0 lasciato*.
...
* **MDVA-39043** (*per Adobe Commerce e Magento Open Source >=2.3.4 &lt;=2.4.3*) - Corregge il problema a causa del quale l&#39;utente amministratore con accesso limitato riceve un errore durante il tentativo di aggiungere il widget *Products* alla pagina CMS.
...
* **MDVA-38852** (*per Adobe Commerce e Magento Open Source >=2.3.0 &lt;=2.3.5-p2*) - Corregge il problema relativo al blocco delle tabelle di cataloghi per progettazione per gli aggiornamenti che riducono significativamente le prestazioni nei casi con diversi ordini paralleli.
...
* **MDVA-38447** (*per Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4*) - Risolve due problemi: in cui *Non visibile singolarmente* prodotti secondari configurabili vengono restituiti nella risposta di GraphQL e ottimizzano la query MySQL per i prodotti GraphQL con filtro categoria.
...
* **MDVA-36021** (*per Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.4*) - Corregge il problema per cui l&#39;errore *Richiama a una funzione membro getId()* viene visualizzato nella pagina dei dettagli dell&#39;ordine in Admin.
...
* **MDVA-37592** (*per Adobe Commerce >=2.3.6 &lt;=2.4.2-p1*) - Corregge il problema se l&#39;ordinamento in base al prezzo non funziona correttamente per i prodotti con prezzo zero assegnati al catalogo condiviso.
...
* **MDVA-37068** (*per Adobe Commerce >=2.3.1 &lt;2.4.4*) - Corregge il problema di visualizzazione dell&#39;aliquota non corretta quando il carrello contiene solo prodotti virtuali.
* **MDVA-38608** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Corregge il problema se le tabelle temporanee non vengono eliminate quando la reindicizzazione non viene completata correttamente.
...
* **MDVA-37916** (*per Adobe Commerce >=2.3.6 &lt;2.4.3*) - Corregge il problema per cui il cliente non viene portato alla pagina di conferma del pagamento quando si utilizza il metodo [!DNL Paypal Payment Advanced].
* **MDVA-37082** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Risolve il problema durante il salvataggio delle scorte personalizzate di prodotti raggruppati causando la visualizzazione del prodotto esaurito nel front-end.
* **MDVA-36572** (*per Adobe Commerce >=2.3.5 &lt;2.4.3*) - Corregge il problema se gli aggiornamenti delle note di credito non causano più aggiornamenti duplicati della prenotazione prodotti nel database.
* **MDVA-38132** (*per Adobe Commerce >=2.3.3 &lt;2.4.3*) - Corregge il problema quando il pannello di amministrazione non è raggiungibile a causa di un errore *di troppi reindirizzamenti*.
...
* **MDVA-36832** (*per Adobe Commerce >=2.3.4 &lt;=2.4.2-p1*) - Risolve il problema relativo alla presenza di immagini duplicate nelle pagine con larghezza di visualizzazione di 768 px.
* **MDVA-37874** (*per Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;=2.4.2-p1*) - Corregge il problema per cui *L&#39;importo dello sconto fisso per l&#39;intero carrello* non viene applicato correttamente a un prodotto bundle contenente più di un&#39;opzione.
...
* **MDVA-37478** (*per Adobe Commerce >=2.3.0 &lt;=2.3.7*) - Corregge il problema a causa del quale Adobe Commerce genera un errore durante la creazione di una fattura parziale per gli ordini effettuati con il metodo di pagamento *Pagamento in conto* tramite API REST.
...
* **MDVA-37225** (*per Adobe Commerce >=2.4.1 &lt;=2.4.2-p1*) - Corregge il problema di blocco del processo di caricamento durante la creazione rapida degli ordini quando è presente un valore intero negli SKU importati.
...
* **MDVA-36286** (*per Adobe Commerce >=2.3.6 &lt;=2.4.2-p1*) - Corregge il problema relativo all&#39;interruzione dell&#39;anteprima del widget prodotti Page Builder se lo stesso SKU ha una posizione diversa nelle sottocategorie.
* **MDVA-30186** (*per Adobe Commerce >=2.3.4 &lt;=2.3.5-p2, >=2.4.0 &lt;=2.4.0-p1, >=2.4.2 &lt;=2.4.2-p1*) - Corregge il problema in cui le opzioni attributo sono ordinate in base al valore dell&#39;opzione invece del conteggio degli elementi attributo nella risposta di GraphQL.
...
* **MDVA-36253** (*per Adobe Commerce >=2.4.0 &lt;=2.4.1-p1*) - Corregge il problema di visualizzazione del subtotale errato nel mini carrello dopo l&#39;eliminazione dell&#39;elemento.
...
* **MDVA-34665** (*per Adobe Commerce >=2.3.4 &lt;=2.3.4-p2*) - Corregge il problema relativo a prodotti in bundle mancanti nelle pagine delle categorie.
...
* **MDVA-36424** (*per Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 || >=2.0.0 &lt;2.2.0*) - Corregge il problema per cui le immagini multimediali allegate agli elementi del Page Builder scompaiono quando il contenuto viene modificato ripetutamente, se l’URL di base del back-end è diverso dall’URL di base della vetrina.
...
* **MDVA-36170** (*per Adobe Commerce >=2.3.4 &lt;2.4.2*) - Questo risolve il problema se la query GraphQL non viene memorizzata in cache utilizzando il tag cache delle categorie.
* **MDVA-33168** (*per Adobe Commerce >=2.3.3 &lt;2.4.2*) - Corregge il problema quando si aggiorna un attributo di prodotto tramite API. Tutti gli altri attributi vengono modificati in un valore vuoto.
* **MDVA-19640** (*per Adobe Commerce >=2.3.0*) - Corregge il problema se [!DNL Advanced Reporting] non visualizza alcun dato.
* **MDVA-11189** (*per Adobe Commerce >=2.3.0 &lt;2.3.5*) - Risolve il problema quando, dopo l&#39;importazione di un file CSV per aggiornare le scorte di prodotto, le righe della tabella `cataloginventory_stock` vengono eliminate.
* **MDVA-26639** (*per Adobe Commerce >=2.3.3-p1 &lt;2.3.6*) - Corregge il problema per cui se viene creato un nuovo modello e-mail di conferma dell&#39;ordine, gli elementi dell&#39;ordine non sono presenti nell&#39;e-mail dell&#39;ordine.
...
* **MDVA-21095** (*per Adobe Commerce >=2.3.0 &lt;2.3.5*) - Corregge il problema quando una query `INSERT INTO search_tmp` non termina dopo l&#39;aggiornamento di massa del valore dell&#39;attributo.
...
* **MDVA-22026** (*per Adobe Commerce >=2.3.2 &lt;2.3.4*) - Risolve il problema che impediva l&#39;importazione di prodotti da file CSV, incluse immagini da URL esterni.
* **MDVA-22383** (*per Adobe Commerce >=2.3.0 &lt;2.3.4*) - Corregge il problema relativo ai tempi lunghi per il salvataggio di un prodotto e alle interruzioni di pagina.
...
* **MDVA-35356** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Corregge il problema relativo alla restituzione del credito dell&#39;archivio non corretta dopo l&#39;annullamento dell&#39;ordine parzialmente fatturato.
* **MDVA-33289** (*per Adobe Commerce >=2.4.0 &lt;2.4.3*) - Corregge il problema di visualizzazione del codice JavaScript non elaborato nell&#39;interfaccia utente degli indirizzi di fatturazione durante l&#39;estrazione se [!DNL Google Tag Manager] è abilitato.
...
* **MDVA-33704** (*per Adobe Commerce >=2.4.0 &lt;2.4.3*) - Risolve il problema che impediva la visualizzazione dell&#39;opzione di spedizione *In store pickup*, anche se configurata per essere disponibile.
* **MDVA-34928** (*per Adobe Commerce >=2.3.5 &lt;2.3.5-p2*) - Corregge il problema della visualizzazione indefinita del caricatore di pagine dopo la rimozione del credito del negozio dal pagamento.
...
* **MDVA-32655** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Corregge lo stato errato del messaggio *in corso* nel messaggio *completo* corretto per il consumatore `quoteItemCleaner` dopo l&#39;eliminazione di diversi prodotti.
* **MDVA-34102** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Corregge se la quantità di scorte predefinite è zero per i prodotti disabilitati nella griglia di prodotto e nelle pagine Modifica prodotto nell&#39;area Amministrazione.
* **MDVA-35286** (*per Adobe Commerce >=2.4.0 &lt;2.4.2*) - Corregge il problema relativo a un errore che si verifica se un cliente ha raggruppato prodotti nel carrello e passa dal pagamento di più indirizzi al pagamento di Onepage.
* **MDVA-35312** (*per Adobe Commerce >=2.4.1-p1 &lt;2.4.2*) - Corregge il codice di risposta 500 quando una richiesta GraphQL vuota.
* **MDVA-34189** (*per Adobe Commerce >=2.3.4 &lt;2.4.3*) - Corregge il timeout di 503 primi byte nelle query [!DNL Visual Merchandiser] durante il caricamento della pagina Categoria amministratore.
* **MDVA-34695** (*per Adobe Commerce >=2.3.0 &lt;2.4.1*) - Corregge i valori negativi `children_count` dopo l&#39;eliminazione delle categorie.
...
* **MDVA-34943** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema in cui l&#39;ordine rapido memorizza nella cache gli SKU immessi in precedenza.
* **MDVA-35197** (*per Adobe Commerce >=2.3.5 &lt;2.4.0*) - Risolve il problema relativo alla presenza di un errore durante l&#39;aggiunta al carrello con GraphQL se i prodotti aggiunti in precedenza non sono più disponibili.
...
* **MDVA-33453** (*per Adobe Commerce >=2.3.6 &lt;2.4.3*) - Corregge il problema relativo all&#39;anteprima del tipo di contenuto Prodotti Page Builder se i prodotti corrispondenti hanno prezzi diversi per ciascun sito Web.
...
* **MDVA-34192** (*per Adobe Commerce >=2.3.4 &lt;2.4.3*) - Risolve il problema dell&#39;impossibilità di modificare o specificare la data di nascita del cliente nel formato gg/mm/aaaa.
...
* **MDVA-33559** (*per Adobe Commerce >=2.3.0 &lt;2.4.3*) - Corregge il problema del pagamento [!DNL PayPal Payflow Pro] non riuscito con errore di formato dell&#39;elenco di parametri di reindirizzamento.
...
* **MDVA-33632** (*per Adobe Commerce >=2.3.0 &lt;2.3.7*) - Corregge l&#39;errore relativo al messaggio di eccezione *Il prodotto è esaurito* che viene ora visualizzato a un utente amministratore quando tenta di riordinare un prodotto esaurito.
...
* **MDVA-33976** (*per Adobe Commerce >=2.3.0 &lt;2.3.7*): è stato risolto il problema che causava la visualizzazione esaurita del prodotto bundle nella vetrina dopo la rimozione della seconda opzione dal prodotto bundle.
* **MDVA-33894** (*per Adobe Commerce >=2.4.0 &lt;2.4.1 con estensione B2B*): sono stati risolti diversi problemi relativi alla funzionalità Ordine rapido, tra cui l&#39;aggiunta e la rimozione di più prodotti e la distinzione tra maiuscole e minuscole per gli SKU.
...
* **MDVA-34100** (*per Adobe Commerce >=2.3.4 &lt;2.4.2 con estensione B2B*) - Risolve il problema che impediva la creazione di un account aziendale dalla pagina della struttura della società.
...
* **MDVA-28511** (*per Adobe Commerce >=2.3.5 &lt;2.3.6*) - Corregge il problema per cui non è possibile completare l&#39;estrazione di [!DNL PayPal] se il campo Nome contiene determinati caratteri (ad esempio lettere maiuscole accentate).
* **MDVA-31519** (*per Adobe Commerce >=2.3.5 &lt;2.3.6*) - Corregge il problema dei timeout di attesa durante il pagamento quando viene utilizzata una regola di vendita a livello di sito.
...
* **MDVA-24201** (*per Adobe Commerce >=2.3.0 &lt;2.3.5*) - Corregge il problema per cui i prezzi non riflettono la regola del prezzo del carrello pianificato fino alla reindicizzazione manuale.
...
* **MDVA-33516** (*per Adobe Commerce >=2.3.0 &lt;2.3.6*) - Risolve il problema che causava un errore durante la modifica di un prodotto bundle in un elenco di richieste di acquisto.
...
* **MDVA-20376** (*per Adobe Commerce >=2.3.2 &lt;2.3.4*) - Corregge il problema con *Nessuna entità di questo tipo con customerId = 1* errore in `exception.log` per i clienti connessi dopo il posizionamento dell&#39;ordine.
...
* **MDVA-13203** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema per cui dopo una reindicizzazione completa viene visualizzato l&#39;errore *Violazione del vincolo di integrità search_tmp_ table*.
* **MDVA-23426** (*per Adobe Commerce >=2.3.3 &lt;2.3.5*) - Corregge il problema per cui le e-mail di notifica inviate da Adobe Commerce contengono un corpo vuoto e il contenuto viene aggiunto come allegato.
...
* **MDVA-31399** (*per Adobe Commerce >=2.3.2 &lt;2.4.2*) - Aggiunge il *Subtotale (incluso Imposta)* opzione per le condizioni della regola di prezzo.
...
* **MDVA-32133** (*per Adobe Commerce >=2.4.0 &lt;2.4.1*) - Corregge il problema di mancato caricamento della raccolta multimediale da Page Builder in alcuni casi.
* **MDVA-12304** (*per Adobe Commerce >=2.3.0*) - Aumenta il numero massimo di cookie da 50 a 200.
* **MDVA-32632** (*per Adobe Commerce >=2.3.2 &lt;2.3.5*) - Corregge il problema per cui gli ordini vengono visualizzati nel sistema di pagamento, ma non in Adobe Commerce.
* **MDVA-32449** (*per Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || >=2.4.1 &lt;2.4.2 con estensione B2B*) - Risolve il problema relativo al caricamento molto lento della cronologia degli ordini o al mancato caricamento di tale cronologia.
...
* **MDVA-30889** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema relativo a un errore che si verifica dopo la fatturazione di un bundle con prodotti virtuali e semplici come opzioni.
...
* **MDVA-32313** (*per Adobe Commerce >=2.3.0 &lt;2.3.4*) - Corregge il problema relativo all&#39;aggiunta di prodotti configurabili alla lista dei desideri con opzioni di configurazione errate.
...
* **MDVA-31640** (*per Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.1*) - Risolve il problema che impediva l’aggiornamento di un prezzo speciale tramite l’API REST.
...
* **MDVA-31150** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema per cui i saldi delle carte di credito e delle carte regalo del negozio non vengono restituiti dalla chiamata API Rest fattura GET, quando la fattura è stata registrata dalla chiamata API Rest e l&#39;ordine è stato parzialmente pagato dai conti carte di credito e gift card del negozio.
* **MDVA-30963** (*per Adobe Commerce >=2.3.2 &lt;2.4.2*) - Corregge il problema per cui i risultati del filtro dei prodotti impostati in modo da contenere solo i valori specificati per l&#39;ambito *Tutte le visualizzazioni archivio* nell&#39;amministratore, includono i prodotti con valori sovrascritti a livello di visualizzazione archivio.
* **MDVA-29954** (*per Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || 2.4.2 con estensione B2B*) - Corregge il problema per cui le *nuove richieste di registrazione società* e *ti hanno collegato a una società* e-mail vengono inviate dall&#39;indirizzo errato.
...
* **MDVA-30428** (*per Adobe Commerce >=2.3.4 &lt;2.3.5*) - Risolve il problema che impediva ai clienti di aggiungere un prodotto alla lista dei desideri se il prodotto è assegnato a un&#39;origine inventario personalizzata.
...
* **MDVA-25028** (*per Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*) - Corregge il problema che si verifica quando gli ordini effettuati utilizzando [!DNL PayPal Payflow Pro] non vengono impostati sullo stato Sospetto di frode quando vengono attivati i filtri antifrode.
...
* **MDVA-31007** (*per Adobe Commerce >=2.4.0 &lt;2.4.1*) - Corregge il problema per cui gli attributi degli indirizzi personalizzati non vengono visualizzati correttamente nella pagina dei dettagli dell&#39;ordine nell&#39;area Il mio account e nel backend.
...
* **MDVA-30945** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema per il quale viene visualizzato un messaggio di errore irreversibile durante l&#39;aggiornamento dei carrelli `Call to a member function getValue() on null in module-configurable-product CartItemProcessor.php`.
...
* **MDVA-30102** (*per Adobe Commerce >=2.3.2 &lt;=2.4.0*): è stato risolto il problema della rapida espansione della cache Redis, poiché il TTL non è disponibile nelle cache di layout.
* **MDVA-30599** (*per Adobe Commerce >=2.3.4 &lt;=2.4.0*) - Corregge il problema per cui le virgolette dei guest create utilizzando l&#39;API non vengono contrassegnate correttamente come virgolette per i clienti connessi.
...
* **MDVA-30357** (*per Adobe Commerce >=2.3.2 &lt;=2.4.0*) - Corregge il problema della creazione di URL di immagine errati durante la generazione di una mappa del sito da parte di cron.
* **MDVA-30565** (*per Adobe Commerce >=2.3.2 &lt;=2.3.3-p1*) - Corregge il problema per cui *Nessuna entità con cartid = 0* viene visualizzato l&#39;errore per il cliente ospite all&#39;estrazione in vetrina se il carrello acquisti persistente è abilitato.
...
* **MDVA-30977** (*per Adobe Commerce >=2.3.4 &lt;=2.3.5-p2*) - Corregge il problema di mancanza di prodotti casuali nelle categorie dopo la reindicizzazione.
...
* **MDVA-30841** (*per Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - È stato risolto il problema relativo alla navigazione su più livelli, in cui il valore *No* per gli attributi di prodotto di tipo booleano non veniva incluso nella navigazione su più livelli se [!DNL Elasticsearch] veniva utilizzato come motore di ricerca.
* **MDVA-28191** (*per Adobe Commerce >=2.3.3 &lt;2.4.2*) - Corregge il problema per cui non vengono caricati metodi di pagamento durante la creazione dell&#39;ordine tramite l&#39;amministratore.
* **MDVA-29959** (*per Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 con estensione B2B*) - Corregge il problema per cui l&#39;utente amministratore con restrizioni con autorizzazioni *Aziende* non è autorizzato a eliminare l&#39;account della società.
* **MDVA-30265** (*per Adobe Commerce >=2.3.3 &lt;2.4.2*) - Corregge il problema di interruzione del funzionamento del collegamento di registrazione della spedizione dopo la creazione della fattura.
* **MDVA-28409** (*per Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - È stato risolto il problema che causava un errore di *memoria insufficiente* nel processo cron `sales_clean_quotes` quando il numero di virgolette scadute nel database era molto elevato.
* **MDVA-30593** (*per Adobe Commerce >=2.3.0 &lt;2.3.4*) - Corregge il problema per cui le virgolette scadute in base all&#39;impostazione della durata delle virgolette non vengono pulite.
* **MDVA-30107** (*per Adobe Commerce >=2.3.0 &lt;2.3.6*) - Corregge il problema per cui il commutatore dell&#39;archivio non funziona come previsto se vengono utilizzati URL di base diversi per le visualizzazioni dell&#39;archivio.
...
* **MDVA-30284** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema in cui l&#39;indicizzatore di Ricerca nel catalogo non riesce a causa del seguente errore *[!DNL Elasticsearch]: è stato superato il limite dei campi totali nell&#39;indice.*
...
* **MDVA-30428** (*per Adobe Commerce >=2.3.3 &lt;2.4.2*) - Risolve il problema che impediva ai clienti di aggiungere un prodotto alla lista dei desideri se il prodotto è assegnato a un&#39;origine inventario personalizzata.
* **MDVA-28661** (*per Adobe Commerce >=2.3.0 &lt;2.4.2 con estensione B2B*) - Corregge il problema che causa un errore nella sezione dell&#39;account aziendale Utenti dopo la modifica dell&#39;amministratore della società.
...
* **MDVA-28656** (*per Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) - Corregge il problema per cui se un ordine è stato effettuato senza che fossero richieste informazioni sul pagamento (ad esempio, con uno sconto del 100%) ed è stata creata una fattura per l&#39;ordine, lo stato dell&#39;ordine cambia in *Chiuso* anziché Completo.
...
* **MDVA-30123** (*per Adobe Commerce >=2.3.4 &lt;2.4.2*) - Corregge il problema di mancata traduzione delle etichette di opzione degli attributi per le query GraphQL.
* **MDVA-29996** (*per Adobe Commerce >=2.3.3 &lt;2.4.2*) - Corregge il problema per cui dopo l&#39;abilitazione dell&#39;autorizzazione per la categoria, la pagina della categoria non viene memorizzata nella cache dalla cache di pagina intera.
...
* **MDVA-30232** (*per Adobe Commerce >=2.3.1 &lt;2.4.1*) - Corregge il problema se l&#39;ordine originale contiene una gift card e non è possibile riordinarlo.
...
* **MDVA-30008** (*per Adobe Commerce >=2.3.0 &lt;2.4.2*) - Corregge il problema B2B che non consente di effettuare ordini tramite Admin API per un prodotto presente in un catalogo pubblico.
...
* **MC-35984** (*per Adobe Commerce >=2.4.0 &lt;2.4.1*) - Corregge il problema che impediva ai commercianti di interagire con gli elementi della pagina Restituzioni dopo aver creato un&#39;etichetta di spedizione per un&#39;autorizzazione di restituzione merce (RMA).
...
* **MDVA-26694** (*per Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0*) - Corregge il problema con le cache di prodotti e cataloghi che scadono ogni giorno, anche se con una pianificazione diversa.
* **MDVA-27825** (*per Adobe Commerce >=2.3.0 &lt;2.4.1*): è stato risolto un problema che impediva l&#39;esportazione di grandi quantità di dati a causa di perdite di memoria.
* **MDVA-29085** (*per Adobe Commerce >=2.3.0 &lt;=2.3.5-p1*) - Corregge il problema B2B che non richiede l&#39;invio di e-mail di nuove società se queste vengono create tramite API.
...
* **MDVA-30131** (*per Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - È stato risolto il problema relativo alla navigazione su più livelli, in cui il valore *No* per gli attributi di prodotto di tipo booleano non veniva incluso nella navigazione su più livelli se [!DNL Elasticsearch] veniva utilizzato come motore di ricerca.
