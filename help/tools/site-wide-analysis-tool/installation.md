...
Se disponi di un’installazione on-premise di Adobe Commerce, devi installare un agente nell’infrastruttura per utilizzare lo strumento. Non è necessario installare l’agente su Adobe Commerce nei progetti di infrastruttura cloud.
...
>[!INFO]
>
>L’agente supporta installazioni Adobe Commerce a più nodi. Devi installare e configurare l’agente su ciascun nodo.
...
- Sistemi operativi
   ...
   - [!DNL Linux x86-64] le distribuzioni, quali [!DNL RedHat Enterprise Linux (RHEL)], [!DNL CentOS], [!DNL Ubuntu], [!DNL Debian]e simili
   >[!IMPORTANT]
   >
...
L&#39;agente richiede l&#39; [[!DNL Commerce Services Connector]](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) estensione da installare nel sistema e [configurato](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) con le chiavi API. Per verificare che l&#39;estensione sia installata, esegui il comando seguente:
...
1. Inserisci il tuo sito web in [modalità di manutenzione](https://devdocs.magento.com/guides/v2.4/install-gde/install/cli/install-cli-subcommands-maint.html).

1. Accedi a [accounts.magento.com](https://account.magento.com/customer/account/login?_ga=2.164207871.117144580.1649172612-1623400270.1640858671).

1. Clic **[!UICONTROL API Portal]**.

1. Fai clic su **[!UICONTROL Delete]** accanto alla chiave API esistente.

1. [Configura](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) una nuova chiave API.

>[!IMPORTANT]
>
> Se generi nuove chiavi nel portale API, aggiorna immediatamente le chiavi API nel [!DNL Admin configuration]. Se si generano nuove chiavi e non le si aggiornano nel [!DNL Admin], le estensioni SaaS non funzioneranno più e perderai dati preziosi.
...
1. [Configurare le chiavi API](https://experienceleague.adobe.com/docs/commerce-merchant-services/user-guides/saas.html) per collegare l&#39;estensione al sistema.
...
1. Creare un file di unità di sistema `(/etc/systemd/system/scheduler.service)` con la seguente configurazione (sostituisci `<filesystemowner>` con l&#39;utente Unix che possiede la directory in cui è installato l&#39;agente).
