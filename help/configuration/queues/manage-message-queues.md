...
>[!INFO]
>
>Se il tuo archivio Adobe Commerce è ospitato sulla piattaforma Cloud, usa [`CRON_CONSUMERS_RUNNER`](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#cron_consumers_runner) per configurare il processo cron `consumers_runner`.
...
- `multiple_processes` - Matrice di coppie chiave-valore che specifica il consumer da eseguire nel numero di processi. Supportato in Commerce 2.4.4 o versione successiva.
  ...
  >[!NOTE]
  >
  >ActiveMQ Artemis (STOMP) è stato introdotto in Adobe Commerce 2.4.5 e versioni successive.
