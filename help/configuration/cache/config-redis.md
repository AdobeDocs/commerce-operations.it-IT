...
Per ottimizzare l’istanza Redis in base alle tue esigenze, puoi ottenere risultati migliori utilizzando un’istanza dedicata per ogni sessione, cache Commerce e cache a pagina intera (FPC).
...
È inoltre possibile abilitare contemporaneamente sia le opzioni RDB che AOF. Per ulteriori dettagli, inclusi i vantaggi e gli svantaggi delle opzioni di persistenza, vedere la [documentazione sulla persistenza Redis](https://redis.io/docs/latest/operate/rs/databases/configure/database-persistence/).

Per l’istanza della cache, imposta l’istanza in modo che sia sufficientemente grande da memorizzare l’intera cache di Commerce.

- I requisiti di dimensione dipendono da diversi fattori come il numero di prodotti e di visualizzazioni dello store.

  Come punto di partenza, utilizzare le dimensioni della cartella `var/cache` nel file system. Ad esempio, se `var/cache` è di 5 GB, configura l&#39;istanza Redis con almeno 5 GB per l&#39;avvio.

- La persistenza non è necessaria per l’istanza della cache perché è possibile ripristinare la cache di Commerce.

  Per ulteriori informazioni, vedere la [Guida alla cache Redis](https://redis.io/docs/latest/develop/use/).
