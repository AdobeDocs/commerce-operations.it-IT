...
description: Scopri le best practice di configurazione per ottimizzare le prestazioni di Adobe Commerce. Scopri le impostazioni e gli strumenti per migliorare il tempo di risposta e la velocità effettiva.
...
Tutte le operazioni asincrone in Commerce vengono eseguite utilizzando il comando Linux `cron`. Vedi [Configura ed esegui cron](../configuration/cli/configure-cron-jobs.md) per configurarlo correttamente.
...
>[!INFO]
>
>Questa opzione funziona anche con [Inserimento ordine asincrono](high-throughput-order-processing.md#asynchronous-order-placement) in combinazione con [Inventory management](https://experienceleague.adobe.com/docs/commerce-admin/inventory/guide-overview.html?lang=it).
...
>[!INFO]
>
>Per ulteriori informazioni sull&#39;ottimizzazione di CSS e JavaScript, vedere [Ottimizzare i file di risorse](/help/implementation-playbook/best-practices/development/optimize-css-js-files.md).

### Suggerimenti per il bundling {#bundling-tips}

* È consigliabile utilizzare strumenti di terze parti per la minimizzazione e il bundling (come [r.js](https://requirejs.org/)). I meccanismi incorporati di Commerce non sono ottimali e vengono forniti come alternative di fallback.
...
* Si sconsiglia di utilizzare impostazioni obsolete, come l’unione di file JS e CSS, in quanto sono state progettate solo per file JS caricati in modo sincrono nella sezione HEAD della pagina. L’utilizzo di questa tecnica può causare il malfunzionamento del bundling e della logica requireJS.

## Convalida dei segmenti cliente

I commercianti che hanno un numero elevato di [segmenti di clienti](https://experienceleague.adobe.com/it/docs/commerce-admin/customers/segments/customer-segments) possono riscontrare un significativo deterioramento delle prestazioni con le azioni dei clienti, ad esempio l&#39;accesso dei clienti e l&#39;aggiunta di prodotti al carrello.
...
Se non desideri che la griglia prodotti sia limitata, ti consigliamo di utilizzare i filtri in modo più preciso affinché la raccolta dei risultati abbia meno elementi di **[!UICONTROL Records Limit]**.
