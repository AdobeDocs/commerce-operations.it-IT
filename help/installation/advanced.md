...
>[!TIP]
>
>Perso? Hai bisogno di aiuto? Prova il nostro [Installazione rapida](composer.md) o [Installazione collaboratore](https://developer.adobe.com/commerce/contributor/guides/install/) guide.
...
In questo argomento viene illustrata l&#39;installazione del software Adobe Commerce mediante CLI. Per informazioni sulla configurazione, vedi [Guida alla configurazione](../configuration/overview.md).
...
* Rivedi [installazione rapida](composer.md) panoramica.

>[!NOTE]
>
>È necessario installare Adobe Commerce da `bin` sottodirectory.
...
>[!NOTE]
>
>Per impostazione predefinita, il programma di installazione non sovrascrive il database se si installa il software nella stessa istanza di database. È possibile utilizzare l’ `cleanup-database` per modificare questo comportamento.

Vedi anche [Aggiornare, reinstallare, disinstallare](tutorials/uninstall.md).
...
Nelle tabelle seguenti vengono descritti i nomi e i valori delle opzioni di installazione. Ad esempio, i comandi di installazione, vedere [Esempi di installazioni localhost](#sample-localhost-installations).
...
Puoi creare l’utente amministratore durante o dopo l’installazione. Se crei l’utente durante l’installazione, sono necessarie tutte le variabili delle credenziali amministratore. Consulta [Esempi di installazioni localhost](#sample-localhost-installations).

Nelle tabelle seguenti sono disponibili molti parametri di installazione, ma non tutti. Per un elenco completo, vedere [Riferimento agli strumenti della riga di comando](https://devdocs.magento.com/guides/v2.4/reference/cli/magento.html).
...
| `--language` | Codice lingua da utilizzare nell’amministrazione e nella vetrina. Se non lo hai già fatto, puoi visualizzare l’elenco dei codici lingua immettendo `bin/magento info:language:list` dalla directory bin.) | No |
| `--currency` | Valuta predefinita da utilizzare nella vetrina. (Se non lo hai già fatto, puoi visualizzare l’elenco delle valute inserendo `bin/magento info:currency:list` dalla directory bin.) | No |
| `--timezone` | Fuso orario predefinito da utilizzare in Admin e storefront. (Se non lo hai già fatto, puoi visualizzare l’elenco dei fusi orari inserendo `bin/magento info:timezone:list` dal `bin/` directory.) | No |
...
| Nome | Valore | Obbligatorio |
|--- |--- |--- |
| `--search-engine` | Versione di Elasticsearch o OpenSearch da utilizzare come motore di ricerca. Il valore predefinito è `elasticsearch7`. L&#39;Elasticsearch 5 è stato dichiarato obsoleto e non è consigliato. | No |
...
>[!NOTE]
>
>Per abilitare o disabilitare i moduli dopo l’installazione di Adobe Commerce, consulta [Abilitare e disabilitare i moduli](tutorials/manage-modules.md).
...
* L’amministratore dispone delle seguenti proprietà:
   ...
   * I nomi e i cognomi sono `Magento User`
...
* È possibile utilizzare il prefisso di incremento dell&#39;ordine cliente `ORD$` (poiché contiene un carattere speciale [`$`], il valore deve essere racchiuso tra virgolette)
