...
La memorizzazione in cache del maggior numero possibile di siti sui dispatcher è la best practice per qualsiasi progetto AEM. L’utilizzo dell’annullamento della validità della cache basato su tempo memorizzerà nella cache le pagine CIF sottoposte a rendering sul lato server per un periodo di tempo limitato. Dopo la scadenza del tempo impostato, la richiesta successiva ricostruirà la pagina dall’editore AEM e da Adobe Commerce GraphQL e la memorizzerà nuovamente nella cache del dispatcher fino alla successiva invalidazione.
...
La memorizzazione in cache a livello di browser viene comunemente eseguita tramite &quot;Cache-Control: max-age=&quot; intestazione di risposta. L&#39;impostazione maxage indica al browser per quanti secondi deve memorizzare in cache il file prima di tentare di &quot;riconvalidare&quot; o richiederlo nuovamente dal sito. Questo concetto di età massima della cache è comunemente noto come &quot;Scadenza cache&quot; o TTL (&quot;Time to Live&quot;). Distribuzione di esperienze commerce su larga scala - Con Adobe Experience Manager, Commerce Integration Framework, Adobe Commerce 7

Alcune aree di un sito AEM/CIF/Adobe Commerce che può essere impostato per essere memorizzato nella cache nel browser del cliente includono:

- Immagini (all&#39;interno del modello di AEM stesso, ad esempio il logo del sito e le immagini di progettazione del modello - le immagini dei prodotti di catalogo saranno chiamate da Adobe Commerce via Flast, la memorizzazione in cache di queste immagini sarà discussa più avanti)
- File di HTML (per pagine modificate raramente - pagina termini e condizioni, ecc.)
- File CSS
...
>[!NOTE]
>
> Ulteriori informazioni più dettagliate su questo argomento sono disponibili nella [esperimenti-aem-dispatcher](https://github.com/adobe/aem-dispatcher-experiments/tree/main/experiments/gracePeriod) Archivio GitHub.

## CIF: memorizzazione in cache GraphQL tramite componenti

I singoli componenti all’interno di AEM possono essere impostati per essere memorizzati nella cache, il che significa che la richiesta GraphQL ad Adobe Commerce viene chiamata una volta e poi le richieste successive, fino al limite di tempo configurato, vengono recuperate dalla cache AEM e non inseriscono ulteriori caricamenti in Adobe Commerce. Ad esempio, la navigazione nel sito basata su una struttura di categorie mostrata su ogni pagina e le opzioni all’interno di una funzionalità di ricerca sfaccettata. Si tratta solo di due aree che richiedono la generazione di query ad alta intensità di risorse su Adobe Commerce, ma è improbabile che si modifichino regolarmente e quindi rappresentano una buona scelta per la memorizzazione in cache. In questo modo, ad esempio, anche quando un PDP o un PLP viene ricostruito dall’editore, la richiesta GraphQL a uso intensivo di risorse per la build di navigazione non colpirà Adobe Commerce e potrebbe essere recuperata dalla cache GraphQL su AEM CIF.
...
Il gclid e fbclid cambieranno con ogni utente che fa clic sull’annuncio, questo è destinato a scopi di tracciamento, ma con le sue impostazioni predefinite, AEM vedrebbe ogni richiesta come una pagina univoca, che bypasserebbe il dispatcher e genererebbe inutili carichi aggiuntivi sull’editore e su Adobe Commerce.
...
>[!NOTE]
>
>Ulteriori letture sull&#39;importanza di definire `ignoreUrlParams` è disponibile in [esperimenti-aem-dispatcher](https://github.com/adobe/aem-dispatcher-experiments/tree/main/experiments/ignoreUrlParams) Archivio GitHub.
